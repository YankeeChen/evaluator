...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_TrackingRadar_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:aggregateOf
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:resource)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($value_TrackingRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_Process_0/RFDevice:supportsTransmitting
    ...
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
        <xqllib:var name="InstantaneousFrequencyMeasurementReceiver_0">{$value_InstantaneousFrequencyMeasurementReceiver_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("InstantaneousFrequencyMeasurementReceiver_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("TrackingRadar_0"))
  return
    $GRAPH_7
return
