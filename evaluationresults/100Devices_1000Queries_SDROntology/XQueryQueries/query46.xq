...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:about)
    for $node_Jammer_0 in $node_TrackingRadar_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_ADC_0 in $node_Jammer_0/Nuvio:aggregateOf
    let $value_ADC_0 := fn:data($node_ADC_0/@rdf:resource)
    where fn:exists($value_ADC_0) and fn:exists($node_Jammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_ADC_0/RFDevice:isProducerOf
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($node_ADC_0) and fn:exists($value_Entity_0)
    return
      <xqllib:result>
        <xqllib:var name="ADC_0">{$value_ADC_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Var0) and $node_Var0/skos:member/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
