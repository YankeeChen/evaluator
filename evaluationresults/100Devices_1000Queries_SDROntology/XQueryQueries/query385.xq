...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xxmini_0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) = 1413579965)]
  let $GRAPH_6 := xqllib:optional($GRAPH_3,$GRAPH_5,("Entity_0"))
  let $GRAPH_7 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Voltage_0 in $node_Entity_0/Nuvio:expresses
    let $value_Voltage_0 := fn:data($node_Voltage_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_Voltage_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("Voltage_0"))
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_0,$GRAPH_14,("USRPB2xxmini_0"))
  return
    $GRAPH_15
return
