...
let $result :=  
  ...
  let $GRAPH_8 :=  
    for $node_LongitudeValueInDegree_0 in $doc_ObjectDescription
    let $value_LongitudeValueInDegree_0 := fn:data($node_LongitudeValueInDegree_0/@rdf:about)
    for $node_DataValue0 in $node_LongitudeValueInDegree_0/RFDevice:hasPositiveIntegerValue
  ...
  let $GRAPH_9 := xqllib:optional($GRAPH_7,$GRAPH_8,("LongitudeValueInDegree_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("LongitudeValueInDegree_0"))
  let $GRAPH_11 :=  
    for $node_Entity_0 in $doc_ObjectDescription[Nuvio:hasProcessQuality/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#VerticalPolarization"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
