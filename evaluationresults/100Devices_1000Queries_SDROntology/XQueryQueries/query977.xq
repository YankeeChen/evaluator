...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseRadar_0 := fn:data($node_PulseRadar_0/@rdf:about)
    for $node_Submarine_0 in $node_PulseRadar_0/RFDevice:isDeployedOn
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:resource)
    where fn:exists($node_PulseRadar_0) and fn:exists($value_Submarine_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseRadar_0">{$value_PulseRadar_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:about)
    for $node_Entity_0 in $node_Submarine_0/RFDevice:supportsJamming
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("Entity_0","Process_0"))
