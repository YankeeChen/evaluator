...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Entity_1) and $node_Entity_1/Nuvio:hasProcessQuantity/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Entity_0 in $node_Modulation_0/RFDevice:hasCapability
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("Modulation_0"))
  ...
  let $GRAPH_11 :=  
    ...
    for $node_Entity_0 in $node_SpatialLinearUnit_0/Nuvio:expresses
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
