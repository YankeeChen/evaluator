...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_Var1) and $node_Var1/Nuvio:expresses/@rdf:resource = $value_Var1
    return
      <xqllib:result>
        <xqllib:var name="Var1">{$value_Var1}</xqllib:var>
      </xqllib:result>
  let $GRAPH_4 :=  
    ...
    for $node_Var1 in $node_Transceiver_0/Nuvio:compositeOf
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_Transceiver_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Transceiver_0 in $node_Producer_0/RFDevice:hasProducer
    let $value_Transceiver_0 := fn:data($node_Transceiver_0/@rdf:resource)
    where fn:exists($node_Producer_0) and fn:exists($value_Transceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("Producer_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("Transceiver_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("Transceiver_0"))
  let $GRAPH_11 :=  
    ...
    where fn:exists($node_Var1) and fn:exists($node_Var2) and $node_Var2/Nuvio:informationSource/@rdf:resource = $value_Var1
    return
      <xqllib:result>
        <xqllib:var name="Var1">{$value_Var1}</xqllib:var>
  ...
  let $GRAPH_12 :=  
    for $node_Var2 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Curve"]
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_11,$GRAPH_12,("Var2"))
  let $GRAPH_14 := xqllib:optional($GRAPH_10,$GRAPH_13,("Var1"))
  let $GRAPH_15 := xqllib:and($GRAPH_3,$GRAPH_14,("Var1"))
  let $GRAPH_16 := xqllib:and($GRAPH_2,$GRAPH_15,("Var1"))
