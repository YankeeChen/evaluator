...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_MonostaticRadar_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_6 :=  
    for $node_Entity_0 in $doc_ObjectDescription[Nuvio:participatesIn/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription100.owl#IEDTrigger_instance2"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_0,$GRAPH_9,("MonostaticRadar_0"))
  return
    $GRAPH_10
return
