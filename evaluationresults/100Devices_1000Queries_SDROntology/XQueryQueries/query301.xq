...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_RadarJammer_0/Nuvio:compositeOf
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($node_RadarJammer_0) and fn:exists($value_USRPB200mini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadarJammer_0 in $node_CrossPolarizationJamming_0/RFDevice:supportsTransmitting
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_CrossPolarizationJamming_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_CrossPolarizationJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Power_15dBm"]/RFDevice:hasTransmitPowerRange
    let $value_CrossPolarizationJamming_0 := fn:data($node_CrossPolarizationJamming_0/@rdf:resource)
    where fn:exists($value_CrossPolarizationJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="CrossPolarizationJamming_0">{$value_CrossPolarizationJamming_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_0,$GRAPH_7,("USRPB200mini_0"))
  return
    $GRAPH_8
return
