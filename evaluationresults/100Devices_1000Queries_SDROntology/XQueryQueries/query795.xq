...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_SuperheterodyneReceiver_0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_SuperheterodyneReceiver_0 in $node_QuantityRange_0/RFDevice:hasFrequencyRange
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($node_QuantityRange_0) and fn:exists($value_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
