...
let $result :=  
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue0"]) = 1126314368)]
  ...
  let $GRAPH_11 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Mixer_0) and $node_Mixer_0/Nuvio:hasParticipant/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_13 :=  
    ...
    for $node_Mixer_0 in $node_IEDJammer_0/Nuvio:compositeOf
    let $value_Mixer_0 := fn:data($node_Mixer_0/@rdf:resource)
    where fn:exists($node_IEDJammer_0) and fn:exists($value_Mixer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_15 :=  
    ...
    for $node_IEDJammer_0 in $node_SweptSpotJamming_0/RFDevice:supportsTuning
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_15,$GRAPH_16,("SweptSpotJamming_0"))
  let $GRAPH_18 := xqllib:optional($GRAPH_14,$GRAPH_17,("IEDJammer_0"))
  let $GRAPH_19 := xqllib:and($GRAPH_13,$GRAPH_18,("IEDJammer_0"))
  let $GRAPH_20 := xqllib:and($GRAPH_12,$GRAPH_19,("Mixer_0"))
  let $GRAPH_21 := xqllib:and($GRAPH_11,$GRAPH_20,("Mixer_0"))
  ...
  let $GRAPH_24 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_InformationRepresentation_0) and fn:exists($node_Situation_0) and $node_InformationRepresentation_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
        <xqllib:var name="InformationRepresentation_0">{$value_InformationRepresentation_0}</xqllib:var>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_25 := xqllib:and($GRAPH_23,$GRAPH_24,())
  let $GRAPH_26 := xqllib:and($GRAPH_4,$GRAPH_25,("Entity_0","Situation_0"))
  let $GRAPH_27 := xqllib:and($GRAPH_3,$GRAPH_26,("Entity_0"))
  let $GRAPH_28 := xqllib:and($GRAPH_2,$GRAPH_27,("Var0"))
  let $GRAPH_29 := xqllib:and($GRAPH_1,$GRAPH_28,("Var0"))
  let $GRAPH_30 := xqllib:and($GRAPH_0,$GRAPH_29,("USRPB200_0"))
  return
    $GRAPH_30
return
