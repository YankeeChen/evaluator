...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:about)
    for $node_Object_0 in $node_ModelX_0/RFDevice:isProducerOf
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($node_ModelX_0) and fn:exists($value_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="ModelX_0">{$value_ModelX_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_AmplitudeModulation_0 in $node_Object_0/RFDevice:hasCapability
    let $value_AmplitudeModulation_0 := fn:data($node_AmplitudeModulation_0/@rdf:resource)
    where fn:exists($value_AmplitudeModulation_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
