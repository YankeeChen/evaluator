...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_DRFMJamming_0/RFDevice:hasCapability
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:resource)
    where fn:exists($node_DRFMJamming_0) and fn:exists($value_JammingSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_DRFMJamming_0 in $node_Entity_0/RFDevice:hasMaxSensedPower
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("DRFMJamming_0"))
