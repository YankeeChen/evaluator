...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Weight_0/RFDevice:hasWeight
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($value_USRPB200mini_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB200mini_0">{$value_USRPB200mini_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_WeightUnit_0 in $node_Var0/Nuvio:hasUnitOfMeasure
    let $value_WeightUnit_0 := fn:data($node_WeightUnit_0/@rdf:resource)
    where fn:exists($node_Var0) and fn:exists($value_WeightUnit_0)
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("WeightUnit_0"))
