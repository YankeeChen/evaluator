...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_XilinxFPGA_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_XilinxFPGA_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_XilinxFPGA_0 := fn:data($node_XilinxFPGA_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_XilinxFPGA_0)
    return
      <xqllib:result>
