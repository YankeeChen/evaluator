...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseDopplerRadar_0 in $node_Jammer_0/Nuvio:compositeOf
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_PulseDopplerRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Jammer_0 in $node_SweepJamming_0/RFDevice:hasCapability
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($value_Jammer_0) and fn:exists($node_SweepJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_SweepJamming_0 in $doc_ObjectDescription
    let $value_SweepJamming_0 := fn:data($node_SweepJamming_0/@rdf:about)
    for $node_Power_0 in $node_SweepJamming_0/RFDevice:hasMaxOutputPower
    let $value_Power_0 := fn:data($node_Power_0/@rdf:resource)
    where fn:exists($value_Power_0) and fn:exists($node_SweepJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("SweepJamming_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("SweepJamming_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("Jammer_0"))
