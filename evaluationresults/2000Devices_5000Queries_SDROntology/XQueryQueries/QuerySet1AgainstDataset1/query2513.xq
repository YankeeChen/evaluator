...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN310_0 in $node_Ship_0/RFDevice:isDeployedOn
    let $value_USRPN310_0 := fn:data($node_USRPN310_0/@rdf:resource)
    where fn:exists($node_Ship_0) and fn:exists($value_USRPN310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Ship_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Moving_instance42"]/RFDevice:supportsTuning
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_0,$GRAPH_5,("USRPN310_0"))
  return
    $GRAPH_6
return
