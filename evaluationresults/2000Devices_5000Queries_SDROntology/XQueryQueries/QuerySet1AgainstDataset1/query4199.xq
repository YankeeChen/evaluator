...
let $result :=  
  ...
  let $GRAPH_6 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Power_0) and $node_Power_0/RFDevice:hasMaxSensedPower/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_6,$GRAPH_9,("Entity_0","Power_0"))
