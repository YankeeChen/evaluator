...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE310_0 in $node_Transmitter_0/Nuvio:compositeOf
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:resource)
    where fn:exists($node_Transmitter_0) and fn:exists($value_USRPE310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Transmitter_0 in $node_Mixer_0/Nuvio:compositeOf
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:resource)
    where fn:exists($node_Mixer_0) and fn:exists($value_Transmitter_0)
    return
      <xqllib:result>
