...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:about)
    for $node_Var0 in $node_BistaticRadar_0/RFDevice:hasCapability
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Jamming_0 in $node_Var0/Nuvio:hasSubprocess
    let $value_Jamming_0 := fn:data($node_Jamming_0/@rdf:resource)
    where fn:exists($value_Jamming_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Jamming_0 in $node_Attribute_0/RFDevice:hasPulseRepetitionInterval
    let $value_Jamming_0 := fn:data($node_Jamming_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_Jamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Attribute_0">{$value_Attribute_0}</xqllib:var>
        <xqllib:var name="Jamming_0">{$value_Jamming_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Jamming_0"))
