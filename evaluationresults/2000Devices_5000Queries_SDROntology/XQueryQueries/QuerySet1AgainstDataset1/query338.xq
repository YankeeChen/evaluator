...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB2xxmini_0 := fn:data($node_USRPB2xxmini_0/@rdf:about)
    for $node_Weight_0 in $node_USRPB2xxmini_0/RFDevice:hasLevel
    let $value_Weight_0 := fn:data($node_Weight_0/@rdf:resource)
    where fn:exists($node_USRPB2xxmini_0) and fn:exists($value_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB2xxmini_0">{$value_USRPB2xxmini_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Ring"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_6 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Var1 in $node_Var0/Nuvio:participatesIn
  ...
  let $GRAPH_10 :=  
    ...
    for $node_Var0 in $node_Object_0/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_12 := xqllib:optional($GRAPH_4,$GRAPH_11,("Var0"))
  let $GRAPH_13 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_0,$GRAPH_17,("USRPB2xxmini_0"))
  return
    $GRAPH_18
return
