...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ContinuousWaveRadar_0 := fn:data($node_ContinuousWaveRadar_0/@rdf:about)
    for $node_Object_0 in $node_ContinuousWaveRadar_0/RFDevice:isProducerOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_ElectricCharge_0 in $node_Object_0/RFDevice:hasLength
    let $value_ElectricCharge_0 := fn:data($node_ElectricCharge_0/@rdf:resource)
    where fn:exists($value_ElectricCharge_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("ElectricCharge_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("ElectricCharge_0"))
