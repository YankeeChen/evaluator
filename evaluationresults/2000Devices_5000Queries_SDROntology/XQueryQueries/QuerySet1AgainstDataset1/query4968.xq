...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Var0 in $node_Amplifier_0/Nuvio:aggregateOf
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_Amplifier_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Amplifier_0"))
  let $GRAPH_7 := ($GRAPH_3,$GRAPH_6)
  let $GRAPH_8 := xqllib:and($GRAPH_1,$GRAPH_7,("Var0"))
  let $GRAPH_9 := xqllib:optional($GRAPH_0,$GRAPH_8,("USRPE3xx_0"))
  return
    $GRAPH_9
return
