...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_ContinuousWaveRadarModulation_0/RFDevice:supportsSpectrumSensing
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:resource)
    where fn:exists($node_ContinuousWaveRadarModulation_0) and fn:exists($value_SearchRadar_0)
    return
      <xqllib:result>
