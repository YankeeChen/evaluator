...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Transceiving_0 in $node_Object_0/RFDevice:hasBandwidthRange
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_Transceiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_2,$GRAPH_6,("Transceiving_0"))
