...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseRadar_0 := fn:data($node_PulseRadar_0/@rdf:about)
    for $node_IEDTrigger_0 in $node_PulseRadar_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:about)
    for $node_Object_0 in $node_IEDTrigger_0/RFDevice:hasFrequencyAccuracy
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="IEDTrigger_0">{$value_IEDTrigger_0}</xqllib:var>
