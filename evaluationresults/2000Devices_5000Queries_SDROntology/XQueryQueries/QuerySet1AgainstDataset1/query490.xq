...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_Process_0 in $node_SurveillanceRadar_0/RFDevice:supportsSpectrumSensing
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($value_Process_0) and fn:exists($node_SurveillanceRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Point_0 in $doc_ObjectDescription
    let $value_Point_0 := fn:data($node_Point_0/@rdf:about)
    for $node_Process_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Process"]
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_Point_0) and fn:exists($node_Process_0) and $node_Point_0/RFDevice:hasTarget/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Point_0">{$value_Point_0}</xqllib:var>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_3 :=  
    for $node_Point_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/sf#Point"]
    let $value_Point_0 := fn:data($node_Point_0/@rdf:about)
    where fn:exists($node_Point_0)
    return
      <xqllib:result>
        <xqllib:var name="Point_0">{$value_Point_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("Point_0"))
