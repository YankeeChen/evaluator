...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_0 in $node_XilinxFPGA_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_XilinxFPGA_0 in $doc_ObjectDescription
    let $value_XilinxFPGA_0 := fn:data($node_XilinxFPGA_0/@rdf:about)
    for $node_XilinxFPGA_1 in $node_XilinxFPGA_0/Nuvio:compositeOf
    let $value_XilinxFPGA_1 := fn:data($node_XilinxFPGA_1/@rdf:resource)
    where fn:exists($node_XilinxFPGA_0) and fn:exists($value_XilinxFPGA_1)
    return
      <xqllib:result>
        <xqllib:var name="XilinxFPGA_0">{$value_XilinxFPGA_0}</xqllib:var>
        <xqllib:var name="XilinxFPGA_1">{$value_XilinxFPGA_1}</xqllib:var>
      </xqllib:result>
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("XilinxFPGA_0"))
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("XilinxFPGA_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_0,$GRAPH_5,("USRPB200_0"))
  return
    $GRAPH_6
return
