...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_GNURadio_0 in $node_Entity_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("InstantaneousFrequencyMeasurementReceiver_0"))
  ...
  let $GRAPH_12 := $GRAPH_11[(data(xqllib:var[@name="DataValue0"]) = -534863506)]
