...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Entity_0 in $node_Process_0/RFDevice:supportsTuning
  ...
  let $GRAPH_6 :=  
    for $node_Cone_0 in $doc_ObjectDescription
    let $value_Cone_0 := fn:data($node_Cone_0/@rdf:about)
    for $node_Process_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Process"]
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_Cone_0) and fn:exists($node_Process_0) and $node_Cone_0/Nuvio:participatesIn/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Cone_0">{$value_Cone_0}</xqllib:var>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 :=  
    for $node_Cone_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Cone"]
    let $value_Cone_0 := fn:data($node_Cone_0/@rdf:about)
    where fn:exists($node_Cone_0)
    return
      <xqllib:result>
        <xqllib:var name="Cone_0">{$value_Cone_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("Cone_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("Process_0"))
  ...
  let $GRAPH_22 := xqllib:and($GRAPH_18,$GRAPH_21,())
  let $GRAPH_23 := xqllib:and($GRAPH_2,$GRAPH_22,("Entity_0","InformationRepresentation_0"))
  let $GRAPH_24 := xqllib:and($GRAPH_1,$GRAPH_23,("Entity_0"))
  let $GRAPH_25 := xqllib:and($GRAPH_0,$GRAPH_24,("SearchRadar_0"))
  return
    $GRAPH_25
return
