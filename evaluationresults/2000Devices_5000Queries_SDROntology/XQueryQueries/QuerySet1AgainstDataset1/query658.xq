...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_InverseGainJamming_0/RFDevice:supportsTuning
  ...
  let $GRAPH_3 :=  
    ...
    for $node_InverseGainJamming_0 in $node_Power_0/RFDevice:hasMaxOutputPower
    let $value_InverseGainJamming_0 := fn:data($node_InverseGainJamming_0/@rdf:resource)
    where fn:exists($value_InverseGainJamming_0) and fn:exists($node_Power_0)
    return
      <xqllib:result>
        <xqllib:var name="InverseGainJamming_0">{$value_InverseGainJamming_0}</xqllib:var>
