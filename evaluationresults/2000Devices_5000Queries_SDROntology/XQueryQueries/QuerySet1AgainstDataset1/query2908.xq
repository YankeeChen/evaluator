...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_SearchRadar_0 in $doc_ObjectDescription[RFDevice:hasSamplingRate/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Weight_instance213"]
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:about)
    where fn:exists($node_SearchRadar_0)
    return
      <xqllib:result>
