...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_Aircraft_0/RFDevice:isDeployedOn
    let $value_USRPN200_0 := fn:data($node_USRPN200_0/@rdf:resource)
    where fn:exists($node_Aircraft_0) and fn:exists($value_USRPN200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Aircraft_0 in $node_Moving_0/RFDevice:supportsSpectrumSensing
