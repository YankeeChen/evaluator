...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:about)
    for $node_Var0 in $node_USRPE310_0/RFDevice:hasCapability
  ...
  let $GRAPH_5 :=  
    ...
    for $node_UnitOfMeasure_0 in $node_Var2/Nuvio:hasUnitOfMeasure
    let $value_UnitOfMeasure_0 := fn:data($node_UnitOfMeasure_0/@rdf:resource)
    where fn:exists($value_UnitOfMeasure_0) and fn:exists($node_Var2)
    return
      <xqllib:result>
