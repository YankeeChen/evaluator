...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:about)
    for $node_Entity_0 in $node_SearchRadar_0/Nuvio:compositeOf
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_ElectricCharge_0) and fn:exists($node_Entity_0) and $node_ElectricCharge_0/RFDevice:hasVoltage/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_15 :=  
    ...
    for $node_Var0 in $node_Entity_1/RFDevice:hasMaxOutputPower
  ...
  let $GRAPH_19 := xqllib:and($GRAPH_11,$GRAPH_18,("InformationEntity_0"))
