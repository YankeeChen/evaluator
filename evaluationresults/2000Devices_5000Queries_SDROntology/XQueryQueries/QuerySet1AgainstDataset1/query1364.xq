...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_IEDTrigger_0 in $doc_ObjectDescription
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:about)
    for $node_Process_0 in $node_IEDTrigger_0/RFDevice:supportsJamming
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
        <xqllib:var name="IEDTrigger_0">{$value_IEDTrigger_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Process_0) and fn:exists($node_Quality_0) and $node_Quality_0/Nuvio:hasProcessQuality/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Quality_0"))
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_0,$GRAPH_9,("ContinuousWaveRadar_0"))
  return
    $GRAPH_10
return
