...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_InformationEntity_0) and fn:exists($node_Var0) and $node_InformationEntity_0/Nuvio:relevantIndividual/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_InformationEntity_0 in $node_Jamming_0/Nuvio:isExpressedBy
    let $value_InformationEntity_0 := fn:data($node_InformationEntity_0/@rdf:resource)
    where fn:exists($value_InformationEntity_0) and fn:exists($node_Jamming_0)
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Jamming_0 in $node_Power_0/RFDevice:hasEffectiveRadiatedPower
    let $value_Jamming_0 := fn:data($node_Jamming_0/@rdf:resource)
    where fn:exists($value_Jamming_0) and fn:exists($node_Power_0)
    return
      <xqllib:result>
        <xqllib:var name="Jamming_0">{$value_Jamming_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("Jamming_0"))
  let $GRAPH_9 := xqllib:optional($GRAPH_3,$GRAPH_8,("InformationEntity_0"))
  let $GRAPH_10 :=  
  ...
  let $GRAPH_14 := xqllib:optional($GRAPH_0,$GRAPH_13,("USRPN2xx_0"))
  return
    $GRAPH_14
return
