...
let $result :=  
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Value_0 in $node_Attribute_0/Nuvio:hasUnitOfMeasure
    let $value_Value_0 := fn:data($node_Value_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_Value_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) != 788005798)]
  let $GRAPH_9 := ($GRAPH_6,$GRAPH_8)
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,())
