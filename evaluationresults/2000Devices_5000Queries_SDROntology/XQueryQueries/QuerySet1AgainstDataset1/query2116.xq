...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_CoverJamming_0/RFDevice:supportsTransmitting
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($node_CoverJamming_0) and fn:exists($value_ElectronicAttackSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_CoverJamming_0 in $node_Quantity_0/RFDevice:hasPulseRepetitionFrequency
    let $value_CoverJamming_0 := fn:data($node_CoverJamming_0/@rdf:resource)
    where fn:exists($value_CoverJamming_0) and fn:exists($node_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="CoverJamming_0">{$value_CoverJamming_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("CoverJamming_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("CoverJamming_0"))
