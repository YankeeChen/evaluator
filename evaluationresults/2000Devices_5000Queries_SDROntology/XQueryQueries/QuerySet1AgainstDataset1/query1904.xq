...
let $result :=  
  ...
  let $GRAPH_13 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_Process_0) and $doc_ObjectDescription[@rdf:ID = "Instance565"]/Nuvio:hasSubprocess/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_0,$GRAPH_16,("RFSystem_0"))
  return
    $GRAPH_17
return
