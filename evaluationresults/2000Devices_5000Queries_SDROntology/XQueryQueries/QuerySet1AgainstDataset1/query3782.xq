...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN3xx_0 in $node_XilinxZynq7000Family_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    for $node_XilinxZynq7000Family_0 in $doc_ObjectDescription
    let $value_XilinxZynq7000Family_0 := fn:data($node_XilinxZynq7000Family_0/@rdf:about)
    for $node_XC7K325TFPGA_0 in $node_XilinxZynq7000Family_0/Nuvio:compositeOf
    let $value_XC7K325TFPGA_0 := fn:data($node_XC7K325TFPGA_0/@rdf:resource)
    where fn:exists($value_XC7K325TFPGA_0) and fn:exists($node_XilinxZynq7000Family_0)
    return
      <xqllib:result>
