...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX3xx_0 in $node_Var0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_InformationContent_0 in $node_Var0/Nuvio:isExpressedBy
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_BarrageJamming_0 in $node_Var0/RFDevice:hasCapability
    let $value_BarrageJamming_0 := fn:data($node_BarrageJamming_0/@rdf:resource)
    where fn:exists($value_BarrageJamming_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 :=  
    for $node_BarrageJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Power_instance12"]/RFDevice:hasPulseWidth
    let $value_BarrageJamming_0 := fn:data($node_BarrageJamming_0/@rdf:resource)
    where fn:exists($value_BarrageJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="BarrageJamming_0">{$value_BarrageJamming_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("BarrageJamming_0"))
  ...
  let $GRAPH_12 :=  
    ...
    for $node_Var0 in $node_InformationContent_0/Nuvio:representedBy
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_11,$GRAPH_12,("InformationContent_0","Var0"))
  let $GRAPH_14 := xqllib:and($GRAPH_1,$GRAPH_13,("Var0"))
  let $GRAPH_15 := xqllib:optional($GRAPH_0,$GRAPH_14,("USRPX3xx_0"))
  return
    $GRAPH_15
return
