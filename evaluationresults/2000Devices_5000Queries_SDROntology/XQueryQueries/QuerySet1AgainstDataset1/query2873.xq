...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:about)
    for $node_Var0 in $node_USRPB2xx_0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#RFSystem"]
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_FrequencyScanRateUnit_0 in $node_Var0/Nuvio:hasUnitOfMeasure
    let $value_FrequencyScanRateUnit_0 := fn:data($node_FrequencyScanRateUnit_0/@rdf:resource)
    where fn:exists($value_FrequencyScanRateUnit_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) <= -631984510)]
  let $GRAPH_8 :=  
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_5,$GRAPH_9,("FrequencyScanRateUnit_0","Var0"))
  let $GRAPH_11 := xqllib:and($GRAPH_4,$GRAPH_10,("Var0"))
  let $GRAPH_12 := xqllib:and($GRAPH_1,$GRAPH_11,("Var0"))
  let $GRAPH_13 := xqllib:and($GRAPH_0,$GRAPH_12,("USRPB2xx_0"))
  return
    $GRAPH_13
return
