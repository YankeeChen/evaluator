...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_i_0 in $node_RadioComponent_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadioComponent_0 in $node_WhipAntenna_0/Nuvio:aggregateOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($value_RadioComponent_0) and fn:exists($node_WhipAntenna_0)
    return
      <xqllib:result>
        <xqllib:var name="RadioComponent_0">{$value_RadioComponent_0}</xqllib:var>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("WhipAntenna_0"))
