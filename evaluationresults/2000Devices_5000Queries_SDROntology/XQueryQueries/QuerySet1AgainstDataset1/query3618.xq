...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:about)
    for $node_Var0 in $node_USRPN210_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_CyclostationaryDetection_0/RFDevice:supportsTuning
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_CyclostationaryDetection_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
