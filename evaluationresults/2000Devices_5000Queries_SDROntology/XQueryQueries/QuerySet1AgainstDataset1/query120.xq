...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_i_0 in $node_Receiver_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Receiver_0 in $node_Object_0/RFDevice:hasFrequencyRange
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_PhaseShiftKeyingModulation_0) and $node_PhaseShiftKeyingModulation_0/RFDevice:hasCapability/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
