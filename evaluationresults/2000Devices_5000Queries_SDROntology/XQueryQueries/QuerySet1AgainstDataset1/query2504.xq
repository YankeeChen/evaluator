...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:about)
    for $node_Var0 in $node_USRPX300_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_Entity_0) and fn:exists($node_Var1) and $node_Entity_0/Nuvio:expresses/@rdf:resource = $value_Var1
    return
      <xqllib:result>
