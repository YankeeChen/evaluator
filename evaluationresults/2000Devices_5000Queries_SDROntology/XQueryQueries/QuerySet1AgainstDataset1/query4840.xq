...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarSystem_0 in $node_DirectConversionReceiver_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_DirectConversionReceiver_0 in $node_FreqencyRange_0/RFDevice:hasFrequencyAccuracy
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($value_DirectConversionReceiver_0) and fn:exists($node_FreqencyRange_0)
    return
      <xqllib:result>
        <xqllib:var name="DirectConversionReceiver_0">{$value_DirectConversionReceiver_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_FreqencyRange_0 in $node_Ratio_0/RFDevice:hasMax
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Ratio_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("FreqencyRange_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("FreqencyRange_0"))
