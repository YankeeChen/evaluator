...
let $result :=  
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Entity_0 in $node_Entity_1/RFDevice:hasCapability
  ...
  let $GRAPH_8 :=  
    ...
    for $node_Entity_0 in $node_MultiSolid_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_MultiSolid_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_8,$GRAPH_9,("MultiSolid_0"))
  ...
  let $GRAPH_12 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $doc_ObjectDescription[@rdf:ID = "AngleUnit_Degree"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
