...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicSupportSystem_0 := fn:data($node_ElectronicSupportSystem_0/@rdf:about)
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_ElectronicSupportSystem_0/Nuvio:aggregateOf
    ...
    where fn:exists($node_ElectronicSupportSystem_0) and fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicSupportSystem_0">{$value_ElectronicSupportSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#ChokeRingAntenna_instance5"]/Nuvio:aggregateOf
    let $value_InstantaneousFrequencyMeasurementReceiver_0 := fn:data($node_InstantaneousFrequencyMeasurementReceiver_0/@rdf:resource)
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("InstantaneousFrequencyMeasurementReceiver_0"))
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("ElectronicSupportSystem_0"))
  return
    $GRAPH_4
return
