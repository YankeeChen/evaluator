...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:about)
    for $node_Processor_0 in $node_USRPE310_0/Nuvio:aggregateOf
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:resource)
    where fn:exists($value_Processor_0) and fn:exists($node_USRPE310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Processor_0 in $node_SuperheterodyneReceiver_0/Nuvio:aggregateOf
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:resource)
    where fn:exists($value_Processor_0) and fn:exists($node_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="Processor_0">{$value_Processor_0}</xqllib:var>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("SuperheterodyneReceiver_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Processor_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Processor_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_0,$GRAPH_7,("USRPE310_0"))
  return
    $GRAPH_8
return
