...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:about)
    for $node_Entity_0 in $node_ElectronicAttackSystem_0/Nuvio:aggregateOf
  ...
  let $GRAPH_10 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Object"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_AntennaPolarization_0 in $node_Var1/RFDevice:hasPolarization
    let $value_AntennaPolarization_0 := fn:data($node_AntennaPolarization_0/@rdf:resource)
    where fn:exists($value_AntennaPolarization_0) and fn:exists($node_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_10,$GRAPH_11,("AntennaPolarization_0"))
  let $GRAPH_13 :=  
  ...
  let $GRAPH_15 := xqllib:optional($GRAPH_9,$GRAPH_14,("Var1"))
  let $GRAPH_16 := xqllib:and($GRAPH_8,$GRAPH_15,("InformationEntity_0"))
