...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_Height_0/RFDevice:hasEffectiveRadiatedPower
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_Height_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Height_0 in $node_Var1/Nuvio:hasValue
    let $value_Height_0 := fn:data($node_Height_0/@rdf:resource)
    where fn:exists($value_Height_0) and fn:exists($node_Var1)
    return
      <xqllib:result>
        <xqllib:var name="Height_0">{$value_Height_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Var1 in $node_SpatialLinearUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_SpatialLinearUnit_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("SpatialLinearUnit_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("Var1"))
  let $GRAPH_9 := xqllib:and($GRAPH_3,$GRAPH_8,("Height_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("Height_0"))
