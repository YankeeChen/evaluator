...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SDR_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_SDR_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_IEDTrigger_0 in $doc_ObjectDescription
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:about)
    for $node_ChokeRingAntenna_0 in $node_IEDTrigger_0/Nuvio:aggregateOf
    let $value_ChokeRingAntenna_0 := fn:data($node_ChokeRingAntenna_0/@rdf:resource)
    where fn:exists($value_ChokeRingAntenna_0) and fn:exists($node_IEDTrigger_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("IEDTrigger_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("SDR_0"))
  return
    $GRAPH_7
return
