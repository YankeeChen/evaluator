...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CR_0 := fn:data($node_CR_0/@rdf:about)
    for $node_Var0 in $node_CR_0/RFDevice:hasCapability
  ...
  let $GRAPH_5 :=  
    for $node_InformationEntity_0 in $doc_ObjectDescription
    let $value_InformationEntity_0 := fn:data($node_InformationEntity_0/@rdf:about)
    for $node_USRPB200mini_0 in $node_InformationEntity_0/Nuvio:expresses
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($node_InformationEntity_0) and fn:exists($value_USRPB200mini_0)
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
  ...
  let $GRAPH_7 :=  
    ...
    for $node_USRPB200mini_0 in $node_Var1/Nuvio:partOf
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($value_USRPB200mini_0) and fn:exists($node_Var1)
    return
      <xqllib:result>
        <xqllib:var name="USRPB200mini_0">{$value_USRPB200mini_0}</xqllib:var>
  ...
  let $GRAPH_8 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_Var2 in $node_Var1/RFDevice:hasWidth
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("USRPB200mini_0"))
  let $GRAPH_11 := ($GRAPH_5,$GRAPH_10)
  let $GRAPH_12 := xqllib:and($GRAPH_3,$GRAPH_11,("InformationEntity_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_2,$GRAPH_12,("InformationContent_0"))
  let $GRAPH_14 :=  
