...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_RadarJamming_0/Nuvio:participatesIn
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_RadarJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_RadarJamming_0 in $node_Attribute_0/RFDevice:hasTransmitPowerRange
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("RadarJamming_0"))
