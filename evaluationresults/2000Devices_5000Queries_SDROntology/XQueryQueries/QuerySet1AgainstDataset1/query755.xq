...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Instance1319"]/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_7 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#USRPE310_instance33"]/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_12 := $GRAPH_11[(data(xqllib:var[@name="DataValue0"]) < -1889351618)]
  ...
  let $GRAPH_17 := xqllib:optional($GRAPH_0,$GRAPH_16,("USRPX3xx_0"))
  return
    $GRAPH_17
return
