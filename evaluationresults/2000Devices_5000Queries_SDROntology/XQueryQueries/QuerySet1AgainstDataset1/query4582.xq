...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:about)
    for $node_MobilePlatform_0 in $node_RadarWarningReceiver_0/RFDevice:isDeployedOn
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:resource)
    where fn:exists($value_MobilePlatform_0) and fn:exists($node_RadarWarningReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_MobilePlatform_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Moving_instance27"]/RFDevice:supportsSpectrumSensing
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:resource)
