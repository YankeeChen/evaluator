...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:about)
    for $node_Var0 in $node_USRPB200mini_i_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var1 in $node_Var0/RFDevice:hasMaxOutputPower
  ...
  let $GRAPH_5 :=  
    for $node_Var0 in $doc_ObjectDescription[RFDevice:hasPulseRepetitionFrequency/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Instance1799"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
