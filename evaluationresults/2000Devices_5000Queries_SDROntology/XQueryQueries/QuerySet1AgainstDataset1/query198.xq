...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:about)
    for $node_Var0 in $node_USRPX300_0/RFDevice:supportsTuning
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Var0) and fn:exists($node_Velocity_0) and $node_Velocity_0/RFDevice:hasEffectiveRadiatedPower/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("Velocity_0"))
