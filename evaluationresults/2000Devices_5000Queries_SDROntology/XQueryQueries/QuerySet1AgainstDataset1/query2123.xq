...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Power_15dBm"]/RFDevice:hasMaxOutputPower
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_11 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $doc_ObjectDescription[@rdf:ID = "MilliampereHour"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_13 := $GRAPH_12[(data(xqllib:var[@name="DataValue0"]) <= -135991397)]
  ...
  let $GRAPH_21 := xqllib:and($GRAPH_2,$GRAPH_20,("Entity_0","InformationContent_0"))
  let $GRAPH_22 := xqllib:and($GRAPH_1,$GRAPH_21,("Entity_0"))
  let $GRAPH_23 := xqllib:optional($GRAPH_0,$GRAPH_22,("Half_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_23
return
