...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseRadar_0 := fn:data($node_PulseRadar_0/@rdf:about)
    for $node_Process_0 in $node_PulseRadar_0/RFDevice:supportsSpectrumSensing
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($value_Process_0) and fn:exists($node_PulseRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Process_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Process" and RFDevice:hasPulseWidth/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Frequency_instance206"]
