...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:about)
    for $node_Var0 in $node_USRPX310_0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_PulseDopplerRadarModulation_0/RFDevice:supportsJamming
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_PulseDopplerRadarModulation_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
