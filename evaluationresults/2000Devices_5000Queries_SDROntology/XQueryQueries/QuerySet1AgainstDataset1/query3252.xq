...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:about)
    for $node_Var0 in $node_USRPB2xx_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_2 :=  
    ...
    for $node_InformationRepresentation_0 in $node_Var0/Nuvio:isExpressedBy
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#IEDJammer"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_5 := $GRAPH_4[not((data(xqllib:var[@name="DataValue0"]) <= 1.57))]
  let $GRAPH_6 :=  
  ...
  let $GRAPH_12 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Power_instance35"]/RFDevice:hasPulseWidth
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
