...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_Var0 in $node_SurveillanceRadar_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.w3.org/2004/02/skos/core#Collection"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_Var0) and fn:exists($node_Var1) and $node_Var1/skos:member/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Var1 in $node_FrequencyScanRate_0/RFDevice:hasBeamwidth
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_FrequencyScanRate_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("FrequencyScanRate_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Var1"))
  let $GRAPH_8 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#VelocityUnit"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:optional($GRAPH_7,$GRAPH_8,("Var0"))
  let $GRAPH_10 :=  
  ...
  let $GRAPH_19 := xqllib:optional($GRAPH_12,$GRAPH_18,("Var0"))
  let $GRAPH_20 := xqllib:and($GRAPH_1,$GRAPH_19,("Var0"))
  let $GRAPH_21 := xqllib:and($GRAPH_0,$GRAPH_20,("SurveillanceRadar_0"))
  return
    $GRAPH_21
return
