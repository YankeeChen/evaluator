...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB205mini_i_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) != xs:double("-65.1387"))]
  let $GRAPH_7 := xqllib:and($GRAPH_4,$GRAPH_6,("Entity_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("Entity_0"))
  let $GRAPH_9 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#Volt"]/Nuvio:hasUnitOfMeasure
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue1"]) <= -414062275)]
  ...
  let $GRAPH_16 := xqllib:optional($GRAPH_2,$GRAPH_15,("Entity_0","InformationContent_0"))
