...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_GNURadio_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:resource)
    where fn:exists($value_GNURadio_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
        <xqllib:var name="GNURadio_0">{$value_GNURadio_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_RadioComponent_0 in $node_Mixer_0/Nuvio:compositeOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($node_Mixer_0) and fn:exists($value_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_RadioComponent_1 in $doc_ObjectDescription
    let $value_RadioComponent_1 := fn:data($node_RadioComponent_1/@rdf:about)
    for $node_Mixer_0 in $node_RadioComponent_1/Nuvio:aggregateOf
    let $value_Mixer_0 := fn:data($node_Mixer_0/@rdf:resource)
    where fn:exists($value_Mixer_0) and fn:exists($node_RadioComponent_1)
    return
      <xqllib:result>
        <xqllib:var name="Mixer_0">{$value_Mixer_0}</xqllib:var>
        <xqllib:var name="RadioComponent_1">{$value_RadioComponent_1}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Mixer_0"))
