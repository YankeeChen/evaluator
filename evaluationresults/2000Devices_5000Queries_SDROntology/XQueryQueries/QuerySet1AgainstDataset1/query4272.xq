...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Var0 in $node_XilinxZynq7020SoC_0/RFDevice:hasTarget
  ...
  let $GRAPH_5 :=  
    for $node_XilinxZynq7020SoC_0 in $doc_ObjectDescription
    let $value_XilinxZynq7020SoC_0 := fn:data($node_XilinxZynq7020SoC_0/@rdf:about)
    for $node_ARMProcessor_0 in $node_XilinxZynq7020SoC_0/Nuvio:aggregateOf
    let $value_ARMProcessor_0 := fn:data($node_ARMProcessor_0/@rdf:resource)
    where fn:exists($value_ARMProcessor_0) and fn:exists($node_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 :=  
    ...
    for $node_ARMProcessor_0 in $node_Entity_0/RFDevice:isProducerOf
    let $value_ARMProcessor_0 := fn:data($node_ARMProcessor_0/@rdf:resource)
    where fn:exists($value_ARMProcessor_0) and fn:exists($node_Entity_0)
    return
      <xqllib:result>
        <xqllib:var name="ARMProcessor_0">{$value_ARMProcessor_0}</xqllib:var>
  ...
  let $GRAPH_10 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#LatitudeValue_instance0"]/Nuvio:hasValue
  ...
  let $GRAPH_14 := xqllib:and($GRAPH_6,$GRAPH_13,("ARMProcessor_0"))
  let $GRAPH_15 := xqllib:and($GRAPH_5,$GRAPH_14,("ARMProcessor_0"))
  let $GRAPH_16 := xqllib:optional($GRAPH_4,$GRAPH_15,("XilinxZynq7020SoC_0"))
  let $GRAPH_17 := xqllib:and($GRAPH_3,$GRAPH_16,("XilinxZynq7020SoC_0"))
  let $GRAPH_18 := xqllib:and($GRAPH_2,$GRAPH_17,("Var0"))
  let $GRAPH_19 := xqllib:and($GRAPH_1,$GRAPH_18,("Var0"))
  let $GRAPH_20 := xqllib:optional($GRAPH_0,$GRAPH_19,("USRPE312_0"))
  return
    $GRAPH_20
return
