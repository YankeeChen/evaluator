...
let $result :=  
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) >= 1309879687)]
  let $GRAPH_9 := ($GRAPH_6,$GRAPH_8)
  let $GRAPH_11 :=  
    for $node_Entity_0 in $doc_ObjectDescription[RFDevice:hasCapability/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Instance2766"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
