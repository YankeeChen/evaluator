...
let $result :=  
  ...
  let $GRAPH_8 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "PowerUnit_instance2"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_14 :=  
    ...
    for $node_Var0 in $node_Var2/RFDevice:hasHeight
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
