...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_RadioComponent_0/Nuvio:compositeOf
  ...
  let $GRAPH_4 :=  
    ...
    for $node_ADC_0 in $node_Producer_0/RFDevice:isProducerOf
    let $value_ADC_0 := fn:data($node_ADC_0/@rdf:resource)
    where fn:exists($value_ADC_0) and fn:exists($node_Producer_0)
    return
      <xqllib:result>
        <xqllib:var name="ADC_0">{$value_ADC_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_0,$GRAPH_9,("USRPB2xx_0"))
  return
    $GRAPH_10
return
