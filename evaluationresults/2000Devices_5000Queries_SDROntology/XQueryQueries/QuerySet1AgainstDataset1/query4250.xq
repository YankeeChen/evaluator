...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN3xx_0 := fn:data($node_USRPN3xx_0/@rdf:about)
    for $node_Entity_0 in $node_USRPN3xx_0/Nuvio:compositeOf
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Entity_0 in $node_USRPE312_0/RFDevice:hasTarget
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_USRPE312_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_8 :=  
    for $node_USRPE312_0 in $doc_ObjectDescription
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:about)
    for $node_Var0 in $node_USRPE312_0/RFDevice:hasHeight
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("USRPE312_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("USRPE312_0"))
