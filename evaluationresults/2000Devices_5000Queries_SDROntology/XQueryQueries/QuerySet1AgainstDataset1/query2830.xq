...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB205mini_i_0 in $node_Var0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_Modulator_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Modulator_0 in $node_Transmitter_0/Nuvio:compositeOf
    let $value_Modulator_0 := fn:data($node_Modulator_0/@rdf:resource)
    where fn:exists($value_Modulator_0) and fn:exists($node_Transmitter_0)
    return
      <xqllib:result>
        <xqllib:var name="Modulator_0">{$value_Modulator_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Transmitter_0 in $node_RadarFrequencyBand_0/RFDevice:hasFrequencyAccuracy
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:resource)
    where fn:exists($node_RadarFrequencyBand_0) and fn:exists($value_Transmitter_0)
    return
      <xqllib:result>
        <xqllib:var name="RadarFrequencyBand_0">{$value_RadarFrequencyBand_0}</xqllib:var>
        <xqllib:var name="Transmitter_0">{$value_Transmitter_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Transmitter_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Transmitter_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Modulator_0"))
  let $GRAPH_9 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_Var0 in $node_Var1/RFDevice:hasVoltage
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
