...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_Entity_0 in $node_SurveillanceRadar_0/RFDevice:hasProducer
  ...
  let $GRAPH_9 :=  
    for $node_Entity_0 in $doc_ObjectDescription[Nuvio:participatesIn/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#RadioFrequencyBand_instance42"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_16 := xqllib:optional($GRAPH_0,$GRAPH_15,("SurveillanceRadar_0"))
  return
    $GRAPH_16
return
