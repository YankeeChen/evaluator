...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_Entity_0/Nuvio:compositeOf
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) = -61875235)]
  let $GRAPH_7 := xqllib:and($GRAPH_4,$GRAPH_6,("Entity_0"))
  let $GRAPH_8 := ($GRAPH_3,$GRAPH_7)
  let $GRAPH_9 :=  
    for $node_Process_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Process"]
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_Process_0) and $doc_ObjectDescription[@rdf:ID = "ContinuousWaveRadarModulation_instance25"]/Nuvio:hasSubprocess/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_12 := xqllib:optional($GRAPH_0,$GRAPH_11,("SurveillanceRadar_0"))
  return
    $GRAPH_12
return
