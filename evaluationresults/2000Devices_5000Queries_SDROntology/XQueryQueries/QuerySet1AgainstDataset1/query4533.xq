...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $node_Entity_0/RFDevice:hasFrequencyRange/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#USRPN2xx_instance2"]/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_8 :=  
    ...
    for $node_USRPN3xx_0 in $node_Var0/RFDevice:hasHeight
  ...
  let $GRAPH_9 := xqllib:optional($GRAPH_7,$GRAPH_8,("USRPN3xx_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("USRPN3xx_0"))
