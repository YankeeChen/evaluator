...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_Entity_1/RFDevice:hasNoiseFigure
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_7 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Situation" and Nuvio:relevantIndividual/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#USRPX300_instance21"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_13 := xqllib:optional($GRAPH_11,$GRAPH_12,("Attribute_0"))
  let $GRAPH_14 := xqllib:and($GRAPH_10,$GRAPH_13,("Process_0"))
