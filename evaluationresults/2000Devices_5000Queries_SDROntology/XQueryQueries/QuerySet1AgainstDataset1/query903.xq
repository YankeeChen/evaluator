...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Triangle_0) and $node_Triangle_0/Nuvio:isExpressedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_0,$GRAPH_6,("USRPX300_0"))
  return
    $GRAPH_7
return
