...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:about)
    for $node_BarrageJamming_0 in $node_JammingSystem_0/RFDevice:supportsTransmitting
    let $value_BarrageJamming_0 := fn:data($node_BarrageJamming_0/@rdf:resource)
    where fn:exists($value_BarrageJamming_0) and fn:exists($node_JammingSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_BarrageJamming_0 in $node_Quantity_0/RFDevice:hasTransmitPowerRange
    let $value_BarrageJamming_0 := fn:data($node_BarrageJamming_0/@rdf:resource)
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("BarrageJamming_0"))
