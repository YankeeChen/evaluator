...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:about)
    for $node_Object_0 in $node_CommunicationsSystem_0/RFDevice:hasProducer
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($node_CommunicationsSystem_0) and fn:exists($value_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="CommunicationsSystem_0">{$value_CommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Gain_0) and fn:exists($node_Object_0) and $node_Gain_0/RFDevice:hasHeight/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) <= 58.4)]
  let $GRAPH_6 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#Decibel"]/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("Gain_0"))
