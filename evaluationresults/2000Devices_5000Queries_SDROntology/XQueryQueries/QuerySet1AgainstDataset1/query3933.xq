...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_PowerUnit_0 in $node_Entity_0/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("PowerUnit_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Entity_0"))
  let $GRAPH_7 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Platform_0 in $node_Entity_0/Nuvio:participatesIn
    let $value_Platform_0 := fn:data($node_Platform_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_Platform_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
