...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_Attribute_0/RFDevice:hasChargingTime
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_ElectronicAttackSystem_0)
    return
      <xqllib:result>
