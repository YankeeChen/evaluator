...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_USRPB200mini_0 in $doc_ObjectDescription[RFDevice:supportsJamming/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#USRP_SpectrumSensing_Instance0"]
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:about)
    where fn:exists($node_USRPB200mini_0)
    return
      <xqllib:result>
