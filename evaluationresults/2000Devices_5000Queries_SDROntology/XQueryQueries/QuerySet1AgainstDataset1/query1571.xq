...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_Var0/RFDevice:supportsTuning
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Transceiving_0) and fn:exists($node_Var0) and $node_Transceiving_0/Nuvio:subprocessOf/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Transceiving_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#RadarJamming"]
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:about)
  ...
  let $GRAPH_5 :=  
    for $node_Transceiving_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#PulseRadar_instance16"]/Nuvio:participatesIn
  ...
  let $GRAPH_7 :=  
    for $node_Transceiving_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#RadarBand_X"]/RFDevice:hasFrequencyAccuracy
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:resource)
    where fn:exists($value_Transceiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_6,$GRAPH_7,("Transceiving_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("Transceiving_0"))
