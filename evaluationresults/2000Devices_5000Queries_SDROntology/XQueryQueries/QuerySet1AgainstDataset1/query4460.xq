...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_BistaticRadar_0 in $node_Process_0/RFDevice:supportsSpectrumSensing
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:resource)
    where fn:exists($value_BistaticRadar_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
        <xqllib:var name="BistaticRadar_0">{$value_BistaticRadar_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_Process_0 in $doc_ObjectDescription
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_Frequency_0 in $node_Process_0/RFDevice:hasPulseWidth
    let $value_Frequency_0 := fn:data($node_Frequency_0/@rdf:resource)
    where fn:exists($value_Frequency_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_0,$GRAPH_6,("BistaticRadar_0"))
  return
    $GRAPH_7
return
