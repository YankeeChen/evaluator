...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB205mini_i_0 in $node_XilinxFPGA_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_XilinxFPGA_0 in $node_Artix7FPGA_0/Nuvio:aggregateOf
    let $value_XilinxFPGA_0 := fn:data($node_XilinxFPGA_0/@rdf:resource)
    where fn:exists($node_Artix7FPGA_0) and fn:exists($value_XilinxFPGA_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_0,$GRAPH_5,("USRPB205mini_i_0"))
  return
    $GRAPH_6
return
