...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200_210_0 := fn:data($node_USRPB200_210_0/@rdf:about)
    for $node_Entity_0 in $node_USRPB200_210_0/RFDevice:hasMaxPowerConsumption
  ...
  let $GRAPH_6 :=  
    ...
    where fn:exists($node_Process_0) and $doc_ObjectDescription[@rdf:ID = "Power_instance33"]/RFDevice:hasMaxOutputPower/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := xqllib:optional($GRAPH_5,$GRAPH_6,("Process_0"))
