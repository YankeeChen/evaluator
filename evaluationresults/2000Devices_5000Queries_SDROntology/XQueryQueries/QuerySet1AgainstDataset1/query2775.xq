...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_SelfProtectionJamming_0 in $node_Var0/Nuvio:subprocessOf
    let $value_SelfProtectionJamming_0 := fn:data($node_SelfProtectionJamming_0/@rdf:resource)
    where fn:exists($value_SelfProtectionJamming_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_SelfProtectionJamming_0 in $node_Power_0/RFDevice:hasPulseRepetitionFrequency
    let $value_SelfProtectionJamming_0 := fn:data($node_SelfProtectionJamming_0/@rdf:resource)
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_2,$GRAPH_6,("SelfProtectionJamming_0"))
