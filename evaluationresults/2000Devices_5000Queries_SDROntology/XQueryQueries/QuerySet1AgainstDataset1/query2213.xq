...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/sf#Point"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_7 :=  
    for $node_Situation_0 in $doc_ObjectDescription
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_OnOffKeyingModulation_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_OnOffKeyingModulation_0 := fn:data($node_OnOffKeyingModulation_0/@rdf:resource)
    where fn:exists($value_OnOffKeyingModulation_0) and fn:exists($node_Situation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 :=  
    for $node_Situation_0 in $doc_ObjectDescription[Nuvio:relevantIndividual/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Aircraft_instance14"]
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("Situation_0"))
  let $GRAPH_10 :=  
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_9,$GRAPH_10,("OnOffKeyingModulation_0"))
