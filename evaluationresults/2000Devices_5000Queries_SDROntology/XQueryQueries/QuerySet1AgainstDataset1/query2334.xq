...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_Var0/RFDevice:supportsTuning
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_StandoffJamming_0) and fn:exists($node_Var0) and $node_StandoffJamming_0/Nuvio:hasSubprocess/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_StandoffJamming_0 in $node_Entity_0/Nuvio:hasProcessQuantity
    let $value_StandoffJamming_0 := fn:data($node_StandoffJamming_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_StandoffJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("ElectronicSupportSystem_0"))
  return
    $GRAPH_7
return
