...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseDopplerRadar_0 in $node_Object_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_2 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Situation_0 in $node_Object_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_OffsetCurve_0) and fn:exists($node_Situation_0) and $node_OffsetCurve_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Situation_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#IEDJammer_instance56"]/Nuvio:relevantIndividual
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:resource)
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("Situation_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Situation_0"))
