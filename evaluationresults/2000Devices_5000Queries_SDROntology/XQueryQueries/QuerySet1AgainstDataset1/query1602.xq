...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:about)
    for $node_PulseDopplerRadarModulation_0 in $node_RadarSystem_0/RFDevice:supportsSpectrumSensing
    let $value_PulseDopplerRadarModulation_0 := fn:data($node_PulseDopplerRadarModulation_0/@rdf:resource)
    where fn:exists($value_PulseDopplerRadarModulation_0) and fn:exists($node_RadarSystem_0)
    return
      <xqllib:result>
