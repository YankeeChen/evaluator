...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_IEDJammer_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_IEDJammer_0 in $node_FreqencyRange_0/RFDevice:hasFrequencyRange
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
    where fn:exists($node_FreqencyRange_0) and fn:exists($value_IEDJammer_0)
    return
      <xqllib:result>
