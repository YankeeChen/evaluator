...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE312_0 in $node_Var0/RFDevice:supportsJamming
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) != -32.55)]
  let $GRAPH_7 :=  
    for $node_UnitOfMeasure_0 in $doc_ObjectDescription
    let $value_UnitOfMeasure_0 := fn:data($node_UnitOfMeasure_0/@rdf:about)
    for $node_Var2 in $node_UnitOfMeasure_0/Nuvio:hasUnitOfMeasure
    let $value_Var2 := fn:data($node_Var2/@rdf:resource)
    where fn:exists($node_UnitOfMeasure_0) and fn:exists($value_Var2)
    return
      <xqllib:result>
        <xqllib:var name="UnitOfMeasure_0">{$value_UnitOfMeasure_0}</xqllib:var>
        <xqllib:var name="Var2">{$value_Var2}</xqllib:var>
      </xqllib:result>
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("Var2"))
  let $GRAPH_9 :=  
    for $node_Var2 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#LongValue"]
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_8,$GRAPH_9,("Var2"))
  let $GRAPH_11 := xqllib:and($GRAPH_4,$GRAPH_10,("Var2"))
  let $GRAPH_12 := xqllib:and($GRAPH_3,$GRAPH_11,("Var1"))
  let $GRAPH_13 := xqllib:and($GRAPH_2,$GRAPH_12,("Var1"))
  let $GRAPH_14 := xqllib:and($GRAPH_1,$GRAPH_13,("Var0"))
  let $GRAPH_15 := xqllib:and($GRAPH_0,$GRAPH_14,("USRPE312_0"))
  return
    $GRAPH_15
return
