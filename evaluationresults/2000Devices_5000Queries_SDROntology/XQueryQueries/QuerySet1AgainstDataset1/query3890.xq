...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:about)
    for $node_EmitterLocationDetection_0 in $node_RadarWarningReceiver_0/RFDevice:supportsJamming
    let $value_EmitterLocationDetection_0 := fn:data($node_EmitterLocationDetection_0/@rdf:resource)
    where fn:exists($value_EmitterLocationDetection_0) and fn:exists($node_RadarWarningReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#USRPE3xx"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("EmitterLocationDetection_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("EmitterLocationDetection_0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("RadarWarningReceiver_0"))
  return
    $GRAPH_8
return
