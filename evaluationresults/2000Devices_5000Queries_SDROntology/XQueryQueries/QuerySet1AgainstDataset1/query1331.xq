...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Full_DuplexCommunicationsSystem_0 in $node_Var0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) >= 67.37)]
  ...
  let $GRAPH_11 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_SpectrumSensing_0) and fn:exists($node_Var0) and $node_SpectrumSensing_0/Nuvio:expresses/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_12 :=  
    for $node_SpectrumSensing_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#FreqencyRange_instance35"]/RFDevice:hasBandwidthRange
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_11,$GRAPH_12,("SpectrumSensing_0"))
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_0,$GRAPH_17,("Full_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_18
return
