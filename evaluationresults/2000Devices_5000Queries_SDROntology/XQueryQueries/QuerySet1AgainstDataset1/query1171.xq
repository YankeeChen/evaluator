...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_OmnidirectionalAntenna_0 in $node_Var0/Nuvio:aggregateOf
    let $value_OmnidirectionalAntenna_0 := fn:data($node_OmnidirectionalAntenna_0/@rdf:resource)
    where fn:exists($value_OmnidirectionalAntenna_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Var2 in $node_Var0/RFDevice:hasGain
