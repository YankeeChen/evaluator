...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX300_0 in $node_Entity_0/RFDevice:hasProducer
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_Receiving_0/RFDevice:supportsJamming
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_Receiving_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Receiving_0"))
  ...
  let $GRAPH_14 := xqllib:and($GRAPH_0,$GRAPH_13,("USRPX300_0"))
  return
    $GRAPH_14
return
