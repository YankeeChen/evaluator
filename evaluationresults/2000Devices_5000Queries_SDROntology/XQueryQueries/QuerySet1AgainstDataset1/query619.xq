...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[RFDevice:hasPolarization/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#AntennaPolarization_instance20"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
