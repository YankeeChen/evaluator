...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_RadarJammer_0/Nuvio:compositeOf
    let $value_USRPN2xx_0 := fn:data($node_USRPN2xx_0/@rdf:resource)
    where fn:exists($node_RadarJammer_0) and fn:exists($value_USRPN2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_RadarJammer_0 in $node_ADC_0/Nuvio:partOf
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_ADC_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    for $node_ADC_0 in $doc_ObjectDescription
    let $value_ADC_0 := fn:data($node_ADC_0/@rdf:about)
    for $node_Attribute_0 in $node_ADC_0/RFDevice:hasResolution
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:resource)
    where fn:exists($node_ADC_0) and fn:exists($value_Attribute_0)
    return
      <xqllib:result>
        <xqllib:var name="ADC_0">{$value_ADC_0}</xqllib:var>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("ADC_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("ADC_0"))
  let $GRAPH_11 := ($GRAPH_3,$GRAPH_10)
  let $GRAPH_12 := xqllib:and($GRAPH_1,$GRAPH_11,("RadarJammer_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_0,$GRAPH_12,("USRPN2xx_0"))
  return
    $GRAPH_13
return
