...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_GNURadio_0 in $node_Weight_0/RFDevice:hasLevel
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:resource)
    where fn:exists($value_GNURadio_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="GNURadio_0">{$value_GNURadio_0}</xqllib:var>
  ...
  let $GRAPH_13 := $GRAPH_12[(data(xqllib:var[@name="DataValue0"]) <= -97.33)]
  ...
  let $GRAPH_19 := xqllib:optional($GRAPH_0,$GRAPH_18,("GNURadio_0"))
  return
    $GRAPH_19
return
