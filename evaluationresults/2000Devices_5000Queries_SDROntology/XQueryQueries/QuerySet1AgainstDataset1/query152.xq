...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_DataValue0 in $node_Entity_0/geosparql:isSimple
  ...
  let $GRAPH_3 := $GRAPH_2[(data(xqllib:var[@name="DataValue0"]) < fn:true())]
  let $GRAPH_5 :=  
  ...
  let $GRAPH_7 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_DataValue1 in $node_Entity_0/RFDevice:hasDoubleValue
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue1"]) = xs:double("-88.99553591319284"))]
