...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_CommunicationsSystem_0 in $doc_ObjectDescription[RFDevice:hasProducer/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Producer_instance326"]
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:about)
    where fn:exists($node_CommunicationsSystem_0)
    return
      <xqllib:result>
