...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_RadioComponent_0/Nuvio:partOf
    let $value_USRPN2xx_0 := fn:data($node_USRPN2xx_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_USRPN2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_RadioComponent_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#SuperheterodyneReceiver_instance52"]/Nuvio:compositeOf
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("USRPN2xx_0"))
  return
    $GRAPH_4
return
