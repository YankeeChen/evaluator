...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_MultiGeometry_0 in $node_Entity_0/geosparql:rcc8eq
    let $value_MultiGeometry_0 := fn:data($node_MultiGeometry_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_MultiGeometry_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("MultiGeometry_0"))
  let $GRAPH_6 :=  
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) != 1660633435)]
