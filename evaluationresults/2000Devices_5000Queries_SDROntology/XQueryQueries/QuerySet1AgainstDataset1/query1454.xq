...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_EnergyDetection_0 in $node_Object_0/Nuvio:participatesIn
    let $value_EnergyDetection_0 := fn:data($node_EnergyDetection_0/@rdf:resource)
    where fn:exists($value_EnergyDetection_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("EnergyDetection_0"))
