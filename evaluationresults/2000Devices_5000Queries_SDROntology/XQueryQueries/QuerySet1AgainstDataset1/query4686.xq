...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN200_0 := fn:data($node_USRPN200_0/@rdf:about)
    for $node_MobilePlatform_0 in $node_USRPN200_0/RFDevice:isDeployedOn
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:resource)
    where fn:exists($value_MobilePlatform_0) and fn:exists($node_USRPN200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_MobilePlatform_0 in $node_Entity_0/RFDevice:supportsTuning
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:resource)
