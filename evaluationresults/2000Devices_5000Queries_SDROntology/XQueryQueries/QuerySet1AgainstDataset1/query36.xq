...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicSupportSystem_0 := fn:data($node_ElectronicSupportSystem_0/@rdf:about)
    for $node_ElectronicWarfare_0 in $node_ElectronicSupportSystem_0/RFDevice:supportsJamming
    let $value_ElectronicWarfare_0 := fn:data($node_ElectronicWarfare_0/@rdf:resource)
    where fn:exists($node_ElectronicSupportSystem_0) and fn:exists($value_ElectronicWarfare_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicSupportSystem_0">{$value_ElectronicSupportSystem_0}</xqllib:var>
