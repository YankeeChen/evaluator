...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_Receiver_0 in $node_Half_DuplexCommunicationsSystem_0/Nuvio:aggregateOf
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:resource)
    where fn:exists($node_Half_DuplexCommunicationsSystem_0) and fn:exists($value_Receiver_0)
    return
      <xqllib:result>
        <xqllib:var name="Half_DuplexCommunicationsSystem_0">{$value_Half_DuplexCommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Receiver_0 in $node_Entity_0/RFDevice:hasFrequencyRange
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:resource)
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) < -1520510316)]
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_1,$GRAPH_11,("Receiver_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_0,$GRAPH_12,("Half_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_13
return
