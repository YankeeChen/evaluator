...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN300_0 := fn:data($node_USRPN300_0/@rdf:about)
    for $node_XilinxZynq7100SoC_0 in $node_USRPN300_0/Nuvio:aggregateOf
    let $value_XilinxZynq7100SoC_0 := fn:data($node_XilinxZynq7100SoC_0/@rdf:resource)
    where fn:exists($node_USRPN300_0) and fn:exists($value_XilinxZynq7100SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPN300_0">{$value_USRPN300_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_XilinxZynq7100SoC_0 in $node_Component_0/Nuvio:compositeOf
    let $value_XilinxZynq7100SoC_0 := fn:data($node_XilinxZynq7100SoC_0/@rdf:resource)
    where fn:exists($node_Component_0) and fn:exists($value_XilinxZynq7100SoC_0)
    return
      <xqllib:result>
