...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:about)
    for $node_Object_0 in $node_CommunicationsSystem_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_Quantity_0) and $node_Quantity_0/RFDevice:hasPolarization/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
