...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_GNURadio_0 in $node_IEDJammer_0/Nuvio:aggregateOf
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:resource)
    where fn:exists($value_GNURadio_0) and fn:exists($node_IEDJammer_0)
    return
      <xqllib:result>
        <xqllib:var name="GNURadio_0">{$value_GNURadio_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_IEDJammer_0 in $node_RadarJamming_0/RFDevice:supportsTransmitting
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
    where fn:exists($value_IEDJammer_0) and fn:exists($node_RadarJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="IEDJammer_0">{$value_IEDJammer_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_RadarJamming_0 in $node_Entity_0/RFDevice:hasPulseRepetitionInterval
    let $value_RadarJamming_0 := fn:data($node_RadarJamming_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_RadarJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $node_Entity_0/skos:broaderTransitive/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_10 :=  
    ...
    let $value_InformationEntity_0 := fn:data($node_InformationEntity_0/@rdf:about)
    where fn:exists($node_AmplitudeShiftKeyingModulation_0) and fn:exists($node_InformationEntity_0) and $node_AmplitudeShiftKeyingModulation_0/Nuvio:isExpressedBy/@rdf:resource = $value_InformationEntity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_12 := xqllib:optional($GRAPH_10,$GRAPH_11,("AmplitudeShiftKeyingModulation_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_9,$GRAPH_12,("InformationEntity_0"))
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_16,$GRAPH_17,("DimensionlessUnit_0"))
  let $GRAPH_19 := xqllib:and($GRAPH_15,$GRAPH_18,())
  let $GRAPH_20 := xqllib:and($GRAPH_6,$GRAPH_19,("Entity_0","Value_0"))
  let $GRAPH_21 := xqllib:and($GRAPH_5,$GRAPH_20,("Entity_0"))
  let $GRAPH_22 := xqllib:and($GRAPH_4,$GRAPH_21,("RadarJamming_0"))
  let $GRAPH_23 := xqllib:and($GRAPH_3,$GRAPH_22,("RadarJamming_0"))
  let $GRAPH_24 := xqllib:and($GRAPH_2,$GRAPH_23,("IEDJammer_0"))
  let $GRAPH_25 := xqllib:and($GRAPH_1,$GRAPH_24,("IEDJammer_0"))
