...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Object_0 in $node_Entity_0/skos:memberList
  ...
  let $GRAPH_11 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_PolynomialSpline_0) and $node_PolynomialSpline_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_15,$GRAPH_16,("PolynomialSpline_0"))
