...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Var0 in $node_FeatureDetection_0/RFDevice:supportsJamming
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_FeatureDetection_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("FeatureDetection_0"))
  let $GRAPH_6 :=  
