...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xxmini_0 in $node_Spartan6FPGA_0/Nuvio:compositeOf
    let $value_USRPB2xxmini_0 := fn:data($node_USRPB2xxmini_0/@rdf:resource)
    where fn:exists($node_Spartan6FPGA_0) and fn:exists($value_USRPB2xxmini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Spartan6FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#InstantaneousFrequencyMeasurementReceiver_instance37"]/Nuvio:partOf
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("Spartan6FPGA_0"))
