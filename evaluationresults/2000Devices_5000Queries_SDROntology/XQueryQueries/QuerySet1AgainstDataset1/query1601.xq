...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_Object_0/RFDevice:isProducerOf
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_Distance_0) and fn:exists($node_Object_0) and $node_Distance_0/RFDevice:hasVoltage/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) = -876411189)]
  let $GRAPH_7 := xqllib:and($GRAPH_4,$GRAPH_6,("Var0"))
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("Distance_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("Distance_0"))
