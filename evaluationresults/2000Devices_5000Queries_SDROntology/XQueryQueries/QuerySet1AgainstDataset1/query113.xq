...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:about)
    for $node_Object_0 in $node_DirectConversionReceiver_0/RFDevice:hasFrequencyRange
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="DirectConversionReceiver_0">{$value_DirectConversionReceiver_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Object_0 in $node_CommunicationsJamming_0/RFDevice:supportsTransmitting
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("CommunicationsJamming_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Object_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("DirectConversionReceiver_0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("USRPX3xx_0"))
  return
    $GRAPH_8
return
