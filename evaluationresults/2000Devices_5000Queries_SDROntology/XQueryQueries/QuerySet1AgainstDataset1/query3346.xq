...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseRadar_0 := fn:data($node_PulseRadar_0/@rdf:about)
    for $node_ContinuousWaveRadarModulation_0 in $node_PulseRadar_0/RFDevice:supportsSpectrumSensing
    let $value_ContinuousWaveRadarModulation_0 := fn:data($node_ContinuousWaveRadarModulation_0/@rdf:resource)
    where fn:exists($value_ContinuousWaveRadarModulation_0) and fn:exists($node_PulseRadar_0)
    return
      <xqllib:result>
