...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE313_0 := fn:data($node_USRPE313_0/@rdf:about)
    for $node_Jammer_0 in $node_USRPE313_0/Nuvio:compositeOf
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($value_Jammer_0) and fn:exists($node_USRPE313_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Jammer_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#FormationJamming_instance4"]/RFDevice:hasCapability
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
