...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_DAC_0/Nuvio:aggregateOf
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($node_DAC_0) and fn:exists($value_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := $GRAPH_9[(data(xqllib:var[@name="DataValue0"]) >= 1228598002)]
  let $GRAPH_11 := xqllib:and($GRAPH_8,$GRAPH_10,("AngleValue_0"))
  ...
  let $GRAPH_18 := xqllib:optional($GRAPH_0,$GRAPH_17,("USRPX310_0"))
  return
    $GRAPH_18
return
