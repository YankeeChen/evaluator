...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseRadar_0 := fn:data($node_PulseRadar_0/@rdf:about)
    for $node_Entity_0 in $node_PulseRadar_0/RFDevice:hasWeight
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_AntennaPolarization_0) and fn:exists($node_Entity_0) and $node_AntennaPolarization_0/Nuvio:hasObjectQuality/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
