...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN300_0 := fn:data($node_USRPN300_0/@rdf:about)
    for $node_MobilePlatform_0 in $node_USRPN300_0/RFDevice:isDeployedOn
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:resource)
    where fn:exists($value_MobilePlatform_0) and fn:exists($node_USRPN300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_MobilePlatform_0 in $doc_ObjectDescription[RFDevice:supportsSpectrumSensing/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Moving_instance9"]
