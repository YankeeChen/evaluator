...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:about)
    for $node_Jammer_0 in $node_TrackingRadar_0/Nuvio:partOf
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($value_Jammer_0) and fn:exists($node_TrackingRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Jammer_0 in $node_SweepJamming_0/RFDevice:supportsSpectrumSensing
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("SweepJamming_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("Jammer_0"))
