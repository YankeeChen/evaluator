...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_GNURadio_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:resource)
    where fn:exists($value_GNURadio_0) and fn:exists($node_IEDTrigger_0)
    return
      <xqllib:result>
        <xqllib:var name="GNURadio_0">{$value_GNURadio_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_IEDTrigger_0 in $node_FreqencyRange_0/RFDevice:hasFrequencyRange
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_3,$GRAPH_12,("FreqencyRange_0"))
  let $GRAPH_14 := xqllib:and($GRAPH_2,$GRAPH_13,("FreqencyRange_0"))
