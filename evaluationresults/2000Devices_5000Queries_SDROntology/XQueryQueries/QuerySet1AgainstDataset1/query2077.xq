...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:about)
    for $node_IEDJammer_0 in $node_USRPB200_0/Nuvio:aggregateOf
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
    where fn:exists($value_IEDJammer_0) and fn:exists($node_USRPB200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_IEDJammer_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Instance853"]/RFDevice:supportsTransmitting
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_0,$GRAPH_5,("USRPB200_0"))
  return
    $GRAPH_6
return
