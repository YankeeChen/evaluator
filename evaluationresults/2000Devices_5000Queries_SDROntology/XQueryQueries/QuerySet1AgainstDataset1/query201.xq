...
let $result :=  
  ...
  let $GRAPH_3 := $GRAPH_2[(data(xqllib:var[@name="DataValue0"]) <= 1.57)]
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_InboundRangeGatePullOffJamming_0) and $node_InboundRangeGatePullOffJamming_0/Nuvio:hasSubprocess/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
