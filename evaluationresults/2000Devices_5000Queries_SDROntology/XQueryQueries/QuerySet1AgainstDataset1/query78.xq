...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_USRPB2xx_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#USRP_SpectrumSensing_Instance0"]/RFDevice:supportsSpectrumSensing
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:resource)
    where fn:exists($value_USRPB2xx_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB2xx_0">{$value_USRPB2xx_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_2 := xqllib:optional($GRAPH_0,$GRAPH_1,("USRPB2xx_0"))
  return
    $GRAPH_2
return
