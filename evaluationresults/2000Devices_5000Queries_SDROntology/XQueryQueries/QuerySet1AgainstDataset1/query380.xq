...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:about)
    for $node_PulseRadarModulation_0 in $node_BistaticRadar_0/RFDevice:supportsJamming
  ...
  let $GRAPH_2 :=  
    for $node_PulseRadarModulation_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#TimeInterval_instance0"]/RFDevice:hasEffectiveRadiatedPower
    let $value_PulseRadarModulation_0 := fn:data($node_PulseRadarModulation_0/@rdf:resource)
