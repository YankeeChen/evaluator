...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Voltage_0 in $node_Object_0/RFDevice:hasWidth
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Var0 in $node_Voltage_0/Nuvio:hasValue
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) >= -1023328791)]
  let $GRAPH_8 :=  
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_2,$GRAPH_11,("Voltage_0"))
