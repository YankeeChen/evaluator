...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:about)
    for $node_Satellite_0 in $node_SDR_0/RFDevice:isDeployedOn
    let $value_Satellite_0 := fn:data($node_Satellite_0/@rdf:resource)
    where fn:exists($node_SDR_0) and fn:exists($value_Satellite_0)
    return
      <xqllib:result>
        <xqllib:var name="SDR_0">{$value_SDR_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("Satellite_0"))
