...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_0 in $node_RadarJammer_0/Nuvio:aggregateOf
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:resource)
    where fn:exists($node_RadarJammer_0) and fn:exists($value_USRPB200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_RadioFrequencyBand_0 in $doc_ObjectDescription
    let $value_RadioFrequencyBand_0 := fn:data($node_RadioFrequencyBand_0/@rdf:about)
    for $node_Quality_0 in $node_RadioFrequencyBand_0/RFDevice:hasMax
    let $value_Quality_0 := fn:data($node_Quality_0/@rdf:resource)
    where fn:exists($value_Quality_0) and fn:exists($node_RadioFrequencyBand_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("RadioFrequencyBand_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("RadarJammer_0"))
  let $GRAPH_11 := xqllib:optional($GRAPH_0,$GRAPH_10,("USRPB200_0"))
  return
    $GRAPH_11
return
