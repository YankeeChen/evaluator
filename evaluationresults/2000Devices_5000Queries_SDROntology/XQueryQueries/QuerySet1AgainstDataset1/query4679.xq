...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Object_0 in $node_Detection_0/RFDevice:supportsTransmitting
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($node_Detection_0) and fn:exists($value_Object_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("Detection_0"))
