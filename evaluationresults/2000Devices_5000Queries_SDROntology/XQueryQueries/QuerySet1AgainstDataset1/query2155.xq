...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_CommunicationsJamming_0/RFDevice:hasCapability
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:resource)
    where fn:exists($node_CommunicationsJamming_0) and fn:exists($value_JammingSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_CommunicationsJamming_0 in $node_Power_0/RFDevice:hasPulseWidth
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) < -6.68)]
  let $GRAPH_8 := xqllib:and($GRAPH_5,$GRAPH_7,("Var0"))
  let $GRAPH_9 := xqllib:and($GRAPH_4,$GRAPH_8,("Power_0"))
  let $GRAPH_10 := ($GRAPH_3,$GRAPH_9)
  let $GRAPH_11 := xqllib:and($GRAPH_1,$GRAPH_10,("CommunicationsJamming_0"))
