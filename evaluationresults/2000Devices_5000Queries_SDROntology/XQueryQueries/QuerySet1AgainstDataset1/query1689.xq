...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_MultiLineString_0 in $node_Entity_0/Nuvio:expresses
    let $value_MultiLineString_0 := fn:data($node_MultiLineString_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_MultiLineString_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
