...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:about)
    for $node_Entity_0 in $node_SearchRadar_0/Nuvio:hasObjectQuantity
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/geosparql#Geometry"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_DataValue0 in $node_Entity_0/geosparql:coordinateDimension
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) <= -1829248940)]
  let $GRAPH_7 :=  
  ...
  let $GRAPH_9 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_DataValue1 in $node_Entity_0/RFDevice:hasPositiveIntegerValue
  ...
  let $GRAPH_10 := $GRAPH_9[(data(xqllib:var[@name="DataValue1"]) < 202431260)]
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_0,$GRAPH_17,("SearchRadar_0"))
  return
    $GRAPH_18
return
