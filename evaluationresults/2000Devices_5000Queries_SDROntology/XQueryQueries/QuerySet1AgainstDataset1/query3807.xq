...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:about)
    for $node_XC6SLX150FPGA_0 in $node_USRPB200mini_i_0/Nuvio:partOf
    let $value_XC6SLX150FPGA_0 := fn:data($node_XC6SLX150FPGA_0/@rdf:resource)
    where fn:exists($node_USRPB200mini_i_0) and fn:exists($value_XC6SLX150FPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB200mini_i_0">{$value_USRPB200mini_i_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_DirectConversionReceiver_0 in $node_XC6SLX150FPGA_0/Nuvio:compositeOf
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($value_DirectConversionReceiver_0) and fn:exists($node_XC6SLX150FPGA_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("DirectConversionReceiver_0"))
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("XC6SLX150FPGA_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_0,$GRAPH_5,("USRPB200mini_i_0"))
  return
    $GRAPH_6
return
