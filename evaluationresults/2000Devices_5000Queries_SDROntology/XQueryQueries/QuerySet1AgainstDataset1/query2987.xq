...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_PulseRadarModulation_0 in $node_Object_0/Nuvio:hasParticipant
    let $value_PulseRadarModulation_0 := fn:data($node_PulseRadarModulation_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_PulseRadarModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_PulseRadarModulation_0 in $node_Attribute_0/RFDevice:hasPulseRepetitionInterval
    let $value_PulseRadarModulation_0 := fn:data($node_PulseRadarModulation_0/@rdf:resource)
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("USRPB2xx_0"))
  return
    $GRAPH_7
return
