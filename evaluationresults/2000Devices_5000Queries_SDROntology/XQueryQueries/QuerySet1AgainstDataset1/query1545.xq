...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_GNURadio_0 in $node_Var0/RFDevice:supportsJamming
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_TimeInterval_0) and fn:exists($node_Var0) and $node_TimeInterval_0/RFDevice:hasHeight/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := $GRAPH_6[not((data(xqllib:var[@name="DataValue0"]) > xs:decimal("0")))]
  let $GRAPH_8 := xqllib:and($GRAPH_5,$GRAPH_7,("Var1"))
  let $GRAPH_9 := xqllib:and($GRAPH_4,$GRAPH_8,("TimeInterval_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_3,$GRAPH_9,("TimeInterval_0"))
