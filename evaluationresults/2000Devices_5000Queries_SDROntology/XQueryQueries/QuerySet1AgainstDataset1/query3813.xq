...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:about)
    for $node_RadioComponent_0 in $node_SDR_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadioComponent_0 in $node_XilinxZynq7100SoC_0/Nuvio:aggregateOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($value_RadioComponent_0) and fn:exists($node_XilinxZynq7100SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="RadioComponent_0">{$value_RadioComponent_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_XilinxZynq7100SoC_0 in $doc_ObjectDescription
    let $value_XilinxZynq7100SoC_0 := fn:data($node_XilinxZynq7100SoC_0/@rdf:about)
    for $node_Processor_0 in $node_XilinxZynq7100SoC_0/Nuvio:partOf
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:resource)
    where fn:exists($value_Processor_0) and fn:exists($node_XilinxZynq7100SoC_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("RadioComponent_0"))
