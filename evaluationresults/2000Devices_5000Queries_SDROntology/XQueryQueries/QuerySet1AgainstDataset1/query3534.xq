...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:about)
    for $node_Submarine_0 in $node_USRPE310_0/RFDevice:isDeployedOn
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:resource)
    where fn:exists($value_Submarine_0) and fn:exists($node_USRPE310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:about)
    for $node_Moving_0 in $node_Submarine_0/RFDevice:hasCapability
