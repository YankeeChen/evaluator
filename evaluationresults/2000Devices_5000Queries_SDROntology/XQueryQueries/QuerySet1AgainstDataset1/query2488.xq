...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:about)
    for $node_Var0 in $node_USRPB200mini_0/RFDevice:hasCapability
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Var1 in $node_Var3/Nuvio:hasValue
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($value_Var1) and fn:exists($node_Var3)
    return
      <xqllib:result>
        <xqllib:var name="Var1">{$value_Var1}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Var3 in $node_Entity_0/Nuvio:hasUnitOfMeasure
    let $value_Var3 := fn:data($node_Var3/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_Var3)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 :=  
    for $node_Var3 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#NonNegativeDecimalValue"]
    let $value_Var3 := fn:data($node_Var3/@rdf:about)
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_8,$GRAPH_9,("Var3"))
  let $GRAPH_11 := xqllib:and($GRAPH_5,$GRAPH_10,("Var3"))
  let $GRAPH_12 := xqllib:and($GRAPH_4,$GRAPH_11,("Var1"))
  let $GRAPH_13 := xqllib:optional($GRAPH_3,$GRAPH_12,("Var1"))
  let $GRAPH_14 := xqllib:and($GRAPH_2,$GRAPH_13,("Var1"))
  let $GRAPH_15 := xqllib:and($GRAPH_1,$GRAPH_14,("Var0"))
  let $GRAPH_16 := xqllib:and($GRAPH_0,$GRAPH_15,("USRPB200mini_0"))
  return
    $GRAPH_16
return
