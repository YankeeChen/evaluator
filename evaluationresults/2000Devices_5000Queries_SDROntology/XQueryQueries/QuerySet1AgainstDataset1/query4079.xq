...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Transmitting_0) and fn:exists($node_Var0) and $node_Transmitting_0/RFDevice:supportsSpectrumSensing/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Freq_61_44MHz"]/RFDevice:hasBeamwidth
