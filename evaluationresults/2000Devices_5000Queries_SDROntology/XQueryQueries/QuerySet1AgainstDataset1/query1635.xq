...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_PulseDopplerRadarModulation_0 in $node_Quantity_0/RFDevice:hasMaxSensedPower
    let $value_PulseDopplerRadarModulation_0 := fn:data($node_PulseDopplerRadarModulation_0/@rdf:resource)
    where fn:exists($value_PulseDopplerRadarModulation_0) and fn:exists($node_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseDopplerRadarModulation_0">{$value_PulseDopplerRadarModulation_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_1,$GRAPH_7,("PulseDopplerRadarModulation_0"))
  let $GRAPH_9 := xqllib:optional($GRAPH_0,$GRAPH_8,("PulseDopplerRadar_0"))
  return
    $GRAPH_9
return
