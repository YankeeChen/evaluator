...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_Spartan3ADSPFPGA_0/Nuvio:partOf
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:resource)
    where fn:exists($node_Spartan3ADSPFPGA_0) and fn:exists($value_USRPN210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Spartan3ADSPFPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Xilinx"]/RFDevice:isProducerOf
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("Spartan3ADSPFPGA_0"))
