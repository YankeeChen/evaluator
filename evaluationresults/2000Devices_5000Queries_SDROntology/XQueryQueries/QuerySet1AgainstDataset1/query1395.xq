...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_Entity_0/RFDevice:supportsJamming
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_USRPN200_0) and $node_USRPN200_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[Nuvio:relevantIndividual/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#RadarWarningReceiver_instance11"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_6,$GRAPH_7,("USRPN200_0"))
  let $GRAPH_9 := ($GRAPH_3,$GRAPH_8)
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("Entity_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_0,$GRAPH_10,("ElectronicSupportSystem_0"))
  return
    $GRAPH_11
return
