...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ContinuousWaveRadar_0 in $node_Entity_0/Nuvio:aggregateOf
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) >= 2127352591)]
  ...
  let $GRAPH_9 :=  
    ...
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    where fn:exists($node_Entity_0) and fn:exists($node_Value_0) and $node_Entity_0/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Value_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
        <xqllib:var name="Value_0">{$value_Value_0}</xqllib:var>
  ...
  let $GRAPH_13 := xqllib:optional($GRAPH_0,$GRAPH_12,("ContinuousWaveRadar_0"))
  return
    $GRAPH_13
return
