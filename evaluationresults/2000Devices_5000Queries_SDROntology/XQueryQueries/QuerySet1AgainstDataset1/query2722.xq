...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Ship_0/RFDevice:isDeployedOn
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($node_Ship_0) and fn:exists($value_USRPB200mini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Ship_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Moving_instance52"]/RFDevice:hasCapability
