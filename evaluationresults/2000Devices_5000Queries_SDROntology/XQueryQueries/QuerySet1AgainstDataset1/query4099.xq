...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_SpotJamming_0 in $node_Power_0/RFDevice:hasTransmitPowerRange
    let $value_SpotJamming_0 := fn:data($node_SpotJamming_0/@rdf:resource)
    where fn:exists($node_Power_0) and fn:exists($value_SpotJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("SpotJamming_0"))
