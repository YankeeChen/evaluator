...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX300_0 in $node_Kintex7FPGA_0/Nuvio:partOf
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:resource)
    where fn:exists($node_Kintex7FPGA_0) and fn:exists($value_USRPX300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Kintex7FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#DirectConversionReceiver_instance11"]/Nuvio:aggregateOf
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("Kintex7FPGA_0"))
