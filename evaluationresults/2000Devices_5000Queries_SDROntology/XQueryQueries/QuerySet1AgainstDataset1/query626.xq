...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_DirectConversionReceiver_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription[Nuvio:partOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Oscillator_instance7"]
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:about)
    where fn:exists($node_DirectConversionReceiver_0)
    return
      <xqllib:result>
