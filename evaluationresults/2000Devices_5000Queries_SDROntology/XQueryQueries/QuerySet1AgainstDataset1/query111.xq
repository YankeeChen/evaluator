...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ContinuousWaveRadar_0 in $node_Attribute_0/RFDevice:hasMaxPowerConsumption
    let $value_ContinuousWaveRadar_0 := fn:data($node_ContinuousWaveRadar_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_ContinuousWaveRadar_0)
    return
      <xqllib:result>
