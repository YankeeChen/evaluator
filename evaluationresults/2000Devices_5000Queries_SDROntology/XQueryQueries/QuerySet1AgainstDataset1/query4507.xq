...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_i_0 in $node_XC6SLX75FPGA_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    for $node_XC6SLX75FPGA_0 in $doc_ObjectDescription
    let $value_XC6SLX75FPGA_0 := fn:data($node_XC6SLX75FPGA_0/@rdf:about)
    for $node_DAC_0 in $node_XC6SLX75FPGA_0/Nuvio:partOf
    let $value_DAC_0 := fn:data($node_DAC_0/@rdf:resource)
    where fn:exists($value_DAC_0) and fn:exists($node_XC6SLX75FPGA_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("XC6SLX75FPGA_0"))
