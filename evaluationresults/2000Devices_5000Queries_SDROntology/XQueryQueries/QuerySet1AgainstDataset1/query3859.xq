...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_Object_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_2 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_NoiseFigure_0 in $node_Object_0/RFDevice:hasHeight
    let $value_NoiseFigure_0 := fn:data($node_NoiseFigure_0/@rdf:resource)
    where fn:exists($value_NoiseFigure_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("NoiseFigure_0"))
