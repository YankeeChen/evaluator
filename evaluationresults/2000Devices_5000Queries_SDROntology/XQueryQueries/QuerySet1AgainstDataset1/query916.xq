...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:about)
    for $node_XilinxZynq7020SoC_0 in $node_USRPE312_0/Nuvio:aggregateOf
    let $value_XilinxZynq7020SoC_0 := fn:data($node_XilinxZynq7020SoC_0/@rdf:resource)
    where fn:exists($node_USRPE312_0) and fn:exists($value_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPE312_0">{$value_USRPE312_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_XilinxZynq7020SoC_0 in $node_Kintex7FPGA_0/Nuvio:partOf
    let $value_XilinxZynq7020SoC_0 := fn:data($node_XilinxZynq7020SoC_0/@rdf:resource)
    where fn:exists($node_Kintex7FPGA_0) and fn:exists($value_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_1,$GRAPH_7,("XilinxZynq7020SoC_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_0,$GRAPH_8,("USRPE312_0"))
  return
    $GRAPH_9
return
