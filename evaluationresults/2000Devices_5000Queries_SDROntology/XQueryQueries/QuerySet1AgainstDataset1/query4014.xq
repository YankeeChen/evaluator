...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#Series7FPGA"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 :=  
    ...
    for $node_Entity_0 in $node_Modulator_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_Modulator_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_11 :=  
    ...
    for $node_Modulator_0 in $node_Process_0/RFDevice:supportsSpectrumSensing
    let $value_Modulator_0 := fn:data($node_Modulator_0/@rdf:resource)
    where fn:exists($value_Modulator_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
        <xqllib:var name="Modulator_0">{$value_Modulator_0}</xqllib:var>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_12 := xqllib:and($GRAPH_10,$GRAPH_11,("Modulator_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_9,$GRAPH_12,("Modulator_0"))
