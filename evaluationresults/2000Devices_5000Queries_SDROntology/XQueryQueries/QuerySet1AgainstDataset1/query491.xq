...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_FrequencyShiftKeyingModulation_0) and $node_FrequencyShiftKeyingModulation_0/RFDevice:supportsJamming/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("FrequencyShiftKeyingModulation_0"))
  let $GRAPH_6 :=  
