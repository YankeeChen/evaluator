...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_Submarine_0/RFDevice:isDeployedOn
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:resource)
    where fn:exists($node_Submarine_0) and fn:exists($value_USRPB2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Submarine_0 in $node_Moving_0/RFDevice:supportsSpectrumSensing
