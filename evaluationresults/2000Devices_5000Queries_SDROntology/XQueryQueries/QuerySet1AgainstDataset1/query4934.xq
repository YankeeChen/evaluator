...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:about)
    for $node_RadioComponent_0 in $node_USRPE3xx_0/Nuvio:aggregateOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($value_RadioComponent_0) and fn:exists($node_USRPE3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Filter_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_Filter_0 := fn:data($node_Filter_0/@rdf:resource)
    where fn:exists($value_Filter_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("Filter_0"))
