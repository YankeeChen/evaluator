...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_RadarModulation_0 in $node_Object_0/RFDevice:hasCapability
    let $value_RadarModulation_0 := fn:data($node_RadarModulation_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_RadarModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
