...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB205mini_i_0 in $node_Entity_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#AbstractSurfacePatch"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("Entity_0"))
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#InformationEntity"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_LatitudeValue_0 in $doc_ObjectDescription
    let $value_LatitudeValue_0 := fn:data($node_LatitudeValue_0/@rdf:about)
    where fn:exists($node_Entity_0) and fn:exists($node_LatitudeValue_0) and $node_LatitudeValue_0/Nuvio:isExpressedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
        <xqllib:var name="LatitudeValue_0">{$value_LatitudeValue_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 :=  
    for $node_LatitudeValue_0 in $doc_ObjectDescription
    let $value_LatitudeValue_0 := fn:data($node_LatitudeValue_0/@rdf:about)
    for $node_DataValue0 in $node_LatitudeValue_0/Nuvio:hasPrecision
    let $value_DataValue0 := fn:data($node_DataValue0)
    where fn:exists($value_DataValue0) and fn:exists($node_LatitudeValue_0)
    return
      <xqllib:result>
        <xqllib:var name="DataValue0">{$value_DataValue0}</xqllib:var>
        <xqllib:var name="LatitudeValue_0">{$value_LatitudeValue_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) < 1673716144)]
  let $GRAPH_8 := xqllib:and($GRAPH_5,$GRAPH_7,("LatitudeValue_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_4,$GRAPH_8,("Entity_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("Entity_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_0,$GRAPH_10,("USRPB205mini_i_0"))
  return
    $GRAPH_11
return
