...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseDopplerRadar_0 in $node_PulseDopplerRadarModulation_0/RFDevice:hasCapability
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:resource)
    where fn:exists($node_PulseDopplerRadarModulation_0) and fn:exists($value_PulseDopplerRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_PulseDopplerRadarModulation_0 in $node_Time_0/Nuvio:hasProcessQuantity
    let $value_PulseDopplerRadarModulation_0 := fn:data($node_PulseDopplerRadarModulation_0/@rdf:resource)
    where fn:exists($value_PulseDopplerRadarModulation_0) and fn:exists($node_Time_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseDopplerRadarModulation_0">{$value_PulseDopplerRadarModulation_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Time_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("PulseDopplerRadarModulation_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("PulseDopplerRadarModulation_0"))
