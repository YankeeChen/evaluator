...
let $result :=  
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) <= -2084982930)]
  ...
  let $GRAPH_13 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_Power_0) and fn:exists($node_Situation_0) and $node_Power_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
  ...
  let $GRAPH_16 := $GRAPH_15[(data(xqllib:var[@name="DataValue1"]) <= -11.22)]
  let $GRAPH_17 := xqllib:and($GRAPH_14,$GRAPH_16,("Var0"))
  let $GRAPH_18 := ($GRAPH_13,$GRAPH_17)
  let $GRAPH_19 := xqllib:and($GRAPH_11,$GRAPH_18,("Situation_0"))
