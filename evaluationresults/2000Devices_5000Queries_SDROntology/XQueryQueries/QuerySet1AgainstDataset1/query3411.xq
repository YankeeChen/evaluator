...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_BistaticRadar_0 in $node_Transmitter_0/Nuvio:aggregateOf
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:resource)
    where fn:exists($value_BistaticRadar_0) and fn:exists($node_Transmitter_0)
    return
      <xqllib:result>
        <xqllib:var name="BistaticRadar_0">{$value_BistaticRadar_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_Transmitter_0 in $doc_ObjectDescription
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:about)
    for $node_RadarFrequencyBand_0 in $node_Transmitter_0/RFDevice:hasFrequencyRange
  ...
  let $GRAPH_5 :=  
    ...
    let $value_RadarFrequencyBand_0 := fn:data($node_RadarFrequencyBand_0/@rdf:about)
    for $node_Frequency_0 in $node_RadarFrequencyBand_0/RFDevice:hasMin
    let $value_Frequency_0 := fn:data($node_Frequency_0/@rdf:resource)
    where fn:exists($value_Frequency_0) and fn:exists($node_RadarFrequencyBand_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_2,$GRAPH_7,("Transmitter_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("Transmitter_0"))
  let $GRAPH_10 := xqllib:optional($GRAPH_0,$GRAPH_9,("BistaticRadar_0"))
  return
    $GRAPH_10
return
