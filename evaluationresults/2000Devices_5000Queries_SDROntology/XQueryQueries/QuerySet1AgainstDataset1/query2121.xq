...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Ship_0) and fn:exists($node_Var0) and $node_Ship_0/Nuvio:participatesIn/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_Ship_0 in $doc_ObjectDescription
    let $value_Ship_0 := fn:data($node_Ship_0/@rdf:about)
    for $node_Var0 in $node_Ship_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Ship_0","Var0"))
  let $GRAPH_8 :=  
