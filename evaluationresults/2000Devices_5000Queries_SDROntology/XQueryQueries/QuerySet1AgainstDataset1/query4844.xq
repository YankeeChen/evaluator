...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    for $node_Entity_0 in $node_PulseDopplerRadar_0/RFDevice:hasChargingTime
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Quality_0 in $node_Entity_0/Nuvio:hasProcessQuality
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("Quality_0"))
  let $GRAPH_5 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_AmplitudeShiftKeyingModulation_0) and fn:exists($node_Entity_0) and $node_AmplitudeShiftKeyingModulation_0/Nuvio:hasParticipant/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
