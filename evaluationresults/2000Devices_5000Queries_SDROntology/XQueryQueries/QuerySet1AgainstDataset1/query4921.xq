...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_DeceptiveCommunicationsJamming_0) and fn:exists($node_Entity_0) and $node_DeceptiveCommunicationsJamming_0/Nuvio:hasSubprocess/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_PowerRange_0 in $doc_ObjectDescription
    let $value_PowerRange_0 := fn:data($node_PowerRange_0/@rdf:about)
    for $node_Situation_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Situation"]
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_PowerRange_0) and fn:exists($node_Situation_0) and $node_PowerRange_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
        <xqllib:var name="PowerRange_0">{$value_PowerRange_0}</xqllib:var>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 :=  
    for $node_Entity_0 in $doc_ObjectDescription
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_5,$GRAPH_6,("Situation_0"))
  let $GRAPH_8 :=  
    for $node_PowerRange_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Frequency_instance58"]/RFDevice:hasMin
    let $value_PowerRange_0 := fn:data($node_PowerRange_0/@rdf:resource)
    where fn:exists($value_PowerRange_0)
    return
      <xqllib:result>
        <xqllib:var name="PowerRange_0">{$value_PowerRange_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("PowerRange_0"))
