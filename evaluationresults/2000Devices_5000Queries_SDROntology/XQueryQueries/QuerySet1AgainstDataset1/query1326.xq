...
let $result :=  
  ...
  let $GRAPH_9 :=  
    ...
    for $node_Entity_0 in $node_PowerRange_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_PowerRange_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_10 :=  
    for $node_PowerRange_0 in $doc_ObjectDescription
    let $value_PowerRange_0 := fn:data($node_PowerRange_0/@rdf:about)
    for $node_Gain_0 in $node_PowerRange_0/RFDevice:hasMax
    let $value_Gain_0 := fn:data($node_Gain_0/@rdf:resource)
    where fn:exists($value_Gain_0) and fn:exists($node_PowerRange_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_10,$GRAPH_11,("Gain_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_9,$GRAPH_12,("PowerRange_0"))
