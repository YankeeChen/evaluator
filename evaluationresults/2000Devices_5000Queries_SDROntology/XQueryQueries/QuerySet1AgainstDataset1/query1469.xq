...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarWarningReceiver_0 in $node_Var0/RFDevice:supportsJamming
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[Nuvio:hasParticipant/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#PulseDopplerRadar_instance7"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("RadarWarningReceiver_0"))
  return
    $GRAPH_7
return
