...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:about)
    for $node_Processor_0 in $node_USRPE3xx_0/Nuvio:compositeOf
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:resource)
    where fn:exists($value_Processor_0) and fn:exists($node_USRPE3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Processor_0 in $node_DirectConversionReceiver_0/Nuvio:partOf
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_Processor_0)
    return
      <xqllib:result>
