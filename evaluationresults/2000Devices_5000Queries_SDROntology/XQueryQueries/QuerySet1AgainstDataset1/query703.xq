...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:about)
    for $node_UnmannedPlatform_0 in $node_CommunicationsSystem_0/RFDevice:isDeployedOn
    let $value_UnmannedPlatform_0 := fn:data($node_UnmannedPlatform_0/@rdf:resource)
    where fn:exists($node_CommunicationsSystem_0) and fn:exists($value_UnmannedPlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="CommunicationsSystem_0">{$value_CommunicationsSystem_0}</xqllib:var>
        <xqllib:var name="UnmannedPlatform_0">{$value_UnmannedPlatform_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_2 := xqllib:optional($GRAPH_0,$GRAPH_1,("CommunicationsSystem_0"))
  return
    $GRAPH_2
return
