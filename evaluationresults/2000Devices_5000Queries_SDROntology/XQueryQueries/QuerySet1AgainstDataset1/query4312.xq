...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Var0 in $node_Entity_0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_9 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_FrequencyAccuracy_0) and $node_FrequencyAccuracy_0/RFDevice:hasWeight/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_9,$GRAPH_10,("FrequencyAccuracy_0"))
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_1,$GRAPH_14,("Ship_0"))
