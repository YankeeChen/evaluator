...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_FPGA_0/Nuvio:partOf
    let $value_USRPB210_0 := fn:data($node_USRPB210_0/@rdf:resource)
    where fn:exists($node_FPGA_0) and fn:exists($value_USRPB210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#OmnidirectionalAntenna_instance0"]/Nuvio:compositeOf
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("FPGA_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("USRPB210_0"))
  return
    $GRAPH_7
return
