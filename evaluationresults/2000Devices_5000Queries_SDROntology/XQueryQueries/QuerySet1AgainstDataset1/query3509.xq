...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_Var0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Situation_0 in $node_XC3SD1800AFPGA_0/Nuvio:relevantIndividual
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:resource)
    where fn:exists($value_Situation_0) and fn:exists($node_XC3SD1800AFPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Situation_0","Var0"))
