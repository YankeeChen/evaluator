...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_210_0 in $node_XilinxFPGA_0/Nuvio:partOf
    let $value_USRPB200_210_0 := fn:data($node_USRPB200_210_0/@rdf:resource)
    where fn:exists($value_USRPB200_210_0) and fn:exists($node_XilinxFPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB200_210_0">{$value_USRPB200_210_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_XilinxFPGA_0 in $node_Jammer_0/Nuvio:partOf
    let $value_XilinxFPGA_0 := fn:data($node_XilinxFPGA_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_XilinxFPGA_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Jammer_0 in $doc_ObjectDescription
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:about)
    for $node_Modulator_0 in $node_Jammer_0/Nuvio:aggregateOf
    let $value_Modulator_0 := fn:data($node_Modulator_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_Modulator_0)
    return
      <xqllib:result>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
