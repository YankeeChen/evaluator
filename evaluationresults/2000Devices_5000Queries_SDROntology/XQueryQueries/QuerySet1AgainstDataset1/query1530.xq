...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_JammingSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_IEDTrigger_0 in $node_Attribute_0/RFDevice:hasNoiseFigure
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_IEDTrigger_0)
    return
      <xqllib:result>
