...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_MonostaticRadar_0 in $node_Modulation_0/RFDevice:supportsSpectrumSensing
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:resource)
    where fn:exists($node_Modulation_0) and fn:exists($value_MonostaticRadar_0)
    return
      <xqllib:result>
