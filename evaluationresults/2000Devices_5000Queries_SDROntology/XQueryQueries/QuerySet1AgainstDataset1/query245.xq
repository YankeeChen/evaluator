...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_Var0/RFDevice:hasCapability
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    where fn:exists($node_Value_0) and $doc_ObjectDescription[@rdf:ID = "Bit"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Value_0
    return
      <xqllib:result>
        <xqllib:var name="Value_0">{$value_Value_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue0"]) < -1981975055)]
  let $GRAPH_10 := xqllib:and($GRAPH_7,$GRAPH_9,("Value_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_6,$GRAPH_10,())
  let $GRAPH_12 := xqllib:and($GRAPH_3,$GRAPH_11,("Value_0","Var0"))
