...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:about)
    for $node_Jammer_0 in $node_ModelX_0/Nuvio:compositeOf
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($value_Jammer_0) and fn:exists($node_ModelX_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_QuantityRange_0 in $node_Jammer_0/RFDevice:hasFrequencyRange
