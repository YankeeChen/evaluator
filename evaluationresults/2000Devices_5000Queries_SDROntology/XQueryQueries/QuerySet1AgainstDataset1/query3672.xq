...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/sf#MultiSurface"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_InformationContent_0) and fn:exists($node_Var0) and $node_InformationContent_0/Nuvio:representedBy/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("InformationContent_0"))
