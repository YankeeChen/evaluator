...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Aircraft_0 in $doc_ObjectDescription
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:about)
    for $node_Entity_0 in $node_Aircraft_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) >= -938251840)]
  ...
  let $GRAPH_11 :=  
    for $node_DAC_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Instance2517"]/Nuvio:compositeOf
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_10,$GRAPH_11,("DAC_0"))
