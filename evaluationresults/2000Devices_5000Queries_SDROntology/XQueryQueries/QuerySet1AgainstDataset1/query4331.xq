...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_Situation_0) and $node_Situation_0/RFDevice:supportsTransmitting/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Situation_0 in $node_Ship_0/Nuvio:relevantIndividual
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:resource)
    where fn:exists($node_Ship_0) and fn:exists($value_Situation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Ship_0 in $node_Moving_0/RFDevice:hasCapability
    let $value_Ship_0 := fn:data($node_Ship_0/@rdf:resource)
    where fn:exists($node_Moving_0) and fn:exists($value_Ship_0)
    return
      <xqllib:result>
