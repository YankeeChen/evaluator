...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_PulseDopplerRadar_0) and fn:exists($node_Var0) and $node_PulseDopplerRadar_0/Nuvio:hasParticipant/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_PulseDopplerRadar_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#PulseDopplerRadarModulation_instance23"]/RFDevice:supportsJamming
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:resource)
    where fn:exists($value_PulseDopplerRadar_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseDopplerRadar_0">{$value_PulseDopplerRadar_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("PulseDopplerRadar_0"))
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("PulseDopplerRadar_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Var0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("USRPX310_0"))
  return
    $GRAPH_8
return
