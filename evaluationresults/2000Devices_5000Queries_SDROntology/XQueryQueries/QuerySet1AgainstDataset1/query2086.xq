...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_Receiver_0/Nuvio:partOf
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:resource)
    where fn:exists($node_Receiver_0) and fn:exists($value_USRPN210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Receiver_0 in $doc_ObjectDescription
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:about)
    for $node_Component_0 in $node_Receiver_0/Nuvio:aggregateOf
