...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xxmini_0 in $node_FPGA_0/Nuvio:aggregateOf
    let $value_USRPB2xxmini_0 := fn:data($node_USRPB2xxmini_0/@rdf:resource)
    where fn:exists($node_FPGA_0) and fn:exists($value_USRPB2xxmini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_FPGA_0 in $node_Object_0/RFDevice:isProducerOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="FPGA_0">{$value_FPGA_0}</xqllib:var>
  ...
  let $GRAPH_19 := xqllib:and($GRAPH_3,$GRAPH_18,("Object_0"))
  let $GRAPH_20 := xqllib:and($GRAPH_2,$GRAPH_19,("FPGA_0"))
  let $GRAPH_21 := xqllib:and($GRAPH_1,$GRAPH_20,("FPGA_0"))
  let $GRAPH_22 := xqllib:and($GRAPH_0,$GRAPH_21,("USRPB2xxmini_0"))
  return
    $GRAPH_22
return
