...
let $result :=  
  ...
  let $GRAPH_4 := $GRAPH_3[(data(xqllib:var[@name="DataValue0"]) > 1504300281)]
  let $GRAPH_5 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Situation"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "USRPB200-210_instance27"]/Nuvio:relevantIndividual/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 :=  
    ...
    for $node_Var0 in $node_Polygon_0/Nuvio:relevantIndividual
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_Polygon_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("Polygon_0"))
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_13,$GRAPH_14,("InformationRepresentation_0"))
  let $GRAPH_16 := xqllib:and($GRAPH_1,$GRAPH_15,("Var0"))
  let $GRAPH_17 := xqllib:and($GRAPH_0,$GRAPH_16,("USRPX300_0"))
  return
    $GRAPH_17
return
