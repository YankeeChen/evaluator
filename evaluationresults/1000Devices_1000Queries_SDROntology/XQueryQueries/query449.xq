...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN200_0 := fn:data($node_USRPN200_0/@rdf:about)
    for $node_IEDTrigger_0 in $node_USRPN200_0/Nuvio:aggregateOf
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:resource)
    where fn:exists($value_IEDTrigger_0) and fn:exists($node_USRPN200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Component_0 in $node_IEDTrigger_0/Nuvio:aggregateOf
    let $value_Component_0 := fn:data($node_Component_0/@rdf:resource)
    where fn:exists($value_Component_0) and fn:exists($node_IEDTrigger_0)
    return
      <xqllib:result>
