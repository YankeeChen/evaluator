...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_SuperheterodyneReceiver_0/Nuvio:compositeOf
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:resource)
    where fn:exists($value_SearchRadar_0) and fn:exists($node_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="SearchRadar_0">{$value_SearchRadar_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_SuperheterodyneReceiver_0 in $doc_ObjectDescription
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:about)
    for $node_Ratio_0 in $node_SuperheterodyneReceiver_0/RFDevice:hasNoiseFigure
    let $value_Ratio_0 := fn:data($node_Ratio_0/@rdf:resource)
    where fn:exists($value_Ratio_0) and fn:exists($node_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
