...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX300_0 in $node_Object_0/Nuvio:partOf
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_USRPX300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_CommunicationsJamming_0) and fn:exists($node_Object_0) and $node_CommunicationsJamming_0/RFDevice:supportsJamming/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_CommunicationsJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Instance1453"]/RFDevice:hasEffectiveRadiatedPower
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("CommunicationsJamming_0"))
