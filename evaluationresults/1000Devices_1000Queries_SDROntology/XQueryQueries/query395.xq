...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Receiver_0 in $node_Object_0/Nuvio:compositeOf
  ...
  let $GRAPH_4 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Modulation_0 in $node_Object_0/Nuvio:participatesIn
    let $value_Modulation_0 := fn:data($node_Modulation_0/@rdf:resource)
    where fn:exists($value_Modulation_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_4,$GRAPH_5,("Modulation_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Object_0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_2,$GRAPH_7,("Receiver_0"))
