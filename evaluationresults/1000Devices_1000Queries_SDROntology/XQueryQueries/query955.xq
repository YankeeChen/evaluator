...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_SweptSpotJamming_0/Nuvio:relevantIndividual
  ...
  let $GRAPH_5 :=  
    for $node_SweptSpotJamming_0 in $doc_ObjectDescription
    let $value_SweptSpotJamming_0 := fn:data($node_SweptSpotJamming_0/@rdf:about)
    for $node_Attribute_0 in $node_SweptSpotJamming_0/RFDevice:hasPulseRepetitionFrequency
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:resource)
    where fn:exists($value_Attribute_0) and fn:exists($node_SweptSpotJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue0"]) < 26.69)]
