...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Object_0 in $node_DeceptiveJamming_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_4 :=  
    for $node_DeceptiveJamming_0 in $doc_ObjectDescription
    let $value_DeceptiveJamming_0 := fn:data($node_DeceptiveJamming_0/@rdf:about)
    for $node_Attribute_0 in $node_DeceptiveJamming_0/RFDevice:hasPulseRepetitionFrequency
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:resource)
    where fn:exists($value_Attribute_0) and fn:exists($node_DeceptiveJamming_0)
    return
      <xqllib:result>
