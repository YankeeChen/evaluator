...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:about)
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_USRPX300_0/Nuvio:compositeOf
    ...
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_USRPX300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_FreqencyRange_0/RFDevice:hasFrequencyRange
    let $value_InstantaneousFrequencyMeasurementReceiver_0 := fn:data($node_InstantaneousFrequencyMeasurementReceiver_0/@rdf:resource)
  ...
  let $GRAPH_5 :=  
    for $node_FreqencyRange_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#NoiseFigure_instance51"]/RFDevice:hasMin
