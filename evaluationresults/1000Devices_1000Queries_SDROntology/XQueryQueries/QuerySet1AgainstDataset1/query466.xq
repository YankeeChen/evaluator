...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:about)
    for $node_Entity_0 in $node_USRPN210_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#CortexA9"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_Oscillator_0/Nuvio:partOf
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_Oscillator_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Oscillator_0"))
  ...
  let $GRAPH_12 :=  
    ...
    for $node_Entity_0 in $node_TimeInstant_0/Nuvio:expresses
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_TimeInstant_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_15 := xqllib:optional($GRAPH_13,$GRAPH_14,("TimeInstant_0"))
  let $GRAPH_16 := xqllib:optional($GRAPH_12,$GRAPH_15,("TimeInstant_0"))
  let $GRAPH_17 := ($GRAPH_11,$GRAPH_16)
