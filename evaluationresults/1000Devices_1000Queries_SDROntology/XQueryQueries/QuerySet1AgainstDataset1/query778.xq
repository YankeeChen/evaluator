...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_Entity_0/Nuvio:compositeOf
  ...
  let $GRAPH_7 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#Millimeter"]/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_11 :=  
    for $node_Situation_0 in $doc_ObjectDescription
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_USRPX310_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:resource)
    where fn:exists($node_Situation_0) and fn:exists($value_USRPX310_0)
    return
      <xqllib:result>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
  ...
  let $GRAPH_13 :=  
    ...
    for $node_USRPX310_0 in $node_Object_0/Nuvio:aggregateOf
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_USRPX310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_16 := xqllib:and($GRAPH_12,$GRAPH_15,("USRPX310_0"))
  let $GRAPH_17 := xqllib:and($GRAPH_11,$GRAPH_16,("USRPX310_0"))
  let $GRAPH_18 := xqllib:and($GRAPH_10,$GRAPH_17,("Situation_0"))
