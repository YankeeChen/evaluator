...
let $result :=  
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) > 767487760)]
  ...
  let $GRAPH_12 :=  
    ...
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    where fn:exists($node_FrequencyScanRateUnit_0) and fn:exists($node_Value_0) and $node_FrequencyScanRateUnit_0/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Value_0
    return
      <xqllib:result>
  ...
  let $GRAPH_17 := xqllib:optional($GRAPH_0,$GRAPH_16,("USRPE313_0"))
  return
    $GRAPH_17
return
