...
let $result :=  
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) = 22.62)]
  let $GRAPH_6 :=  
  ...
  let $GRAPH_8 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_DataValue1 in $node_Var1/RFDevice:hasDecimalValue
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue1"]) != 500580169)]
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_3,$GRAPH_12,("Var1"))
