...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:about)
    for $node_Jammer_0 in $node_USRPE3xx_0/Nuvio:aggregateOf
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($value_Jammer_0) and fn:exists($node_USRPE3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Jammer_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Producer_instance243"]/RFDevice:isProducerOf
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
