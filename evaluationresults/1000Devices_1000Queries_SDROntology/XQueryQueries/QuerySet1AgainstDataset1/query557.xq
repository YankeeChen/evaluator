...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_InformationContent_0 in $node_Var1/Nuvio:isExpressedBy
    let $value_InformationContent_0 := fn:data($node_InformationContent_0/@rdf:resource)
    where fn:exists($value_InformationContent_0) and fn:exists($node_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_5,$GRAPH_6,("InformationEntity_0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_4,$GRAPH_7,("InformationRepresentation_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_3,$GRAPH_8,("InformationContent_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("Var1"))
  let $GRAPH_11 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_Var0 in $node_Var1/Nuvio:hasProcessQuantity
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_10,$GRAPH_11,("Var0","Var1"))
  let $GRAPH_13 := xqllib:and($GRAPH_1,$GRAPH_12,("Var0"))
  let $GRAPH_14 := xqllib:and($GRAPH_0,$GRAPH_13,("USRPB205mini_i_0"))
  return
    $GRAPH_14
return
