...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_Satellite_0 in $node_SurveillanceRadar_0/RFDevice:isDeployedOn
    let $value_Satellite_0 := fn:data($node_Satellite_0/@rdf:resource)
    where fn:exists($value_Satellite_0) and fn:exists($node_SurveillanceRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Satellite_0 := fn:data($node_Satellite_0/@rdf:about)
    for $node_Process_0 in $node_Satellite_0/RFDevice:supportsSpectrumSensing
