...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_CyclostationaryDetection_0) and fn:exists($node_Entity_0) and $node_CyclostationaryDetection_0/RFDevice:hasCapability/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
