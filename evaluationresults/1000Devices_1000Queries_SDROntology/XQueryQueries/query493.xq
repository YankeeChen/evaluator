...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_210_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_USRPB200_210_0 := fn:data($node_USRPB200_210_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_USRPB200_210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_RadioComponent_0 in $doc_ObjectDescription
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:about)
    for $node_Receiver_0 in $node_RadioComponent_0/Nuvio:partOf
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_Receiver_0)
    return
      <xqllib:result>
        <xqllib:var name="RadioComponent_0">{$value_RadioComponent_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Receiver_0 in $node_Attribute_0/RFDevice:hasNoiseFigure
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_Receiver_0)
    return
      <xqllib:result>
