...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CR_0 in $node_DirectConversionReceiver_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:about)
    for $node_NoiseFigure_0 in $node_DirectConversionReceiver_0/RFDevice:hasNoiseFigure
    let $value_NoiseFigure_0 := fn:data($node_NoiseFigure_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_NoiseFigure_0)
    return
      <xqllib:result>
        <xqllib:var name="DirectConversionReceiver_0">{$value_DirectConversionReceiver_0}</xqllib:var>
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) = -71.45)]
  let $GRAPH_9 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_10 := $GRAPH_9[(data(xqllib:var[@name="DataValue1"]) < 1924974425)]
  let $GRAPH_11 := ($GRAPH_8,$GRAPH_10)
  let $GRAPH_12 := xqllib:and($GRAPH_5,$GRAPH_11,("Var0"))
  let $GRAPH_13 := ($GRAPH_4,$GRAPH_12)
  let $GRAPH_14 := xqllib:and($GRAPH_2,$GRAPH_13,("NoiseFigure_0"))
  let $GRAPH_15 := xqllib:and($GRAPH_1,$GRAPH_14,("DirectConversionReceiver_0"))
  let $GRAPH_16 := xqllib:and($GRAPH_0,$GRAPH_15,("CR_0"))
  return
    $GRAPH_16
return
