...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_Receiver_0/Nuvio:partOf
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:resource)
    where fn:exists($node_Receiver_0) and fn:exists($value_USRPB2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Receiver_0 in $node_HardwareComponent_0/Nuvio:compositeOf
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:resource)
    where fn:exists($node_HardwareComponent_0) and fn:exists($value_Receiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("HardwareComponent_0"))
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("Receiver_0"))
