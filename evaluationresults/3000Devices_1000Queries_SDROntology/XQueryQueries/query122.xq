...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB210_0 := fn:data($node_USRPB210_0/@rdf:about)
    for $node_Var0 in $node_USRPB210_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_4 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#Percentage"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_Var1)
    return
      <xqllib:result>
