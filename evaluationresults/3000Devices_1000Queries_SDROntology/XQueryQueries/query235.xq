...
let $result :=  
  ...
  let $GRAPH_12 :=  
    ...
    for $node_Var0 in $node_Var2/Nuvio:informationSource
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_14 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "UnmannedPlatform_instance27"]/Nuvio:participatesIn/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
