...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_GNURadio_0 in $node_Transmitter_0/Nuvio:compositeOf
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:resource)
    where fn:exists($value_GNURadio_0) and fn:exists($node_Transmitter_0)
    return
      <xqllib:result>
        <xqllib:var name="GNURadio_0">{$value_GNURadio_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Transmitter_0 in $node_Transceiving_0/RFDevice:supportsSpectrumSensing
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:resource)
    where fn:exists($node_Transceiving_0) and fn:exists($value_Transmitter_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Transceiving_0 in $node_Quantity_0/RFDevice:hasEffectiveRadiatedPower
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:resource)
    where fn:exists($node_Quantity_0) and fn:exists($value_Transceiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_0,$GRAPH_10,("GNURadio_0"))
  return
    $GRAPH_11
return
