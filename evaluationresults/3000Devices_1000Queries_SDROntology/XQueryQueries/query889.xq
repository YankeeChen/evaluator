...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Full_DuplexCommunicationsSystem_0 in $node_Receiving_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Process_0 in $node_ArcByCenterPoint_0/RFDevice:hasTarget
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_ArcByCenterPoint_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    for $node_ArcByCenterPoint_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#ArcByCenterPoint"]
    let $value_ArcByCenterPoint_0 := fn:data($node_ArcByCenterPoint_0/@rdf:about)
    where fn:exists($node_ArcByCenterPoint_0)
    return
      <xqllib:result>
        <xqllib:var name="ArcByCenterPoint_0">{$value_ArcByCenterPoint_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("ArcByCenterPoint_0"))
