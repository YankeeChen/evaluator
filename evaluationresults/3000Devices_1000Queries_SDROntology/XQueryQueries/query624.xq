...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_Process_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_ElectronicSupport_0 in $node_Process_0/Nuvio:hasSubprocess
    let $value_ElectronicSupport_0 := fn:data($node_ElectronicSupport_0/@rdf:resource)
    where fn:exists($value_ElectronicSupport_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
