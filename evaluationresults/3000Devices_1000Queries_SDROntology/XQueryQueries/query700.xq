...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_Object_0/RFDevice:hasProducer
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_ElectronicAttack_0 in $node_Object_0/Nuvio:hasParticipant
    let $value_ElectronicAttack_0 := fn:data($node_ElectronicAttack_0/@rdf:resource)
    where fn:exists($value_ElectronicAttack_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
