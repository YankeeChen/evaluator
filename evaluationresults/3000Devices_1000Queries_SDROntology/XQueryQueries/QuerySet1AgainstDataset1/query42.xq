...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_210_0 in $node_Var0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#FrequencyUnit_instance19"]/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) >= -1999350589)]
  ...
  let $GRAPH_9 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Surface_0) and $node_Surface_0/Nuvio:isExpressedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
