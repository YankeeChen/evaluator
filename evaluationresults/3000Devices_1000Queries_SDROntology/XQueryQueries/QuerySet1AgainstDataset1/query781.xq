...
let $result :=  
  ...
  let $GRAPH_8 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_ArcByCenterPoint_0 in $node_Entity_0/Nuvio:isExpressedBy
    let $value_ArcByCenterPoint_0 := fn:data($node_ArcByCenterPoint_0/@rdf:resource)
    where fn:exists($value_ArcByCenterPoint_0) and fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_13 := $GRAPH_12[(data(xqllib:var[@name="DataValue0"]) > 436007389)]
