...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarWarningReceiver_0 in $node_Entity_0/RFDevice:hasCapability
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Bezier_0 in $node_Entity_0/Nuvio:relevantIndividual
    let $value_Bezier_0 := fn:data($node_Bezier_0/@rdf:resource)
    where fn:exists($value_Bezier_0) and fn:exists($node_Entity_0)
    return
      <xqllib:result>
