...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX300_0 in $node_MannedPlatform_0/RFDevice:isDeployedOn
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:resource)
    where fn:exists($node_MannedPlatform_0) and fn:exists($value_USRPX300_0)
    return
      <xqllib:result>
        <xqllib:var name="MannedPlatform_0">{$value_MannedPlatform_0}</xqllib:var>
        <xqllib:var name="USRPX300_0">{$value_USRPX300_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_2 := xqllib:and($GRAPH_0,$GRAPH_1,("USRPX300_0"))
  return
    $GRAPH_2
return
