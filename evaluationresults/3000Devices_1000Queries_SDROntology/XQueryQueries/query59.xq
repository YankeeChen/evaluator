...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:about)
    for $node_Entity_0 in $node_GNURadio_0/Nuvio:aggregateOf
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_QuadratureAmplitudeModulation_0) and $node_QuadratureAmplitudeModulation_0/Nuvio:hasSubprocess/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Attribute_0","Entity_0"))
