...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ModelX_0 in $node_Object_0/RFDevice:hasProducer
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_NoiseJamming_0) and fn:exists($node_Object_0) and $node_NoiseJamming_0/RFDevice:supportsSpectrumSensing/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_NoiseJamming_0 in $doc_ObjectDescription
    let $value_NoiseJamming_0 := fn:data($node_NoiseJamming_0/@rdf:about)
    for $node_Quantity_0 in $node_NoiseJamming_0/RFDevice:hasEffectiveRadiatedPower
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("NoiseJamming_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("NoiseJamming_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Object_0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("ModelX_0"))
  return
    $GRAPH_8
return
