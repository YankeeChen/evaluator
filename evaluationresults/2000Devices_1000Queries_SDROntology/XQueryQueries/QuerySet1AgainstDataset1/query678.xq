...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_USRPN200_0 := fn:data($node_USRPN200_0/@rdf:resource)
    where fn:exists($node_MobilePlatform_0) and fn:exists($value_USRPN200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_MobilePlatform_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Moving_instance49"]/RFDevice:supportsTuning
