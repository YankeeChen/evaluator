...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_Quality_0) and fn:exists($node_Situation_0) and $node_Quality_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Situation_0 in $doc_ObjectDescription
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_XC7K325TFPGA_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_XC7K325TFPGA_0 := fn:data($node_XC7K325TFPGA_0/@rdf:resource)
    where fn:exists($node_Situation_0) and fn:exists($value_XC7K325TFPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,())
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Quality_0","Situation_0"))
