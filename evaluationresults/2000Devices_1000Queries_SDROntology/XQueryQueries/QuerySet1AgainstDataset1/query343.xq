...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX3xx_0 in $node_Jammer_0/Nuvio:partOf
    let $value_USRPX3xx_0 := fn:data($node_USRPX3xx_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_USRPX3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Jammer_0 in $node_RangeGatePullOffJamming_0/RFDevice:supportsJamming
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($value_Jammer_0) and fn:exists($node_RangeGatePullOffJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_RangeGatePullOffJamming_0 in $node_Quantity_0/RFDevice:hasPulseRepetitionFrequency
    let $value_RangeGatePullOffJamming_0 := fn:data($node_RangeGatePullOffJamming_0/@rdf:resource)
    where fn:exists($node_Quantity_0) and fn:exists($value_RangeGatePullOffJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Quantity_0">{$value_Quantity_0}</xqllib:var>
        <xqllib:var name="RangeGatePullOffJamming_0">{$value_RangeGatePullOffJamming_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("RangeGatePullOffJamming_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("RangeGatePullOffJamming_0"))
  let $GRAPH_9 := ($GRAPH_3,$GRAPH_8)
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("Jammer_0"))
