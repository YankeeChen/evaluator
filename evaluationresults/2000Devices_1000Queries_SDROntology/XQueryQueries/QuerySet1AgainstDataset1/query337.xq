...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SDR_0 in $node_Satellite_0/RFDevice:isDeployedOn
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:resource)
    where fn:exists($value_SDR_0) and fn:exists($node_Satellite_0)
    return
      <xqllib:result>
        <xqllib:var name="SDR_0">{$value_SDR_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Satellite_0 in $node_Entity_0/RFDevice:supportsJamming
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_InformationEntity_1) and $node_InformationEntity_1/Nuvio:isExpressedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Frequency_0 in $node_Var0/RFDevice:hasPulseRepetitionInterval
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_1,$GRAPH_10,("Satellite_0"))
  let $GRAPH_12 := xqllib:optional($GRAPH_0,$GRAPH_11,("SDR_0"))
  return
    $GRAPH_12
return
