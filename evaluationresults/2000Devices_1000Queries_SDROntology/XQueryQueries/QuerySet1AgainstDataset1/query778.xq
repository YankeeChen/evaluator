...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE313_0 := fn:data($node_USRPE313_0/@rdf:about)
    for $node_Processor_0 in $node_USRPE313_0/Nuvio:partOf
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:resource)
    where fn:exists($value_Processor_0) and fn:exists($node_USRPE313_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Processor_0 in $node_DAC_0/Nuvio:partOf
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:resource)
    where fn:exists($node_DAC_0) and fn:exists($value_Processor_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_DAC_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Instance1700"]/Nuvio:partOf
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("DAC_0"))
