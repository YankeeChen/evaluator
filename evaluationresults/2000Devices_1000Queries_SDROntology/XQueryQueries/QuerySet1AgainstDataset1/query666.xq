...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Attribute_1 in $node_Entity_0/Nuvio:isExpressedBy
    let $value_Attribute_1 := fn:data($node_Attribute_1/@rdf:resource)
    where fn:exists($value_Attribute_1) and fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_Attribute_0/RFDevice:hasEffectiveRadiatedPower
