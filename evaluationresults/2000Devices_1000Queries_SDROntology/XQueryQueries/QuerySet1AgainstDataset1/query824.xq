...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_BistaticRadar_0 in $doc_ObjectDescription[Nuvio:partOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#SuperheterodyneReceiver_instance18"]
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:about)
    where fn:exists($node_BistaticRadar_0)
    return
      <xqllib:result>
