...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_Object_0/RFDevice:hasProducer
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Object_0 in $node_RangeGatePullOffJamming_0/Nuvio:participatesIn
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($value_Object_0) and fn:exists($node_RangeGatePullOffJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_RangeGatePullOffJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Power_instance5"]/RFDevice:hasPulseRepetitionFrequency
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_0,$GRAPH_7,("USRPN2xx_0"))
  return
    $GRAPH_8
return
