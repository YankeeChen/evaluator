...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE313_0 in $node_Processor_0/Nuvio:compositeOf
    let $value_USRPE313_0 := fn:data($node_USRPE313_0/@rdf:resource)
    where fn:exists($node_Processor_0) and fn:exists($value_USRPE313_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Processor_0 in $doc_ObjectDescription[RFDevice:isProducerOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription2000.owl#Producer_instance552"]
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:about)
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("Processor_0"))
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("USRPE313_0"))
  return
    $GRAPH_4
return
