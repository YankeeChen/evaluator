...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_FixedBandwidthSensing_0) and fn:exists($node_Var0) and $node_FixedBandwidthSensing_0/Nuvio:relevantIndividual/@rdf:resource = $value_Var0
    return
      <xqllib:result>
