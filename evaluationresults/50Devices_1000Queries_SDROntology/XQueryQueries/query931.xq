...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:about)
    for $node_UnmannedPlatform_0 in $node_SDR_0/RFDevice:isDeployedOn
    let $value_UnmannedPlatform_0 := fn:data($node_UnmannedPlatform_0/@rdf:resource)
    where fn:exists($node_SDR_0) and fn:exists($value_UnmannedPlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="SDR_0">{$value_SDR_0}</xqllib:var>
        <xqllib:var name="UnmannedPlatform_0">{$value_UnmannedPlatform_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_2 := xqllib:optional($GRAPH_0,$GRAPH_1,("SDR_0"))
  return
    $GRAPH_2
return
