...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_RadarJamming_0 in $node_Object_0/Nuvio:participatesIn
    let $value_RadarJamming_0 := fn:data($node_RadarJamming_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_RadarJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_RadarJamming_0 in $doc_ObjectDescription[Nuvio:hasParticipant/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription50.owl#PulseDopplerRadar_instance1"]
