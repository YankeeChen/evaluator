...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:about)
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_GNURadio_0/Nuvio:partOf
    ...
    where fn:exists($node_GNURadio_0) and fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="GNURadio_0">{$value_GNURadio_0}</xqllib:var>
