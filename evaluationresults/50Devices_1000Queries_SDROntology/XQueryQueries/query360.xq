...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadioComponent_0 in $node_ADC_0/Nuvio:partOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($node_ADC_0) and fn:exists($value_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_ADC_0 in $node_Entity_0/RFDevice:hasResolution
    let $value_ADC_0 := fn:data($node_ADC_0/@rdf:resource)
    where fn:exists($value_ADC_0) and fn:exists($node_Entity_0)
    return
      <xqllib:result>
        <xqllib:var name="ADC_0">{$value_ADC_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#USRPE312"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_7 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription50.owl#Power_instance2"]/RFDevice:hasEffectiveRadiatedPower
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_3,$GRAPH_10,("ADC_0"))
