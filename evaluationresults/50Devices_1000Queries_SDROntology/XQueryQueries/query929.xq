...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Var1 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#Decibel"]/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_7 :=  
    ...
    for $node_Var1 in $node_SpatialUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_SpatialUnit_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_5,$GRAPH_9,("Var1"))
  let $GRAPH_11 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
  ...
  let $GRAPH_12 := $GRAPH_11[not((data(xqllib:var[@name="DataValue0"]) > xs:decimal("0")))]
  let $GRAPH_13 := ($GRAPH_10,$GRAPH_12)
  let $GRAPH_14 := xqllib:and($GRAPH_3,$GRAPH_13,("Var1"))
  let $GRAPH_15 := xqllib:and($GRAPH_2,$GRAPH_14,("Var0"))
  let $GRAPH_16 := xqllib:and($GRAPH_1,$GRAPH_15,("Var0"))
  let $GRAPH_17 := xqllib:and($GRAPH_0,$GRAPH_16,("USRPB205mini_i_0"))
  return
    $GRAPH_17
return
