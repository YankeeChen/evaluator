...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_IEDJammer_0/Nuvio:aggregateOf
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:resource)
    where fn:exists($node_IEDJammer_0) and fn:exists($value_USRPN210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_IEDJammer_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription50.owl#SpotJamming_instance0"]/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("IEDJammer_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("USRPN210_0"))
  return
    $GRAPH_7
return
