...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN3xx_0 in $node_Receiver_0/Nuvio:compositeOf
    let $value_USRPN3xx_0 := fn:data($node_USRPN3xx_0/@rdf:resource)
    where fn:exists($node_Receiver_0) and fn:exists($value_USRPN3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Receiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription50.owl#RFFrontEnd_instance1"]/Nuvio:aggregateOf
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("Receiver_0"))
