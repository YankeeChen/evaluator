...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_ContinuousWaveRadar_0 in $doc_ObjectDescription[RFDevice:isDeployedOn/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription50.owl#Ship_instance0"]
    let $value_ContinuousWaveRadar_0 := fn:data($node_ContinuousWaveRadar_0/@rdf:about)
    where fn:exists($node_ContinuousWaveRadar_0)
    return
      <xqllib:result>
