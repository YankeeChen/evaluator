...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE313_0 in $node_Entity_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#CompositeSolid"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
