...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX310_0 in $node_Series7FPGA_0/Nuvio:compositeOf
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:resource)
    where fn:exists($node_Series7FPGA_0) and fn:exists($value_USRPX310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("Series7FPGA_0"))
