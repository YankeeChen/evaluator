...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_Entity_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription50.owl#DataSize_instance2"]/RFDevice:hasResolution
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Entity_0 in $node_RadarModulation_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_RadarModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("RadarModulation_0"))
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_0,$GRAPH_11,("PulseRadar_0"))
  return
    $GRAPH_12
return
