...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_0 in $node_Var0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_4 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_InformationEntity_0 in $node_Var1/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_6 :=  
    for $node_Var2 in $doc_ObjectDescription
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
    for $node_AnalogComponent_0 in $node_Var2/Nuvio:aggregateOf
    let $value_AnalogComponent_0 := fn:data($node_AnalogComponent_0/@rdf:resource)
    where fn:exists($value_AnalogComponent_0) and fn:exists($node_Var2)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("AnalogComponent_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("Var2"))
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("InformationEntity_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_3,$GRAPH_10,("Var1"))
  let $GRAPH_12 := xqllib:and($GRAPH_2,$GRAPH_11,("Var0"))
  let $GRAPH_13 := xqllib:and($GRAPH_1,$GRAPH_12,("Var0"))
  let $GRAPH_14 := xqllib:and($GRAPH_0,$GRAPH_13,("USRPB200_0"))
  return
    $GRAPH_14
return
