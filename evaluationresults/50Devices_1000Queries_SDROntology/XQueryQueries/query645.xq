...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_Jammer_0/Nuvio:aggregateOf
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Jammer_0 in $node_SelfProtectionJamming_0/RFDevice:hasCapability
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($value_Jammer_0) and fn:exists($node_SelfProtectionJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("Jammer_0"))
