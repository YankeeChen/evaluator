...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_Entity_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#FrequencyModulation"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_5 :=  
    ...
    let $value_InformationEntity_0 := fn:data($node_InformationEntity_0/@rdf:about)
    for $node_MultiGeometry_0 in $doc_ObjectDescription
    let $value_MultiGeometry_0 := fn:data($node_MultiGeometry_0/@rdf:about)
    where fn:exists($node_InformationEntity_0) and fn:exists($node_MultiGeometry_0) and $node_MultiGeometry_0/Nuvio:expresses/@rdf:resource = $value_InformationEntity_0
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
        <xqllib:var name="MultiGeometry_0">{$value_MultiGeometry_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 :=  
    for $node_MultiGeometry_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#MultiGeometry"]
    let $value_MultiGeometry_0 := fn:data($node_MultiGeometry_0/@rdf:about)
    where fn:exists($node_MultiGeometry_0)
    return
      <xqllib:result>
        <xqllib:var name="MultiGeometry_0">{$value_MultiGeometry_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("MultiGeometry_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("InformationEntity_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("Entity_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_0,$GRAPH_9,("USRPN2xx_0"))
  return
    $GRAPH_10
return
