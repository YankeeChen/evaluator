...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#CubicSpline"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_11 := xqllib:optional($GRAPH_0,$GRAPH_10,("USRPE3xx_0"))
  return
    $GRAPH_11
return
