...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_QuantityRange_0 in $node_Var0/RFDevice:hasFrequencyAccuracy
  ...
  let $GRAPH_7 :=  
    for $node_QuantityRange_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#Freq_30kHz"]/RFDevice:hasMin
    let $value_QuantityRange_0 := fn:data($node_QuantityRange_0/@rdf:resource)
