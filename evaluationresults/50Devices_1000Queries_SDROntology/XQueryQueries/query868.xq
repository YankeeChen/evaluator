...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RFSystem_0 := fn:data($node_RFSystem_0/@rdf:about)
    for $node_Entity_0 in $node_RFSystem_0/RFDevice:hasProducer
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_InformationEntity_0) and $doc_ObjectDescription[@rdf:ID = "Meter"]/Nuvio:expresses/@rdf:resource = $value_InformationEntity_0
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
      </xqllib:result>
