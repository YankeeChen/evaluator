...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Satellite_0 in $doc_ObjectDescription[RFDevice:supportsTuning/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription50.owl#Moving_instance1"]
    let $value_Satellite_0 := fn:data($node_Satellite_0/@rdf:about)
    where fn:exists($node_Satellite_0)
    return
      <xqllib:result>
