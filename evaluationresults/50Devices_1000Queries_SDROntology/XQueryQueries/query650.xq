...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Process_1) and $node_Process_1/Nuvio:expresses/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_IntValue_0 in $node_Entity_0/Nuvio:relevantIndividual
    let $value_IntValue_0 := fn:data($node_IntValue_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_IntValue_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_IntValue_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#IntValue"]
    let $value_IntValue_0 := fn:data($node_IntValue_0/@rdf:about)
    where fn:exists($node_IntValue_0)
    return
      <xqllib:result>
        <xqllib:var name="IntValue_0">{$value_IntValue_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_11 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Process_0) and $node_Process_0/RFDevice:supportsTransmitting/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
