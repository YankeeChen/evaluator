...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_Satellite_0/RFDevice:isDeployedOn
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:resource)
    where fn:exists($node_Satellite_0) and fn:exists($value_USRPB2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Satellite_0 in $doc_ObjectDescription[RFDevice:supportsTransmitting/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription50.owl#Moving_instance1"]
    let $value_Satellite_0 := fn:data($node_Satellite_0/@rdf:about)
