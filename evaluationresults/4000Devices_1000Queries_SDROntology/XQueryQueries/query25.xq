...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Jammer_0/Nuvio:compositeOf
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_USRPB200mini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Entity_1) and $node_Entity_1/Nuvio:expresses/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_AmplitudeModulation_0) and fn:exists($node_Entity_0) and $node_AmplitudeModulation_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_8 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_QuadratureAmplitudeModulation_0 in $node_Entity_0/Nuvio:relevantIndividual
    let $value_QuadratureAmplitudeModulation_0 := fn:data($node_QuadratureAmplitudeModulation_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_QuadratureAmplitudeModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_8,$GRAPH_9,("QuadratureAmplitudeModulation_0"))
  let $GRAPH_11 :=  
  ...
  let $GRAPH_16 :=  
    ...
    for $node_Jammer_0 in $node_Entity_2/RFDevice:isProducerOf
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($node_Entity_2) and fn:exists($value_Jammer_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_2">{$value_Entity_2}</xqllib:var>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_17 := xqllib:optional($GRAPH_15,$GRAPH_16,("Jammer_0"))
  let $GRAPH_18 := xqllib:and($GRAPH_2,$GRAPH_17,("Jammer_0"))
  let $GRAPH_19 := xqllib:and($GRAPH_1,$GRAPH_18,("Jammer_0"))
  let $GRAPH_20 := xqllib:and($GRAPH_0,$GRAPH_19,("USRPB200mini_0"))
  return
    $GRAPH_20
return
