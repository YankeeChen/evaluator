...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Process_0 in $node_Mixer_0/Nuvio:hasParticipant
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_Mixer_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:optional($GRAPH_3,$GRAPH_4,("Mixer_0"))
