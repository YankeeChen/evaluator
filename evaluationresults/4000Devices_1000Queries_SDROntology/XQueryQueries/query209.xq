...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:about)
    for $node_Entity_0 in $node_MonostaticRadar_0/RFDevice:hasMaxPowerConsumption
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Value_0 in $node_InformationUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Value_0 := fn:data($node_Value_0/@rdf:resource)
    where fn:exists($node_InformationUnit_0) and fn:exists($value_Value_0)
    return
      <xqllib:result>
