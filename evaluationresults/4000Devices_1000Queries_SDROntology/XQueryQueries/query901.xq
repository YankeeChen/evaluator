...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN310_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_USRPN310_0 := fn:data($node_USRPN310_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_USRPN310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_RadioComponent_0 in $doc_ObjectDescription
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:about)
    for $node_Object_0 in $node_RadioComponent_0/RFDevice:hasProducer
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Object_0) and $doc_ObjectDescription[@rdf:ID = "Frequency_instance458"]/RFDevice:hasLevel/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
      </xqllib:result>
