...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX310_0 in $node_Kintex7FPGA_0/Nuvio:partOf
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:resource)
    where fn:exists($node_Kintex7FPGA_0) and fn:exists($value_USRPX310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Kintex7FPGA_0 in $node_Antenna_0/Nuvio:partOf
    let $value_Kintex7FPGA_0 := fn:data($node_Kintex7FPGA_0/@rdf:resource)
    where fn:exists($node_Antenna_0) and fn:exists($value_Kintex7FPGA_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Antenna_0 in $node_Var0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_6 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Frequency_instance210"]/RFDevice:hasWidth
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("Antenna_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_1,$GRAPH_10,("Kintex7FPGA_0"))
