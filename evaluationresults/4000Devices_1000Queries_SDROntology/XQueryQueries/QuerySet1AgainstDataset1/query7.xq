...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarSystem_0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_4 :=  
    ...
    for $node_InformationEntity_0 in $node_RFFrontEnd_0/Nuvio:isExpressedBy
    let $value_InformationEntity_0 := fn:data($node_InformationEntity_0/@rdf:resource)
    where fn:exists($value_InformationEntity_0) and fn:exists($node_RFFrontEnd_0)
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_0,$GRAPH_6,("RadarSystem_0"))
  return
    $GRAPH_7
return
