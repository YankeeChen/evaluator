...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_210_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_USRPN300_0 in $node_Entity_0/Nuvio:participatesIn
  ...
  let $GRAPH_7 :=  
    ...
    for $node_USRPN300_0 in $node_Var0/Nuvio:partOf
    let $value_USRPN300_0 := fn:data($node_USRPN300_0/@rdf:resource)
    where fn:exists($value_USRPN300_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
        <xqllib:var name="USRPN300_0">{$value_USRPN300_0}</xqllib:var>
  ...
  let $GRAPH_8 :=  
    for $node_Var0 in $doc_ObjectDescription[RFDevice:hasGain/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#Freq_122_88MHz"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_9 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Freq_125MHz"]/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_14 := xqllib:optional($GRAPH_6,$GRAPH_13,("USRPN300_0"))
