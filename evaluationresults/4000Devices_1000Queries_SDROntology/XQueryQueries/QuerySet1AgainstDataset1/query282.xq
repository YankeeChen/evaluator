...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RFSystem_0 := fn:data($node_RFSystem_0/@rdf:about)
    for $node_Var0 in $node_RFSystem_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Point"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) > 943355695)]
  ...
  let $GRAPH_19 := xqllib:and($GRAPH_0,$GRAPH_18,("RFSystem_0"))
  return
    $GRAPH_19
return
