...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_USRPB200mini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_RadioComponent_0 in $doc_ObjectDescription
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:about)
    for $node_Object_0 in $node_RadioComponent_0/RFDevice:isProducerOf
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($value_Object_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_PhaseModulation_0) and $node_PhaseModulation_0/Nuvio:hasParticipant/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("PhaseModulation_0"))
