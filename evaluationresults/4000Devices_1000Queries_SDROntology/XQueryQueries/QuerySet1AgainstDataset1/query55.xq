...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/sf#MultiCurve"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_LatitudeValueInDegree_0 in $node_Entity_0/Nuvio:hasValue
  ...
  let $GRAPH_11 :=  
    ...
    let $value_LatitudeValueInDegree_0 := fn:data($node_LatitudeValueInDegree_0/@rdf:about)
    for $node_DataValue0 in $node_LatitudeValueInDegree_0/RFDevice:hasFloatValue
  ...
  let $GRAPH_12 := $GRAPH_11[(data(xqllib:var[@name="DataValue0"]) <= xs:decimal("90"))]
  let $GRAPH_13 := xqllib:and($GRAPH_10,$GRAPH_12,("LatitudeValueInDegree_0"))
  let $GRAPH_14 := xqllib:and($GRAPH_9,$GRAPH_13,("LatitudeValueInDegree_0"))
