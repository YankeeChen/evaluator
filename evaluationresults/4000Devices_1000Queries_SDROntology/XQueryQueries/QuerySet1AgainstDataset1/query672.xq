...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_Attribute_0/Nuvio:expresses
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#Port"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_11 :=  
    ...
    for $node_Var0 in $node_AnalogComponent_0/Nuvio:relevantIndividual
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_AnalogComponent_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_11,$GRAPH_12,("AnalogComponent_0"))
