...
let $result :=  
  ...
  let $GRAPH_6 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_MultiSurface_0) and $node_MultiSurface_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_7 :=  
    ...
    for $node_Entity_0 in $node_ModelX_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_8 :=  
    ...
    for $node_ModelX_0 in $node_Entity_1/RFDevice:hasHeight
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:resource)
    where fn:exists($node_Entity_1) and fn:exists($value_ModelX_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_1">{$value_Entity_1}</xqllib:var>
        <xqllib:var name="ModelX_0">{$value_ModelX_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("ModelX_0"))
