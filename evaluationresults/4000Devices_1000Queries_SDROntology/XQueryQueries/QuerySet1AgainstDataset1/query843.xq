...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_RadarModulation_0/RFDevice:supportsJamming
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:resource)
    where fn:exists($node_RadarModulation_0) and fn:exists($value_SurveillanceRadar_0)
    return
      <xqllib:result>
