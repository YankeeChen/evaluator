...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:about)
    for $node_Entity_0 in $node_TrackingRadar_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_Mixer_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_Mixer_0 := fn:data($node_Mixer_0/@rdf:resource)
    where fn:exists($value_Mixer_0) and fn:exists($node_Situation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_0,$GRAPH_6,("TrackingRadar_0"))
  return
    $GRAPH_7
return
