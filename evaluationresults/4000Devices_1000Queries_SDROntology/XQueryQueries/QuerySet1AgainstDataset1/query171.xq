...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:about)
    for $node_Entity_0 in $node_GNURadio_0/Nuvio:partOf
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#CompositeSurface"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_MultiPolygon_0 in $doc_ObjectDescription
    let $value_MultiPolygon_0 := fn:data($node_MultiPolygon_0/@rdf:about)
    where fn:exists($node_Entity_0) and fn:exists($node_MultiPolygon_0) and $node_MultiPolygon_0/Nuvio:hasParticipant/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
        <xqllib:var name="MultiPolygon_0">{$value_MultiPolygon_0}</xqllib:var>
      </xqllib:result>
