...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CR_0 in $node_Jammer_0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Jammer_0 in $node_FreqencyRange_0/RFDevice:hasFrequencyAccuracy
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($node_FreqencyRange_0) and fn:exists($value_Jammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_FreqencyRange_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#Freq_330MHz"]/RFDevice:hasMax
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Jammer_0"))
