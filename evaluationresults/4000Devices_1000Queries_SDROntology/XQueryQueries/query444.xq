...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[RFDevice:hasBandwidthRange/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#RadioFrequencyBand_instance260"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
