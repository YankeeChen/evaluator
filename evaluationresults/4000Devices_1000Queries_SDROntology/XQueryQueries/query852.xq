...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Var0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) <= xs:double("-57.673763"))]
  let $GRAPH_8 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_15 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Freq_800MHz"]/Nuvio:hasObjectQuantity
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_0,$GRAPH_17,("USRPX300_0"))
  return
    $GRAPH_18
return
