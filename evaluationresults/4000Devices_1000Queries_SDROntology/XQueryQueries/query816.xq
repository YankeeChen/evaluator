...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CommunicationsSystem_0 in $node_Var0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Transmitting_0) and fn:exists($node_Var0) and $node_Transmitting_0/Nuvio:subprocessOf/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Transmitting_0 in $doc_ObjectDescription
    let $value_Transmitting_0 := fn:data($node_Transmitting_0/@rdf:about)
    for $node_Attribute_0 in $node_Transmitting_0/RFDevice:hasMaxOutputPower
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:resource)
    where fn:exists($value_Attribute_0) and fn:exists($node_Transmitting_0)
    return
      <xqllib:result>
        <xqllib:var name="Attribute_0">{$value_Attribute_0}</xqllib:var>
        <xqllib:var name="Transmitting_0">{$value_Transmitting_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("Transmitting_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Transmitting_0"))
  let $GRAPH_7 :=  
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("Attribute_0","Var0"))
  let $GRAPH_11 := xqllib:and($GRAPH_1,$GRAPH_10,("Var0"))
  let $GRAPH_12 := xqllib:optional($GRAPH_0,$GRAPH_11,("CommunicationsSystem_0"))
  return
    $GRAPH_12
return
