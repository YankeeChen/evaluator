...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN3xx_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_USRPN3xx_0 := fn:data($node_USRPN3xx_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_USRPN3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_RadioComponent_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:compositeOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($value_RadioComponent_0)
    return
      <xqllib:result>
