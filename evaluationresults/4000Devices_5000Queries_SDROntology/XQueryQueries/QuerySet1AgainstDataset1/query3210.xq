...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_InformationEntity_0 in $node_IEDTrigger_0/Nuvio:isExpressedBy
    let $value_InformationEntity_0 := fn:data($node_InformationEntity_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_InformationEntity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_CortexA9_0 in $node_Var0/Nuvio:hasParticipant
    let $value_CortexA9_0 := fn:data($node_CortexA9_0/@rdf:resource)
    where fn:exists($value_CortexA9_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("CortexA9_0"))
