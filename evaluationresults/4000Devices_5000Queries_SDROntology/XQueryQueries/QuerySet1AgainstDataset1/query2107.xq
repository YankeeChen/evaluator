...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB205mini_i_0 in $node_Var0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_4 :=  
    for $node_Var2 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Value"]
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
  ...
  let $GRAPH_5 :=  
    for $node_Var2 in $doc_ObjectDescription
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
    for $node_DataValue0 in $node_Var2/RFDevice:hasFloatValue
  ...
  let $GRAPH_6 := $GRAPH_5[not((data(xqllib:var[@name="DataValue0"]) > xs:decimal("10")))]
  let $GRAPH_7 := xqllib:optional($GRAPH_4,$GRAPH_6,("Var2"))
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("Var2"))
  ...
  let $GRAPH_13 := xqllib:optional($GRAPH_0,$GRAPH_12,("USRPB205mini_i_0"))
  return
    $GRAPH_13
return
