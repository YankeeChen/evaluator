...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_USRPN200_0 := fn:data($node_USRPN200_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_USRPN200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_IEDTrigger_0 in $doc_ObjectDescription
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:about)
    for $node_Antenna_0 in $node_IEDTrigger_0/Nuvio:aggregateOf
    let $value_Antenna_0 := fn:data($node_Antenna_0/@rdf:resource)
    where fn:exists($value_Antenna_0) and fn:exists($node_IEDTrigger_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Antenna_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#Antenna"]
    let $value_Antenna_0 := fn:data($node_Antenna_0/@rdf:about)
    where fn:exists($node_Antenna_0)
    return
      <xqllib:result>
        <xqllib:var name="Antenna_0">{$value_Antenna_0}</xqllib:var>
      </xqllib:result>
