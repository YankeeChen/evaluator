...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Jammer_0 in $node_DRFMJamming_0/RFDevice:hasCapability
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($node_DRFMJamming_0) and fn:exists($value_Jammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_DRFMJamming_0 in $node_Quantity_0/RFDevice:hasMaxSensedPower
