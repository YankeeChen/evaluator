...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Attribute_0 in $node_Var0/Nuvio:hasProcessQuantity
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue0"]) > xs:double("-91.7315"))]
  ...
  let $GRAPH_13 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_CrossPolarizationJamming_0) and fn:exists($node_Var0) and $node_CrossPolarizationJamming_0/Nuvio:expresses/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_20 := xqllib:and($GRAPH_0,$GRAPH_19,("USRPB200mini_i_0"))
  return
    $GRAPH_20
return
