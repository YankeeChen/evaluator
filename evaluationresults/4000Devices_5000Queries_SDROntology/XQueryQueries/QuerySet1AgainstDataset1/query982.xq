...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CR_0 in $node_Ship_0/RFDevice:isDeployedOn
    let $value_CR_0 := fn:data($node_CR_0/@rdf:resource)
    where fn:exists($value_CR_0) and fn:exists($node_Ship_0)
    return
      <xqllib:result>
        <xqllib:var name="CR_0">{$value_CR_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("Ship_0"))
