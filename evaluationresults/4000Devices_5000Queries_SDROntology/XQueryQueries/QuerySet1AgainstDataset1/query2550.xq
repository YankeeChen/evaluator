...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:about)
    for $node_Var0 in $node_USRPX310_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Var0 in $node_Jammer_0/Nuvio:aggregateOf
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Jammer_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#RadioFrequencyBand_instance118"]/RFDevice:hasFrequencyAccuracy
