...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#LatitudeValueInRadian"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_0,$GRAPH_11,("USRPB2xxmini_0"))
  return
    $GRAPH_12
return
