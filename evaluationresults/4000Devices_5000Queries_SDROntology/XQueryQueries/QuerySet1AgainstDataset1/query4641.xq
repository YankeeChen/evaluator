...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE313_0 in $node_Var0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#FeatureDetection"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Var0) and fn:exists($node_XC7K325TFPGA_0) and $node_XC7K325TFPGA_0/Nuvio:expresses/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("XC7K325TFPGA_0"))
  ...
  let $GRAPH_8 :=  
    ...
    for $node_Var0 in $node_LinearRing_0/Nuvio:relevantIndividual
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_LinearRing_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_0,$GRAPH_11,("USRPE313_0"))
  return
    $GRAPH_12
return
