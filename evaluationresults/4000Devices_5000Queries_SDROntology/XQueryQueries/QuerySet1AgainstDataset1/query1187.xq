...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:about)
    for $node_Var0 in $node_SearchRadar_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_2 :=  
    ...
    for $node_RangeGatePullOffJamming_0 in $node_Var0/Nuvio:hasSubprocess
    let $value_RangeGatePullOffJamming_0 := fn:data($node_RangeGatePullOffJamming_0/@rdf:resource)
    where fn:exists($value_RangeGatePullOffJamming_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_RangeGatePullOffJamming_0 in $node_Entity_0/RFDevice:hasEffectiveRadiatedPower
    let $value_RangeGatePullOffJamming_0 := fn:data($node_RangeGatePullOffJamming_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_RangeGatePullOffJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Entity_0 in $node_Attribute_0/RFDevice:hasPulseRepetitionInterval
  ...
  let $GRAPH_13 := $GRAPH_12[(data(xqllib:var[@name="DataValue0"]) >= -1370582151)]
  let $GRAPH_14 := xqllib:and($GRAPH_11,$GRAPH_13,("Entity_0"))
  let $GRAPH_15 :=  
  ...
  let $GRAPH_20 := xqllib:and($GRAPH_3,$GRAPH_19,("RangeGatePullOffJamming_0"))
  let $GRAPH_21 := xqllib:optional($GRAPH_2,$GRAPH_20,("RangeGatePullOffJamming_0"))
  let $GRAPH_22 := xqllib:and($GRAPH_1,$GRAPH_21,("Var0"))
  let $GRAPH_23 := xqllib:and($GRAPH_0,$GRAPH_22,("SearchRadar_0"))
  return
    $GRAPH_23
return
