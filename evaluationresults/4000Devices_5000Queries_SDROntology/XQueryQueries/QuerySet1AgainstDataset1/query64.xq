...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN300_0 in $node_Object_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_AmplitudeShiftKeyingModulation_0 in $node_Object_0/Nuvio:participatesIn
    let $value_AmplitudeShiftKeyingModulation_0 := fn:data($node_AmplitudeShiftKeyingModulation_0/@rdf:resource)
    where fn:exists($value_AmplitudeShiftKeyingModulation_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
