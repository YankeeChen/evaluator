...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Width_0 in $node_Object_0/RFDevice:hasMaxPowerConsumption
    let $value_Width_0 := fn:data($node_Width_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_Width_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("Width_0"))
