...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarSystem_0 in $node_Var0/RFDevice:hasCapability
  ...
  let $GRAPH_5 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Power_instance13"]/RFDevice:hasPulseRepetitionInterval
  ...
  let $GRAPH_7 :=  
    ...
    for $node_Var0 in $node_RadarJammer_0/RFDevice:hasTarget
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_RadarJammer_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("RadarJammer_0"))
