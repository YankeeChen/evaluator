...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_Entity_0/RFDevice:hasHeight
  ...
  let $GRAPH_6 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Weight_0) and $node_Weight_0/RFDevice:hasNoiseFigure/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_7,$GRAPH_10,("Weight_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_6,$GRAPH_11,("Weight_0"))
