...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_SelfProtectionJamming_0/RFDevice:supportsTransmitting
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:resource)
    where fn:exists($value_JammingSystem_0) and fn:exists($node_SelfProtectionJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="JammingSystem_0">{$value_JammingSystem_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_SelfProtectionJamming_0 in $doc_ObjectDescription
    let $value_SelfProtectionJamming_0 := fn:data($node_SelfProtectionJamming_0/@rdf:about)
    for $node_Attribute_0 in $node_SelfProtectionJamming_0/RFDevice:hasPulseRepetitionInterval
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:resource)
    where fn:exists($value_Attribute_0) and fn:exists($node_SelfProtectionJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("SelfProtectionJamming_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("JammingSystem_0"))
  return
    $GRAPH_7
return
