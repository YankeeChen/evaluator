...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:about)
    for $node_FPGA_0 in $node_USRPB200_0/Nuvio:compositeOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0) and fn:exists($node_USRPB200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Producer_instance671"]/RFDevice:isProducerOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("USRPB200_0"))
  return
    $GRAPH_7
return
