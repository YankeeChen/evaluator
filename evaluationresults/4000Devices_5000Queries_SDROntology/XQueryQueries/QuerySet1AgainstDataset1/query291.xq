...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_SpatialUnit_0) and $node_SpatialUnit_0/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) > -399053632)]
  let $GRAPH_6 :=  
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("Entity_0","SpatialUnit_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("Entity_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("Entity_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_0,$GRAPH_10,("SearchRadar_0"))
  return
    $GRAPH_11
return
