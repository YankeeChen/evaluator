...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN310_0 in $node_Processor_0/Nuvio:compositeOf
    let $value_USRPN310_0 := fn:data($node_USRPN310_0/@rdf:resource)
    where fn:exists($node_Processor_0) and fn:exists($value_USRPN310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Processor_0 in $doc_ObjectDescription
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:about)
    for $node_Mixer_0 in $node_Processor_0/Nuvio:partOf
    let $value_Mixer_0 := fn:data($node_Mixer_0/@rdf:resource)
    where fn:exists($value_Mixer_0) and fn:exists($node_Processor_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Mixer_0 in $node_IEDTrigger_0/Nuvio:aggregateOf
    let $value_Mixer_0 := fn:data($node_Mixer_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_Mixer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    for $node_IEDTrigger_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#RadioFrequencyBand_instance47"]/RFDevice:hasFrequencyAccuracy
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("IEDTrigger_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("Mixer_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_3,$GRAPH_8,("Mixer_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("Processor_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_1,$GRAPH_10,("Processor_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_0,$GRAPH_11,("USRPN310_0"))
  return
    $GRAPH_12
return
