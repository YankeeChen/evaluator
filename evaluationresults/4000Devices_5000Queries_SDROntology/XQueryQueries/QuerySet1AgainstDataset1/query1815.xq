...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:about)
    for $node_MannedPlatform_0 in $node_USRPB200mini_i_0/RFDevice:isDeployedOn
    let $value_MannedPlatform_0 := fn:data($node_MannedPlatform_0/@rdf:resource)
    where fn:exists($value_MannedPlatform_0) and fn:exists($node_USRPB200mini_i_0)
    return
      <xqllib:result>
        <xqllib:var name="MannedPlatform_0">{$value_MannedPlatform_0}</xqllib:var>
        <xqllib:var name="USRPB200mini_i_0">{$value_USRPB200mini_i_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_2 := xqllib:and($GRAPH_0,$GRAPH_1,("USRPB200mini_i_0"))
  return
    $GRAPH_2
return
