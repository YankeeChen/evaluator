...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_Process_0/RFDevice:supportsTransmitting
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:resource)
    where fn:exists($node_Process_0) and fn:exists($value_SearchRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Process_0 in $node_DataSize_0/RFDevice:hasMaxOutputPower
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_DataSize_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("DataSize_0"))
