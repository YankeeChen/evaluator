...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_MonostaticRadar_0 in $node_Entity_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_AmplitudeShiftKeyingModulation_0) and fn:exists($node_Entity_0) and $node_AmplitudeShiftKeyingModulation_0/Nuvio:hasSubprocess/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_Situation_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#FreqencyRange_instance4"]/Nuvio:relevantIndividual
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,())
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Entity_0","Situation_0"))
