...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN300_0 in $node_Entity_0/RFDevice:hasLevel
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Object_0) and $doc_ObjectDescription[@rdf:ID = "Weight_instance215"]/RFDevice:hasSamplingRate/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
      </xqllib:result>
