...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Var0) and fn:exists($node_Voltage_0) and $node_Voltage_0/RFDevice:hasPulseRepetitionInterval/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Voltage_0 in $node_Var1/Nuvio:hasValue
    let $value_Voltage_0 := fn:data($node_Voltage_0/@rdf:resource)
    where fn:exists($node_Var1) and fn:exists($value_Voltage_0)
    return
      <xqllib:result>
