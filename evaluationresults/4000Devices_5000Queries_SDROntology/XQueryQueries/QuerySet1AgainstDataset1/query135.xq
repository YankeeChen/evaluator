...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_IEDJammer_0 in $doc_ObjectDescription
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:about)
    for $node_Process_0 in $node_IEDJammer_0/RFDevice:supportsJamming
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_IEDJammer_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
        <xqllib:var name="IEDJammer_0">{$value_IEDJammer_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Angle_0) and fn:exists($node_Process_0) and $node_Angle_0/RFDevice:hasTransmitPowerRange/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_4,$GRAPH_5,("Angle_0"))
