...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Entity_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_5 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Weight_0) and $node_Weight_0/RFDevice:hasWidth/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("Weight_0"))
  let $GRAPH_8 :=  
