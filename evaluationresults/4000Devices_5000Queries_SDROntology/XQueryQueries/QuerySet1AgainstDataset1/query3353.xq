...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Polygon"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue0"]) >= xs:double("-62.396286"))]
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_1,$GRAPH_14,("MobilePlatform_0"))
