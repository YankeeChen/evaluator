...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:about)
    for $node_NoiseJamming_0 in $node_JammingSystem_0/RFDevice:hasCapability
    let $value_NoiseJamming_0 := fn:data($node_NoiseJamming_0/@rdf:resource)
    where fn:exists($node_JammingSystem_0) and fn:exists($value_NoiseJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="JammingSystem_0">{$value_JammingSystem_0}</xqllib:var>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("NoiseJamming_0"))
