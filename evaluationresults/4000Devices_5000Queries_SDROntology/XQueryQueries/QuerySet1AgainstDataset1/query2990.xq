...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_SDR_0 in $doc_ObjectDescription[Nuvio:compositeOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#InstantaneousFrequencyMeasurementReceiver_instance123"]
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:about)
    where fn:exists($node_SDR_0)
    return
      <xqllib:result>
