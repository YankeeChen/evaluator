...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:about)
    for $node_InverseGainJamming_0 in $node_JammingSystem_0/RFDevice:supportsTransmitting
    let $value_InverseGainJamming_0 := fn:data($node_InverseGainJamming_0/@rdf:resource)
    where fn:exists($value_InverseGainJamming_0) and fn:exists($node_JammingSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_InverseGainJamming_0 in $node_Entity_0/RFDevice:hasMaxOutputPower
    let $value_InverseGainJamming_0 := fn:data($node_InverseGainJamming_0/@rdf:resource)
  ...
  let $GRAPH_6 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_FrequencyScanRate_0 in $node_Entity_0/RFDevice:hasBeamwidth
  ...
  let $GRAPH_10 := $GRAPH_9[(data(xqllib:var[@name="DataValue0"]) >= -1306684391)]
  let $GRAPH_11 := xqllib:and($GRAPH_8,$GRAPH_10,("Var0"))
  let $GRAPH_12 := xqllib:and($GRAPH_7,$GRAPH_11,("FrequencyScanRate_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_6,$GRAPH_12,("FrequencyScanRate_0"))
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_2,$GRAPH_16,("InverseGainJamming_0"))
  let $GRAPH_18 := xqllib:and($GRAPH_1,$GRAPH_17,("InverseGainJamming_0"))
  let $GRAPH_19 := xqllib:and($GRAPH_0,$GRAPH_18,("JammingSystem_0"))
  return
    $GRAPH_19
return
