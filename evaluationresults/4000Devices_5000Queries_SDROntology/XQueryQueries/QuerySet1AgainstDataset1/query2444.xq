...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_CommunicationsJamming_0) and fn:exists($node_Object_0) and $node_CommunicationsJamming_0/Nuvio:participatesIn/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_CommunicationsJamming_0 in $doc_ObjectDescription[Nuvio:hasProcessQuantity/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Power_instance60"]
    let $value_CommunicationsJamming_0 := fn:data($node_CommunicationsJamming_0/@rdf:about)
