...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:about)
    for $node_Submarine_0 in $node_RadarSystem_0/RFDevice:isDeployedOn
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:resource)
    where fn:exists($node_RadarSystem_0) and fn:exists($value_Submarine_0)
    return
      <xqllib:result>
        <xqllib:var name="RadarSystem_0">{$value_RadarSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_AntennaPolarization_0) and fn:exists($node_Process_0) and $node_AntennaPolarization_0/Nuvio:hasProcessQuality/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
