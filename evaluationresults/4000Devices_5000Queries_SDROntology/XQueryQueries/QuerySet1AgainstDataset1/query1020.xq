...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CommunicationsSystem_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:resource)
    where fn:exists($value_CommunicationsSystem_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
        <xqllib:var name="CommunicationsSystem_0">{$value_CommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_RadioComponent_0 in $doc_ObjectDescription
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:about)
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_RadioComponent_0/Nuvio:partOf
    ...
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("RadioComponent_0"))
