...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Full_DuplexCommunicationsSystem_0 := fn:data($node_Full_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_GroundPlatform_0 in $node_Full_DuplexCommunicationsSystem_0/RFDevice:isDeployedOn
    let $value_GroundPlatform_0 := fn:data($node_GroundPlatform_0/@rdf:resource)
    where fn:exists($node_Full_DuplexCommunicationsSystem_0) and fn:exists($value_GroundPlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="Full_DuplexCommunicationsSystem_0">{$value_Full_DuplexCommunicationsSystem_0}</xqllib:var>
