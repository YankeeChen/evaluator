...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_SuperheterodyneReceiver_0 in $node_SurveillanceRadar_0/Nuvio:aggregateOf
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($value_SuperheterodyneReceiver_0) and fn:exists($node_SurveillanceRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:about)
    for $node_NoiseFigure_0 in $node_SuperheterodyneReceiver_0/RFDevice:hasNoiseFigure
    let $value_NoiseFigure_0 := fn:data($node_NoiseFigure_0/@rdf:resource)
    where fn:exists($value_NoiseFigure_0) and fn:exists($node_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) <= -49.56)]
  let $GRAPH_7 := xqllib:and($GRAPH_4,$GRAPH_6,("Var0"))
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("NoiseFigure_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("SuperheterodyneReceiver_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("SuperheterodyneReceiver_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_0,$GRAPH_10,("SurveillanceRadar_0"))
  return
    $GRAPH_11
return
