...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Object_0 in $node_Gain_0/RFDevice:hasBeamwidth
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($node_Gain_0) and fn:exists($value_Object_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) > -11.87)]
  let $GRAPH_8 := xqllib:and($GRAPH_5,$GRAPH_7,("Var0"))
  let $GRAPH_9 := ($GRAPH_4,$GRAPH_8)
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("Gain_0"))
