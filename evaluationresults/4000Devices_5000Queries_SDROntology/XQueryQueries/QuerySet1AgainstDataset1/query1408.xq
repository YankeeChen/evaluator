...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_MonostaticRadar_0 in $node_Transmitter_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    for $node_Transmitter_0 in $doc_ObjectDescription[RFDevice:hasCapability/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Instance1813"]
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:about)
    where fn:exists($node_Transmitter_0)
    return
      <xqllib:result>
