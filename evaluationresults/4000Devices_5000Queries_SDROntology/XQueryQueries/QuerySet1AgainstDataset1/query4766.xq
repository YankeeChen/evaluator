...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:about)
    for $node_Ship_0 in $node_JammingSystem_0/RFDevice:isDeployedOn
    let $value_Ship_0 := fn:data($node_Ship_0/@rdf:resource)
    where fn:exists($node_JammingSystem_0) and fn:exists($value_Ship_0)
    return
      <xqllib:result>
        <xqllib:var name="JammingSystem_0">{$value_JammingSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_Ship_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Moving_instance74"]/RFDevice:supportsTuning
    let $value_Ship_0 := fn:data($node_Ship_0/@rdf:resource)
