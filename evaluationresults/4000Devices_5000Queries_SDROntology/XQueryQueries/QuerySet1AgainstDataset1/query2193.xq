...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_Var0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "AntennaPolarization_instance41"]/Nuvio:hasProcessQuality/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_0,$GRAPH_6,("USRPB210_0"))
  return
    $GRAPH_7
return
