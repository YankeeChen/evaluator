...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_XC6SLX75FPGA_0/Nuvio:compositeOf
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:resource)
    where fn:exists($value_USRPB2xx_0) and fn:exists($node_XC6SLX75FPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB2xx_0">{$value_USRPB2xx_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_XC6SLX75FPGA_0 in $node_Kintex7FPGA_0/Nuvio:compositeOf
    let $value_XC6SLX75FPGA_0 := fn:data($node_XC6SLX75FPGA_0/@rdf:resource)
    where fn:exists($node_Kintex7FPGA_0) and fn:exists($value_XC6SLX75FPGA_0)
    return
      <xqllib:result>
