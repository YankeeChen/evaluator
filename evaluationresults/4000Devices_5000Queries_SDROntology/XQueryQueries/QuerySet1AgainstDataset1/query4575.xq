...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Var0 in $node_SweepJamming_0/RFDevice:supportsJamming
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_SweepJamming_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("SweepJamming_0"))
  let $GRAPH_7 :=  
  ...
  let $GRAPH_11 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_CumulativeDetection_0 in $node_Var0/Nuvio:isExpressedBy
    let $value_CumulativeDetection_0 := fn:data($node_CumulativeDetection_0/@rdf:resource)
    where fn:exists($value_CumulativeDetection_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
