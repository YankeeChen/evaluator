...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX310_0 in $node_Var0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_OnOffKeyingModulation_0 in $node_Var0/RFDevice:supportsSpectrumSensing
    let $value_OnOffKeyingModulation_0 := fn:data($node_OnOffKeyingModulation_0/@rdf:resource)
    where fn:exists($value_OnOffKeyingModulation_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("OnOffKeyingModulation_0"))
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("Var0"))
  let $GRAPH_6 := xqllib:and($GRAPH_0,$GRAPH_5,("USRPX310_0"))
  return
    $GRAPH_6
return
