...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:about)
    for $node_DirectConversionReceiver_0 in $node_USRPE3xx_0/Nuvio:aggregateOf
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($value_DirectConversionReceiver_0) and fn:exists($node_USRPE3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription[RFDevice:hasNoiseFigure/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#NoiseFigure_instance125"]
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:about)
    where fn:exists($node_DirectConversionReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_0,$GRAPH_5,("USRPE3xx_0"))
  return
    $GRAPH_6
return
