...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:about)
    for $node_RadarJammer_0 in $node_GNURadio_0/Nuvio:compositeOf
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_GNURadio_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
        <xqllib:var name="GNURadio_0">{$value_GNURadio_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadarJammer_0 in $node_BarrageJamming_0/RFDevice:supportsJamming
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
