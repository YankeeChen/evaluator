...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_SuperheterodyneReceiver_0 in $node_AnalogComponent_0/Nuvio:compositeOf
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($node_AnalogComponent_0) and fn:exists($value_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="AnalogComponent_0">{$value_AnalogComponent_0}</xqllib:var>
        <xqllib:var name="SuperheterodyneReceiver_0">{$value_SuperheterodyneReceiver_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("SuperheterodyneReceiver_0"))
