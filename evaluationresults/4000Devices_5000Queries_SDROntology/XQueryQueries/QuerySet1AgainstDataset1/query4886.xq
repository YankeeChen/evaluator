...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN3xx_0 := fn:data($node_USRPN3xx_0/@rdf:about)
    for $node_Entity_0 in $node_USRPN3xx_0/RFDevice:hasProducer
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_NoiseFigure_0) and $node_NoiseFigure_0/Nuvio:isExpressedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue0"]) >= -1607687430)]
  ...
  let $GRAPH_12 :=  
    ...
    for $node_Entity_0 in $node_FPGA_0/Nuvio:hasParticipant
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_16 := xqllib:optional($GRAPH_0,$GRAPH_15,("USRPN3xx_0"))
  return
    $GRAPH_16
return
