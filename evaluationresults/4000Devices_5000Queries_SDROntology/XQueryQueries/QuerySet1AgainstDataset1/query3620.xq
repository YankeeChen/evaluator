...
let $result :=  
  ...
  let $GRAPH_7 :=  
    for $node_Var2 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#GroundPlatform"]
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
  ...
  let $GRAPH_8 :=  
    for $node_Var2 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#TerrainBounceJamming"]
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
    where fn:exists($node_Var2)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_6,$GRAPH_9,("Var2"))
