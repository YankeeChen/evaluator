...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SDR_0 in $node_SuperheterodyneReceiver_0/Nuvio:compositeOf
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:resource)
    where fn:exists($value_SDR_0) and fn:exists($node_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="SDR_0">{$value_SDR_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_SuperheterodyneReceiver_0 in $node_RadarFrequencyBand_0/RFDevice:hasFrequencyRange
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($node_RadarFrequencyBand_0) and fn:exists($value_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_RadarFrequencyBand_0 in $node_Attribute_0/RFDevice:hasMin
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("SuperheterodyneReceiver_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_1,$GRAPH_10,("SuperheterodyneReceiver_0"))
