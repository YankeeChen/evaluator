...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_RadioComponent_0/Nuvio:aggregateOf
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_USRPE3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_FeatureDetection_0 in $node_Entity_0/Nuvio:subprocessOf
    let $value_FeatureDetection_0 := fn:data($node_FeatureDetection_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_FeatureDetection_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) >= 568783540)]
  ...
  let $GRAPH_13 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_SpatialVolumeUnit_0 in $node_Var0/Nuvio:relevantIndividual
    let $value_SpatialVolumeUnit_0 := fn:data($node_SpatialVolumeUnit_0/@rdf:resource)
    where fn:exists($value_SpatialVolumeUnit_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_15 :=  
    ...
    for $node_Var0 in $node_VoltageUnit_0/Nuvio:relevantIndividual
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_15,$GRAPH_16,("VoltageUnit_0"))
  let $GRAPH_18 :=  
  ...
  let $GRAPH_20 := ($GRAPH_13,$GRAPH_19)
  ...
  let $GRAPH_24 := xqllib:and($GRAPH_2,$GRAPH_23,("RadioComponent_0"))
  let $GRAPH_25 := xqllib:and($GRAPH_1,$GRAPH_24,("RadioComponent_0"))
  let $GRAPH_26 := xqllib:and($GRAPH_0,$GRAPH_25,("USRPE3xx_0"))
  return
    $GRAPH_26
return
