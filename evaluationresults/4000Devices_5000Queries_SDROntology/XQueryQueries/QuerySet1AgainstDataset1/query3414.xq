...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_210_0 in $node_Var0/RFDevice:supportsJamming
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Quality_0 in $node_Var0/Nuvio:hasProcessQuality
    let $value_Quality_0 := fn:data($node_Quality_0/@rdf:resource)
    where fn:exists($value_Quality_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
