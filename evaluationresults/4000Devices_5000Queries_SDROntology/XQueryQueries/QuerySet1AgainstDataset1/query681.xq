...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:about)
    for $node_Transmitter_0 in $node_USRPB200mini_i_0/Nuvio:aggregateOf
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:resource)
    where fn:exists($value_Transmitter_0) and fn:exists($node_USRPB200mini_i_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Transmitter_0 in $node_RadioFrequencyBand_0/RFDevice:hasFrequencyRange
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:resource)
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("USRPB200mini_i_0"))
  return
    $GRAPH_7
return
