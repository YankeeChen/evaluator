...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RFSystem_0 := fn:data($node_RFSystem_0/@rdf:about)
    for $node_Entity_0 in $node_RFSystem_0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_FPGA_0/Nuvio:partOf
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_FPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    for $node_FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#XilinxZynq7020SoC_instance30"]/Nuvio:partOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="FPGA_0">{$value_FPGA_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("FPGA_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("FPGA_0"))
  let $GRAPH_9 :=  
  ...
  let $GRAPH_12 := $GRAPH_11[(data(xqllib:var[@name="DataValue0"]) >= 26.52)]
  ...
  let $GRAPH_22 := xqllib:optional($GRAPH_0,$GRAPH_21,("RFSystem_0"))
  return
    $GRAPH_22
return
