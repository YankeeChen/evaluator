...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ModelX_0 in $node_Transmitting_0/RFDevice:supportsSpectrumSensing
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:resource)
    where fn:exists($value_ModelX_0) and fn:exists($node_Transmitting_0)
    return
      <xqllib:result>
        <xqllib:var name="ModelX_0">{$value_ModelX_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Transmitting_0"))
