...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_i_0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_9 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_FrequencyModulation_0 in $node_Entity_0/Nuvio:relevantIndividual
    let $value_FrequencyModulation_0 := fn:data($node_FrequencyModulation_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_FrequencyModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_15 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Aircraft_0 in $node_Var0/RFDevice:isDeployedOn
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:resource)
    where fn:exists($value_Aircraft_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_15,$GRAPH_16,("Aircraft_0"))
  ...
  let $GRAPH_21 := xqllib:and($GRAPH_12,$GRAPH_20,("Var0"))
  let $GRAPH_22 := xqllib:and($GRAPH_11,$GRAPH_21,("InformationEntity_0"))
