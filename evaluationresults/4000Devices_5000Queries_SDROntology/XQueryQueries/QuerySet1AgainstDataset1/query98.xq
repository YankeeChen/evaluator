...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Spartan6FPGA_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_Spartan6FPGA_0 in $doc_ObjectDescription
    let $value_Spartan6FPGA_0 := fn:data($node_Spartan6FPGA_0/@rdf:about)
    for $node_Component_0 in $node_Spartan6FPGA_0/Nuvio:aggregateOf
    let $value_Component_0 := fn:data($node_Component_0/@rdf:resource)
    where fn:exists($value_Component_0) and fn:exists($node_Spartan6FPGA_0)
    return
      <xqllib:result>
