...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_DirectConversionReceiver_0/Nuvio:partOf
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_SearchRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Instance4215"]/RFDevice:supportsJamming
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("SearchRadar_0"))
  return
    $GRAPH_7
return
