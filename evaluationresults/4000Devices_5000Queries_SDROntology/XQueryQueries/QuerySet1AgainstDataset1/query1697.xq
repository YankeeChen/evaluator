...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:about)
    for $node_Quantity_0 in $node_SuperheterodyneReceiver_0/RFDevice:hasNoiseFigure
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:resource)
    where fn:exists($value_Quantity_0) and fn:exists($node_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
