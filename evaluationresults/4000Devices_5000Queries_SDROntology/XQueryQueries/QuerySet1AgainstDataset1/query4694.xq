...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX310_0 in $node_Var0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Var2 in $node_Var0/RFDevice:hasSamplingRate
  ...
  let $GRAPH_7 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_AbstractGeometry_0 in $node_Var0/Nuvio:expresses
    let $value_AbstractGeometry_0 := fn:data($node_AbstractGeometry_0/@rdf:resource)
    where fn:exists($value_AbstractGeometry_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
