...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB205mini_i_0 in $node_MannedPlatform_0/RFDevice:isDeployedOn
    let $value_USRPB205mini_i_0 := fn:data($node_USRPB205mini_i_0/@rdf:resource)
    where fn:exists($node_MannedPlatform_0) and fn:exists($value_USRPB205mini_i_0)
    return
      <xqllib:result>
        <xqllib:var name="MannedPlatform_0">{$value_MannedPlatform_0}</xqllib:var>
        <xqllib:var name="USRPB205mini_i_0">{$value_USRPB205mini_i_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_2 := xqllib:and($GRAPH_0,$GRAPH_1,("USRPB205mini_i_0"))
  return
    $GRAPH_2
return
