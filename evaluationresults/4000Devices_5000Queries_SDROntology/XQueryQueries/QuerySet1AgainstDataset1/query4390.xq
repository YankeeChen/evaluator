...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX310_0 in $node_Submarine_0/RFDevice:isDeployedOn
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:resource)
    where fn:exists($node_Submarine_0) and fn:exists($value_USRPX310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Submarine_0 in $doc_ObjectDescription[RFDevice:supportsJamming/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Moving_instance27"]
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:about)
    where fn:exists($node_Submarine_0)
    return
      <xqllib:result>
