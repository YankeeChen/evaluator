...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:about)
    for $node_Var0 in $node_USRPE310_0/RFDevice:supportsJamming
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "AntennaPolarization_instance2"]/Nuvio:hasObjectQuality/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
