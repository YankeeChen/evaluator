...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_0 in $node_RadioComponent_0/Nuvio:partOf
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_USRPB200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadioComponent_0 in $node_Modulator_0/Nuvio:aggregateOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($node_Modulator_0) and fn:exists($value_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Modulator_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Producer_instance186"]/RFDevice:hasProducer
    let $value_Modulator_0 := fn:data($node_Modulator_0/@rdf:resource)
    where fn:exists($value_Modulator_0)
    return
      <xqllib:result>
        <xqllib:var name="Modulator_0">{$value_Modulator_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 :=  
    for $node_Modulator_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Producer_instance1234"]/RFDevice:hasProducer
    let $value_Modulator_0 := fn:data($node_Modulator_0/@rdf:resource)
    where fn:exists($value_Modulator_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("RadioComponent_0"))
