...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:about)
    for $node_Ship_0 in $node_USRPB200mini_0/RFDevice:isDeployedOn
    let $value_Ship_0 := fn:data($node_Ship_0/@rdf:resource)
    where fn:exists($value_Ship_0) and fn:exists($node_USRPB200mini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) != 39.7)]
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_1,$GRAPH_11,("Ship_0"))
