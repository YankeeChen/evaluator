...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarWarningReceiver_0 in $node_ElectronicSupport_0/RFDevice:supportsTransmitting
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:resource)
    where fn:exists($node_ElectronicSupport_0) and fn:exists($value_RadarWarningReceiver_0)
    return
      <xqllib:result>
