...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_RadarModulation_0 in $node_SurveillanceRadar_0/RFDevice:supportsSpectrumSensing
    let $value_RadarModulation_0 := fn:data($node_RadarModulation_0/@rdf:resource)
    where fn:exists($value_RadarModulation_0) and fn:exists($node_SurveillanceRadar_0)
    return
      <xqllib:result>
