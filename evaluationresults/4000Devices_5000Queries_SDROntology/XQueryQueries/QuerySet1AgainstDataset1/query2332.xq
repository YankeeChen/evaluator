...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_ElectronicSupportSystem_0 := fn:data($node_ElectronicSupportSystem_0/@rdf:resource)
    where fn:exists($value_ElectronicSupportSystem_0) and fn:exists($node_MobilePlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicSupportSystem_0">{$value_ElectronicSupportSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_MobilePlatform_0 in $node_Process_0/RFDevice:supportsJamming
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_Ratio_0 in $node_Process_0/RFDevice:hasMaxSensedPower
    let $value_Ratio_0 := fn:data($node_Ratio_0/@rdf:resource)
    where fn:exists($node_Process_0) and fn:exists($value_Ratio_0)
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
