...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_Entity_0/RFDevice:hasProducer
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#PulseRadar"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_RadarJammer_0/Nuvio:aggregateOf
  ...
  let $GRAPH_6 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_AbstractGeometry_0) and fn:exists($node_Entity_0) and $node_AbstractGeometry_0/Nuvio:isExpressedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_0,$GRAPH_9,("JammingSystem_0"))
  return
    $GRAPH_10
return
