...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_BistaticRadar_0 in $node_Weight_0/RFDevice:hasNoiseFigure
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:resource)
    where fn:exists($value_BistaticRadar_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="BistaticRadar_0">{$value_BistaticRadar_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[Nuvio:hasUnitOfMeasure/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#WeightUnit_instance13"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("Weight_0"))
