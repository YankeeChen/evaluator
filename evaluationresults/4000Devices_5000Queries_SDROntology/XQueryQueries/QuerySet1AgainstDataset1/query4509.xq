...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Entity_0 in $node_Feature_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_Feature_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    ...
    let $value_Feature_0 := fn:data($node_Feature_0/@rdf:about)
    where fn:exists($node_Feature_0)
    return
      <xqllib:result>
        <xqllib:var name="Feature_0">{$value_Feature_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("Feature_0"))
