...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_TerrainBounceJamming_0 in $node_Var0/Nuvio:subprocessOf
    let $value_TerrainBounceJamming_0 := fn:data($node_TerrainBounceJamming_0/@rdf:resource)
    where fn:exists($value_TerrainBounceJamming_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Power_0 in $node_TerrainBounceJamming_0/RFDevice:hasPulseRepetitionFrequency
    let $value_Power_0 := fn:data($node_Power_0/@rdf:resource)
    where fn:exists($value_Power_0) and fn:exists($node_TerrainBounceJamming_0)
    return
      <xqllib:result>
