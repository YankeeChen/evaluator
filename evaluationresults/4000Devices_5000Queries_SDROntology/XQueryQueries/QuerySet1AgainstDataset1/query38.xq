...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ModelX_0 in $node_Process_0/RFDevice:hasCapability
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Process_0 in $node_AngleModulation_0/Nuvio:subprocessOf
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_AngleModulation_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
