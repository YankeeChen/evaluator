...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Object_0 in $node_Entity_0/Nuvio:hasParticipant
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Value_0 in $node_Object_1/Nuvio:hasUnitOfMeasure
    let $value_Value_0 := fn:data($node_Value_0/@rdf:resource)
    where fn:exists($node_Object_1) and fn:exists($value_Value_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_1">{$value_Object_1}</xqllib:var>
        <xqllib:var name="Value_0">{$value_Value_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("Value_0"))
