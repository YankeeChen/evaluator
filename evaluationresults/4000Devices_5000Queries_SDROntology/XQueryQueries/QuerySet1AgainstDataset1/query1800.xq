...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Data_14bit"]/RFDevice:hasResolution
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_InformationEntity_0) and fn:exists($node_Var0) and $node_InformationEntity_0/Nuvio:hasParticipant/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_InformationEntity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#SurveillanceRadar_instance18"]/Nuvio:expresses
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_0,$GRAPH_7,("USRPN210_0"))
  return
    $GRAPH_8
return
