...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ContinuousWaveRadar_0 in $node_Process_0/RFDevice:supportsTuning
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Process_0) and fn:exists($node_Tuning_0) and $node_Tuning_0/Nuvio:subprocessOf/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
