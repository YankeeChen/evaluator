...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_AbstractParametricCurveSurface_0) and fn:exists($node_Var0) and $node_AbstractParametricCurveSurface_0/Nuvio:participatesIn/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) >= 286734343)]
