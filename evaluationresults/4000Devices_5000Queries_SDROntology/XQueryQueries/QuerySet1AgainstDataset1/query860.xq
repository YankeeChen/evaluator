...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:about)
    for $node_Entity_0 in $node_CommunicationsSystem_0/RFDevice:isProducerOf
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) = 1803951379)]
  ...
  let $GRAPH_13 :=  
    for $node_Situation_0 in $doc_ObjectDescription
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_DoubleValue_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_DoubleValue_0 := fn:data($node_DoubleValue_0/@rdf:resource)
    where fn:exists($value_DoubleValue_0) and fn:exists($node_Situation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_2,$GRAPH_14,("Entity_0","Situation_0"))
  let $GRAPH_16 := xqllib:and($GRAPH_1,$GRAPH_15,("Entity_0"))
  let $GRAPH_17 := xqllib:and($GRAPH_0,$GRAPH_16,("CommunicationsSystem_0"))
  return
    $GRAPH_17
return
