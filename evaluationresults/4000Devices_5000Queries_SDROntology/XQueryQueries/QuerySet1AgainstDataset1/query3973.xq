...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_Submarine_0/RFDevice:isDeployedOn
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:resource)
    where fn:exists($node_Submarine_0) and fn:exists($value_USRPE3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Submarine_0 in $node_Entity_0/RFDevice:hasCapability
  ...
  let $GRAPH_13 := $GRAPH_12[(data(xqllib:var[@name="DataValue0"]) <= 1132225867)]
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_1,$GRAPH_17,("Submarine_0"))
