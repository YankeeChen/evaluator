...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:about)
    for $node_PulseRadarModulation_0 in $node_SearchRadar_0/RFDevice:supportsSpectrumSensing
    let $value_PulseRadarModulation_0 := fn:data($node_PulseRadarModulation_0/@rdf:resource)
    where fn:exists($value_PulseRadarModulation_0) and fn:exists($node_SearchRadar_0)
    return
      <xqllib:result>
