...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    where fn:exists($node_SpatialAreaUnit_0) and fn:exists($node_Value_0) and $node_SpatialAreaUnit_0/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Value_0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) > -2139897430)]
