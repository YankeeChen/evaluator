...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN2xx_0 := fn:data($node_USRPN2xx_0/@rdf:about)
    for $node_Object_0 in $node_USRPN2xx_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Object_0) and $node_Entity_0/RFDevice:hasCapability/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Process" and Nuvio:hasSubprocess/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Instance4973"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
