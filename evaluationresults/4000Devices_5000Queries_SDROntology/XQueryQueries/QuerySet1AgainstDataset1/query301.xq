...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_Var0/Nuvio:hasObjectQuantity
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Transmitting_0) and $node_Transmitting_0/Nuvio:hasSubprocess/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    for $node_Transmitting_0 in $doc_ObjectDescription
    let $value_Transmitting_0 := fn:data($node_Transmitting_0/@rdf:about)
    for $node_Entity_0 in $node_Transmitting_0/RFDevice:hasPulseWidth
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_4,$GRAPH_5,("Entity_0","Transmitting_0"))
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_0,$GRAPH_9,("Half_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_10
return
