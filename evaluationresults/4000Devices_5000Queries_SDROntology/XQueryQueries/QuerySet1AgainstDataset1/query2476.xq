...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_TrackingRadar_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_2 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#JammingSystem"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_Attribute_0/Nuvio:subprocessOf
  ...
  let $GRAPH_7 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Quantity_0) and $node_Quantity_0/RFDevice:hasPolarization/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
