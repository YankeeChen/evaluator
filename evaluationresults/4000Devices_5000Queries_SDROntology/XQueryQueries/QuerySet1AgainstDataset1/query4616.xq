...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_CrossEyeJamming_0/RFDevice:supportsSpectrumSensing
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($node_CrossEyeJamming_0) and fn:exists($value_ElectronicAttackSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_CrossEyeJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Power_instance16"]/RFDevice:hasPulseRepetitionFrequency
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("CrossEyeJamming_0"))
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("ElectronicAttackSystem_0"))
  return
    $GRAPH_4
return
