...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_210_0 in $node_Object_0/RFDevice:hasProducer
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_InboundRangeGatePullOffJamming_0) and fn:exists($node_Object_0) and $node_InboundRangeGatePullOffJamming_0/RFDevice:supportsTransmitting/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("InboundRangeGatePullOffJamming_0"))
