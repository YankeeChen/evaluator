...
let $result :=  
  ...
  let $GRAPH_6 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_Cylinder_0 in $node_Process_0/Nuvio:participatesIn
    let $value_Cylinder_0 := fn:data($node_Cylinder_0/@rdf:resource)
    where fn:exists($value_Cylinder_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
