...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $doc_ObjectDescription[RFDevice:hasFrequencyRange/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#RadioFrequencyBand_instance66"]
    let $value_InstantaneousFrequencyMeasurementReceiver_0 := fn:data($node_InstantaneousFrequencyMeasurementReceiver_0/@rdf:about)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0)
    return
      <xqllib:result>
