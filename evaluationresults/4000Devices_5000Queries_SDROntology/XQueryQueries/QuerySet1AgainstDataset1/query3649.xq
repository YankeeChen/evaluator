...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_0 in $node_Entity_0/RFDevice:hasGain
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) < 992780837)]
  ...
  let $GRAPH_10 := $GRAPH_9[(data(xqllib:var[@name="DataValue1"]) != -1198250022)]
  ...
  let $GRAPH_13 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_UnitOfMeasure_0) and $node_UnitOfMeasure_0/Nuvio:subprocessOf/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
