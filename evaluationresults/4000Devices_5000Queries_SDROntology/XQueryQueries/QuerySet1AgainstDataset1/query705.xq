...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseDopplerRadar_0 in $node_MannedPlatform_0/RFDevice:isDeployedOn
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:resource)
    where fn:exists($node_MannedPlatform_0) and fn:exists($value_PulseDopplerRadar_0)
    return
      <xqllib:result>
