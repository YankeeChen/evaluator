...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:about)
    for $node_Var0 in $node_USRPE312_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_PhaseModulation_0/Nuvio:hasParticipant
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_PhaseModulation_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
