...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#XC7K325TFPGA"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Var0 in $node_DipoleAntenna_0/Nuvio:partOf
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_DipoleAntenna_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("DipoleAntenna_0"))
  ...
  let $GRAPH_14 :=  
    ...
    for $node_Var0 in $node_Var1/RFDevice:hasSamplingRate
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
