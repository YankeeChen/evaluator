...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_MonostaticRadar_0 in $doc_ObjectDescription[RFDevice:supportsJamming/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#PulseRadarModulation_instance14"]
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:about)
    where fn:exists($node_MonostaticRadar_0)
    return
      <xqllib:result>
