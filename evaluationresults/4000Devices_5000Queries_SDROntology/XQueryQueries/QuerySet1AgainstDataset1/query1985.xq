...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Full_DuplexCommunicationsSystem_0 := fn:data($node_Full_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_MobilePlatform_0 in $node_Full_DuplexCommunicationsSystem_0/RFDevice:isDeployedOn
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:resource)
    where fn:exists($node_Full_DuplexCommunicationsSystem_0) and fn:exists($value_MobilePlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="Full_DuplexCommunicationsSystem_0">{$value_Full_DuplexCommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Process_0) and fn:exists($node_Quantity_0) and $node_Quantity_0/Nuvio:hasProcessQuantity/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("MobilePlatform_0"))
