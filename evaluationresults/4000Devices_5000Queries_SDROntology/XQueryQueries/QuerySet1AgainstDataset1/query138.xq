...
let $result :=  
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("UnitOfMeasure_0"))
  let $GRAPH_7 :=  
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue0"]) >= xs:decimal("0"))]
  ...
  let $GRAPH_15 :=  
    ...
    for $node_Var1 in $node_PhaseModulation_0/Nuvio:relevantIndividual
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_PhaseModulation_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_16 :=  
    ...
    for $node_Var1 in $node_EnergyDetection_0/Nuvio:relevantIndividual
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_EnergyDetection_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_20 := xqllib:optional($GRAPH_14,$GRAPH_19,("Var1"))
  let $GRAPH_21 := xqllib:and($GRAPH_13,$GRAPH_20,("InformationEntity_0"))
