...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:about)
    for $node_Submarine_0 in $node_RadarWarningReceiver_0/RFDevice:isDeployedOn
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:resource)
    where fn:exists($node_RadarWarningReceiver_0) and fn:exists($value_Submarine_0)
    return
      <xqllib:result>
        <xqllib:var name="RadarWarningReceiver_0">{$value_RadarWarningReceiver_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    for $node_Submarine_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Moving_instance63"]/RFDevice:hasCapability
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:resource)
