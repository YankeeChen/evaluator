...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX310_0 in $node_Receiver_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Receiver_0 in $node_Transceiving_0/RFDevice:supportsSpectrumSensing
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:resource)
    where fn:exists($value_Receiver_0) and fn:exists($node_Transceiving_0)
    return
      <xqllib:result>
        <xqllib:var name="Receiver_0">{$value_Receiver_0}</xqllib:var>
