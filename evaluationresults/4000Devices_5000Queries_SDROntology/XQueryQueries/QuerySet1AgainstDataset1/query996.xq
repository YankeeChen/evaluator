...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB205mini_i_0 := fn:data($node_USRPB205mini_i_0/@rdf:about)
    for $node_Var0 in $node_USRPB205mini_i_0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_Var1) and $doc_ObjectDescription[@rdf:ID = "FrequencyUnit_instance0"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Var1
    return
      <xqllib:result>
        <xqllib:var name="Var1">{$value_Var1}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) != -1542598793)]
  let $GRAPH_6 := xqllib:and($GRAPH_3,$GRAPH_5,("Var1"))
