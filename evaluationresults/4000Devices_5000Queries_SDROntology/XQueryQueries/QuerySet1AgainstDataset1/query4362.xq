...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:partOf
    let $value_USRPB210_0 := fn:data($node_USRPB210_0/@rdf:resource)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($value_USRPB210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_RadarJammer_0/Nuvio:aggregateOf
    ...
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_RadarJammer_0)
    return
      <xqllib:result>
        <xqllib:var name="InstantaneousFrequencyMeasurementReceiver_0">{$value_InstantaneousFrequencyMeasurementReceiver_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("ElectronicWarfare_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("RadarJammer_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_3,$GRAPH_8,("RadarJammer_0"))
  let $GRAPH_10 := xqllib:optional($GRAPH_2,$GRAPH_9,("InstantaneousFrequencyMeasurementReceiver_0"))
