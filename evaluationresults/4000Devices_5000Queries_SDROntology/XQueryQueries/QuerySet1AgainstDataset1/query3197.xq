...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:about)
    for $node_Object_0 in $node_USRPX300_0/RFDevice:isProducerOf
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($value_Object_0) and fn:exists($node_USRPX300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Object_0 in $node_ContinuousWaveRadarModulation_0/RFDevice:hasCapability
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
