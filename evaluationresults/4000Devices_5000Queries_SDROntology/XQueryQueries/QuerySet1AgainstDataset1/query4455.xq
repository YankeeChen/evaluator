...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX3xx_0 in $node_IEDTrigger_0/Nuvio:partOf
    let $value_USRPX3xx_0 := fn:data($node_USRPX3xx_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_USRPX3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_IEDTrigger_0 in $doc_ObjectDescription
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:about)
    for $node_Entity_0 in $node_IEDTrigger_0/RFDevice:hasFrequencyAccuracy
