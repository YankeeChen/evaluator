...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SDR_0 in $node_DirectConversionReceiver_0/Nuvio:aggregateOf
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_SDR_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Object_0 in $doc_ObjectDescription[Nuvio:hasObjectQuality/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#AntennaPolarization_instance36"]
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("DirectConversionReceiver_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("DirectConversionReceiver_0"))
