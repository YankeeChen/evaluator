...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SDR_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:resource)
    where fn:exists($node_MobilePlatform_0) and fn:exists($value_SDR_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Process_0 in $doc_ObjectDescription
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_RadarJamming_0 in $node_Process_0/Nuvio:subprocessOf
    let $value_RadarJamming_0 := fn:data($node_RadarJamming_0/@rdf:resource)
    where fn:exists($node_Process_0) and fn:exists($value_RadarJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
