...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE310_0 in $node_Object_0/RFDevice:hasProducer
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_USRPE310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_ContinuousWaveRadarModulation_0) and fn:exists($node_Object_0) and $node_ContinuousWaveRadarModulation_0/Nuvio:participatesIn/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
