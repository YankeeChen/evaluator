...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CR_0 in $node_Var0/RFDevice:supportsJamming
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_CrossEyeJamming_0) and fn:exists($node_Var0) and $node_CrossEyeJamming_0/Nuvio:hasSubprocess/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_CrossEyeJamming_0 in $node_Power_0/RFDevice:hasMaxOutputPower
    let $value_CrossEyeJamming_0 := fn:data($node_CrossEyeJamming_0/@rdf:resource)
    where fn:exists($value_CrossEyeJamming_0) and fn:exists($node_Power_0)
    return
      <xqllib:result>
        <xqllib:var name="CrossEyeJamming_0">{$value_CrossEyeJamming_0}</xqllib:var>
  ...
  let $GRAPH_7 :=  
    ...
    for $node_Var1 in $node_PowerUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_PowerUnit_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_3,$GRAPH_14,("CrossEyeJamming_0"))
  let $GRAPH_16 := xqllib:and($GRAPH_2,$GRAPH_15,("CrossEyeJamming_0"))
