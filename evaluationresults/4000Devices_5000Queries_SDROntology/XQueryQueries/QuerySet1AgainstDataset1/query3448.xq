...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Half_DuplexCommunicationsSystem_0 in $node_Var0/RFDevice:hasCapability
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Length_0 in $node_Var0/RFDevice:hasMaxSensedPower
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Length_0 in $node_Var1/Nuvio:hasValue
    let $value_Length_0 := fn:data($node_Length_0/@rdf:resource)
    where fn:exists($value_Length_0) and fn:exists($node_Var1)
    return
      <xqllib:result>
        <xqllib:var name="Length_0">{$value_Length_0}</xqllib:var>
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) <= -1390972733)]
  let $GRAPH_9 := xqllib:and($GRAPH_6,$GRAPH_8,("Var1"))
  let $GRAPH_10 := xqllib:and($GRAPH_5,$GRAPH_9,("Length_0"))
  let $GRAPH_11 := xqllib:optional($GRAPH_4,$GRAPH_10,("Length_0"))
  let $GRAPH_12 := ($GRAPH_3,$GRAPH_11)
  let $GRAPH_13 := xqllib:and($GRAPH_1,$GRAPH_12,("Var0"))
  let $GRAPH_14 := xqllib:optional($GRAPH_0,$GRAPH_13,("Half_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_14
return
