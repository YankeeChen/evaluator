...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:about)
    for $node_Ship_0 in $node_USRPB2xx_0/RFDevice:isDeployedOn
    let $value_Ship_0 := fn:data($node_Ship_0/@rdf:resource)
    where fn:exists($value_Ship_0) and fn:exists($node_USRPB2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Ship_0 in $node_Moving_0/RFDevice:supportsJamming
    let $value_Ship_0 := fn:data($node_Ship_0/@rdf:resource)
