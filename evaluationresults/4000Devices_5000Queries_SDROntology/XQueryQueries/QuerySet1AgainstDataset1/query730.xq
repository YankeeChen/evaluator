...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_CoverJamming_0 in $node_Entity_0/Nuvio:expresses
    let $value_CoverJamming_0 := fn:data($node_CoverJamming_0/@rdf:resource)
    where fn:exists($value_CoverJamming_0) and fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_CoverJamming_0 in $node_Entity_1/RFDevice:hasPulseRepetitionFrequency
    let $value_CoverJamming_0 := fn:data($node_CoverJamming_0/@rdf:resource)
    where fn:exists($value_CoverJamming_0) and fn:exists($node_Entity_1)
    return
      <xqllib:result>
        <xqllib:var name="CoverJamming_0">{$value_CoverJamming_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Entity_0","Process_0"))
