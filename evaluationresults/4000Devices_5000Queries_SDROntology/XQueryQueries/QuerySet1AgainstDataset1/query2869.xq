...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:about)
    for $node_FPGA_0 in $node_USRPB200_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_FPGA_0 in $node_Producer_0/RFDevice:isProducerOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0) and fn:exists($node_Producer_0)
    return
      <xqllib:result>
        <xqllib:var name="FPGA_0">{$value_FPGA_0}</xqllib:var>
