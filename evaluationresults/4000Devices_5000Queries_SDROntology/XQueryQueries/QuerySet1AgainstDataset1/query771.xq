...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:about)
    for $node_Var0 in $node_TrackingRadar_0/RFDevice:hasCapability
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_TrackingRadar_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
        <xqllib:var name="TrackingRadar_0">{$value_TrackingRadar_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_DataSize_0) and fn:exists($node_Var0) and $node_DataSize_0/RFDevice:hasPulseRepetitionInterval/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_7 :=  
    ...
    for $node_DataSize_0 in $node_Var1/Nuvio:hasValue
    let $value_DataSize_0 := fn:data($node_DataSize_0/@rdf:resource)
    where fn:exists($value_DataSize_0) and fn:exists($node_Var1)
    return
      <xqllib:result>
        <xqllib:var name="DataSize_0">{$value_DataSize_0}</xqllib:var>
        <xqllib:var name="Var1">{$value_Var1}</xqllib:var>
      </xqllib:result>
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("DataSize_0"))
