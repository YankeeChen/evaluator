...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_i_0 in $node_Var0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_Var1/RFDevice:hasMaxSensedPower
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) != 62.74)]
  let $GRAPH_7 := xqllib:and($GRAPH_4,$GRAPH_6,("Var2"))
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("Var1"))
  let $GRAPH_9 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_Var3 in $node_Var1/Nuvio:hasValue
    let $value_Var3 := fn:data($node_Var3/@rdf:resource)
    where fn:exists($node_Var1) and fn:exists($value_Var3)
    return
      <xqllib:result>
        <xqllib:var name="Var1">{$value_Var1}</xqllib:var>
  ...
  let $GRAPH_10 :=  
    for $node_Var3 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#PositiveDecimalValue"]
  ...
  let $GRAPH_11 :=  
    for $node_Var3 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#Decibel_milliwatts"]/Nuvio:hasUnitOfMeasure
    let $value_Var3 := fn:data($node_Var3/@rdf:resource)
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_10,$GRAPH_11,("Var3"))
  let $GRAPH_13 := xqllib:and($GRAPH_9,$GRAPH_12,("Var3"))
  let $GRAPH_14 := xqllib:and($GRAPH_8,$GRAPH_13,("Var1"))
  let $GRAPH_15 := xqllib:and($GRAPH_2,$GRAPH_14,("Var1"))
  let $GRAPH_16 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
  ...
  let $GRAPH_17 := xqllib:optional($GRAPH_15,$GRAPH_16,("Var0","Var1"))
