...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:about)
    for $node_Entity_0 in $node_BistaticRadar_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Circle"]
  ...
  let $GRAPH_6 :=  
    for $node_Arc_0 in $doc_ObjectDescription
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Arc_0) and fn:exists($node_Entity_0) and $node_Arc_0/Nuvio:isExpressedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_6,$GRAPH_7,("Arc_0"))
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_0,$GRAPH_12,("BistaticRadar_0"))
  return
    $GRAPH_13
return
