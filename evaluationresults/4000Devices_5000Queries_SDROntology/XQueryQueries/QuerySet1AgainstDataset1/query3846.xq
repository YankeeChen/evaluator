...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_MobilePlatform_0 in $node_Entity_0/RFDevice:hasCapability
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) != -1456425894)]
  let $GRAPH_9 :=  
  ...
  let $GRAPH_15 :=  
    ...
    for $node_Entity_0 in $node_RFFrontEnd_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_16 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Entity_0 in $node_Var0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_13,$GRAPH_17,("Entity_0","Var0"))
  let $GRAPH_19 := xqllib:and($GRAPH_3,$GRAPH_18,("Entity_0"))
  let $GRAPH_20 := xqllib:and($GRAPH_2,$GRAPH_19,("MobilePlatform_0"))
  let $GRAPH_21 := xqllib:and($GRAPH_1,$GRAPH_20,("MobilePlatform_0"))
  let $GRAPH_22 := xqllib:optional($GRAPH_0,$GRAPH_21,("GNURadio_0"))
  return
    $GRAPH_22
return
