...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Receiver_0/Nuvio:aggregateOf
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($node_Receiver_0) and fn:exists($value_USRPB200mini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Receiver_0 in $doc_ObjectDescription[Nuvio:aggregateOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#IEDJammer_instance250"]
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:about)
    where fn:exists($node_Receiver_0)
    return
      <xqllib:result>
