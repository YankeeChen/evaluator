...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:about)
    for $node_Object_0 in $node_USRPX300_0/RFDevice:hasProducer
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and $doc_ObjectDescription[@rdf:ID = "RadarJamming_instance57"]/RFDevice:supportsJamming/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("Object_0"))
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("USRPX300_0"))
  return
    $GRAPH_4
return
