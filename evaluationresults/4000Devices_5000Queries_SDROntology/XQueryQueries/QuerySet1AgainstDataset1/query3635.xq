...
let $result :=  
  ...
  let $GRAPH_11 :=  
    ...
    for $node_DRFMJamming_0 in $node_Object_0/RFDevice:supportsJamming
    let $value_DRFMJamming_0 := fn:data($node_DRFMJamming_0/@rdf:resource)
    where fn:exists($value_DRFMJamming_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_12 :=  
    ...
    for $node_Quantity_0 in $node_DRFMJamming_0/Nuvio:hasProcessQuantity
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:resource)
    where fn:exists($node_DRFMJamming_0) and fn:exists($value_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="DRFMJamming_0">{$value_DRFMJamming_0}</xqllib:var>
        <xqllib:var name="Quantity_0">{$value_Quantity_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_13 := xqllib:and($GRAPH_11,$GRAPH_12,("DRFMJamming_0"))
  let $GRAPH_14 := xqllib:and($GRAPH_10,$GRAPH_13,("Object_0"))
  let $GRAPH_15 := xqllib:and($GRAPH_1,$GRAPH_14,("Entity_0"))
  let $GRAPH_16 := xqllib:and($GRAPH_0,$GRAPH_15,("BistaticRadar_0"))
  return
    $GRAPH_16
return
