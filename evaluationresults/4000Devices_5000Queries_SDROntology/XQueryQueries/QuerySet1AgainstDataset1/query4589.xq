...
let $result :=  
  ...
  let $GRAPH_14 := $GRAPH_13[(data(xqllib:var[@name="DataValue0"]) = -940391525)]
  ...
  let $GRAPH_17 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#CoverPulsesJamming"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
  ...
  let $GRAPH_18 :=  
    for $node_Var1 in $doc_ObjectDescription[RFDevice:hasPulseWidth/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Power_instance50"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_26 := xqllib:and($GRAPH_0,$GRAPH_25,("ModelX_0"))
  return
    $GRAPH_26
return
