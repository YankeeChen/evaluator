...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Var0 in $node_CR_0/Nuvio:relevantIndividual
  ...
  let $GRAPH_8 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_Object_0) and $doc_ObjectDescription[@rdf:ID = "AntennaPolarization_instance22"]/RFDevice:hasPolarization/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("Object_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("CR_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_5,$GRAPH_10,("CR_0"))
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_0,$GRAPH_17,("GNURadio_0"))
  return
    $GRAPH_18
return
