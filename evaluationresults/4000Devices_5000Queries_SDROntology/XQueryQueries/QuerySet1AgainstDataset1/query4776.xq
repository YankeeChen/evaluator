...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_ElectricChargeUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($node_ElectricChargeUnit_0) and fn:exists($value_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("ElectricChargeUnit_0"))
  let $GRAPH_7 :=  
    for $node_Object_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Object" and Nuvio:hasParticipant/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Instance5340"]
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
