...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_PulseDopplerRadarModulation_0/RFDevice:supportsSpectrumSensing
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:resource)
    where fn:exists($node_PulseDopplerRadarModulation_0) and fn:exists($value_SurveillanceRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_PulseDopplerRadarModulation_0 in $doc_ObjectDescription
    let $value_PulseDopplerRadarModulation_0 := fn:data($node_PulseDopplerRadarModulation_0/@rdf:about)
    for $node_Quantity_0 in $node_PulseDopplerRadarModulation_0/RFDevice:hasEffectiveRadiatedPower
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:resource)
    where fn:exists($node_PulseDopplerRadarModulation_0) and fn:exists($value_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseDopplerRadarModulation_0">{$value_PulseDopplerRadarModulation_0}</xqllib:var>
        <xqllib:var name="Quantity_0">{$value_Quantity_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("PulseDopplerRadarModulation_0"))
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("PulseDopplerRadarModulation_0"))
  let $GRAPH_6 := xqllib:optional($GRAPH_0,$GRAPH_5,("SurveillanceRadar_0"))
  return
    $GRAPH_6
return
