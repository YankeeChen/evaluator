...
let $result :=  
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) != 51.52)]
  ...
  let $GRAPH_9 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_FreqencyRange_0 in $node_Process_0/Nuvio:hasParticipant
    let $value_FreqencyRange_0 := fn:data($node_FreqencyRange_0/@rdf:resource)
    where fn:exists($value_FreqencyRange_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_2,$GRAPH_12,("Entity_0","Process_0"))
