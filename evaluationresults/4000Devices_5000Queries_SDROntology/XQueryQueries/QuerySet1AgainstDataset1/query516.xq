...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN200_0 := fn:data($node_USRPN200_0/@rdf:about)
    for $node_XC3SD1800AFPGA_0 in $node_USRPN200_0/Nuvio:aggregateOf
    let $value_XC3SD1800AFPGA_0 := fn:data($node_XC3SD1800AFPGA_0/@rdf:resource)
    where fn:exists($node_USRPN200_0) and fn:exists($value_XC3SD1800AFPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPN200_0">{$value_USRPN200_0}</xqllib:var>
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("XC3SD1800AFPGA_0"))
