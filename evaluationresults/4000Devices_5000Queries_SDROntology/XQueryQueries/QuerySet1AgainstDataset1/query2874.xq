...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_Var0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#MultiCurve"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_8 :=  
    ...
    for $node_Situation_0 in $node_Composite_0/Nuvio:relevantIndividual
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:resource)
    where fn:exists($node_Composite_0) and fn:exists($value_Situation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("Situation_0"))
