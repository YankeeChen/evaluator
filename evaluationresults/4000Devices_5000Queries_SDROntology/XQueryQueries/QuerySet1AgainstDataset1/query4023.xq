...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE310_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:aggregateOf
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:resource)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($value_USRPE310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Instance5991"]/Nuvio:partOf
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("USRPE310_0"))
  return
    $GRAPH_7
return
