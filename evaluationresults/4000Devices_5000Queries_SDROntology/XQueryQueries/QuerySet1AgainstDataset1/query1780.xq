...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Value" and Nuvio:hasUnitOfMeasure/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#InformationUnit_instance0"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_BarrageJamming_0) and fn:exists($node_Situation_0) and $node_BarrageJamming_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_9,$GRAPH_10,("BarrageJamming_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_8,$GRAPH_11,("Situation_0"))
