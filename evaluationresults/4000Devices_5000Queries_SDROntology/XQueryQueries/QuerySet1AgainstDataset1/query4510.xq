...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE310_0 in $node_Var0/RFDevice:supportsTuning
  ...
  let $GRAPH_8 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $doc_ObjectDescription[@rdf:ID = "Frequency_instance310"]/Nuvio:hasProcessQuantity/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_11 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_InformationEntity_0) and fn:exists($node_Object_0) and $node_InformationEntity_0/Nuvio:hasParticipant/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_12 :=  
    for $node_InformationEntity_0 in $doc_ObjectDescription
    let $value_InformationEntity_0 := fn:data($node_InformationEntity_0/@rdf:about)
    for $node_PolyhedralSurface_0 in $node_InformationEntity_0/Nuvio:isExpressedBy
    let $value_PolyhedralSurface_0 := fn:data($node_PolyhedralSurface_0/@rdf:resource)
    where fn:exists($node_InformationEntity_0) and fn:exists($value_PolyhedralSurface_0)
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
        <xqllib:var name="PolyhedralSurface_0">{$value_PolyhedralSurface_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_13 :=  
    for $node_PolyhedralSurface_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/sf#PolyhedralSurface"]
    let $value_PolyhedralSurface_0 := fn:data($node_PolyhedralSurface_0/@rdf:about)
    where fn:exists($node_PolyhedralSurface_0)
    return
      <xqllib:result>
        <xqllib:var name="PolyhedralSurface_0">{$value_PolyhedralSurface_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_14 := xqllib:and($GRAPH_12,$GRAPH_13,("PolyhedralSurface_0"))
  let $GRAPH_15 := xqllib:and($GRAPH_11,$GRAPH_14,("InformationEntity_0"))
  let $GRAPH_16 := xqllib:and($GRAPH_10,$GRAPH_15,("Object_0"))
