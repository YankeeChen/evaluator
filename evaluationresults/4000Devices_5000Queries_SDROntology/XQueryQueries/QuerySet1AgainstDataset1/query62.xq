...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_TrackingRadar_0 in $doc_ObjectDescription[RFDevice:isProducerOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Producer_instance1056"]
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:about)
    where fn:exists($node_TrackingRadar_0)
    return
      <xqllib:result>
