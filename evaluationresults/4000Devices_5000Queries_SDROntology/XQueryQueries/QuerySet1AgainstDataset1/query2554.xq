...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_RadioComponent_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_RadioComponent_0 in $doc_ObjectDescription
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:about)
    for $node_ChokeRingAntenna_0 in $node_RadioComponent_0/Nuvio:aggregateOf
    let $value_ChokeRingAntenna_0 := fn:data($node_ChokeRingAntenna_0/@rdf:resource)
    where fn:exists($value_ChokeRingAntenna_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
