...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Var0 in $node_USRPN300_0/Nuvio:relevantIndividual
  ...
  let $GRAPH_5 :=  
    for $node_USRPN300_0 in $doc_ObjectDescription
    let $value_USRPN300_0 := fn:data($node_USRPN300_0/@rdf:about)
    for $node_Var1 in $node_USRPN300_0/RFDevice:supportsSpectrumSensing
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_USRPN300_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
        <xqllib:var name="USRPN300_0">{$value_USRPN300_0}</xqllib:var>
        <xqllib:var name="Var1">{$value_Var1}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("USRPN300_0"))
  ...
  let $GRAPH_14 := xqllib:optional($GRAPH_12,$GRAPH_13,("Power_0"))
  let $GRAPH_15 := xqllib:and($GRAPH_11,$GRAPH_14,("Power_0"))
  let $GRAPH_16 := xqllib:optional($GRAPH_10,$GRAPH_15,("Var0"))
  let $GRAPH_17 := xqllib:and($GRAPH_1,$GRAPH_16,("Var0"))
  let $GRAPH_18 := xqllib:and($GRAPH_0,$GRAPH_17,("RFSystem_0"))
  return
    $GRAPH_18
return
