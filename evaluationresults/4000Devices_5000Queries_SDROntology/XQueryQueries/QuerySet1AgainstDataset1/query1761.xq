...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Var0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#SDR"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Var0 in $node_Quantity_0/RFDevice:hasBeamwidth
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_10 := $GRAPH_9[(data(xqllib:var[@name="DataValue0"]) > 1690309454)]
