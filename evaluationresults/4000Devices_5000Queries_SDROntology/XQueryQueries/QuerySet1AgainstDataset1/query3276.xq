...
let $result :=  
  ...
  let $GRAPH_5 := $GRAPH_4[not((data(xqllib:var[@name="DataValue0"]) >= xs:decimal("-180")))]
  ...
  let $GRAPH_9 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_DigitalModulation_0) and fn:exists($node_Entity_0) and $node_DigitalModulation_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_10 :=  
    ...
    for $node_Entity_0 in $node_Half_DuplexCommunicationsSystem_0/Nuvio:relevantIndividual
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_9,$GRAPH_12,("DigitalModulation_0","Entity_0"))
  let $GRAPH_14 :=  
