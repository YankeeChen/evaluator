...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_GroundPlatform_0 in $node_Var0/RFDevice:isDeployedOn
    let $value_GroundPlatform_0 := fn:data($node_GroundPlatform_0/@rdf:resource)
    where fn:exists($value_GroundPlatform_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_21 := xqllib:and($GRAPH_0,$GRAPH_20,("ContinuousWaveRadar_0"))
  return
    $GRAPH_21
return
