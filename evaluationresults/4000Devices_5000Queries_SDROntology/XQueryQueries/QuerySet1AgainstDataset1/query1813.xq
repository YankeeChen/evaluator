...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:about)
    for $node_Object_0 in $node_RadarWarningReceiver_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and $doc_ObjectDescription[@rdf:ID = "Instance3603"]/RFDevice:supportsTuning/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
      </xqllib:result>
