...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE313_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:compositeOf
    let $value_USRPE313_0 := fn:data($node_USRPE313_0/@rdf:resource)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($value_USRPE313_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#ChokeRingAntenna_instance14"]/Nuvio:aggregateOf
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_0,$GRAPH_3,("USRPE313_0"))
  return
    $GRAPH_4
return
