...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_RadarJammer_0 in $node_CoverPulsesJamming_0/RFDevice:hasCapability
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_CoverPulsesJamming_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("CoverPulsesJamming_0"))
