...
let $result :=  
  ...
  let $GRAPH_9 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_RadioFrequencyBand_0) and $node_RadioFrequencyBand_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_11 :=  
    ...
    for $node_Entity_0 in $node_AbstractGeometry_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($node_AbstractGeometry_0) and fn:exists($value_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_14 := xqllib:and($GRAPH_9,$GRAPH_13,("Entity_0","RadioFrequencyBand_0"))
  ...
  let $GRAPH_19 := ($GRAPH_15,$GRAPH_18)
  let $GRAPH_20 :=  
    for $node_Object_0 in $doc_ObjectDescription[RFDevice:hasPolarization/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#AntennaPolarization_instance22"]
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
  ...
  let $GRAPH_24 := xqllib:optional($GRAPH_2,$GRAPH_23,("Var0"))
  let $GRAPH_25 := xqllib:and($GRAPH_1,$GRAPH_24,("Var0"))
  let $GRAPH_26 := xqllib:and($GRAPH_0,$GRAPH_25,("TrackingRadar_0"))
  return
    $GRAPH_26
return
