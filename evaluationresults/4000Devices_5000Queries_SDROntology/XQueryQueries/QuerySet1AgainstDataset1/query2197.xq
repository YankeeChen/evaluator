...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_Full_DuplexCommunicationsSystem_0 in $doc_ObjectDescription[Nuvio:compositeOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#IEDTrigger_instance136"]
    let $value_Full_DuplexCommunicationsSystem_0 := fn:data($node_Full_DuplexCommunicationsSystem_0/@rdf:about)
    where fn:exists($node_Full_DuplexCommunicationsSystem_0)
    return
      <xqllib:result>
