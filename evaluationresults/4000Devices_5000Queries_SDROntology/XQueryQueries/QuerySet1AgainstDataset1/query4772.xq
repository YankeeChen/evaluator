...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_MonostaticRadar_0 in $node_Entity_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) < 970587000)]
  ...
  let $GRAPH_10 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Width_0) and $node_Width_0/RFDevice:hasEffectiveRadiatedPower/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
