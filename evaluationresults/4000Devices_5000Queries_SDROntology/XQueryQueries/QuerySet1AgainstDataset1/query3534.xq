...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_TIN_0 in $node_Var0/RFDevice:hasTarget
    let $value_TIN_0 := fn:data($node_TIN_0/@rdf:resource)
    where fn:exists($value_TIN_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("TIN_0"))
  ...
  let $GRAPH_10 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/sf#MultiSurface"]
