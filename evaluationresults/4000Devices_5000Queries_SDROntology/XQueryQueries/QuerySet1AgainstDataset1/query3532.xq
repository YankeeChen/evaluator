...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_AngleModulation_0 in $node_Var0/Nuvio:subprocessOf
    let $value_AngleModulation_0 := fn:data($node_AngleModulation_0/@rdf:resource)
    where fn:exists($value_AngleModulation_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
