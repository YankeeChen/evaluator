...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Full_DuplexCommunicationsSystem_0 := fn:data($node_Full_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_Transceiving_0 in $node_Full_DuplexCommunicationsSystem_0/RFDevice:supportsSpectrumSensing
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:resource)
    where fn:exists($node_Full_DuplexCommunicationsSystem_0) and fn:exists($value_Transceiving_0)
    return
      <xqllib:result>
        <xqllib:var name="Full_DuplexCommunicationsSystem_0">{$value_Full_DuplexCommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("Full_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_4
return
