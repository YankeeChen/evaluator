...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_Object_0/RFDevice:isProducerOf
  ...
  let $GRAPH_2 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_InverseGainJamming_0 in $node_Object_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_4 :=  
    ...
    for $node_InverseGainJamming_0 in $node_Entity_0/RFDevice:hasPulseRepetitionFrequency
    let $value_InverseGainJamming_0 := fn:data($node_InverseGainJamming_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_InverseGainJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_TimeInterval_0) and $node_TimeInterval_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("TimeInterval_0"))
  let $GRAPH_9 :=  
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_5,$GRAPH_14,("Entity_0","InformationContent_0"))
  let $GRAPH_16 := xqllib:and($GRAPH_4,$GRAPH_15,("Entity_0"))
  let $GRAPH_17 := xqllib:and($GRAPH_3,$GRAPH_16,("InverseGainJamming_0"))
  let $GRAPH_18 := xqllib:optional($GRAPH_2,$GRAPH_17,("InverseGainJamming_0"))
  let $GRAPH_19 := xqllib:and($GRAPH_1,$GRAPH_18,("Object_0"))
  let $GRAPH_20 := xqllib:and($GRAPH_0,$GRAPH_19,("ElectronicSupportSystem_0"))
  return
    $GRAPH_20
return
