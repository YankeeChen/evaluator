...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:about)
    for $node_Process_0 in $node_MonostaticRadar_0/RFDevice:supportsJamming
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_MonostaticRadar_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
        <xqllib:var name="MonostaticRadar_0">{$value_MonostaticRadar_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Detection_0 in $node_Process_0/Nuvio:subprocessOf
    let $value_Detection_0 := fn:data($node_Detection_0/@rdf:resource)
    where fn:exists($value_Detection_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("MonostaticRadar_0"))
  return
    $GRAPH_4
return
