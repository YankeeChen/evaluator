...
let $result :=  
  ...
  let $GRAPH_6 :=  
    for $node_Power_1 in $doc_ObjectDescription
    let $value_Power_1 := fn:data($node_Power_1/@rdf:about)
    for $node_Var0 in $node_Power_1/RFDevice:hasMaxOutputPower
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_Power_1) and fn:exists($value_Var0)
    return
      <xqllib:result>
        <xqllib:var name="Power_1">{$value_Power_1}</xqllib:var>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
