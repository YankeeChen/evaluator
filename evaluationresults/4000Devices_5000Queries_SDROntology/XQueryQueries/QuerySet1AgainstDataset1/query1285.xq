...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:about)
    for $node_FPGA_0 in $node_USRPB2xx_0/Nuvio:aggregateOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0) and fn:exists($node_USRPB2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:about)
    where fn:exists($node_FPGA_0) and $node_FPGA_0/Nuvio:partOf/@rdf:resource = $value_FPGA_0
    return
      <xqllib:result>
        <xqllib:var name="FPGA_0">{$value_FPGA_0}</xqllib:var>
      </xqllib:result>
