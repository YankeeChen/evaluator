...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE310_0 in $node_Satellite_0/RFDevice:isDeployedOn
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:resource)
    where fn:exists($node_Satellite_0) and fn:exists($value_USRPE310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Satellite_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Moving_instance71"]/RFDevice:hasCapability
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("Satellite_0"))
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("Satellite_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_0,$GRAPH_5,("USRPE310_0"))
  return
    $GRAPH_6
return
