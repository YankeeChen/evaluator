...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_InformationContent_0 in $node_Var1/Nuvio:expresses
  ...
  let $GRAPH_7 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#MultiSurface"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_4,$GRAPH_16,("InformationContent_0","Var1"))
  let $GRAPH_18 := xqllib:and($GRAPH_3,$GRAPH_17,("Var1"))
  let $GRAPH_19 := xqllib:and($GRAPH_2,$GRAPH_18,("Var1"))
