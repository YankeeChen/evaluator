...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_USRPB200_210_0/Nuvio:relevantIndividual
  ...
  let $GRAPH_4 :=  
    for $node_DigitalComponent_0 in $doc_ObjectDescription
    let $value_DigitalComponent_0 := fn:data($node_DigitalComponent_0/@rdf:about)
    for $node_Entity_0 in $node_DigitalComponent_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($node_DigitalComponent_0) and fn:exists($value_Entity_0)
    return
      <xqllib:result>
        <xqllib:var name="DigitalComponent_0">{$value_DigitalComponent_0}</xqllib:var>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 :=  
    for $node_DigitalComponent_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#DigitalComponent"]
    let $value_DigitalComponent_0 := fn:data($node_DigitalComponent_0/@rdf:about)
    where fn:exists($node_DigitalComponent_0)
    return
      <xqllib:result>
        <xqllib:var name="DigitalComponent_0">{$value_DigitalComponent_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("DigitalComponent_0"))
  ...
  let $GRAPH_8 :=  
    ...
    for $node_USRPB200_210_0 in $node_Satellite_0/RFDevice:isDeployedOn
    let $value_USRPB200_210_0 := fn:data($node_USRPB200_210_0/@rdf:resource)
    where fn:exists($node_Satellite_0) and fn:exists($value_USRPB200_210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_8,$GRAPH_9,("Satellite_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_7,$GRAPH_10,("USRPB200_210_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_2,$GRAPH_11,("Entity_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_1,$GRAPH_12,("Entity_0"))
  let $GRAPH_14 := xqllib:and($GRAPH_0,$GRAPH_13,("ElectronicAttackSystem_0"))
  return
    $GRAPH_14
return
