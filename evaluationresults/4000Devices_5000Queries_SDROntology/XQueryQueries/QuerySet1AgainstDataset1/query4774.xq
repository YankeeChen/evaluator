...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_Process_0/RFDevice:supportsJamming
  ...
  let $GRAPH_2 :=  
    for $node_Process_0 in $doc_ObjectDescription
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_AbstractGriddedSurface_0 in $node_Process_0/RFDevice:hasTarget
    let $value_AbstractGriddedSurface_0 := fn:data($node_AbstractGriddedSurface_0/@rdf:resource)
    where fn:exists($value_AbstractGriddedSurface_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("AbstractGriddedSurface_0"))
