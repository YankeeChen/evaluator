...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_TerrainBounceJamming_0/RFDevice:supportsTransmitting
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:resource)
    where fn:exists($value_JammingSystem_0) and fn:exists($node_TerrainBounceJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="JammingSystem_0">{$value_JammingSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_TerrainBounceJamming_0 in $node_Quantity_0/RFDevice:hasPulseRepetitionInterval
    let $value_TerrainBounceJamming_0 := fn:data($node_TerrainBounceJamming_0/@rdf:resource)
    where fn:exists($node_Quantity_0) and fn:exists($value_TerrainBounceJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("TerrainBounceJamming_0"))
