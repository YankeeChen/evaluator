...
let $result :=  
  ...
  let $GRAPH_7 :=  
    ...
    where fn:exists($node_InformationEntity_0) and fn:exists($node_Solid_0) and $node_Solid_0/Nuvio:isExpressedBy/@rdf:resource = $value_InformationEntity_0
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
        <xqllib:var name="Solid_0">{$value_Solid_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_8 := xqllib:optional($GRAPH_6,$GRAPH_7,("InformationEntity_0"))
