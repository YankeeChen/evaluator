...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Curve_0) and fn:exists($node_Var0) and $node_Curve_0/RFDevice:hasTarget/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Curve_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/sf#Curve"]
    let $value_Curve_0 := fn:data($node_Curve_0/@rdf:about)
    where fn:exists($node_Curve_0)
    return
      <xqllib:result>
        <xqllib:var name="Curve_0">{$value_Curve_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("Curve_0"))
