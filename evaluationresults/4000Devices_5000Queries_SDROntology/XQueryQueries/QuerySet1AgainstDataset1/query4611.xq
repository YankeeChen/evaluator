...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarWarningReceiver_0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_Jammer_0/Nuvio:partOf
  ...
  let $GRAPH_5 :=  
    for $node_Jammer_0 in $doc_ObjectDescription
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:about)
    for $node_Object_0 in $node_Jammer_0/RFDevice:hasFrequencyAccuracy
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_4,$GRAPH_7,("Jammer_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_3,$GRAPH_8,("Jammer_0"))
  let $GRAPH_10 :=  
    for $node_Quantity_0 in $doc_ObjectDescription
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:about)
    for $node_Entity_0 in $node_Quantity_0/Nuvio:hasProcessQuantity
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_15 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_InformationRepresentation_0) and $node_InformationRepresentation_0/Nuvio:represents/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_13,$GRAPH_17,("Entity_0"))
  ...
  let $GRAPH_23 := xqllib:optional($GRAPH_0,$GRAPH_22,("RadarWarningReceiver_0"))
  return
    $GRAPH_23
return
