...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_UnmannedPlatform_0/RFDevice:isDeployedOn
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:resource)
    where fn:exists($value_SurveillanceRadar_0) and fn:exists($node_UnmannedPlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="SurveillanceRadar_0">{$value_SurveillanceRadar_0}</xqllib:var>
