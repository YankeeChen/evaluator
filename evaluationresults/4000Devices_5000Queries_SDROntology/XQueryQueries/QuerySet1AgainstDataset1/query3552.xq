...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_Entity_0/Nuvio:compositeOf
  ...
  let $GRAPH_7 := $GRAPH_6[(not((data(xqllib:var[@name="DataValue0"]) <= xs:decimal("100"))) and (data(xqllib:var[@name="DataValue0"]) >= xs:decimal("0")))]
