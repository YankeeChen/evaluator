...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Series7FPGA_0 in $node_Entity_0/Nuvio:compositeOf
    let $value_Series7FPGA_0 := fn:data($node_Series7FPGA_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_Series7FPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Series7FPGA_0 in $node_Producer_0/RFDevice:isProducerOf
    let $value_Series7FPGA_0 := fn:data($node_Series7FPGA_0/@rdf:resource)
    where fn:exists($node_Producer_0) and fn:exists($value_Series7FPGA_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 :=  
    ...
    for $node_Entity_0 in $node_Series7FPGA_0/RFDevice:hasProducer
  ...
  let $GRAPH_9 := xqllib:optional($GRAPH_7,$GRAPH_8,("Series7FPGA_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("Entity_0","Series7FPGA_0"))
