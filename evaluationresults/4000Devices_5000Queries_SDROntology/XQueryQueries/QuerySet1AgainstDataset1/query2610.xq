...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Port_0 in $node_Var0/Nuvio:participatesIn
    let $value_Port_0 := fn:data($node_Port_0/@rdf:resource)
    where fn:exists($value_Port_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
