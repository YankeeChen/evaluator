...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE312_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:resource)
    where fn:exists($node_MobilePlatform_0) and fn:exists($value_USRPE312_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_MobilePlatform_0 in $node_Process_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_3 :=  
    for $node_PolyhedralSurface_0 in $doc_ObjectDescription
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_PolyhedralSurface_0) and fn:exists($node_Process_0) and $node_PolyhedralSurface_0/Nuvio:participatesIn/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("PolyhedralSurface_0"))
