...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_Entity_0 in $node_SurveillanceRadar_0/Nuvio:compositeOf
  ...
  let $GRAPH_9 := xqllib:optional($GRAPH_7,$GRAPH_8,("Attribute_0"))
  ...
  let $GRAPH_12 :=  
    ...
    for $node_Value_0 in $node_SpatialAreaUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Value_0 := fn:data($node_Value_0/@rdf:resource)
    where fn:exists($node_SpatialAreaUnit_0) and fn:exists($value_Value_0)
    return
      <xqllib:result>
