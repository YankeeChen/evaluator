...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Value_0 in $node_SpatialVolumeUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Value_0 := fn:data($node_Value_0/@rdf:resource)
    where fn:exists($node_SpatialVolumeUnit_0) and fn:exists($value_Value_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) = 2057729812)]
  let $GRAPH_6 := xqllib:and($GRAPH_3,$GRAPH_5,("Value_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_2,$GRAPH_6,("Value_0"))
