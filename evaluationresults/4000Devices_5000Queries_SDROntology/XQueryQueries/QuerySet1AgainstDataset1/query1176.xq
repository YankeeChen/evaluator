...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CR_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_3 :=  
    for $node_IntValue_0 in $doc_ObjectDescription
    let $value_IntValue_0 := fn:data($node_IntValue_0/@rdf:about)
    for $node_Entity_0 in $node_IntValue_0/Nuvio:isExpressedBy
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_IntValue_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
        <xqllib:var name="IntValue_0">{$value_IntValue_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_4 :=  
    for $node_IntValue_0 in $doc_ObjectDescription
    let $value_IntValue_0 := fn:data($node_IntValue_0/@rdf:about)
    for $node_DataValue0 in $node_IntValue_0/RFDevice:hasPositiveIntegerValue
    let $value_DataValue0 := fn:data($node_DataValue0)
    where fn:exists($value_DataValue0) and fn:exists($node_IntValue_0)
    return
      <xqllib:result>
        <xqllib:var name="DataValue0">{$value_DataValue0}</xqllib:var>
        <xqllib:var name="IntValue_0">{$value_IntValue_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("IntValue_0"))
  let $GRAPH_6 :=  
