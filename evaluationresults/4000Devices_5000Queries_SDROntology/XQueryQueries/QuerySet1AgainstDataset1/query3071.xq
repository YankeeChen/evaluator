...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_Entity_0/RFDevice:hasSamplingRate
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_AntennaPolarization_0) and fn:exists($node_Entity_0) and $node_AntennaPolarization_0/Nuvio:hasProcessQuality/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:optional($GRAPH_3,$GRAPH_4,("AntennaPolarization_0"))
