...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_BlinkingJamming_0/RFDevice:supportsJamming
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($node_BlinkingJamming_0) and fn:exists($value_ElectronicAttackSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_BlinkingJamming_0 in $node_Entity_0/RFDevice:hasPulseRepetitionInterval
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#AbstractGeometry"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_14 := xqllib:and($GRAPH_2,$GRAPH_13,("BlinkingJamming_0"))
  let $GRAPH_15 := xqllib:and($GRAPH_1,$GRAPH_14,("BlinkingJamming_0"))
