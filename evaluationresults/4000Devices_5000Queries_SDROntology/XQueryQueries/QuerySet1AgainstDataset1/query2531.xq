...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE312_0 in $node_Transmitter_0/Nuvio:compositeOf
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:resource)
    where fn:exists($node_Transmitter_0) and fn:exists($value_USRPE312_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Transmitter_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Transceiving_instance100"]/RFDevice:supportsTransmitting
