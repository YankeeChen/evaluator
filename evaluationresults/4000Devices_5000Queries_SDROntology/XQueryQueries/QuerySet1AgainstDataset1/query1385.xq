...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_TrackingRadar_0 in $node_SuperheterodyneReceiver_0/Nuvio:aggregateOf
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:resource)
    where fn:exists($node_SuperheterodyneReceiver_0) and fn:exists($value_TrackingRadar_0)
    return
      <xqllib:result>
