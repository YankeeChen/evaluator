...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Freq_122_88MHz"]/Nuvio:hasObjectQuantity
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Var0 in $node_Object_0/RFDevice:isProducerOf
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_Object_0) and $doc_ObjectDescription[@rdf:ID = "Frequency_instance121"]/RFDevice:hasNoiseFigure/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_12 := xqllib:optional($GRAPH_0,$GRAPH_11,("USRPN300_0"))
  return
    $GRAPH_12
return
