...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Half_DuplexCommunicationsSystem_0 in $node_Entity_0/RFDevice:hasWeight
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Concept_0) and fn:exists($node_Entity_0) and $node_Concept_0/skos:inScheme/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Concept_0 in $node_Object_0/skos:semanticRelation
    let $value_Concept_0 := fn:data($node_Concept_0/@rdf:resource)
    where fn:exists($value_Concept_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="Concept_0">{$value_Concept_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Concept_0","Entity_0"))
  let $GRAPH_8 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_9 := $GRAPH_8[not((data(xqllib:var[@name="DataValue0"]) <= 1.57))]
  ...
  let $GRAPH_13 := $GRAPH_12[(data(xqllib:var[@name="DataValue1"]) > 3.2)]
  let $GRAPH_14 := xqllib:and($GRAPH_11,$GRAPH_13,("Var0"))
  let $GRAPH_15 := xqllib:and($GRAPH_10,$GRAPH_14,("Entity_0"))
  let $GRAPH_16 :=  
    for $node_Entity_0 in $doc_ObjectDescription[RFDevice:hasPulseWidth/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Frequency_instance311"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_20 := xqllib:and($GRAPH_0,$GRAPH_19,("Half_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_20
return
