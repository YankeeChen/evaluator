...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_BistaticRadar_0 in $node_Entity_0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_12 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Process" and RFDevice:hasTransmitPowerRange/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Frequency_instance444"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
