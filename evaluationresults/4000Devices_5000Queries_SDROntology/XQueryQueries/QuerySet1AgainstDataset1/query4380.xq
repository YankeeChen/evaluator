...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_Satellite_0/RFDevice:isDeployedOn
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:resource)
    where fn:exists($node_Satellite_0) and fn:exists($value_SurveillanceRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Process_0) and fn:exists($node_TrackingRadar_0) and $node_TrackingRadar_0/Nuvio:hasParticipant/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("TrackingRadar_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Process_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Satellite_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("Satellite_0"))
