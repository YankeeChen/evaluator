...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE312_0 in $node_Submarine_0/RFDevice:isDeployedOn
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:resource)
    where fn:exists($node_Submarine_0) and fn:exists($value_USRPE312_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Submarine_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Moving_instance21"]/RFDevice:supportsTransmitting
