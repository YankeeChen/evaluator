...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:aggregateOf
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:resource)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($value_USRPB200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $doc_ObjectDescription
    let $value_InstantaneousFrequencyMeasurementReceiver_0 := fn:data($node_InstantaneousFrequencyMeasurementReceiver_0/@rdf:about)
    for $node_Receiving_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/RFDevice:supportsSpectrumSensing
    let $value_Receiving_0 := fn:data($node_Receiving_0/@rdf:resource)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($value_Receiving_0)
    return
      <xqllib:result>
        <xqllib:var name="InstantaneousFrequencyMeasurementReceiver_0">{$value_InstantaneousFrequencyMeasurementReceiver_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Receiving_0 in $node_Entity_0/RFDevice:hasBandwidthRange
    let $value_Receiving_0 := fn:data($node_Receiving_0/@rdf:resource)
  ...
  let $GRAPH_8 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Producer_0 in $node_Entity_0/RFDevice:hasProducer
  ...
  let $GRAPH_12 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $doc_ObjectDescription[@rdf:ID = "FrequencyUnit_instance13"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_13 := xqllib:and($GRAPH_11,$GRAPH_12,("Entity_0"))
  let $GRAPH_14 := xqllib:and($GRAPH_8,$GRAPH_13,("Entity_0"))
  let $GRAPH_15 :=  
  ...
  let $GRAPH_19 := xqllib:optional($GRAPH_4,$GRAPH_18,("Receiving_0"))
  let $GRAPH_20 := xqllib:and($GRAPH_3,$GRAPH_19,("Receiving_0"))
  let $GRAPH_21 := xqllib:and($GRAPH_2,$GRAPH_20,("InstantaneousFrequencyMeasurementReceiver_0"))
  let $GRAPH_22 := xqllib:and($GRAPH_1,$GRAPH_21,("InstantaneousFrequencyMeasurementReceiver_0"))
  let $GRAPH_23 := xqllib:and($GRAPH_0,$GRAPH_22,("USRPB200_0"))
  return
    $GRAPH_23
return
