...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#Transmitter"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Angle_0) and fn:exists($node_Var0) and $node_Angle_0/RFDevice:hasSamplingRate/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_9 :=  
    ...
    for $node_Var0 in $node_Var2/RFDevice:hasWidth
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
