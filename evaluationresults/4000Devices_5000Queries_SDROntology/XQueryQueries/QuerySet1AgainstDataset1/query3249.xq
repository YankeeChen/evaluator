...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_Satellite_0/RFDevice:isDeployedOn
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:resource)
    where fn:exists($value_JammingSystem_0) and fn:exists($node_Satellite_0)
    return
      <xqllib:result>
        <xqllib:var name="JammingSystem_0">{$value_JammingSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Satellite_0 in $node_Process_0/RFDevice:hasCapability
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Process_0 in $node_Percentage_0/RFDevice:hasPulseRepetitionInterval
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) >= xs:decimal("0"))]
  let $GRAPH_7 := xqllib:and($GRAPH_4,$GRAPH_6,("Var0"))
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("Percentage_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("Process_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("Satellite_0"))
