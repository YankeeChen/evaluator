...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Var1 in $node_Var0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[RFDevice:hasCapacity/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#Time_120min"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
