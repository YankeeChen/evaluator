...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_TrackingRadar_0 in $node_Object_0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_CrossEyeJamming_0) and fn:exists($node_Object_0) and $node_CrossEyeJamming_0/RFDevice:supportsTransmitting/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("CrossEyeJamming_0"))
