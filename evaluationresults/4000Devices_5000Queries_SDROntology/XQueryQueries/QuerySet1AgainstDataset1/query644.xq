...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN300_0 in $node_RadioComponent_0/Nuvio:partOf
  ...
  let $GRAPH_4 :=  
    for $node_RadioComponent_0 in $doc_ObjectDescription
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:about)
    for $node_CortexA9_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_CortexA9_0 := fn:data($node_CortexA9_0/@rdf:resource)
    where fn:exists($value_CortexA9_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_CortexA9_0 in $node_XilinxFPGA_0/Nuvio:compositeOf
    let $value_CortexA9_0 := fn:data($node_CortexA9_0/@rdf:resource)
    where fn:exists($value_CortexA9_0) and fn:exists($node_XilinxFPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="CortexA9_0">{$value_CortexA9_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("XilinxFPGA_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("CortexA9_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("CortexA9_0"))
