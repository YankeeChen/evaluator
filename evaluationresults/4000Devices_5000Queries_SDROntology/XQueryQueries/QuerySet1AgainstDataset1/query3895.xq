...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RFSystem_0 in $node_Entity_0/RFDevice:hasLevel
  ...
  let $GRAPH_4 := $GRAPH_3[(data(xqllib:var[@name="DataValue0"]) >= xs:decimal("-180"))]
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue1"]) > xs:decimal("0"))]
