...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:about)
    for $node_IEDJammer_0 in $node_SDR_0/Nuvio:compositeOf
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
    where fn:exists($value_IEDJammer_0) and fn:exists($node_SDR_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_IEDJammer_0 in $node_CrossPolarizationJamming_0/RFDevice:hasCapability
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
    where fn:exists($node_CrossPolarizationJamming_0) and fn:exists($value_IEDJammer_0)
    return
      <xqllib:result>
