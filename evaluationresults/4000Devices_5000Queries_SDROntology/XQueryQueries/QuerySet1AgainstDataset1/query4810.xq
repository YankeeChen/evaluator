...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_XilinxZynq7000Family_0/Nuvio:partOf
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:resource)
    where fn:exists($value_USRPE3xx_0) and fn:exists($node_XilinxZynq7000Family_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPE3xx_0">{$value_USRPE3xx_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_XilinxZynq7000Family_0 in $node_XilinxZynq7100SoC_0/Nuvio:compositeOf
    let $value_XilinxZynq7000Family_0 := fn:data($node_XilinxZynq7000Family_0/@rdf:resource)
    where fn:exists($value_XilinxZynq7000Family_0) and fn:exists($node_XilinxZynq7100SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="XilinxZynq7000Family_0">{$value_XilinxZynq7000Family_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_XilinxZynq7100SoC_0 in $node_Entity_0/Nuvio:compositeOf
    let $value_XilinxZynq7100SoC_0 := fn:data($node_XilinxZynq7100SoC_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_XilinxZynq7100SoC_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("XilinxZynq7100SoC_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("XilinxZynq7100SoC_0"))
