...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_RadarJammer_0 in $doc_ObjectDescription
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:about)
    for $node_Object_0 in $node_RadarJammer_0/Nuvio:compositeOf
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($value_Object_0) and fn:exists($node_RadarJammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_FrequencyShiftKeyingModulation_0) and fn:exists($node_Object_0) and $node_FrequencyShiftKeyingModulation_0/Nuvio:participatesIn/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
