...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_Entity_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Situation_0 in $node_Var0/Nuvio:hasUnitOfMeasure
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:resource)
    where fn:exists($value_Situation_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Situation_0 in $node_Value_0/Nuvio:relevantIndividual
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:resource)
    where fn:exists($value_Situation_0) and fn:exists($node_Value_0)
    return
      <xqllib:result>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    for $node_Angle_0 in $doc_ObjectDescription
    let $value_Angle_0 := fn:data($node_Angle_0/@rdf:about)
    for $node_AngleValue_0 in $node_Angle_0/Nuvio:hasValue
    let $value_AngleValue_0 := fn:data($node_AngleValue_0/@rdf:resource)
    where fn:exists($value_AngleValue_0) and fn:exists($node_Angle_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) > 870888398)]
  let $GRAPH_9 := xqllib:and($GRAPH_6,$GRAPH_8,("AngleValue_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_5,$GRAPH_9,("Angle_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_4,$GRAPH_10,("Situation_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_3,$GRAPH_11,("Situation_0"))
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_0,$GRAPH_16,("USRPN200_0"))
  return
    $GRAPH_17
return
