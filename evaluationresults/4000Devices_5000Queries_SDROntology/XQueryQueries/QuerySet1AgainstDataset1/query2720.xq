...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB205mini_i_0 in $node_Object_0/RFDevice:hasProducer
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_Voltage_0) and $node_Voltage_0/RFDevice:hasSamplingRate/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("Voltage_0"))
