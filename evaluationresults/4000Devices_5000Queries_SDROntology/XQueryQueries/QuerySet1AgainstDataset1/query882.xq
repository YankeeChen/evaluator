...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_CoverPulsesJamming_0) and fn:exists($node_Object_0) and $node_CoverPulsesJamming_0/Nuvio:hasParticipant/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_10 :=  
    ...
    for $node_Var0 in $node_RadioFrequencyBand_0/RFDevice:hasFrequencyRange
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
