...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_TrackingRadar_0 in $node_Jammer_0/Nuvio:aggregateOf
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_TrackingRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Jammer_0 in $doc_ObjectDescription[RFDevice:hasFrequencyAccuracy/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#RadarFrequencyBand_instance63"]
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:about)
    where fn:exists($node_Jammer_0)
    return
      <xqllib:result>
