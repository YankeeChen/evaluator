...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX3xx_0 := fn:data($node_USRPX3xx_0/@rdf:about)
    for $node_Var0 in $node_USRPX3xx_0/Nuvio:compositeOf
  ...
  let $GRAPH_10 :=  
    ...
    for $node_Object_0 in $node_SpotJamming_0/Nuvio:hasParticipant
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($value_Object_0) and fn:exists($node_SpotJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
