...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_QuadratureAmplitudeModulation_0 in $node_Var0/Nuvio:hasSubprocess
    let $value_QuadratureAmplitudeModulation_0 := fn:data($node_QuadratureAmplitudeModulation_0/@rdf:resource)
    where fn:exists($value_QuadratureAmplitudeModulation_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("QuadratureAmplitudeModulation_0"))
