...
let $result :=  
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) != -1408870172)]
  ...
  let $GRAPH_11 :=  
    for $node_Var0 in $doc_ObjectDescription[Nuvio:isExpressedBy/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription4000.owl#Instance4922"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_15 := xqllib:optional($GRAPH_0,$GRAPH_14,("SearchRadar_0"))
  return
    $GRAPH_15
return
