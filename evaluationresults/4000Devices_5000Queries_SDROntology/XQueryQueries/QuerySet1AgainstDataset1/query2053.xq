...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:about)
    for $node_ContinuousWaveRadarModulation_0 in $node_BistaticRadar_0/RFDevice:supportsTuning
    let $value_ContinuousWaveRadarModulation_0 := fn:data($node_ContinuousWaveRadarModulation_0/@rdf:resource)
    where fn:exists($node_BistaticRadar_0) and fn:exists($value_ContinuousWaveRadarModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="BistaticRadar_0">{$value_BistaticRadar_0}</xqllib:var>
