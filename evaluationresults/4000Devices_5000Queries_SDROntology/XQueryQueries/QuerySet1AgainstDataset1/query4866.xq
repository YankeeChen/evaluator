...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $doc_ObjectDescription
    let $value_InstantaneousFrequencyMeasurementReceiver_0 := fn:data($node_InstantaneousFrequencyMeasurementReceiver_0/@rdf:about)
    for $node_FreqencyRange_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/RFDevice:hasFrequencyAccuracy
    let $value_FreqencyRange_0 := fn:data($node_FreqencyRange_0/@rdf:resource)
    where fn:exists($value_FreqencyRange_0) and fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_FreqencyRange_0 in $node_DataSize_0/RFDevice:hasMin
    let $value_FreqencyRange_0 := fn:data($node_FreqencyRange_0/@rdf:resource)
    where fn:exists($node_DataSize_0) and fn:exists($value_FreqencyRange_0)
    return
      <xqllib:result>
