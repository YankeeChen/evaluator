...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB210_0 := fn:data($node_USRPB210_0/@rdf:about)
    for $node_DirectConversionReceiver_0 in $node_USRPB210_0/Nuvio:aggregateOf
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($value_DirectConversionReceiver_0) and fn:exists($node_USRPB210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_DirectConversionReceiver_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($value_DirectConversionReceiver_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
        <xqllib:var name="DirectConversionReceiver_0">{$value_DirectConversionReceiver_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_RadioComponent_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription4000.owl#IEDTrigger_instance146"]/Nuvio:partOf
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_0,$GRAPH_5,("USRPB210_0"))
  return
    $GRAPH_6
return
