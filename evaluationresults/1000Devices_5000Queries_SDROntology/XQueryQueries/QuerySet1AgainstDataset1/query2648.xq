...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Instance1196"]/RFDevice:hasCapability
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Var0 in $node_CrossEyeJamming_0/Nuvio:isExpressedBy
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_CrossEyeJamming_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 :=  
    for $node_CrossEyeJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Power_45W"]/RFDevice:hasPulseRepetitionInterval
    let $value_CrossEyeJamming_0 := fn:data($node_CrossEyeJamming_0/@rdf:resource)
    where fn:exists($value_CrossEyeJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="CrossEyeJamming_0">{$value_CrossEyeJamming_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("CrossEyeJamming_0"))
