...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_MobilePlatform_0 in $doc_ObjectDescription[RFDevice:supportsJamming/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance22"]
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:about)
    where fn:exists($node_MobilePlatform_0)
    return
      <xqllib:result>
