...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN2xx_0 := fn:data($node_USRPN2xx_0/@rdf:about)
    for $node_SuperheterodyneReceiver_0 in $node_USRPN2xx_0/Nuvio:compositeOf
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($value_SuperheterodyneReceiver_0) and fn:exists($node_USRPN2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:about)
    for $node_PhasedArrayAntenna_0 in $node_SuperheterodyneReceiver_0/Nuvio:partOf
    let $value_PhasedArrayAntenna_0 := fn:data($node_PhasedArrayAntenna_0/@rdf:resource)
    where fn:exists($value_PhasedArrayAntenna_0) and fn:exists($node_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_PhasedArrayAntenna_0 in $node_Var0/RFDevice:supportsTuning
    let $value_PhasedArrayAntenna_0 := fn:data($node_PhasedArrayAntenna_0/@rdf:resource)
    where fn:exists($value_PhasedArrayAntenna_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
        <xqllib:var name="PhasedArrayAntenna_0">{$value_PhasedArrayAntenna_0}</xqllib:var>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("Attribute_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("Var0"))
  let $GRAPH_11 := ($GRAPH_5,$GRAPH_10)
  let $GRAPH_12 := xqllib:and($GRAPH_3,$GRAPH_11,("PhasedArrayAntenna_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_2,$GRAPH_12,("SuperheterodyneReceiver_0"))
