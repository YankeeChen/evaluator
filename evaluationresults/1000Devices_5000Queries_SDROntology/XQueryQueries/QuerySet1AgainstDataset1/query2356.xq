...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_SuperheterodyneReceiver_0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_SuperheterodyneReceiver_0 in $node_Transceiving_0/RFDevice:supportsTuning
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($value_SuperheterodyneReceiver_0) and fn:exists($node_Transceiving_0)
    return
      <xqllib:result>
        <xqllib:var name="SuperheterodyneReceiver_0">{$value_SuperheterodyneReceiver_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_0,$GRAPH_5,("USRPN210_0"))
  return
    $GRAPH_6
return
