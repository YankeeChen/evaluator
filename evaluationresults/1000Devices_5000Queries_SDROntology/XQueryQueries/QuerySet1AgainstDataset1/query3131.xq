...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarWarningReceiver_0 in $node_GroundPlatform_0/RFDevice:isDeployedOn
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:resource)
    where fn:exists($node_GroundPlatform_0) and fn:exists($value_RadarWarningReceiver_0)
    return
      <xqllib:result>
