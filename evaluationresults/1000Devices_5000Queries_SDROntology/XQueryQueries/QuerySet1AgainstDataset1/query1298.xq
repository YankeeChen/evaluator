...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    where fn:exists($node_InformationEntity_0) and fn:exists($node_QuantityRange_0) and $node_QuantityRange_0/Nuvio:expresses/@rdf:resource = $value_InformationEntity_0
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_QuantityRange_0 in $node_Entity_1/RFDevice:hasMin
    let $value_QuantityRange_0 := fn:data($node_QuantityRange_0/@rdf:resource)
    where fn:exists($node_Entity_1) and fn:exists($value_QuantityRange_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_1">{$value_Entity_1}</xqllib:var>
        <xqllib:var name="QuantityRange_0">{$value_QuantityRange_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("QuantityRange_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("InformationEntity_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_3,$GRAPH_8,("InformationContent_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("InformationRepresentation_0"))
