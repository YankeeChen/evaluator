...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:about)
    for $node_Entity_0 in $node_USRPN210_0/RFDevice:isProducerOf
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_Situation_0) and $doc_ObjectDescription[@rdf:ID = "USRPN300_instance13"]/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 :=  
    for $node_Situation_0 in $doc_ObjectDescription
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_EnergyDetection_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_EnergyDetection_0 := fn:data($node_EnergyDetection_0/@rdf:resource)
    where fn:exists($value_EnergyDetection_0) and fn:exists($node_Situation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("EnergyDetection_0"))
