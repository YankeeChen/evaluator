...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#RadarFrequencyBand_instance16"]/RFDevice:hasFrequencyAccuracy
  ...
  let $GRAPH_8 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#Byte"]/Nuvio:hasUnitOfMeasure
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_5,$GRAPH_9,("Entity_0"))
