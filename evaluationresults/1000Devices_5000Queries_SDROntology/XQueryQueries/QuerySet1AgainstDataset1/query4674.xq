...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:about)
    for $node_DirectConversionReceiver_0 in $node_USRPX300_0/Nuvio:compositeOf
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($value_DirectConversionReceiver_0) and fn:exists($node_USRPX300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("DirectConversionReceiver_0"))
