...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:about)
    for $node_Ship_0 in $node_MonostaticRadar_0/RFDevice:isDeployedOn
    let $value_Ship_0 := fn:data($node_Ship_0/@rdf:resource)
    where fn:exists($node_MonostaticRadar_0) and fn:exists($value_Ship_0)
    return
      <xqllib:result>
        <xqllib:var name="MonostaticRadar_0">{$value_MonostaticRadar_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_Ship_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance4"]/RFDevice:supportsTuning
    let $value_Ship_0 := fn:data($node_Ship_0/@rdf:resource)
