...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_Receiver_0/Nuvio:partOf
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:resource)
    where fn:exists($node_Receiver_0) and fn:exists($value_USRPE3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Receiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Producer_instance325"]/RFDevice:hasProducer
