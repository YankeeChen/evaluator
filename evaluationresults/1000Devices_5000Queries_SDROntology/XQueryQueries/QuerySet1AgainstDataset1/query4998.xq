...
let $result :=  
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) <= 1837641216)]
  let $GRAPH_6 :=  
  ...
  let $GRAPH_13 :=  
    ...
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    where fn:exists($node_Value_0) and $doc_ObjectDescription[@rdf:ID = "PowerUnit_instance0"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Value_0
    return
      <xqllib:result>
        <xqllib:var name="Value_0">{$value_Value_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_14 := xqllib:optional($GRAPH_12,$GRAPH_13,("Value_0"))
