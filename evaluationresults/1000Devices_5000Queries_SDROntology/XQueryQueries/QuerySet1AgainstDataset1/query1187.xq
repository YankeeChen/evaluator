...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_RadarJammer_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    for $node_RadarJammer_0 in $doc_ObjectDescription
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:about)
    for $node_Component_0 in $node_RadarJammer_0/Nuvio:partOf
    let $value_Component_0 := fn:data($node_Component_0/@rdf:resource)
    where fn:exists($value_Component_0) and fn:exists($node_RadarJammer_0)
    return
      <xqllib:result>
