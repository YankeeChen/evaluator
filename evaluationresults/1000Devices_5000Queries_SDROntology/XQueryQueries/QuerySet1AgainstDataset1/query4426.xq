...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_ElectronicAttackSystem_0 in $doc_ObjectDescription[RFDevice:hasNoiseFigure/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Weight_instance46"]
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:about)
    where fn:exists($node_ElectronicAttackSystem_0)
    return
      <xqllib:result>
