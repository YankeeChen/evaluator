...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN3xx_0 in $node_Var0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Var0) and fn:exists($node_Width_0) and $node_Width_0/RFDevice:hasEffectiveRadiatedPower/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("USRPN3xx_0"))
  return
    $GRAPH_8
return
