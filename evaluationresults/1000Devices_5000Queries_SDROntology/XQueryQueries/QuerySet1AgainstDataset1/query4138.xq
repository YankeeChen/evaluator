...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:about)
    for $node_Object_0 in $node_USRPE312_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and $doc_ObjectDescription[@rdf:ID = "Gain_instance3"]/RFDevice:hasNoiseFigure/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
      </xqllib:result>
