...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:aggregateOf
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:resource)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($value_USRPE3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_RadarFrequencyBand_0/RFDevice:hasFrequencyRange
    ...
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_RadarFrequencyBand_0)
    return
      <xqllib:result>
        <xqllib:var name="InstantaneousFrequencyMeasurementReceiver_0">{$value_InstantaneousFrequencyMeasurementReceiver_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadarFrequencyBand_0 in $node_Quantity_0/RFDevice:hasMin
    let $value_RadarFrequencyBand_0 := fn:data($node_RadarFrequencyBand_0/@rdf:resource)
    where fn:exists($node_Quantity_0) and fn:exists($value_RadarFrequencyBand_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("RadarFrequencyBand_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("InstantaneousFrequencyMeasurementReceiver_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_0,$GRAPH_7,("USRPE3xx_0"))
  return
    $GRAPH_8
return
