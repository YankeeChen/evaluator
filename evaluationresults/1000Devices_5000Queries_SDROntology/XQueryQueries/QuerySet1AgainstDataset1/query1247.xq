...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CR_0 := fn:data($node_CR_0/@rdf:about)
    for $node_IEDJammer_0 in $node_CR_0/Nuvio:partOf
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
    where fn:exists($node_CR_0) and fn:exists($value_IEDJammer_0)
    return
      <xqllib:result>
        <xqllib:var name="CR_0">{$value_CR_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    for $node_RadarFrequencyBand_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#NoiseFigure_instance6"]/RFDevice:hasMax
