...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:about)
    for $node_EmitterIdentification_0 in $node_RadarWarningReceiver_0/RFDevice:supportsTuning
    let $value_EmitterIdentification_0 := fn:data($node_EmitterIdentification_0/@rdf:resource)
    where fn:exists($value_EmitterIdentification_0) and fn:exists($node_RadarWarningReceiver_0)
    return
      <xqllib:result>
