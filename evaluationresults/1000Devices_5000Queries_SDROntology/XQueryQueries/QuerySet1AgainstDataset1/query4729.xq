...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_TimeInstant_0 in $node_Object_0/RFDevice:hasLength
    let $value_TimeInstant_0 := fn:data($node_TimeInstant_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_TimeInstant_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    let $value_TimeInstant_0 := fn:data($node_TimeInstant_0/@rdf:about)
    for $node_Var0 in $node_TimeInstant_0/Nuvio:hasValue
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_TimeInstant_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
        <xqllib:var name="TimeInstant_0">{$value_TimeInstant_0}</xqllib:var>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("TimeInstant_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("TimeInstant_0"))
