...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN300_0 := fn:data($node_USRPN300_0/@rdf:about)
    for $node_Satellite_0 in $node_USRPN300_0/RFDevice:isDeployedOn
    let $value_Satellite_0 := fn:data($node_Satellite_0/@rdf:resource)
    where fn:exists($value_Satellite_0) and fn:exists($node_USRPN300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Satellite_0 := fn:data($node_Satellite_0/@rdf:about)
    for $node_Entity_0 in $node_Satellite_0/RFDevice:hasCapability
