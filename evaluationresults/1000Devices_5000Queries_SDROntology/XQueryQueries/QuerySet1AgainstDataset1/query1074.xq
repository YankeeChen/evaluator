...
let $result :=  
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Entity_0 in $node_Attribute_0/RFDevice:hasResolution
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_14 :=  
    for $node_Situation_0 in $doc_ObjectDescription
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_ConceptScheme_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_ConceptScheme_0 := fn:data($node_ConceptScheme_0/@rdf:resource)
    where fn:exists($value_ConceptScheme_0) and fn:exists($node_Situation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_15 :=  
    ...
    for $node_ConceptScheme_0 in $node_Collection_0/skos:hasTopConcept
    let $value_ConceptScheme_0 := fn:data($node_ConceptScheme_0/@rdf:resource)
    where fn:exists($node_Collection_0) and fn:exists($value_ConceptScheme_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_15,$GRAPH_16,("Collection_0"))
  let $GRAPH_18 := xqllib:and($GRAPH_14,$GRAPH_17,("ConceptScheme_0"))
  let $GRAPH_19 := xqllib:and($GRAPH_13,$GRAPH_18,())
