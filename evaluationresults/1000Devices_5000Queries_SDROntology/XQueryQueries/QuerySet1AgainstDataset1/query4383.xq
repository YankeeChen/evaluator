...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_ModelX_0 in $doc_ObjectDescription[Nuvio:hasObjectQuantity/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Weight_instance4"]
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:about)
    where fn:exists($node_ModelX_0)
    return
      <xqllib:result>
