...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CommunicationsSystem_0 in $node_Entity_0/RFDevice:hasHeight
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_InboundRangeGatePullOffJamming_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_InboundRangeGatePullOffJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_7 :=  
    for $node_InboundRangeGatePullOffJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Power_45W"]/RFDevice:hasMaxSensedPower
    let $value_InboundRangeGatePullOffJamming_0 := fn:data($node_InboundRangeGatePullOffJamming_0/@rdf:resource)
    where fn:exists($value_InboundRangeGatePullOffJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="InboundRangeGatePullOffJamming_0">{$value_InboundRangeGatePullOffJamming_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_8 := ($GRAPH_6,$GRAPH_7)
  let $GRAPH_9 := xqllib:and($GRAPH_4,$GRAPH_8,("InboundRangeGatePullOffJamming_0"))
