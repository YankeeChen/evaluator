...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:about)
    for $node_Transmitter_0 in $node_JammingSystem_0/Nuvio:aggregateOf
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:resource)
    where fn:exists($node_JammingSystem_0) and fn:exists($value_Transmitter_0)
    return
      <xqllib:result>
        <xqllib:var name="JammingSystem_0">{$value_JammingSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_Transmitter_0 in $doc_ObjectDescription[RFDevice:hasFrequencyAccuracy/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#RadarFrequencyBand_instance41"]
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:about)
    where fn:exists($node_Transmitter_0)
    return
      <xqllib:result>
