...
let $result :=  
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) < 596041190)]
  ...
  let $GRAPH_11 :=  
    for $node_Value_0 in $doc_ObjectDescription
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    for $node_Process_0 in $node_Value_0/Nuvio:hasUnitOfMeasure
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($value_Process_0) and fn:exists($node_Value_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_13 := $GRAPH_12[(data(xqllib:var[@name="DataValue1"]) < 8554853)]
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_10,$GRAPH_16,("Value_0"))
