...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_SearchRadar_0 in $doc_ObjectDescription[RFDevice:isDeployedOn/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Satellite_instance8"]
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:about)
    where fn:exists($node_SearchRadar_0)
    return
      <xqllib:result>
