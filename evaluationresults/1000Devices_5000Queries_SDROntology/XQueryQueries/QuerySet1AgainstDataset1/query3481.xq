...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB210_0 := fn:data($node_USRPB210_0/@rdf:about)
    for $node_Object_0 in $node_USRPB210_0/RFDevice:isProducerOf
  ...
  let $GRAPH_4 :=  
    for $node_SweptSpotJamming_0 in $doc_ObjectDescription
    let $value_SweptSpotJamming_0 := fn:data($node_SweptSpotJamming_0/@rdf:about)
    for $node_Quantity_0 in $node_SweptSpotJamming_0/Nuvio:hasProcessQuantity
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:resource)
    where fn:exists($value_Quantity_0) and fn:exists($node_SweptSpotJamming_0)
    return
      <xqllib:result>
