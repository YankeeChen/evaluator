...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#ADC_instance33"]/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_RFFrontEnd_0/Nuvio:isExpressedBy
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_5 :=  
    for $node_RFFrontEnd_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Filter_instance4"]/Nuvio:compositeOf
    let $value_RFFrontEnd_0 := fn:data($node_RFFrontEnd_0/@rdf:resource)
    where fn:exists($value_RFFrontEnd_0)
    return
      <xqllib:result>
        <xqllib:var name="RFFrontEnd_0">{$value_RFFrontEnd_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("RFFrontEnd_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("RFFrontEnd_0"))
