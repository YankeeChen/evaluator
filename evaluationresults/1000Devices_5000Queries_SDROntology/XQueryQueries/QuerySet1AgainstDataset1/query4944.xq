...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_FPGA_0/Nuvio:compositeOf
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:resource)
    where fn:exists($node_FPGA_0) and fn:exists($value_USRPN210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Quality_0 in $node_Object_0/Nuvio:hasObjectQuality
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("FPGA_0"))
