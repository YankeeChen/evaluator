...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX300_0 in $node_Object_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_EmitterIdentification_0) and fn:exists($node_Object_0) and $node_EmitterIdentification_0/Nuvio:hasParticipant/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_0,$GRAPH_5,("USRPX300_0"))
  return
    $GRAPH_6
return
