...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:about)
    for $node_Aircraft_0 in $node_MonostaticRadar_0/RFDevice:isDeployedOn
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:resource)
    where fn:exists($value_Aircraft_0) and fn:exists($node_MonostaticRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Aircraft_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance22"]/RFDevice:supportsSpectrumSensing
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:resource)
