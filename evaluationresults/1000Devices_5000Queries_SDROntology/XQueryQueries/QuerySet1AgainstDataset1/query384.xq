...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:about)
    for $node_FPGA_0 in $node_USRPB200mini_0/Nuvio:compositeOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0) and fn:exists($node_USRPB200mini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_FPGA_0 in $node_Entity_0/Nuvio:aggregateOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Producer_instance258"]/RFDevice:isProducerOf
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("FPGA_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_0,$GRAPH_10,("USRPB200mini_0"))
  return
    $GRAPH_11
return
