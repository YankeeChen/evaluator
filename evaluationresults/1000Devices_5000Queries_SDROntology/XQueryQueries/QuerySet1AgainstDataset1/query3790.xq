...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_Weight_0/RFDevice:hasMaxPowerConsumption
    let $value_PulseRadar_0 := fn:data($node_PulseRadar_0/@rdf:resource)
    where fn:exists($value_PulseRadar_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseRadar_0">{$value_PulseRadar_0}</xqllib:var>
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) = xs:double("47.27301"))]
  let $GRAPH_7 := xqllib:and($GRAPH_4,$GRAPH_6,("Var0"))
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("Var0"))
  let $GRAPH_9 := xqllib:optional($GRAPH_2,$GRAPH_8,("Weight_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("Weight_0"))
  let $GRAPH_11 := xqllib:optional($GRAPH_0,$GRAPH_10,("PulseRadar_0"))
  return
    $GRAPH_11
return
