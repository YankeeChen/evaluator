...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_210_0 in $node_Jammer_0/Nuvio:compositeOf
    let $value_USRPB200_210_0 := fn:data($node_USRPB200_210_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_USRPB200_210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Jammer_0 in $doc_ObjectDescription
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:about)
    for $node_RangeGatePullOffJamming_0 in $node_Jammer_0/RFDevice:supportsJamming
    let $value_RangeGatePullOffJamming_0 := fn:data($node_RangeGatePullOffJamming_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_RangeGatePullOffJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_RangeGatePullOffJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Instance85"]/RFDevice:hasPulseWidth
    let $value_RangeGatePullOffJamming_0 := fn:data($node_RangeGatePullOffJamming_0/@rdf:resource)
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("RangeGatePullOffJamming_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Jammer_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_0,$GRAPH_7,("USRPB200_210_0"))
  return
    $GRAPH_8
return
