...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_Var0/Nuvio:participatesIn
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Var0 in $node_StandoffJamming_0/RFDevice:supportsTuning
  ...
  let $GRAPH_6 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_StandoffJamming_0 in $node_Entity_0/RFDevice:hasTransmitPowerRange
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("Entity_0","Var0"))
  let $GRAPH_9 :=  
  ...
  let $GRAPH_16 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#EnergyDetection"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
  ...
  let $GRAPH_17 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_Entity_1 in $node_Var1/Nuvio:representedBy
    let $value_Entity_1 := fn:data($node_Entity_1/@rdf:resource)
    where fn:exists($value_Entity_1) and fn:exists($node_Var1)
    return
      <xqllib:result>
