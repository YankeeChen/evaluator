...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE313_0 in $node_Entity_0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_CyclostationaryDetection_0) and fn:exists($node_Entity_0) and $node_CyclostationaryDetection_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#RadarFrequencyBand_instance40"]/Nuvio:relevantIndividual
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_5,$GRAPH_6,("CyclostationaryDetection_0"))
