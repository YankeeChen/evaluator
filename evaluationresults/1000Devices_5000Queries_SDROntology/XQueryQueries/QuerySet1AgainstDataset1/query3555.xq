...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "Frequency_instance48"]/RFDevice:hasWidth/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_9 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#XilinxZynq7020SoC"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
  ...
  let $GRAPH_14 := xqllib:optional($GRAPH_2,$GRAPH_13,("UnitOfMeasure_0","Var0"))
