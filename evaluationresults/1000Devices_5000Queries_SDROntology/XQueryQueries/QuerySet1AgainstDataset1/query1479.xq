...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_DirectConversionReceiver_0/Nuvio:compositeOf
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_JammingSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#RadarBand_C"]/RFDevice:hasFrequencyAccuracy
