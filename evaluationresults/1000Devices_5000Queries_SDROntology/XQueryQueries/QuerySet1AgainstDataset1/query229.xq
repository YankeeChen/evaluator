...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RFSystem_0 in $node_Weight_0/RFDevice:hasWeight
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) <= 2005492006)]
  let $GRAPH_6 :=  
  ...
  let $GRAPH_9 :=  
    ...
    for $node_Entity_0 in $node_Geometry_0/geosparql:defaultGeometry
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue1"]) > fn:true())]
  let $GRAPH_12 := xqllib:and($GRAPH_9,$GRAPH_11,("Geometry_0"))
  let $GRAPH_13 :=  
  ...
  let $GRAPH_19 :=  
    for $node_Value_0 in $doc_ObjectDescription
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    for $node_DataValue2 in $node_Value_0/Nuvio:hasPrecision
    let $value_DataValue2 := fn:data($node_DataValue2)
    where fn:exists($value_DataValue2) and fn:exists($node_Value_0)
    return
      <xqllib:result>
        <xqllib:var name="DataValue2">{$value_DataValue2}</xqllib:var>
        <xqllib:var name="Value_0">{$value_Value_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_20 := $GRAPH_19[(data(xqllib:var[@name="DataValue2"]) != -900546772)]
  let $GRAPH_21 := xqllib:optional($GRAPH_18,$GRAPH_20,("Value_0"))
  let $GRAPH_22 := xqllib:and($GRAPH_17,$GRAPH_21,("Value_0"))
  let $GRAPH_23 := xqllib:and($GRAPH_6,$GRAPH_22,("Entity_0"))
  let $GRAPH_24 := xqllib:and($GRAPH_5,$GRAPH_23,("Var0"))
  let $GRAPH_25 := xqllib:and($GRAPH_3,$GRAPH_24,("Var0"))
  let $GRAPH_26 := xqllib:and($GRAPH_2,$GRAPH_25,("Weight_0"))
  let $GRAPH_27 := xqllib:and($GRAPH_1,$GRAPH_26,("Weight_0"))
  let $GRAPH_28 := xqllib:and($GRAPH_0,$GRAPH_27,("RFSystem_0"))
  return
    $GRAPH_28
return
