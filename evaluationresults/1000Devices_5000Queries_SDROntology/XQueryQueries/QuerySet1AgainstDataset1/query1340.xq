...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ContinuousWaveRadar_0 := fn:data($node_ContinuousWaveRadar_0/@rdf:about)
    for $node_RadioComponent_0 in $node_ContinuousWaveRadar_0/Nuvio:aggregateOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($node_ContinuousWaveRadar_0) and fn:exists($value_RadioComponent_0)
    return
      <xqllib:result>
        <xqllib:var name="ContinuousWaveRadar_0">{$value_ContinuousWaveRadar_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    for $node_DipoleAntenna_0 in $doc_ObjectDescription
    let $value_DipoleAntenna_0 := fn:data($node_DipoleAntenna_0/@rdf:about)
    for $node_Attribute_0 in $node_DipoleAntenna_0/RFDevice:hasLevel
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:resource)
    where fn:exists($value_Attribute_0) and fn:exists($node_DipoleAntenna_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_3,$GRAPH_9,("DipoleAntenna_0"))
