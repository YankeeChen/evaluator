...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_CommunicationsJamming_0/RFDevice:supportsTransmitting
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($node_CommunicationsJamming_0) and fn:exists($value_ElectronicAttackSystem_0)
    return
      <xqllib:result>
