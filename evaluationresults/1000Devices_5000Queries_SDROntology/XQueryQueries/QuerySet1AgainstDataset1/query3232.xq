...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:about)
    for $node_FPGA_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("FPGA_0"))
