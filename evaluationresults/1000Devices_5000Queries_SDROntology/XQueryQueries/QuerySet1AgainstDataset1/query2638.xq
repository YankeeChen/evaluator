...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN300_0 in $node_Var0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_CyclostationaryDetection_0) and fn:exists($node_Var0) and $node_CyclostationaryDetection_0/RFDevice:hasCapability/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("CyclostationaryDetection_0"))
  let $GRAPH_5 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_Var0 in $node_Var1/Nuvio:hasObjectQuantity
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("USRPN300_0"))
  return
    $GRAPH_8
return
