...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Freq_153_6MHz"]/RFDevice:hasNoiseFigure
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_6 :=  
    for $node_Var0 in $doc_ObjectDescription[RFDevice:hasProducer/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Producer_instance17"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
