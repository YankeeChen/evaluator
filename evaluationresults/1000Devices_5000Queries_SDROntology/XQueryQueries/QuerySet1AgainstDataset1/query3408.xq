...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_EmitterLocationDetection_0/RFDevice:supportsTuning
  ...
  let $GRAPH_3 :=  
    for $node_EmitterLocationDetection_0 in $doc_ObjectDescription
    let $value_EmitterLocationDetection_0 := fn:data($node_EmitterLocationDetection_0/@rdf:about)
    for $node_Var0 in $node_EmitterLocationDetection_0/Nuvio:participatesIn
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_EmitterLocationDetection_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
        <xqllib:var name="EmitterLocationDetection_0">{$value_EmitterLocationDetection_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Var0 in $node_RangeGatePullOffJamming_0/RFDevice:supportsTransmitting
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_RangeGatePullOffJamming_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("RangeGatePullOffJamming_0"))
  let $GRAPH_7 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Weight_instance92"]/RFDevice:hasLevel
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("EmitterLocationDetection_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_1,$GRAPH_10,("EmitterLocationDetection_0"))
