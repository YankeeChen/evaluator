...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_Process_0 in $node_Half_DuplexCommunicationsSystem_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Process_0) and fn:exists($node_Spartan6FPGA_0) and $node_Spartan6FPGA_0/Nuvio:participatesIn/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
