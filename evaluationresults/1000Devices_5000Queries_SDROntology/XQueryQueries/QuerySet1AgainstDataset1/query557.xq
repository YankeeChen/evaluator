...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_SearchRadar_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#ContinuousWaveRadarModulation_instance4"]/RFDevice:supportsSpectrumSensing
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:resource)
    where fn:exists($value_SearchRadar_0)
    return
      <xqllib:result>
        <xqllib:var name="SearchRadar_0">{$value_SearchRadar_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_2 := xqllib:and($GRAPH_0,$GRAPH_1,("SearchRadar_0"))
  return
    $GRAPH_2
return
