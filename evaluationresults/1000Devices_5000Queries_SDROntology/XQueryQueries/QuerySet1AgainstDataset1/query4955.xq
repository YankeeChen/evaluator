...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Length_0) and fn:exists($node_Var0) and $node_Length_0/RFDevice:hasLength/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Length_0 in $doc_ObjectDescription
    let $value_Length_0 := fn:data($node_Length_0/@rdf:about)
    for $node_Var1 in $node_Length_0/Nuvio:hasValue
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_Length_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
        <xqllib:var name="Length_0">{$value_Length_0}</xqllib:var>
        <xqllib:var name="Var1">{$value_Var1}</xqllib:var>
      </xqllib:result>
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("Length_0"))
