...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_Weight_0/RFDevice:hasCapacity
    let $value_USRPN200_0 := fn:data($node_USRPN200_0/@rdf:resource)
    where fn:exists($value_USRPN200_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPN200_0">{$value_USRPN200_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Kilogram"]/Nuvio:hasUnitOfMeasure
