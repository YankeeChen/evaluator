...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xxmini_0 in $node_DirectConversionReceiver_0/Nuvio:aggregateOf
    let $value_USRPB2xxmini_0 := fn:data($node_USRPB2xxmini_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_USRPB2xxmini_0)
    return
      <xqllib:result>
