...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_Weight_0/RFDevice:hasNoiseFigure
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:resource)
    where fn:exists($value_USRPE3xx_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPE3xx_0">{$value_USRPE3xx_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#WeightUnit_instance4"]/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Weight_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Weight_0"))
