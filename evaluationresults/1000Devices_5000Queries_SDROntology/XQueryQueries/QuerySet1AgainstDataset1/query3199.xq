...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_RadarSystem_0/Nuvio:hasParticipant
  ...
  let $GRAPH_4 :=  
    ...
    for $node_RadarSystem_0 in $node_DirectConversionReceiver_0/Nuvio:compositeOf
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_RadarSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_DirectConversionReceiver_0 in $node_Quantity_0/RFDevice:hasNoiseFigure
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($value_DirectConversionReceiver_0) and fn:exists($node_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="DirectConversionReceiver_0">{$value_DirectConversionReceiver_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("DirectConversionReceiver_0"))
  let $GRAPH_11 := xqllib:optional($GRAPH_3,$GRAPH_10,("RadarSystem_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_2,$GRAPH_11,("RadarSystem_0"))
