...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarWarningReceiver_0 in $node_Jammer_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Jammer_0 in $node_Jamming_0/RFDevice:hasCapability
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($value_Jammer_0) and fn:exists($node_Jamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Jamming_0 in $node_Power_0/RFDevice:hasMaxOutputPower
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_4,$GRAPH_5,("Power_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Power_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Jamming_0"))
