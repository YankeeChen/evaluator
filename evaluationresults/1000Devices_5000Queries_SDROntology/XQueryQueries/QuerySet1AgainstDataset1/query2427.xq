...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN300_0 in $node_XilinxZynq7000Family_0/Nuvio:partOf
    let $value_USRPN300_0 := fn:data($node_USRPN300_0/@rdf:resource)
    where fn:exists($value_USRPN300_0) and fn:exists($node_XilinxZynq7000Family_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPN300_0">{$value_USRPN300_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_XilinxZynq7000Family_0 in $node_Object_0/RFDevice:isProducerOf
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("XilinxZynq7000Family_0"))
