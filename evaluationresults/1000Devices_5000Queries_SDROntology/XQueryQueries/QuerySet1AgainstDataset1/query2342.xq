...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) <= -1678747511)]
  ...
  let $GRAPH_9 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Quantity_0) and $node_Quantity_0/RFDevice:hasMaxSensedPower/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
