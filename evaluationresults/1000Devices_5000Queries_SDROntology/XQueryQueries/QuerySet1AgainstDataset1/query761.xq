...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#BistaticRadar"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#FrequencyModulation"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := $GRAPH_6[((data(xqllib:var[@name="DataValue0"]) >= xs:decimal("-180")) and (data(xqllib:var[@name="DataValue0"]) <= xs:decimal("180")))]
  ...
  let $GRAPH_9 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_StandoffJamming_0) and fn:exists($node_Var0) and $node_StandoffJamming_0/Nuvio:expresses/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_9,$GRAPH_10,("StandoffJamming_0"))
  ...
  let $GRAPH_16 := xqllib:and($GRAPH_0,$GRAPH_15,("USRPB200mini_i_0"))
  return
    $GRAPH_16
return
