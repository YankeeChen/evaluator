...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarSystem_0 in $node_SuperheterodyneReceiver_0/Nuvio:compositeOf
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:resource)
    where fn:exists($value_RadarSystem_0) and fn:exists($node_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="RadarSystem_0">{$value_RadarSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_SuperheterodyneReceiver_0 in $node_Entity_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#GeodesicString"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
