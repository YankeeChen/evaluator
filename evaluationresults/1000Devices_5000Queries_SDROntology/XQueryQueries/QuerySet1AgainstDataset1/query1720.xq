...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_Var0/RFDevice:supportsTuning
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Moving_0 in $node_Var0/Nuvio:subprocessOf
    let $value_Moving_0 := fn:data($node_Moving_0/@rdf:resource)
    where fn:exists($value_Moving_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
