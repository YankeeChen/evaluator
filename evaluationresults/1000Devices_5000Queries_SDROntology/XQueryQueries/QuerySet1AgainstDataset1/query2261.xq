...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_Object_0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_ElectronicProtection_0) and fn:exists($node_Object_0) and $node_ElectronicProtection_0/RFDevice:supportsSpectrumSensing/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_0,$GRAPH_5,("USRPB210_0"))
  return
    $GRAPH_6
return
