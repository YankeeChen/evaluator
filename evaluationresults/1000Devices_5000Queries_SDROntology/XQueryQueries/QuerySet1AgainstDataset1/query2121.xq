...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_MonostaticRadar_0 in $node_Var0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[RFDevice:hasBandwidthRange/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#RadarFrequencyBand_instance6"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
