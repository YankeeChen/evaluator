...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_USRPB2xx_0 in $doc_ObjectDescription[Nuvio:aggregateOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Spartan6FPGA_instance4"]
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:about)
    where fn:exists($node_USRPB2xx_0)
    return
      <xqllib:result>
