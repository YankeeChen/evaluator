...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX310_0 in $node_Jammer_0/Nuvio:aggregateOf
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_USRPX310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Jammer_0 in $node_Filter_0/Nuvio:aggregateOf
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($node_Filter_0) and fn:exists($value_Jammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("Filter_0"))
