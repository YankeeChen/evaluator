...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_Entity_0/RFDevice:hasCapability
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_Half_DuplexCommunicationsSystem_0) and fn:exists($node_Process_0) and $node_Half_DuplexCommunicationsSystem_0/Nuvio:participatesIn/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("Process_0"))
