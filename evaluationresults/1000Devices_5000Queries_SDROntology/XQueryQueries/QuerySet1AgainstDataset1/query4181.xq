...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    for $node_RadioFrequencyBand_0 in $node_Entity_0/RFDevice:hasMax
  ...
  let $GRAPH_7 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_LineString_0) and $node_LineString_0/Nuvio:isExpressedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("RadioFrequencyBand_0"))
