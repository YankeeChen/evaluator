...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RFSystem_0 := fn:data($node_RFSystem_0/@rdf:about)
    for $node_Entity_0 in $node_RFSystem_0/Nuvio:partOf
  ...
  let $GRAPH_8 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Process" and Nuvio:hasProcessQuality/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#AntennaPolarization_instance2"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
