...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_GNURadio_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:resource)
    where fn:exists($value_GNURadio_0) and fn:exists($node_MobilePlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="GNURadio_0">{$value_GNURadio_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_MobilePlatform_0 in $node_Process_0/RFDevice:supportsTuning
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Process_0 in $node_MultiPoint_0/Nuvio:participatesIn
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_MultiPoint_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
