...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_210_0 in $node_XC6SLX150FPGA_0/Nuvio:compositeOf
    let $value_USRPB200_210_0 := fn:data($node_USRPB200_210_0/@rdf:resource)
    where fn:exists($value_USRPB200_210_0) and fn:exists($node_XC6SLX150FPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB200_210_0">{$value_USRPB200_210_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_XC6SLX150FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Xilinx"]/RFDevice:hasProducer
    let $value_XC6SLX150FPGA_0 := fn:data($node_XC6SLX150FPGA_0/@rdf:resource)
    where fn:exists($value_XC6SLX150FPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="XC6SLX150FPGA_0">{$value_XC6SLX150FPGA_0}</xqllib:var>
      </xqllib:result>
