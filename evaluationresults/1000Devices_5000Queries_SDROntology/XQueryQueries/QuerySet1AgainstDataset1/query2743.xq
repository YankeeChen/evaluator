...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_Object_0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_Weight_0) and $node_Weight_0/RFDevice:hasLevel/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("Weight_0"))
