...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB205mini_i_0 in $node_FPGA_0/Nuvio:compositeOf
    let $value_USRPB205mini_i_0 := fn:data($node_USRPB205mini_i_0/@rdf:resource)
    where fn:exists($node_FPGA_0) and fn:exists($value_USRPB205mini_i_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Filter_instance3"]/Nuvio:partOf
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("FPGA_0"))
