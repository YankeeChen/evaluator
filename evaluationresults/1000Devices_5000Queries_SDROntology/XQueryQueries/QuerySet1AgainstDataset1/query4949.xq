...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:about)
    for $node_Entity_0 in $node_BistaticRadar_0/Nuvio:compositeOf
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Object_0 in $node_Entity_0/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_6 :=  
    ...
    for $node_QuadratureAmplitudeModulation_0 in $node_Object_0/RFDevice:supportsTransmitting
    let $value_QuadratureAmplitudeModulation_0 := fn:data($node_QuadratureAmplitudeModulation_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_QuadratureAmplitudeModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_9 := xqllib:optional($GRAPH_5,$GRAPH_8,("Object_0"))
