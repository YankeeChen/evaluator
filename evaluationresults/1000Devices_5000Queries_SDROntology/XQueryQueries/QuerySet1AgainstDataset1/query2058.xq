...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_Satellite_0/RFDevice:isDeployedOn
    let $value_USRPB210_0 := fn:data($node_USRPB210_0/@rdf:resource)
    where fn:exists($node_Satellite_0) and fn:exists($value_USRPB210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Satellite_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance24"]/RFDevice:supportsTuning
