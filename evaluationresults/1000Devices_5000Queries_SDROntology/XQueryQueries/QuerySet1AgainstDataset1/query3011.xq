...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:aggregateOf
  ...
  let $GRAPH_4 :=  
    ...
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_Component_0/Nuvio:compositeOf
    ...
    where fn:exists($node_Component_0) and fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("USRPN2xx_0"))
  return
    $GRAPH_7
return
