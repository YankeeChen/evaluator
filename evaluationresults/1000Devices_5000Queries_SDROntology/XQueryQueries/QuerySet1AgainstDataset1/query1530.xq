...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_Transmitting_0 in $node_Half_DuplexCommunicationsSystem_0/RFDevice:supportsTuning
    let $value_Transmitting_0 := fn:data($node_Transmitting_0/@rdf:resource)
    where fn:exists($node_Half_DuplexCommunicationsSystem_0) and fn:exists($value_Transmitting_0)
    return
      <xqllib:result>
        <xqllib:var name="Half_DuplexCommunicationsSystem_0">{$value_Half_DuplexCommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    for $node_Transmitting_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Power_instance15"]/RFDevice:hasTransmitPowerRange
    let $value_Transmitting_0 := fn:data($node_Transmitting_0/@rdf:resource)
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("Transmitting_0"))
