...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_BarrageJamming_0/RFDevice:hasCapability
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($node_BarrageJamming_0) and fn:exists($value_ElectronicAttackSystem_0)
    return
      <xqllib:result>
