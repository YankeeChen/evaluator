...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_GNURadio_0 in $node_RadarJammer_0/Nuvio:aggregateOf
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:resource)
    where fn:exists($value_GNURadio_0) and fn:exists($node_RadarJammer_0)
    return
      <xqllib:result>
        <xqllib:var name="GNURadio_0">{$value_GNURadio_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_RadarJammer_0 in $node_Object_0/RFDevice:hasFrequencyAccuracy
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Object_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Instance1487"]/RFDevice:hasWeight
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_0,$GRAPH_5,("GNURadio_0"))
  return
    $GRAPH_6
return
