...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_JammingSystem_0 in $doc_ObjectDescription[RFDevice:hasCapability/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#FormationJamming_instance0"]
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:about)
    where fn:exists($node_JammingSystem_0)
    return
      <xqllib:result>
