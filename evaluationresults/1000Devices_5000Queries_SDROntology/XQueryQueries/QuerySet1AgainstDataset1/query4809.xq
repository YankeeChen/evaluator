...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN2xx_0 := fn:data($node_USRPN2xx_0/@rdf:about)
    for $node_Var0 in $node_USRPN2xx_0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Attribute_0 in $node_Var0/RFDevice:hasEffectiveRadiatedPower
  ...
  let $GRAPH_10 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_SpatialUnit_0 in $node_Var1/Nuvio:hasUnitOfMeasure
    let $value_SpatialUnit_0 := fn:data($node_SpatialUnit_0/@rdf:resource)
    where fn:exists($value_SpatialUnit_0) and fn:exists($node_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_15 := xqllib:optional($GRAPH_0,$GRAPH_14,("USRPN2xx_0"))
  return
    $GRAPH_15
return
