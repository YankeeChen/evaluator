...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:about)
    for $node_Var0 in $node_USRPB200mini_0/Nuvio:compositeOf
  ...
  let $GRAPH_4 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_NoiseJamming_0 in $node_Object_0/Nuvio:hasParticipant
    let $value_NoiseJamming_0 := fn:data($node_NoiseJamming_0/@rdf:resource)
    where fn:exists($value_NoiseJamming_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("USRPB200mini_0"))
  return
    $GRAPH_8
return
