...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $node_Entity_0/skos:memberList/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_4 :=  
    ...
    for $node_Process_0 in $node_Entity_1/RFDevice:hasMaxOutputPower
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Entity_0","Process_0"))
