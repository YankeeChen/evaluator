...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN3xx_0 := fn:data($node_USRPN3xx_0/@rdf:about)
    for $node_Entity_0 in $node_USRPN3xx_0/RFDevice:isProducerOf
  ...
  let $GRAPH_7 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_InformationEntity_0) and $node_InformationEntity_0/Nuvio:representedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_9 := xqllib:optional($GRAPH_7,$GRAPH_8,("InformationEntity_0"))
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_0,$GRAPH_16,("USRPN3xx_0"))
  return
    $GRAPH_17
return
