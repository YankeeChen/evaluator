...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE310_0 in $node_XilinxZynq7020SoC_0/Nuvio:partOf
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:resource)
    where fn:exists($value_USRPE310_0) and fn:exists($node_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPE310_0">{$value_USRPE310_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_XilinxZynq7020SoC_0 in $node_CortexA9_0/Nuvio:partOf
    let $value_XilinxZynq7020SoC_0 := fn:data($node_XilinxZynq7020SoC_0/@rdf:resource)
    where fn:exists($node_CortexA9_0) and fn:exists($value_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
