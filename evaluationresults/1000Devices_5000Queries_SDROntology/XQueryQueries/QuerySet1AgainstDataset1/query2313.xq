...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_Ship_0/RFDevice:isDeployedOn
    let $value_PulseRadar_0 := fn:data($node_PulseRadar_0/@rdf:resource)
    where fn:exists($value_PulseRadar_0) and fn:exists($node_Ship_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseRadar_0">{$value_PulseRadar_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_Ship_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance18"]/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("Ship_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("PulseRadar_0"))
  return
    $GRAPH_7
return
