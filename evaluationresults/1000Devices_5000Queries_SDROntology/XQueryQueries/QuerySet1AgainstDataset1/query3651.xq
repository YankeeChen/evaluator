...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_USRPN2xx_0 in $doc_ObjectDescription[RFDevice:isProducerOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Producer_instance208"]
    let $value_USRPN2xx_0 := fn:data($node_USRPN2xx_0/@rdf:about)
    where fn:exists($node_USRPN2xx_0)
    return
      <xqllib:result>
