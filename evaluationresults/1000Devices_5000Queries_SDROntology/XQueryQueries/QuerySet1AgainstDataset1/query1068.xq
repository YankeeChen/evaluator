...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_MonostaticRadar_0 in $node_PulseRadarModulation_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_4 :=  
    ...
    for $node_PulseRadarModulation_0 in $node_Entity_0/RFDevice:hasEffectiveRadiatedPower
  ...
  let $GRAPH_6 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Attribute_0 in $node_Object_0/RFDevice:hasPolarization
