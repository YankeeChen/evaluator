...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:about)
    for $node_PulseRadarModulation_0 in $node_RadarSystem_0/RFDevice:supportsTransmitting
    let $value_PulseRadarModulation_0 := fn:data($node_PulseRadarModulation_0/@rdf:resource)
    where fn:exists($value_PulseRadarModulation_0) and fn:exists($node_RadarSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_PulseRadarModulation_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Frequency_instance42"]/RFDevice:hasPulseWidth
    let $value_PulseRadarModulation_0 := fn:data($node_PulseRadarModulation_0/@rdf:resource)
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("PulseRadarModulation_0"))
