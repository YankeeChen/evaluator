...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE313_0 := fn:data($node_USRPE313_0/@rdf:about)
    for $node_Var0 in $node_USRPE313_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Producer_instance88"]/RFDevice:hasProducer
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Var1 in $node_Var0/Nuvio:hasObjectQuantity
