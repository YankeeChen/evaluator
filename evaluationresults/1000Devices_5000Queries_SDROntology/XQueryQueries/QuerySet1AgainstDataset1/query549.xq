...
let $result :=  
  ...
  let $GRAPH_8 :=  
    ...
    for $node_Var0 in $node_InformationContent_0/Nuvio:represents
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_9 :=  
    ...
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_InformationContent_0) and fn:exists($node_Var0) and $node_Var0/Nuvio:representedBy/@rdf:resource = $value_InformationContent_0
    return
      <xqllib:result>
        <xqllib:var name="InformationContent_0">{$value_InformationContent_0}</xqllib:var>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_10 := xqllib:and($GRAPH_8,$GRAPH_9,("InformationContent_0","Var0"))
