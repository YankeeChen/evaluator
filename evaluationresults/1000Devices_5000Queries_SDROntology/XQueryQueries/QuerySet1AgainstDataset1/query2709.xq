...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:about)
    for $node_Entity_0 in $node_RadarSystem_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Entity_1) and $node_Entity_1/skos:hasTopConcept/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_Entity_2/RFDevice:supportsJamming
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
