...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_RadarSystem_0 in $doc_ObjectDescription[RFDevice:isDeployedOn/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#GroundPlatform_instance2"]
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:about)
    where fn:exists($node_RadarSystem_0)
    return
      <xqllib:result>
