...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SDR_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:partOf
  ...
  let $GRAPH_4 :=  
    ...
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_Producer_0/RFDevice:isProducerOf
    ...
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_Producer_0)
    return
      <xqllib:result>
        <xqllib:var name="InstantaneousFrequencyMeasurementReceiver_0">{$value_InstantaneousFrequencyMeasurementReceiver_0}</xqllib:var>
