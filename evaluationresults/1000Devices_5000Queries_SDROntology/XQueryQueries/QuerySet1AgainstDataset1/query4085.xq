...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CommunicationsSystem_0 in $node_Process_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_Frequency_0) and fn:exists($node_Process_0) and $node_Frequency_0/RFDevice:hasMaxSensedPower/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
