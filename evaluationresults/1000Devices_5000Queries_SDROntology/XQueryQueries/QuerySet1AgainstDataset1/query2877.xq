...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_i_0 in $node_RadioComponent_0/Nuvio:compositeOf
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_USRPB200mini_i_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_RadioComponent_0 in $node_Jammer_0/Nuvio:partOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Jammer_0 in $node_Object_0/RFDevice:hasFrequencyAccuracy
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Jammer_0"))
