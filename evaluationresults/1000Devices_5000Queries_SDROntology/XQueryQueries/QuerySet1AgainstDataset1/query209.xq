...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:about)
    for $node_SweptSpotJamming_0 in $node_JammingSystem_0/RFDevice:hasCapability
    let $value_SweptSpotJamming_0 := fn:data($node_SweptSpotJamming_0/@rdf:resource)
    where fn:exists($node_JammingSystem_0) and fn:exists($value_SweptSpotJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="JammingSystem_0">{$value_JammingSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_SweptSpotJamming_0 in $node_Power_0/RFDevice:hasPulseRepetitionFrequency
    let $value_SweptSpotJamming_0 := fn:data($node_SweptSpotJamming_0/@rdf:resource)
    where fn:exists($node_Power_0) and fn:exists($value_SweptSpotJamming_0)
    return
      <xqllib:result>
