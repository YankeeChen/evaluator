...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:about)
    for $node_Aircraft_0 in $node_USRPX310_0/RFDevice:isDeployedOn
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:resource)
    where fn:exists($value_Aircraft_0) and fn:exists($node_USRPX310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Aircraft_0 in $doc_ObjectDescription[RFDevice:hasCapability/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance24"]
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:about)
    where fn:exists($node_Aircraft_0)
    return
      <xqllib:result>
