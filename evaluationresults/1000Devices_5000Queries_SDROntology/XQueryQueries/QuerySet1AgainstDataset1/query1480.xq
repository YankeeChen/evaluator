...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_Jamming_0/RFDevice:supportsJamming
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($value_ElectronicAttackSystem_0) and fn:exists($node_Jamming_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicAttackSystem_0">{$value_ElectronicAttackSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Jamming_0 in $node_Quantity_0/RFDevice:hasMaxOutputPower
    let $value_Jamming_0 := fn:data($node_Jamming_0/@rdf:resource)
    where fn:exists($value_Jamming_0) and fn:exists($node_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="Jamming_0">{$value_Jamming_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Jamming_0"))
