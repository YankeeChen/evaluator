...
let $result :=  
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) >= 49348583)]
  ...
  let $GRAPH_13 :=  
    ...
    where fn:exists($node_MonostaticRadar_0) and fn:exists($node_Situation_0) and $node_MonostaticRadar_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
  ...
  let $GRAPH_14 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_Antenna_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_Antenna_0 := fn:data($node_Antenna_0/@rdf:resource)
    where fn:exists($value_Antenna_0) and fn:exists($node_Situation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_13,$GRAPH_16,("MonostaticRadar_0","Situation_0"))
