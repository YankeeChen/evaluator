...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX3xx_0 in $node_Entity_0/RFDevice:hasWidth
  ...
  let $GRAPH_4 := $GRAPH_3[(data(xqllib:var[@name="DataValue0"]) != -63.88)]
  let $GRAPH_6 :=  
    for $node_SplineCurve_0 in $doc_ObjectDescription
    let $value_SplineCurve_0 := fn:data($node_SplineCurve_0/@rdf:about)
    for $node_Entity_0 in $node_SplineCurve_0/RFDevice:hasTarget
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_SplineCurve_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
        <xqllib:var name="SplineCurve_0">{$value_SplineCurve_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 :=  
    for $node_SplineCurve_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#SplineCurve"]
    let $value_SplineCurve_0 := fn:data($node_SplineCurve_0/@rdf:about)
    where fn:exists($node_SplineCurve_0)
    return
      <xqllib:result>
        <xqllib:var name="SplineCurve_0">{$value_SplineCurve_0}</xqllib:var>
      </xqllib:result>
