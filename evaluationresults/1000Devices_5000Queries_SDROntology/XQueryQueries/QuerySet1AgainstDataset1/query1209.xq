...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    for $node_JammingSystem_0 in $node_RangeGatePullOffJamming_0/RFDevice:supportsTuning
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:resource)
    where fn:exists($value_JammingSystem_0) and fn:exists($node_RangeGatePullOffJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="JammingSystem_0">{$value_JammingSystem_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("JammingSystem_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_3,$GRAPH_8,("JammingSystem_0"))
  let $GRAPH_10 :=  
  ...
  let $GRAPH_12 :=  
    ...
    for $node_Process_0 in $node_RFFrontEnd_0/Nuvio:participatesIn
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($value_Process_0) and fn:exists($node_RFFrontEnd_0)
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
  ...
  let $GRAPH_14 := xqllib:and($GRAPH_2,$GRAPH_13,("Entity_0","Process_0"))
