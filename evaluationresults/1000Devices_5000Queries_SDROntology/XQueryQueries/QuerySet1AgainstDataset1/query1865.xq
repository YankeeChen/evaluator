...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_USRPE3xx_0 in $doc_ObjectDescription[RFDevice:supportsTransmitting/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#USRP_SpectrumSensing_Instance0"]
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:about)
    where fn:exists($node_USRPE3xx_0)
    return
      <xqllib:result>
