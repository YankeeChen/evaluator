...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:about)
    for $node_Var0 in $node_GNURadio_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_AnalogComponent_0) and fn:exists($node_Var0) and $node_AnalogComponent_0/RFDevice:hasTarget/@rdf:resource = $value_Var0
    return
      <xqllib:result>
