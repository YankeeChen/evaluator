...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CR_0 in $node_Var0/RFDevice:supportsTuning
  ...
  let $GRAPH_2 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#CortexA9"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_SpatialUnit_0) and fn:exists($node_Var0) and $node_SpatialUnit_0/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Var0
    return
      <xqllib:result>
