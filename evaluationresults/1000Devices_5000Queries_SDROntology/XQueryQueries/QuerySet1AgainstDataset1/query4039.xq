...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Battery_0) and fn:exists($node_Entity_0) and $node_Battery_0/Nuvio:isExpressedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    for $node_Battery_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#Battery"]
    let $value_Battery_0 := fn:data($node_Battery_0/@rdf:about)
    where fn:exists($node_Battery_0)
    return
      <xqllib:result>
        <xqllib:var name="Battery_0">{$value_Battery_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Battery_0 in $node_Entity_0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("Battery_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("Battery_0","Entity_0"))
