...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($value_ElectronicAttackSystem_0) and fn:exists($node_IEDTrigger_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicAttackSystem_0">{$value_ElectronicAttackSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_IEDTrigger_0 in $doc_ObjectDescription
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:about)
    for $node_DipoleAntenna_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_DipoleAntenna_0 := fn:data($node_DipoleAntenna_0/@rdf:resource)
    where fn:exists($value_DipoleAntenna_0) and fn:exists($node_IEDTrigger_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_XC3SD3400AFPGA_0 in $node_DipoleAntenna_0/Nuvio:partOf
    let $value_XC3SD3400AFPGA_0 := fn:data($node_XC3SD3400AFPGA_0/@rdf:resource)
    where fn:exists($node_DipoleAntenna_0) and fn:exists($value_XC3SD3400AFPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="DipoleAntenna_0">{$value_DipoleAntenna_0}</xqllib:var>
        <xqllib:var name="XC3SD3400AFPGA_0">{$value_XC3SD3400AFPGA_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("DipoleAntenna_0"))
