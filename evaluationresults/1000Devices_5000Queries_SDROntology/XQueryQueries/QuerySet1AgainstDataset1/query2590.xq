...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:about)
    for $node_Entity_0 in $node_USRPB200_0/RFDevice:hasProducer
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) >= -248198680)]
  let $GRAPH_8 := ($GRAPH_5,$GRAPH_7)
  let $GRAPH_9 :=  
    for $node_Arc_0 in $doc_ObjectDescription
    let $value_Arc_0 := fn:data($node_Arc_0/@rdf:about)
    for $node_Entity_0 in $node_Arc_0/Nuvio:isExpressedBy
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($node_Arc_0) and fn:exists($value_Entity_0)
    return
      <xqllib:result>
