...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX300_0 in $node_IEDTrigger_0/Nuvio:partOf
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_USRPX300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_IEDTrigger_0 in $node_Entity_0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) >= 405203310)]
  let $GRAPH_6 := xqllib:and($GRAPH_3,$GRAPH_5,("Entity_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_2,$GRAPH_6,("IEDTrigger_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_1,$GRAPH_7,("IEDTrigger_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_0,$GRAPH_8,("USRPX300_0"))
  return
    $GRAPH_9
return
