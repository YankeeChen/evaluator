...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN310_0 in $node_Entity_0/RFDevice:hasWeight
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $doc_ObjectDescription[@rdf:ID = "Producer_instance170"]/Nuvio:expresses/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_6 :=  
    for $node_Situation_0 in $doc_ObjectDescription
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_USRPE310_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:resource)
    where fn:exists($node_Situation_0) and fn:exists($value_USRPE310_0)
    return
      <xqllib:result>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
  ...
  let $GRAPH_7 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_USRPE310_0 in $node_Entity_0/RFDevice:hasVoltage
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:resource)
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("USRPE310_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("Situation_0"))
