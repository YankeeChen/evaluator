...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RFSystem_0 in $node_Transmitter_0/Nuvio:compositeOf
    let $value_RFSystem_0 := fn:data($node_RFSystem_0/@rdf:resource)
    where fn:exists($value_RFSystem_0) and fn:exists($node_Transmitter_0)
    return
      <xqllib:result>
        <xqllib:var name="RFSystem_0">{$value_RFSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    for $node_Transmitter_0 in $doc_ObjectDescription
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:about)
    for $node_FPGA_0 in $node_Transmitter_0/Nuvio:partOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0) and fn:exists($node_Transmitter_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Producer_0 in $node_FPGA_0/RFDevice:hasProducer
    let $value_Producer_0 := fn:data($node_Producer_0/@rdf:resource)
    where fn:exists($node_FPGA_0) and fn:exists($value_Producer_0)
    return
      <xqllib:result>
        <xqllib:var name="FPGA_0">{$value_FPGA_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("FPGA_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Transmitter_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_0,$GRAPH_7,("RFSystem_0"))
  return
    $GRAPH_8
return
