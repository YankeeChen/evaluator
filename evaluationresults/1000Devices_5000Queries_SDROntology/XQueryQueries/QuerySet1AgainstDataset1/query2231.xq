...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:about)
    for $node_Object_0 in $node_USRPB200_0/Nuvio:partOf
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($value_Object_0) and fn:exists($node_USRPB200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Object_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Instance162"]/Nuvio:participatesIn
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
