...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_Var0/Nuvio:aggregateOf
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_Object_0) and $doc_ObjectDescription[@rdf:ID = "Weight_instance82"]/RFDevice:hasSamplingRate/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_7 :=  
    ...
    for $node_Var0 in $node_Object_1/RFDevice:isProducerOf
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
