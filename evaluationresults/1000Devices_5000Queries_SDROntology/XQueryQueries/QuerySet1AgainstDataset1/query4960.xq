...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:about)
    for $node_Entity_0 in $node_USRPX300_0/RFDevice:hasGain
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_MonostaticRadar_0) and fn:exists($node_Var0) and $node_MonostaticRadar_0/Nuvio:relevantIndividual/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("MonostaticRadar_0"))
