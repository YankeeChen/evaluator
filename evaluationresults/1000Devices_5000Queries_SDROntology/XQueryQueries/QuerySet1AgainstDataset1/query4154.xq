...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RFSystem_0 in $node_RadarJammer_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    for $node_RadarJammer_0 in $doc_ObjectDescription
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:about)
    for $node_Object_0 in $node_RadarJammer_0/RFDevice:hasFrequencyRange
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Object_0 in $node_Quality_0/RFDevice:hasPolarization
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
