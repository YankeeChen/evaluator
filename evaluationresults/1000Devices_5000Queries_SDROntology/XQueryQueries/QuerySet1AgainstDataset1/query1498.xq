...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:about)
    for $node_UnmannedPlatform_0 in $node_MonostaticRadar_0/RFDevice:isDeployedOn
    let $value_UnmannedPlatform_0 := fn:data($node_UnmannedPlatform_0/@rdf:resource)
    where fn:exists($node_MonostaticRadar_0) and fn:exists($value_UnmannedPlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="MonostaticRadar_0">{$value_MonostaticRadar_0}</xqllib:var>
