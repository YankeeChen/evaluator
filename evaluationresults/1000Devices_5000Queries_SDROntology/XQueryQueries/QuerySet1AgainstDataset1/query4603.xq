...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Power_45W"]/RFDevice:hasMaxOutputPower
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_NoiseFigure_0) and fn:exists($node_Var0) and $node_NoiseFigure_0/RFDevice:hasEffectiveRadiatedPower/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue0"]) < -6.55)]
  let $GRAPH_10 := xqllib:and($GRAPH_7,$GRAPH_9,("Var1"))
  let $GRAPH_11 := xqllib:and($GRAPH_6,$GRAPH_10,("NoiseFigure_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_5,$GRAPH_11,("NoiseFigure_0"))
