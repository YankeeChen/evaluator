...
let $result :=  
  ...
  let $GRAPH_4 := $GRAPH_3[(data(xqllib:var[@name="DataValue0"]) < xs:decimal("8"))]
  let $GRAPH_5 := xqllib:and($GRAPH_2,$GRAPH_4,("Var1"))
  let $GRAPH_6 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Var0 in $node_Entity_0/Nuvio:expresses
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_7 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Weight_instance7"]/RFDevice:hasWeight
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
