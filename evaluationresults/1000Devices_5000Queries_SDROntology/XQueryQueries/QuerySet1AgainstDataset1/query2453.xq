...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:about)
    for $node_Var0 in $node_ElectronicAttackSystem_0/RFDevice:hasCapability
  ...
  let $GRAPH_9 :=  
    ...
    for $node_Var0 in $node_BarrageJamming_0/Nuvio:relevantIndividual
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_BarrageJamming_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_13 := $GRAPH_12[(data(xqllib:var[@name="DataValue0"]) > 299011802)]
  let $GRAPH_14 := xqllib:and($GRAPH_11,$GRAPH_13,("Value_0"))
  let $GRAPH_15 := xqllib:and($GRAPH_10,$GRAPH_14,())
  let $GRAPH_16 := xqllib:and($GRAPH_3,$GRAPH_15,("Value_0","Var0"))
  let $GRAPH_17 :=  
  ...
  let $GRAPH_22 := xqllib:optional($GRAPH_0,$GRAPH_21,("ElectronicAttackSystem_0"))
  return
    $GRAPH_22
return
