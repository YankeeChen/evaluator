...
let $result :=  
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue1"]) > xs:double("-96.261444"))]
  ...
  let $GRAPH_16 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_DataValue2 in $node_Entity_0/Nuvio:hasPrecision
    let $value_DataValue2 := fn:data($node_DataValue2)
    where fn:exists($value_DataValue2) and fn:exists($node_Entity_0)
    return
      <xqllib:result>
        <xqllib:var name="DataValue2">{$value_DataValue2}</xqllib:var>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_17 := $GRAPH_16[(data(xqllib:var[@name="DataValue2"]) < 1457862353)]
  let $GRAPH_18 :=  
  ...
  let $GRAPH_20 := xqllib:and($GRAPH_15,$GRAPH_19,("Entity_0","VoltageUnit_0"))
