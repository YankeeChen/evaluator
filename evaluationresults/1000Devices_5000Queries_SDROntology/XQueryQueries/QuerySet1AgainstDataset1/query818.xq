...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_TerrainBounceJamming_0) and $node_TerrainBounceJamming_0/RFDevice:supportsJamming/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("USRPX310_0"))
  return
    $GRAPH_4
return
