...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Var0) and fn:exists($node_Weight_0) and $node_Weight_0/RFDevice:hasPulseWidth/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    for $node_Weight_0 in $doc_ObjectDescription
    let $value_Weight_0 := fn:data($node_Weight_0/@rdf:about)
    for $node_Var1 in $node_Weight_0/Nuvio:hasValue
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($value_Var1) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_0,$GRAPH_8,("ElectronicSupportSystem_0"))
  return
    $GRAPH_9
return
