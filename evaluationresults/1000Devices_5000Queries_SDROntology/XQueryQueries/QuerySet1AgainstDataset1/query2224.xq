...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX310_0 in $node_DirectConversionReceiver_0/Nuvio:partOf
    let $value_USRPX310_0 := fn:data($node_USRPX310_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_USRPX310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:about)
    for $node_HardwareComponent_0 in $node_DirectConversionReceiver_0/Nuvio:compositeOf
    let $value_HardwareComponent_0 := fn:data($node_HardwareComponent_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_HardwareComponent_0)
    return
      <xqllib:result>
        <xqllib:var name="DirectConversionReceiver_0">{$value_DirectConversionReceiver_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("HardwareComponent_0"))
