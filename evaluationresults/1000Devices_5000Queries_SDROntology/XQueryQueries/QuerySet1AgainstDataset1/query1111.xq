...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_BistaticRadar_0 in $node_Var0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Point_0) and fn:exists($node_Var0) and $node_Point_0/Nuvio:hasParticipant/@rdf:resource = $value_Var0
    return
      <xqllib:result>
