...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:about)
    for $node_Var0 in $node_ModelX_0/RFDevice:supportsTuning
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#AbstractGeometricPrimitive"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#PulseRadarModulation"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_0,$GRAPH_9,("ModelX_0"))
  return
    $GRAPH_10
return
