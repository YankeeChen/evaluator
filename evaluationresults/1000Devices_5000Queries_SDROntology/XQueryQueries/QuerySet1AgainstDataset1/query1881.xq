...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:about)
    for $node_PulseDopplerRadarModulation_0 in $node_MonostaticRadar_0/RFDevice:supportsJamming
    let $value_PulseDopplerRadarModulation_0 := fn:data($node_PulseDopplerRadarModulation_0/@rdf:resource)
    where fn:exists($node_MonostaticRadar_0) and fn:exists($value_PulseDopplerRadarModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="MonostaticRadar_0">{$value_MonostaticRadar_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_PulseDopplerRadarModulation_0 in $node_TimeInterval_0/RFDevice:hasEffectiveRadiatedPower
    let $value_PulseDopplerRadarModulation_0 := fn:data($node_PulseDopplerRadarModulation_0/@rdf:resource)
