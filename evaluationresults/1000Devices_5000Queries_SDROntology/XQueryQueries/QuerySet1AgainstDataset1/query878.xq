...
let $result :=  
  ...
  let $GRAPH_8 :=  
    for $node_RadarFrequencyBand_0 in $doc_ObjectDescription[RFDevice:hasMax/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Frequency_instance71"]
    let $value_RadarFrequencyBand_0 := fn:data($node_RadarFrequencyBand_0/@rdf:about)
    where fn:exists($node_RadarFrequencyBand_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:optional($GRAPH_7,$GRAPH_8,("RadarFrequencyBand_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("RadarFrequencyBand_0"))
