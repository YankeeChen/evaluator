...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarSystem_0 in $node_PulseDopplerRadarModulation_0/RFDevice:supportsSpectrumSensing
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:resource)
    where fn:exists($node_PulseDopplerRadarModulation_0) and fn:exists($value_RadarSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_PulseDopplerRadarModulation_0 in $node_TimeInterval_0/RFDevice:hasMaxSensedPower
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("TimeInterval_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("TimeInterval_0"))
