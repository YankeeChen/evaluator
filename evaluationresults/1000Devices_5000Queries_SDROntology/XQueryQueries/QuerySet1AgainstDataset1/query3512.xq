...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_RadarJammer_0 in $node_Half_DuplexCommunicationsSystem_0/Nuvio:partOf
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_Half_DuplexCommunicationsSystem_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
        <xqllib:var name="Half_DuplexCommunicationsSystem_0">{$value_Half_DuplexCommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadarJammer_0 in $node_SweepJamming_0/RFDevice:supportsTuning
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("SweepJamming_0"))
