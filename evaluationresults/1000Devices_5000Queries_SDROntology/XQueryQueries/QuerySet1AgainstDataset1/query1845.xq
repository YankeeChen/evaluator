...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:about)
    for $node_Object_0 in $node_USRPE312_0/RFDevice:hasProducer
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($value_Object_0) and fn:exists($node_USRPE312_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Object_0 in $node_Ratio_0/RFDevice:hasMaxPowerConsumption
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($value_Object_0) and fn:exists($node_Ratio_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("Ratio_0"))
