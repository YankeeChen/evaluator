...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_BistaticRadar_0 in $doc_ObjectDescription[RFDevice:hasVoltage/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Weight_instance82"]
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:about)
    where fn:exists($node_BistaticRadar_0)
    return
      <xqllib:result>
