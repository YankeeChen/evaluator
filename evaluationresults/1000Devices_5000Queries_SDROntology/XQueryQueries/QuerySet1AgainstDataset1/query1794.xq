...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:about)
    for $node_Var0 in $node_SDR_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_ElectronicSupport_0) and fn:exists($node_Var0) and $node_ElectronicSupport_0/Nuvio:subprocessOf/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("ElectronicSupport_0"))
