...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_Process_0) and $node_Process_0/Nuvio:participatesIn/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_Percentage_0) and fn:exists($node_Process_0) and $node_Percentage_0/RFDevice:hasPulseRepetitionInterval/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := $GRAPH_6[(not((data(xqllib:var[@name="DataValue0"]) >= xs:decimal("0"))) or (data(xqllib:var[@name="DataValue0"]) <= xs:decimal("100")))]
  let $GRAPH_8 :=  
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_4,$GRAPH_10,("Percentage_0"))
  let $GRAPH_12 := ($GRAPH_3,$GRAPH_11)
  let $GRAPH_13 := xqllib:and($GRAPH_1,$GRAPH_12,("Object_0"))
  let $GRAPH_14 := xqllib:optional($GRAPH_0,$GRAPH_13,("CommunicationsSystem_0"))
  return
    $GRAPH_14
return
