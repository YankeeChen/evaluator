...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Aircraft_0/RFDevice:isDeployedOn
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($node_Aircraft_0) and fn:exists($value_USRPB200mini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Aircraft_0 in $doc_ObjectDescription
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:about)
    for $node_Process_0 in $node_Aircraft_0/RFDevice:supportsTuning
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_Aircraft_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
        <xqllib:var name="Aircraft_0">{$value_Aircraft_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Process_0) and $doc_ObjectDescription[@rdf:ID = "Instance1328"]/Nuvio:hasSubprocess/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
      </xqllib:result>
