...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Var2 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#PositiveDecimalValue"]
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("Var2"))
  ...
  let $GRAPH_9 :=  
    ...
    for $node_Var0 in $node_AdjustableBandwidthSensing_0/Nuvio:relevantIndividual
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_AdjustableBandwidthSensing_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
