...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarSystem_0 in $node_Process_0/RFDevice:supportsJamming
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:resource)
    where fn:exists($node_Process_0) and fn:exists($value_RadarSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_EnergyDetection_0) and fn:exists($node_Process_0) and $node_EnergyDetection_0/Nuvio:hasSubprocess/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
