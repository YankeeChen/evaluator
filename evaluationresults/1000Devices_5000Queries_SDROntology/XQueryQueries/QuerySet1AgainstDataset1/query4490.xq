...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Distance_0 in $node_Object_0/RFDevice:hasChargingTime
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Distance_0 in $node_Var1/Nuvio:hasValue
    let $value_Distance_0 := fn:data($node_Distance_0/@rdf:resource)
    where fn:exists($value_Distance_0) and fn:exists($node_Var1)
    return
      <xqllib:result>
        <xqllib:var name="Distance_0">{$value_Distance_0}</xqllib:var>
  ...
  let $GRAPH_11 := xqllib:optional($GRAPH_2,$GRAPH_10,("Object_0","Var0"))
