...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Half_DuplexCommunicationsSystem_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:resource)
    where fn:exists($value_Half_DuplexCommunicationsSystem_0) and fn:exists($node_MobilePlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="Half_DuplexCommunicationsSystem_0">{$value_Half_DuplexCommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_MobilePlatform_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance7"]/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("Half_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_7
return
