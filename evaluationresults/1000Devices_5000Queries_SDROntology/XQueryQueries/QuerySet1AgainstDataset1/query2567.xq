...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ContinuousWaveRadar_0 in $node_Process_0/RFDevice:hasCapability
  ...
  let $GRAPH_2 :=  
    for $node_Process_0 in $doc_ObjectDescription
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_FeatureDetection_0 in $node_Process_0/Nuvio:subprocessOf
    let $value_FeatureDetection_0 := fn:data($node_FeatureDetection_0/@rdf:resource)
    where fn:exists($value_FeatureDetection_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("FeatureDetection_0"))
