...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_NoiseFigure_0) and fn:exists($node_Object_0) and $node_NoiseFigure_0/RFDevice:hasBeamwidth/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) >= -81.24)]
  let $GRAPH_6 := xqllib:and($GRAPH_3,$GRAPH_5,("Var0"))
  let $GRAPH_7 := xqllib:and($GRAPH_2,$GRAPH_6,("NoiseFigure_0"))
