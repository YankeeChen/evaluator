...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE312_0 in $node_Satellite_0/RFDevice:isDeployedOn
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:resource)
    where fn:exists($node_Satellite_0) and fn:exists($value_USRPE312_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Satellite_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance14"]/RFDevice:hasCapability
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("USRPE312_0"))
  return
    $GRAPH_4
return
