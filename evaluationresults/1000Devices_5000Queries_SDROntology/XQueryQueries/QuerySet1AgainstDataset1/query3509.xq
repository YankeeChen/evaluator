...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ModelX_0 in $node_Receiving_0/RFDevice:supportsJamming
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:resource)
    where fn:exists($value_ModelX_0) and fn:exists($node_Receiving_0)
    return
      <xqllib:result>
        <xqllib:var name="ModelX_0">{$value_ModelX_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Receiving_0 in $node_Object_0/RFDevice:hasFrequencyRange
    let $value_Receiving_0 := fn:data($node_Receiving_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_Receiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_AdjustableBandwidthSensing_0) and fn:exists($node_Object_0) and $node_AdjustableBandwidthSensing_0/Nuvio:hasParticipant/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("AdjustableBandwidthSensing_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Object_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Receiving_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_0,$GRAPH_7,("ModelX_0"))
  return
    $GRAPH_8
return
