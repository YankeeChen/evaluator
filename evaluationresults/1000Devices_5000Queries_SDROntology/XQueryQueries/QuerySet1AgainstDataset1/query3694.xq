...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    where fn:exists($node_Value_0) and $doc_ObjectDescription[@rdf:ID = "AngleUnit_Degree"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Value_0
    return
      <xqllib:result>
        <xqllib:var name="Value_0">{$value_Value_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) > 1571111606)]
  let $GRAPH_7 := xqllib:and($GRAPH_4,$GRAPH_6,("Value_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("Value_0"))
