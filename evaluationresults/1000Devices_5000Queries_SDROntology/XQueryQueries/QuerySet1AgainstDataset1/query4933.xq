...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX300_0 in $node_SuperheterodyneReceiver_0/Nuvio:compositeOf
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:resource)
    where fn:exists($node_SuperheterodyneReceiver_0) and fn:exists($value_USRPX300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_SuperheterodyneReceiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#RFFrontEnd_instance23"]/Nuvio:aggregateOf
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("SuperheterodyneReceiver_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("USRPX300_0"))
  return
    $GRAPH_7
return
