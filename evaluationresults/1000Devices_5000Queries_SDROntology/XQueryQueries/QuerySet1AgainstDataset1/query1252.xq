...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_Var0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_AdjustableBandwidthSensing_0) and fn:exists($node_Var0) and $node_AdjustableBandwidthSensing_0/Nuvio:hasSubprocess/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_AdjustableBandwidthSensing_0 in $doc_ObjectDescription[RFDevice:hasBandwidthRange/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#RadarFrequencyBand_instance0"]
    let $value_AdjustableBandwidthSensing_0 := fn:data($node_AdjustableBandwidthSensing_0/@rdf:about)
  ...
  let $GRAPH_5 := xqllib:optional($GRAPH_3,$GRAPH_4,("AdjustableBandwidthSensing_0"))
  let $GRAPH_6 :=  
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue0"]) <= 390289253)]
