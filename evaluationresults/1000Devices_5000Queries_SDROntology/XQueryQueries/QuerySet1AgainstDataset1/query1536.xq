...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN310_0 in $node_Submarine_0/RFDevice:isDeployedOn
    let $value_USRPN310_0 := fn:data($node_USRPN310_0/@rdf:resource)
    where fn:exists($node_Submarine_0) and fn:exists($value_USRPN310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Submarine_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance13"]/RFDevice:supportsTuning
