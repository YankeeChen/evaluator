...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN310_0 := fn:data($node_USRPN310_0/@rdf:about)
    for $node_Var0 in $node_USRPN310_0/RFDevice:supportsTuning
  ...
  let $GRAPH_4 :=  
    ...
    for $node_FreqencyRange_0 in $node_Angle_0/RFDevice:hasMin
    let $value_FreqencyRange_0 := fn:data($node_FreqencyRange_0/@rdf:resource)
    where fn:exists($node_Angle_0) and fn:exists($value_FreqencyRange_0)
    return
      <xqllib:result>
