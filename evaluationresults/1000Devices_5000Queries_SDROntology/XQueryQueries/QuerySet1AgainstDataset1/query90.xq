...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE310_0 in $node_Entity_0/RFDevice:hasMaxPowerConsumption
  ...
  let $GRAPH_4 := $GRAPH_3[not((data(xqllib:var[@name="DataValue0"]) <= xs:decimal("90")))]
  ...
  let $GRAPH_7 :=  
    for $node_Situation_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Producer_instance321"]/Nuvio:relevantIndividual
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:resource)
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("Situation_0"))
