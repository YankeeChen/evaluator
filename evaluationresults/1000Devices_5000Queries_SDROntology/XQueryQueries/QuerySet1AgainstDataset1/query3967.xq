...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_Transmitting_0 in $node_Half_DuplexCommunicationsSystem_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Transmitting_0 in $node_Entity_0/RFDevice:hasEffectiveRadiatedPower
    let $value_Transmitting_0 := fn:data($node_Transmitting_0/@rdf:resource)
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue0"]) != -131515003)]
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_1,$GRAPH_17,("Transmitting_0"))
