...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_Entity_0/RFDevice:supportsTuning
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Entity_0 in $node_ARMProcessor_0/Nuvio:aggregateOf
  ...
  let $GRAPH_6 :=  
    ...
    for $node_ARMProcessor_0 in $node_Oscillator_0/Nuvio:aggregateOf
    let $value_ARMProcessor_0 := fn:data($node_ARMProcessor_0/@rdf:resource)
    where fn:exists($value_ARMProcessor_0) and fn:exists($node_Oscillator_0)
    return
      <xqllib:result>
        <xqllib:var name="ARMProcessor_0">{$value_ARMProcessor_0}</xqllib:var>
        <xqllib:var name="Oscillator_0">{$value_Oscillator_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("ARMProcessor_0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_4,$GRAPH_7,("Entity_0"))
  let $GRAPH_9 :=  
    for $node_OffsetCurve_0 in $doc_ObjectDescription
    let $value_OffsetCurve_0 := fn:data($node_OffsetCurve_0/@rdf:about)
    for $node_Entity_0 in $node_OffsetCurve_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_OffsetCurve_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
        <xqllib:var name="OffsetCurve_0">{$value_OffsetCurve_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_10 :=  
    for $node_OffsetCurve_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#OffsetCurve"]
    let $value_OffsetCurve_0 := fn:data($node_OffsetCurve_0/@rdf:about)
    where fn:exists($node_OffsetCurve_0)
    return
      <xqllib:result>
        <xqllib:var name="OffsetCurve_0">{$value_OffsetCurve_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_11 := xqllib:and($GRAPH_9,$GRAPH_10,("OffsetCurve_0"))
  let $GRAPH_12 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_15 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Ratio_0 in $node_Entity_0/RFDevice:hasLength
    let $value_Ratio_0 := fn:data($node_Ratio_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_Ratio_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_19 := xqllib:and($GRAPH_0,$GRAPH_18,("SearchRadar_0"))
  return
    $GRAPH_19
return
