...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_LatitudeValueInRadian_0 in $node_Entity_0/Nuvio:relevantIndividual
    let $value_LatitudeValueInRadian_0 := fn:data($node_LatitudeValueInRadian_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_LatitudeValueInRadian_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("LatitudeValueInRadian_0"))
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_0,$GRAPH_11,("SurveillanceRadar_0"))
  return
    $GRAPH_12
return
