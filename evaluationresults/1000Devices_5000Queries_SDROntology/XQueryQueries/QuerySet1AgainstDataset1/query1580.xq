...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_IEDJammer_0 in $node_Half_DuplexCommunicationsSystem_0/Nuvio:compositeOf
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
    where fn:exists($node_Half_DuplexCommunicationsSystem_0) and fn:exists($value_IEDJammer_0)
    return
      <xqllib:result>
        <xqllib:var name="Half_DuplexCommunicationsSystem_0">{$value_Half_DuplexCommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_IEDJammer_0 in $node_Object_0/RFDevice:hasFrequencyAccuracy
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("IEDJammer_0"))
