...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Half_DuplexCommunicationsSystem_0 in $node_Transceiving_0/RFDevice:supportsTuning
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:about)
    where fn:exists($node_Attribute_0) and fn:exists($node_Transceiving_0) and $node_Attribute_0/Nuvio:hasProcessQuality/@rdf:resource = $value_Transceiving_0
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("Half_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_8
return
