...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN300_0 in $node_RadarJammer_0/Nuvio:partOf
    let $value_USRPN300_0 := fn:data($node_USRPN300_0/@rdf:resource)
    where fn:exists($node_RadarJammer_0) and fn:exists($value_USRPN300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadarJammer_0 in $node_Producer_0/RFDevice:hasProducer
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_Producer_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
