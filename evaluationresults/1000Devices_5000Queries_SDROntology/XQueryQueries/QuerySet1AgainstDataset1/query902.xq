...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_QuantityRange_0 in $node_Weight_0/RFDevice:hasMax
    let $value_QuantityRange_0 := fn:data($node_QuantityRange_0/@rdf:resource)
    where fn:exists($value_QuantityRange_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="QuantityRange_0">{$value_QuantityRange_0}</xqllib:var>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("Weight_0"))
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_0,$GRAPH_14,("USRPX3xx_0"))
  return
    $GRAPH_15
return
