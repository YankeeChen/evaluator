...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_RadioComponent_0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadioComponent_0 in $node_CortexA9_0/Nuvio:partOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($node_CortexA9_0) and fn:exists($value_RadioComponent_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_CortexA9_0 in $node_Object_0/RFDevice:isProducerOf
    let $value_CortexA9_0 := fn:data($node_CortexA9_0/@rdf:resource)
    where fn:exists($value_CortexA9_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="CortexA9_0">{$value_CortexA9_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_3,$GRAPH_9,("CortexA9_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_2,$GRAPH_10,("RadioComponent_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_1,$GRAPH_11,("RadioComponent_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_0,$GRAPH_12,("SearchRadar_0"))
  return
    $GRAPH_13
return
