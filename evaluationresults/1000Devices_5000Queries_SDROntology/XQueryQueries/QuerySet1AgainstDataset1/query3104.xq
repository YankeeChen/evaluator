...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:about)
    for $node_RadarJammer_0 in $node_BistaticRadar_0/Nuvio:compositeOf
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_BistaticRadar_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
        <xqllib:var name="BistaticRadar_0">{$value_BistaticRadar_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_RadarJammer_0 in $doc_ObjectDescription[RFDevice:hasFrequencyAccuracy/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#FreqencyRange_instance48"]
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:about)
    where fn:exists($node_RadarJammer_0)
    return
      <xqllib:result>
