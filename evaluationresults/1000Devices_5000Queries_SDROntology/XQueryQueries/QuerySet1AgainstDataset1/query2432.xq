...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_Object_0 in $node_Half_DuplexCommunicationsSystem_0/RFDevice:hasProducer
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_Time_0) and $node_Time_0/RFDevice:hasHeight/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Time_0"))
