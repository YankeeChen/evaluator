...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_XilinxFPGA_0 in $doc_ObjectDescription
    let $value_XilinxFPGA_0 := fn:data($node_XilinxFPGA_0/@rdf:about)
    for $node_Entity_0 in $node_XilinxFPGA_0/RFDevice:isProducerOf
  ...
  let $GRAPH_6 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Shell_0) and $node_Shell_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_4,$GRAPH_11,("XilinxFPGA_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_1,$GRAPH_12,("XilinxFPGA_0"))
