...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_PulseRadarModulation_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_2 :=  
    ...
    for $node_PulseRadarModulation_0 in $node_Attribute_0/RFDevice:hasTransmitPowerRange
    let $value_PulseRadarModulation_0 := fn:data($node_PulseRadarModulation_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_PulseRadarModulation_0)
    return
      <xqllib:result>
