...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_PulseDopplerRadar_0 in $doc_ObjectDescription[RFDevice:supportsSpectrumSensing/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#PulseDopplerRadarModulation_instance11"]
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    where fn:exists($node_PulseDopplerRadar_0)
    return
      <xqllib:result>
