...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:about)
    for $node_XilinxZynq7000Family_0 in $node_USRPE3xx_0/Nuvio:aggregateOf
    let $value_XilinxZynq7000Family_0 := fn:data($node_XilinxZynq7000Family_0/@rdf:resource)
    where fn:exists($node_USRPE3xx_0) and fn:exists($value_XilinxZynq7000Family_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPE3xx_0">{$value_USRPE3xx_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("USRPE3xx_0"))
  return
    $GRAPH_4
return
