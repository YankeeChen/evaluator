...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Situation_0 in $doc_ObjectDescription
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    for $node_CircleByCenterPoint_0 in $node_Situation_0/Nuvio:relevantIndividual
    let $value_CircleByCenterPoint_0 := fn:data($node_CircleByCenterPoint_0/@rdf:resource)
    where fn:exists($value_CircleByCenterPoint_0) and fn:exists($node_Situation_0)
    return
      <xqllib:result>
