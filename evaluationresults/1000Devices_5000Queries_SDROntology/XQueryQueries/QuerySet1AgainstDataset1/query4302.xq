...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_InformationEntity_0 in $node_Entity_0/RFDevice:hasCapability
  ...
  let $GRAPH_5 :=  
    ...
    for $node_AmplitudeShiftKeyingModulation_0 in $node_InformationEntity_0/Nuvio:expresses
    let $value_AmplitudeShiftKeyingModulation_0 := fn:data($node_AmplitudeShiftKeyingModulation_0/@rdf:resource)
    where fn:exists($value_AmplitudeShiftKeyingModulation_0) and fn:exists($node_InformationEntity_0)
    return
      <xqllib:result>
