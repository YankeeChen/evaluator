...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_Entity_0/RFDevice:hasWeight
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) >= -2012282561)]
  ...
  let $GRAPH_10 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Full_DuplexCommunicationsSystem_0 in $node_Entity_0/Nuvio:isExpressedBy
  ...
  let $GRAPH_11 :=  
    for $node_Full_DuplexCommunicationsSystem_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Producer_instance67"]/RFDevice:hasProducer
    let $value_Full_DuplexCommunicationsSystem_0 := fn:data($node_Full_DuplexCommunicationsSystem_0/@rdf:resource)
    where fn:exists($value_Full_DuplexCommunicationsSystem_0)
    return
      <xqllib:result>
        <xqllib:var name="Full_DuplexCommunicationsSystem_0">{$value_Full_DuplexCommunicationsSystem_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_14 := xqllib:and($GRAPH_2,$GRAPH_13,("Entity_0","Entity_1"))
