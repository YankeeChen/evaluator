...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CommunicationsSystem_0 in $node_Transmitting_0/RFDevice:supportsJamming
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Transmitting_0 in $node_Attribute_0/Nuvio:hasProcessQuantity
    let $value_Transmitting_0 := fn:data($node_Transmitting_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_Transmitting_0)
    return
      <xqllib:result>
