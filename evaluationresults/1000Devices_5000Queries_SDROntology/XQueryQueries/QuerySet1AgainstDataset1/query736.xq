...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN310_0 := fn:data($node_USRPN310_0/@rdf:about)
    for $node_Object_0 in $node_USRPN310_0/RFDevice:isProducerOf
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($value_Object_0) and fn:exists($node_USRPN310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Object_0 in $node_Voltage_0/RFDevice:hasHeight
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
