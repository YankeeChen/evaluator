...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Transmitting_0 in $node_Object_0/RFDevice:supportsSpectrumSensing
    let $value_Transmitting_0 := fn:data($node_Transmitting_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_Transmitting_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_Transmitting_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Power_15dBm"]/RFDevice:hasMaxOutputPower
    let $value_Transmitting_0 := fn:data($node_Transmitting_0/@rdf:resource)
