...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Percentage_0 in $node_Object_0/RFDevice:hasBeamwidth
    let $value_Percentage_0 := fn:data($node_Percentage_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_Percentage_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("Percentage_0"))
