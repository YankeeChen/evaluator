...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_BistaticRadar_0 in $node_RadioComponent_0/Nuvio:aggregateOf
    let $value_BistaticRadar_0 := fn:data($node_BistaticRadar_0/@rdf:resource)
    where fn:exists($value_BistaticRadar_0) and fn:exists($node_RadioComponent_0)
    return
      <xqllib:result>
        <xqllib:var name="BistaticRadar_0">{$value_BistaticRadar_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadioComponent_0 in $node_XilinxZynq7020SoC_0/Nuvio:aggregateOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
    where fn:exists($value_RadioComponent_0) and fn:exists($node_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="RadioComponent_0">{$value_RadioComponent_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_XilinxZynq7020SoC_0 in $node_DirectConversionReceiver_0/Nuvio:partOf
    let $value_XilinxZynq7020SoC_0 := fn:data($node_XilinxZynq7020SoC_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#RadioFrequencyBand_instance18"]/RFDevice:hasFrequencyRange
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("DirectConversionReceiver_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("DirectConversionReceiver_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("XilinxZynq7020SoC_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_3,$GRAPH_10,("XilinxZynq7020SoC_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_2,$GRAPH_11,("RadioComponent_0"))
