...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_ContinuousWaveRadar_0 in $doc_ObjectDescription[Nuvio:partOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Transmitter_instance2"]
    let $value_ContinuousWaveRadar_0 := fn:data($node_ContinuousWaveRadar_0/@rdf:about)
    where fn:exists($node_ContinuousWaveRadar_0)
    return
      <xqllib:result>
