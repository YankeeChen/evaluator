...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:about)
    for $node_Submarine_0 in $node_ModelX_0/RFDevice:isDeployedOn
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:resource)
    where fn:exists($node_ModelX_0) and fn:exists($value_Submarine_0)
    return
      <xqllib:result>
        <xqllib:var name="ModelX_0">{$value_ModelX_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Submarine_0 in $node_Entity_0/RFDevice:supportsJamming
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:resource)
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_1,$GRAPH_11,("Submarine_0"))
  let $GRAPH_13 := xqllib:optional($GRAPH_0,$GRAPH_12,("ModelX_0"))
  return
    $GRAPH_13
return
