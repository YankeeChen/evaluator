...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_i_0 in $node_RadarJammer_0/Nuvio:aggregateOf
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:resource)
    where fn:exists($node_RadarJammer_0) and fn:exists($value_USRPB200mini_i_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_RadarJammer_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#FreqencyRange_instance33"]/RFDevice:hasFrequencyRange
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("USRPB200mini_i_0"))
  return
    $GRAPH_4
return
