...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_FrequencyScanRateUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_FrequencyScanRateUnit_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) = -190794627)]
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("FrequencyScanRateUnit_0"))
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_0,$GRAPH_14,("USRPE3xx_0"))
  return
    $GRAPH_15
return
