...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Process_0 in $node_PhaseModulation_0/Nuvio:subprocessOf
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_PhaseModulation_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("PhaseModulation_0"))
