...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CommunicationsSystem_0 in $node_Aircraft_0/RFDevice:isDeployedOn
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:resource)
    where fn:exists($node_Aircraft_0) and fn:exists($value_CommunicationsSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Aircraft_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance7"]/RFDevice:hasCapability
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:resource)
    where fn:exists($value_Aircraft_0)
    return
      <xqllib:result>
        <xqllib:var name="Aircraft_0">{$value_Aircraft_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 := ($GRAPH_3,$GRAPH_4)
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("Aircraft_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("CommunicationsSystem_0"))
  return
    $GRAPH_7
return
