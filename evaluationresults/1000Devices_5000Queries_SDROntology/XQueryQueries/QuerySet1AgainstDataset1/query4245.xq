...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_XC6SLX150FPGA_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_XC6SLX150FPGA_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_5 :=  
    ...
    let $value_InformationContent_0 := fn:data($node_InformationContent_0/@rdf:about)
    where fn:exists($node_Entity_1) and fn:exists($node_InformationContent_0) and $node_Entity_1/Nuvio:representedBy/@rdf:resource = $value_InformationContent_0
    return
      <xqllib:result>
