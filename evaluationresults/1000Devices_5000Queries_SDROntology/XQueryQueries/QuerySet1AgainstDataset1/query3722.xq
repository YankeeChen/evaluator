...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    for $node_GroundPlatform_0 in $node_PulseDopplerRadar_0/RFDevice:isDeployedOn
    let $value_GroundPlatform_0 := fn:data($node_GroundPlatform_0/@rdf:resource)
    where fn:exists($value_GroundPlatform_0) and fn:exists($node_PulseDopplerRadar_0)
    return
      <xqllib:result>
