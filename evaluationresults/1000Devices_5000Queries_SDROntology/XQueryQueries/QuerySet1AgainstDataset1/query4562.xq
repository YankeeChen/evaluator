...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:about)
    for $node_Var0 in $node_CommunicationsSystem_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_RadarFrequencyBand_0 in $node_Var0/RFDevice:hasFrequencyAccuracy
    let $value_RadarFrequencyBand_0 := fn:data($node_RadarFrequencyBand_0/@rdf:resource)
    where fn:exists($value_RadarFrequencyBand_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
