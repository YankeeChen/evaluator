...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RFSystem_0 := fn:data($node_RFSystem_0/@rdf:about)
    for $node_SuperheterodyneReceiver_0 in $node_RFSystem_0/Nuvio:partOf
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($node_RFSystem_0) and fn:exists($value_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="RFSystem_0">{$value_RFSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_SuperheterodyneReceiver_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#RadioFrequencyBand_instance9"]/RFDevice:hasFrequencyRange
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
