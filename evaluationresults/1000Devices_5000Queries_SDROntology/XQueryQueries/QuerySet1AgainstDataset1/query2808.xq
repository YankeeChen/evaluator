...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_0 in $node_Spartan6FPGA_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Spartan6FPGA_0 in $node_Mixer_0/Nuvio:aggregateOf
    let $value_Spartan6FPGA_0 := fn:data($node_Spartan6FPGA_0/@rdf:resource)
    where fn:exists($node_Mixer_0) and fn:exists($value_Spartan6FPGA_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Spartan6FPGA_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Spartan6FPGA_0"))
