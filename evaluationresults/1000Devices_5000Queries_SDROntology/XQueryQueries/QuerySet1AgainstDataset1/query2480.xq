...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_InformationEntity_0 := fn:data($node_InformationEntity_0/@rdf:about)
    for $node_LongValue_0 in $doc_ObjectDescription
    let $value_LongValue_0 := fn:data($node_LongValue_0/@rdf:about)
    where fn:exists($node_InformationEntity_0) and fn:exists($node_LongValue_0) and $node_LongValue_0/Nuvio:isExpressedBy/@rdf:resource = $value_InformationEntity_0
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
        <xqllib:var name="LongValue_0">{$value_LongValue_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 :=  
    for $node_LongValue_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#LongValue"]
    let $value_LongValue_0 := fn:data($node_LongValue_0/@rdf:about)
    where fn:exists($node_LongValue_0)
    return
      <xqllib:result>
        <xqllib:var name="LongValue_0">{$value_LongValue_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 :=  
    for $node_LongValue_0 in $doc_ObjectDescription
    let $value_LongValue_0 := fn:data($node_LongValue_0/@rdf:about)
    for $node_DataValue0 in $node_LongValue_0/RFDevice:hasDoubleValue
    let $value_DataValue0 := fn:data($node_DataValue0)
    where fn:exists($value_DataValue0) and fn:exists($node_LongValue_0)
    return
      <xqllib:result>
        <xqllib:var name="DataValue0">{$value_DataValue0}</xqllib:var>
        <xqllib:var name="LongValue_0">{$value_LongValue_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_7 := xqllib:optional($GRAPH_5,$GRAPH_6,("LongValue_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("LongValue_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_3,$GRAPH_8,("InformationEntity_0"))
