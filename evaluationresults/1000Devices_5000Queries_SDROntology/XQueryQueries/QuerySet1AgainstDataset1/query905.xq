...
let $result :=  
  ...
  let $GRAPH_10 :=  
    ...
    for $node_Value_0 in $node_VoltageUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Value_0 := fn:data($node_Value_0/@rdf:resource)
    where fn:exists($value_Value_0) and fn:exists($node_VoltageUnit_0)
    return
      <xqllib:result>
        <xqllib:var name="Value_0">{$value_Value_0}</xqllib:var>
  ...
  let $GRAPH_12 := $GRAPH_11[(data(xqllib:var[@name="DataValue0"]) < 412443106)]
  let $GRAPH_13 := xqllib:optional($GRAPH_10,$GRAPH_12,("Value_0"))
  let $GRAPH_14 := xqllib:and($GRAPH_9,$GRAPH_13,())
  let $GRAPH_15 := xqllib:and($GRAPH_6,$GRAPH_14,("Entity_0","Value_0"))
  let $GRAPH_16 := xqllib:and($GRAPH_5,$GRAPH_15,("Entity_0"))
  let $GRAPH_17 := xqllib:optional($GRAPH_4,$GRAPH_16,("Var0"))
