...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Object_0 in $node_Entity_0/skos:inScheme
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Object_0) and $doc_ObjectDescription[@rdf:ID = "Instance264"]/RFDevice:hasNoiseFigure/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue0"]) < -546093310)]
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_2,$GRAPH_14,("Entity_0","Process_0"))
