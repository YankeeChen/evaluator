...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_UnitOfMeasure_0) and $node_UnitOfMeasure_0/RFDevice:supportsTuning/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) > -99414770)]
  ...
  let $GRAPH_14 := $GRAPH_13[(data(xqllib:var[@name="DataValue1"]) < -53.53)]
  ...
  let $GRAPH_25 := xqllib:and($GRAPH_0,$GRAPH_24,("ModelX_0"))
  return
    $GRAPH_25
return
