...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE313_0 in $node_Submarine_0/RFDevice:isDeployedOn
    let $value_USRPE313_0 := fn:data($node_USRPE313_0/@rdf:resource)
    where fn:exists($node_Submarine_0) and fn:exists($value_USRPE313_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Submarine_0 in $doc_ObjectDescription[RFDevice:supportsTransmitting/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Moving_instance2"]
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:about)
