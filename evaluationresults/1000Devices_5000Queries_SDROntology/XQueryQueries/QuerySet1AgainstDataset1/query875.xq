...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:resource)
    where fn:exists($node_MobilePlatform_0) and fn:exists($value_USRPN210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Process_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Length_27_7cm"]/RFDevice:hasMaxSensedPower
