...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Entity_0 in $node_Var0/RFDevice:hasPulseWidth
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_Situation_0) and $doc_ObjectDescription[@rdf:ID = "SDR_instance0"]/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 :=  
    ...
    for $node_Situation_0 in $node_QuantityRange_0/Nuvio:relevantIndividual
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:resource)
    where fn:exists($node_QuantityRange_0) and fn:exists($value_Situation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 :=  
    ...
    for $node_QuantityRange_0 in $node_Length_0/RFDevice:hasMin
    let $value_QuantityRange_0 := fn:data($node_QuantityRange_0/@rdf:resource)
    where fn:exists($node_Length_0) and fn:exists($value_QuantityRange_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_8,$GRAPH_9,("Length_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_7,$GRAPH_10,("QuantityRange_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_6,$GRAPH_11,("QuantityRange_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_5,$GRAPH_12,("Situation_0"))
  let $GRAPH_14 := xqllib:and($GRAPH_4,$GRAPH_13,("Situation_0"))
  ...
  let $GRAPH_20 := xqllib:optional($GRAPH_16,$GRAPH_19,("Var0"))
  let $GRAPH_21 := xqllib:and($GRAPH_1,$GRAPH_20,("Var0"))
  let $GRAPH_22 := xqllib:and($GRAPH_0,$GRAPH_21,("USRPB200mini_0"))
  return
    $GRAPH_22
return
