...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:about)
    for $node_Quantity_0 in $node_RadarWarningReceiver_0/RFDevice:hasMaxPowerConsumption
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:resource)
    where fn:exists($value_Quantity_0) and fn:exists($node_RadarWarningReceiver_0)
    return
      <xqllib:result>
