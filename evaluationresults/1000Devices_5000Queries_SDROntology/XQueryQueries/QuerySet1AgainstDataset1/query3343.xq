...
let $result :=  
  ...
  let $GRAPH_8 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription1000.owl#Power_instance26"]/RFDevice:hasPulseRepetitionInterval
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_11 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Cylinder_0) and fn:exists($node_Var0) and $node_Cylinder_0/Nuvio:hasParticipant/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_12 :=  
    for $node_Cylinder_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Cylinder"]
    let $value_Cylinder_0 := fn:data($node_Cylinder_0/@rdf:about)
    where fn:exists($node_Cylinder_0)
    return
      <xqllib:result>
        <xqllib:var name="Cylinder_0">{$value_Cylinder_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_13 := xqllib:and($GRAPH_11,$GRAPH_12,("Cylinder_0"))
