...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB205mini_i_0 in $node_Var0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    where fn:exists($node_AngleUnit_0) and fn:exists($node_Var1) and $node_AngleUnit_0/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Var1
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Var1"))
  let $GRAPH_7 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#NoiseFigure"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_8 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Var2 in $node_Var0/Nuvio:hasValue
    let $value_Var2 := fn:data($node_Var2/@rdf:resource)
    where fn:exists($node_Var0) and fn:exists($value_Var2)
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
  ...
  let $GRAPH_11 :=  
    ...
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
    for $node_DataValue1 in $node_Var2/Nuvio:hasPrecision
    let $value_DataValue1 := fn:data($node_DataValue1)
    where fn:exists($value_DataValue1) and fn:exists($node_Var2)
    return
      <xqllib:result>
        <xqllib:var name="DataValue1">{$value_DataValue1}</xqllib:var>
        <xqllib:var name="Var2">{$value_Var2}</xqllib:var>
      </xqllib:result>
  let $GRAPH_12 := $GRAPH_11[(data(xqllib:var[@name="DataValue1"]) <= -1707047388)]
  let $GRAPH_13 := xqllib:and($GRAPH_10,$GRAPH_12,("Var2"))
  let $GRAPH_14 := xqllib:and($GRAPH_9,$GRAPH_13,("Var2"))
  let $GRAPH_15 := xqllib:and($GRAPH_8,$GRAPH_14,("Var2"))
  let $GRAPH_16 := xqllib:optional($GRAPH_7,$GRAPH_15,("Var0"))
