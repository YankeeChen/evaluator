...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_TrackingRadar_0/Nuvio:participatesIn
  ...
  let $GRAPH_4 :=  
    ...
    for $node_TrackingRadar_0 in $node_Transmitter_0/Nuvio:partOf
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:resource)
    where fn:exists($value_TrackingRadar_0) and fn:exists($node_Transmitter_0)
    return
      <xqllib:result>
        <xqllib:var name="TrackingRadar_0">{$value_TrackingRadar_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("TrackingRadar_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("TrackingRadar_0"))
