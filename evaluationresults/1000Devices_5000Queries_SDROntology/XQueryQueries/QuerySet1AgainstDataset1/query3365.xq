...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:about)
    for $node_PulseDopplerRadarModulation_0 in $node_TrackingRadar_0/RFDevice:hasCapability
    let $value_PulseDopplerRadarModulation_0 := fn:data($node_PulseDopplerRadarModulation_0/@rdf:resource)
    where fn:exists($value_PulseDopplerRadarModulation_0) and fn:exists($node_TrackingRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    let $value_PulseDopplerRadarModulation_0 := fn:data($node_PulseDopplerRadarModulation_0/@rdf:about)
    for $node_Entity_0 in $node_PulseDopplerRadarModulation_0/RFDevice:hasMaxSensedPower
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_PulseDopplerRadarModulation_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Cone"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("PulseDopplerRadarModulation_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_0,$GRAPH_10,("TrackingRadar_0"))
  return
    $GRAPH_11
return
