...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_BSpline_0 in $node_Process_0/Nuvio:hasParticipant
    let $value_BSpline_0 := fn:data($node_BSpline_0/@rdf:resource)
    where fn:exists($value_BSpline_0) and fn:exists($node_Process_0)
    return
      <xqllib:result>
