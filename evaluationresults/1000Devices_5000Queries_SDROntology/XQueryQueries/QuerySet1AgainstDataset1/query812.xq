...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Full_DuplexCommunicationsSystem_0 in $node_Component_0/Nuvio:aggregateOf
    let $value_Full_DuplexCommunicationsSystem_0 := fn:data($node_Full_DuplexCommunicationsSystem_0/@rdf:resource)
    where fn:exists($node_Component_0) and fn:exists($value_Full_DuplexCommunicationsSystem_0)
    return
      <xqllib:result>
