...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xxmini_0 in $node_Ship_0/RFDevice:isDeployedOn
    let $value_USRPB2xxmini_0 := fn:data($node_USRPB2xxmini_0/@rdf:resource)
    where fn:exists($node_Ship_0) and fn:exists($value_USRPB2xxmini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("USRPB2xxmini_0"))
  return
    $GRAPH_8
return
