...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:about)
    for $node_Process_0 in $node_ModelX_0/RFDevice:supportsSpectrumSensing
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_ModelX_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
        <xqllib:var name="ModelX_0">{$value_ModelX_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Process_0 in $node_Kintex7FPGA_0/Nuvio:participatesIn
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_Kintex7FPGA_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("Kintex7FPGA_0"))
