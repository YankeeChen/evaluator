...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:about)
    for $node_Weight_0 in $node_USRPB2xx_0/RFDevice:hasSamplingRate
    let $value_Weight_0 := fn:data($node_Weight_0/@rdf:resource)
    where fn:exists($node_USRPB2xx_0) and fn:exists($value_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB2xx_0">{$value_USRPB2xx_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#SplineCurve"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_2,$GRAPH_7,("Weight_0"))
