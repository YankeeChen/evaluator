...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_Entity_0/RFDevice:hasProducer
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#SDR"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_MannedPlatform_0 in $node_Entity_0/RFDevice:isDeployedOn
    let $value_MannedPlatform_0 := fn:data($node_MannedPlatform_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_MannedPlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
        <xqllib:var name="MannedPlatform_0">{$value_MannedPlatform_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_5 :=  
    for $node_MannedPlatform_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#MannedPlatform"]
    let $value_MannedPlatform_0 := fn:data($node_MannedPlatform_0/@rdf:about)
    where fn:exists($node_MannedPlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="MannedPlatform_0">{$value_MannedPlatform_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("MannedPlatform_0"))
  ...
  let $GRAPH_10 := $GRAPH_9[(data(xqllib:var[@name="DataValue0"]) = 80.89)]
  ...
  let $GRAPH_17 := xqllib:and($GRAPH_0,$GRAPH_16,("USRPE3xx_0"))
  return
    $GRAPH_17
return
