...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    for $node_Entity_0 in $node_PulseDopplerRadar_0/RFDevice:supportsSpectrumSensing
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_PulseDopplerRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $doc_ObjectDescription[@rdf:ID = "WeightUnit_instance15"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue0"]) > 830669842)]
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_16,$GRAPH_17,("Quality_0"))
