...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "TimeUnit_instance2"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) != 121816987)]
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue1"]) > xs:double("0.0"))]
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_1,$GRAPH_12,("Weight_0"))
