...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_Receiving_0) and $node_Receiving_0/Nuvio:hasParticipant/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_5,$GRAPH_6,("Receiving_0"))
  let $GRAPH_8 :=  
    for $node_SweepJamming_0 in $doc_ObjectDescription
    let $value_SweepJamming_0 := fn:data($node_SweepJamming_0/@rdf:about)
    for $node_Process_0 in $node_SweepJamming_0/Nuvio:hasSubprocess
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($value_Process_0) and fn:exists($node_SweepJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("Entity_0","Process_0"))
