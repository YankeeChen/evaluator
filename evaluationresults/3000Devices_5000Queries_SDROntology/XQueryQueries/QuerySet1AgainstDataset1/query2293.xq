...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:about)
    for $node_Entity_0 in $node_USRPE3xx_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_ChokeRingAntenna_0 in $node_Entity_0/Nuvio:relevantIndividual
  ...
  let $GRAPH_5 :=  
    ...
    for $node_ChokeRingAntenna_0 in $node_Quantity_0/RFDevice:hasMaxPowerConsumption
    let $value_ChokeRingAntenna_0 := fn:data($node_ChokeRingAntenna_0/@rdf:resource)
    where fn:exists($value_ChokeRingAntenna_0) and fn:exists($node_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="ChokeRingAntenna_0">{$value_ChokeRingAntenna_0}</xqllib:var>
        <xqllib:var name="Quantity_0">{$value_Quantity_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("ChokeRingAntenna_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("ChokeRingAntenna_0"))
  let $GRAPH_8 :=  
  ...
  let $GRAPH_12 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $doc_ObjectDescription[@rdf:ID = "AntennaPolarization_instance1"]/Nuvio:hasProcessQuality/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_16 := xqllib:and($GRAPH_0,$GRAPH_15,("USRPE3xx_0"))
  return
    $GRAPH_16
return
