...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_Ship_0/RFDevice:isDeployedOn
    let $value_USRPB210_0 := fn:data($node_USRPB210_0/@rdf:resource)
    where fn:exists($node_Ship_0) and fn:exists($value_USRPB210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Ship_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Moving_instance47"]/RFDevice:supportsJamming
