...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX3xx_0 in $node_Entity_0/RFDevice:isProducerOf
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Triangle"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#AntennaPolarization_instance19"]/RFDevice:hasPolarization
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_0,$GRAPH_7,("USRPX3xx_0"))
  return
    $GRAPH_8
return
