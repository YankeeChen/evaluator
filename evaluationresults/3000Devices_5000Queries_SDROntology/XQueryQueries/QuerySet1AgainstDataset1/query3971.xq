...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_SpotJamming_0 in $node_Quantity_0/RFDevice:hasPulseRepetitionFrequency
    let $value_SpotJamming_0 := fn:data($node_SpotJamming_0/@rdf:resource)
    where fn:exists($node_Quantity_0) and fn:exists($value_SpotJamming_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("SpotJamming_0"))
