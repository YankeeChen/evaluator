...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE313_0 := fn:data($node_USRPE313_0/@rdf:about)
    for $node_Entity_0 in $node_USRPE313_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_CyclostationaryDetection_0) and fn:exists($node_Object_0) and $node_CyclostationaryDetection_0/Nuvio:hasParticipant/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:optional($GRAPH_3,$GRAPH_4,("CyclostationaryDetection_0"))
