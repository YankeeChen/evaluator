...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_IEDTrigger_0 in $node_Process_0/RFDevice:hasCapability
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_Process_0) and $node_Process_0/Nuvio:participatesIn/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
      </xqllib:result>
