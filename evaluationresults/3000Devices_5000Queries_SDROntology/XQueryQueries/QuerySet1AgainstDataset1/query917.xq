...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Half_DuplexCommunicationsSystem_0 in $node_Transmitting_0/RFDevice:supportsJamming
  ...
  let $GRAPH_3 :=  
    for $node_Transmitting_0 in $doc_ObjectDescription[RFDevice:hasEffectiveRadiatedPower/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Power_instance34"]
    let $value_Transmitting_0 := fn:data($node_Transmitting_0/@rdf:about)
    where fn:exists($node_Transmitting_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("Transmitting_0"))
