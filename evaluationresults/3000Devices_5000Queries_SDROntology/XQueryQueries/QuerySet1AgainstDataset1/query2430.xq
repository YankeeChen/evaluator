...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:about)
    for $node_DirectConversionReceiver_0 in $node_USRPN210_0/Nuvio:aggregateOf
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($value_DirectConversionReceiver_0) and fn:exists($node_USRPN210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Quantity_0 in $node_DirectConversionReceiver_0/RFDevice:hasNoiseFigure
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="DirectConversionReceiver_0">{$value_DirectConversionReceiver_0}</xqllib:var>
