...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_GNURadio_0 := fn:data($node_GNURadio_0/@rdf:about)
    for $node_Aircraft_0 in $node_GNURadio_0/RFDevice:isDeployedOn
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:resource)
    where fn:exists($value_Aircraft_0) and fn:exists($node_GNURadio_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Aircraft_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Moving_instance72"]/RFDevice:supportsTransmitting
    let $value_Aircraft_0 := fn:data($node_Aircraft_0/@rdf:resource)
