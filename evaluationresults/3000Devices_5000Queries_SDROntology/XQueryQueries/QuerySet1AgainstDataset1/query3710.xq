...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Entity_0 in $node_Concept_0/Nuvio:isExpressedBy
  ...
  let $GRAPH_7 :=  
    ...
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Concept_0) and fn:exists($node_Entity_0) and $node_Entity_0/skos:hasTopConcept/@rdf:resource = $value_Concept_0
    return
      <xqllib:result>
        <xqllib:var name="Concept_0">{$value_Concept_0}</xqllib:var>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("Concept_0","Entity_0"))
  let $GRAPH_9 :=  
    for $node_Quality_0 in $doc_ObjectDescription
    let $value_Quality_0 := fn:data($node_Quality_0/@rdf:about)
    for $node_Entity_0 in $node_Quality_0/RFDevice:hasCapacity
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_9,$GRAPH_10,("Quality_0"))
  let $GRAPH_12 := ($GRAPH_8,$GRAPH_11)
  let $GRAPH_13 := ($GRAPH_4,$GRAPH_12)
  let $GRAPH_14 :=  
    for $node_Process_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Instance2249"]/RFDevice:hasPulseRepetitionInterval
