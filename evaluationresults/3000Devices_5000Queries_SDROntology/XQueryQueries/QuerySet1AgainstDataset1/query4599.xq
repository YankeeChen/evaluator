...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:about)
    for $node_Transmitter_0 in $node_USRPB2xx_0/Nuvio:partOf
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:resource)
    where fn:exists($value_Transmitter_0) and fn:exists($node_USRPB2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:about)
    for $node_PhasedArrayAntenna_0 in $node_Transmitter_0/Nuvio:aggregateOf
    let $value_PhasedArrayAntenna_0 := fn:data($node_PhasedArrayAntenna_0/@rdf:resource)
    where fn:exists($value_PhasedArrayAntenna_0) and fn:exists($node_Transmitter_0)
    return
      <xqllib:result>
