...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Process_0 in $node_ElectronicWarfare_0/Nuvio:subprocessOf
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_ElectronicWarfare_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("ElectronicWarfare_0"))
