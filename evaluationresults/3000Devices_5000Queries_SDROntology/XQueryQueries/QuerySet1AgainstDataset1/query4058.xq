...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Full_DuplexCommunicationsSystem_0 := fn:data($node_Full_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_Object_0 in $node_Full_DuplexCommunicationsSystem_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_DeceptiveJamming_0 in $node_Object_0/Nuvio:participatesIn
    let $value_DeceptiveJamming_0 := fn:data($node_DeceptiveJamming_0/@rdf:resource)
    where fn:exists($value_DeceptiveJamming_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
