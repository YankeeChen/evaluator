...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarSystem_0 in $node_IEDJammer_0/Nuvio:compositeOf
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:resource)
    where fn:exists($node_IEDJammer_0) and fn:exists($value_RadarSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_IEDJammer_0 in $node_InverseGainJamming_0/RFDevice:supportsSpectrumSensing
