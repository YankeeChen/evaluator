...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Var0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_IEDTrigger_0 in $doc_ObjectDescription
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:about)
    for $node_Entity_0 in $node_IEDTrigger_0/RFDevice:hasFrequencyRange
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Entity_0","Var0"))
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_8,$GRAPH_9,("Value_0"))
