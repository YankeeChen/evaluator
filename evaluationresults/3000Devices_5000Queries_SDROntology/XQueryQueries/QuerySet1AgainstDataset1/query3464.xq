...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_IEDJammer_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    for $node_IEDJammer_0 in $doc_ObjectDescription
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:about)
    for $node_DeceptiveJamming_0 in $node_IEDJammer_0/RFDevice:supportsTransmitting
    let $value_DeceptiveJamming_0 := fn:data($node_DeceptiveJamming_0/@rdf:resource)
    where fn:exists($value_DeceptiveJamming_0) and fn:exists($node_IEDJammer_0)
    return
      <xqllib:result>
