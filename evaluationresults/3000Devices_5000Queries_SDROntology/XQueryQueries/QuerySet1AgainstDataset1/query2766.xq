...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarSystem_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:resource)
    where fn:exists($node_MobilePlatform_0) and fn:exists($value_RadarSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_MobilePlatform_0 in $doc_ObjectDescription
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:about)
    for $node_Entity_0 in $node_MobilePlatform_0/RFDevice:supportsTuning
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue0"]) >= -1875919131)]
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_1,$GRAPH_17,("MobilePlatform_0"))
