...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_DRFMJamming_0) and fn:exists($node_Var0) and $node_DRFMJamming_0/Nuvio:subprocessOf/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_DRFMJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Power_instance44"]/RFDevice:hasPulseWidth
    let $value_DRFMJamming_0 := fn:data($node_DRFMJamming_0/@rdf:resource)
    where fn:exists($value_DRFMJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="DRFMJamming_0">{$value_DRFMJamming_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("DRFMJamming_0"))
  let $GRAPH_7 := xqllib:optional($GRAPH_3,$GRAPH_6,("DRFMJamming_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Var0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("Var0"))
  let $GRAPH_10 := xqllib:optional($GRAPH_0,$GRAPH_9,("GNURadio_0"))
  return
    $GRAPH_10
return
