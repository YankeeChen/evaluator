...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Bezier"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_Transceiving_0 in $node_Entity_0/RFDevice:supportsJamming
  ...
  let $GRAPH_7 :=  
    for $node_Transceiving_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#BlinkingJamming"]
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:about)
    where fn:exists($node_Transceiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,("Transceiving_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_5,$GRAPH_10,("Transceiving_0"))
