...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_0 in $node_Entity_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Process_0) and fn:exists($node_SpectrumSensing_0) and $node_SpectrumSensing_0/Nuvio:subprocessOf/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_SpectrumSensing_0 in $doc_ObjectDescription
    let $value_SpectrumSensing_0 := fn:data($node_SpectrumSensing_0/@rdf:about)
    for $node_RadioFrequencyBand_0 in $node_SpectrumSensing_0/RFDevice:hasBandwidthRange
    let $value_RadioFrequencyBand_0 := fn:data($node_RadioFrequencyBand_0/@rdf:resource)
    where fn:exists($value_RadioFrequencyBand_0) and fn:exists($node_SpectrumSensing_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("SpectrumSensing_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Process_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("Entity_0"))
  let $GRAPH_10 := xqllib:optional($GRAPH_0,$GRAPH_9,("USRPB200_0"))
  return
    $GRAPH_10
return
