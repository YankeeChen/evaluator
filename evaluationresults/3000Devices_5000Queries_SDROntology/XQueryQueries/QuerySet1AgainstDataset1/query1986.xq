...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Clothoid_0) and fn:exists($node_Var0) and $node_Clothoid_0/RFDevice:hasTarget/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("Clothoid_0"))
  let $GRAPH_6 :=  
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue0"]) <= -5.73)]
