...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Half_DuplexCommunicationsSystem_0 in $node_Entity_0/Nuvio:partOf
  ...
  let $GRAPH_10 :=  
    ...
    for $node_Entity_0 in $node_AnalogModulation_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($node_AnalogModulation_0) and fn:exists($value_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_10,$GRAPH_11,("AnalogModulation_0"))
