...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CommunicationsSystem_0 in $node_Entity_0/RFDevice:hasMaxPowerConsumption
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) = -1955008880)]
  ...
  let $GRAPH_10 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_DataValue1 in $node_Var0/RFDevice:hasPositiveIntegerValue
  ...
  let $GRAPH_11 := $GRAPH_10[not((data(xqllib:var[@name="DataValue1"]) > xs:decimal("0")))]
