...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CR_0 := fn:data($node_CR_0/@rdf:about)
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_CR_0/Nuvio:compositeOf
    ...
    where fn:exists($node_CR_0) and fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="CR_0">{$value_CR_0}</xqllib:var>
