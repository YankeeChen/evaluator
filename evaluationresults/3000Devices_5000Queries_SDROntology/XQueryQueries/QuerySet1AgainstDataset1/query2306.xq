...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:about)
    for $node_RangeGatePullOffJamming_0 in $node_ElectronicAttackSystem_0/RFDevice:hasCapability
    let $value_RangeGatePullOffJamming_0 := fn:data($node_RangeGatePullOffJamming_0/@rdf:resource)
    where fn:exists($node_ElectronicAttackSystem_0) and fn:exists($value_RangeGatePullOffJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicAttackSystem_0">{$value_ElectronicAttackSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_RangeGatePullOffJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Power_instance38"]/RFDevice:hasTransmitPowerRange
    let $value_RangeGatePullOffJamming_0 := fn:data($node_RangeGatePullOffJamming_0/@rdf:resource)
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("RangeGatePullOffJamming_0"))
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("RangeGatePullOffJamming_0"))
