...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_TrackingRadar_0 in $node_IEDTrigger_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_IEDTrigger_0 in $node_ChokeRingAntenna_0/Nuvio:compositeOf
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:resource)
    where fn:exists($node_ChokeRingAntenna_0) and fn:exists($value_IEDTrigger_0)
    return
      <xqllib:result>
