...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_Entity_0/Nuvio:aggregateOf
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_LineString_0) and fn:exists($node_Situation_0) and $node_LineString_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_2,$GRAPH_6,("Situation_0"))
