...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_TrackingRadar_0 := fn:data($node_TrackingRadar_0/@rdf:about)
    for $node_IEDTrigger_0 in $node_TrackingRadar_0/Nuvio:compositeOf
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:resource)
    where fn:exists($value_IEDTrigger_0) and fn:exists($node_TrackingRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_IEDTrigger_0 in $node_Entity_0/Nuvio:compositeOf
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:resource)
  ...
  let $GRAPH_5 := xqllib:optional($GRAPH_3,$GRAPH_4,("Object_0"))
