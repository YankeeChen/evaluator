...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_MonostaticRadar_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#PulseDopplerRadarModulation_instance32"]/RFDevice:supportsSpectrumSensing
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:resource)
    where fn:exists($value_MonostaticRadar_0)
    return
      <xqllib:result>
        <xqllib:var name="MonostaticRadar_0">{$value_MonostaticRadar_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_2 := xqllib:optional($GRAPH_0,$GRAPH_1,("MonostaticRadar_0"))
  return
    $GRAPH_2
return
