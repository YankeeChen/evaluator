...
let $result :=  
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) != 22.13)]
  let $GRAPH_8 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
    for $node_InformationUnit_0 in $node_Var1/Nuvio:hasUnitOfMeasure
    let $value_InformationUnit_0 := fn:data($node_InformationUnit_0/@rdf:resource)
    where fn:exists($value_InformationUnit_0) and fn:exists($node_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_2,$GRAPH_11,("Power_0"))
