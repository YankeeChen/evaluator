...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_PulseRadar_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#InstantaneousFrequencyMeasurementReceiver_instance132"]/Nuvio:compositeOf
    let $value_PulseRadar_0 := fn:data($node_PulseRadar_0/@rdf:resource)
    where fn:exists($value_PulseRadar_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseRadar_0">{$value_PulseRadar_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_2 := xqllib:optional($GRAPH_0,$GRAPH_1,("PulseRadar_0"))
  return
    $GRAPH_2
return
