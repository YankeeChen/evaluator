...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN310_0 := fn:data($node_USRPN310_0/@rdf:about)
    for $node_DirectConversionReceiver_0 in $node_USRPN310_0/Nuvio:compositeOf
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($value_DirectConversionReceiver_0) and fn:exists($node_USRPN310_0)
    return
      <xqllib:result>
