...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ContinuousWaveRadar_0 := fn:data($node_ContinuousWaveRadar_0/@rdf:about)
    for $node_Jammer_0 in $node_ContinuousWaveRadar_0/Nuvio:partOf
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($node_ContinuousWaveRadar_0) and fn:exists($value_Jammer_0)
    return
      <xqllib:result>
        <xqllib:var name="ContinuousWaveRadar_0">{$value_ContinuousWaveRadar_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_DAC_0 in $node_Jammer_0/Nuvio:aggregateOf
    let $value_DAC_0 := fn:data($node_DAC_0/@rdf:resource)
    where fn:exists($value_DAC_0) and fn:exists($node_Jammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("DAC_0"))
