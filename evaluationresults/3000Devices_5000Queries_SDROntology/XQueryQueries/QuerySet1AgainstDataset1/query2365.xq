...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:about)
    for $node_InboundRangeGatePullOffJamming_0 in $node_JammingSystem_0/RFDevice:supportsTuning
    let $value_InboundRangeGatePullOffJamming_0 := fn:data($node_InboundRangeGatePullOffJamming_0/@rdf:resource)
    where fn:exists($value_InboundRangeGatePullOffJamming_0) and fn:exists($node_JammingSystem_0)
    return
      <xqllib:result>
