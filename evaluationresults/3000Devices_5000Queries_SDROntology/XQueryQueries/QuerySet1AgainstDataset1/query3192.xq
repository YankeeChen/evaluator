...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_Situation_0/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) != 1231785488)]
  ...
  let $GRAPH_8 :=  
    ...
    for $node_Situation_0 in $node_SpotJamming_0/Nuvio:relevantIndividual
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:resource)
    where fn:exists($value_Situation_0) and fn:exists($node_SpotJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Situation_0">{$value_Situation_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_8,$GRAPH_9,("SpotJamming_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_7,$GRAPH_10,("Situation_0"))
  let $GRAPH_12 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Frequency_instance271"]/RFDevice:hasMaxOutputPower
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
