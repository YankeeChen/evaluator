...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Situation" and Nuvio:relevantIndividual/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#RadarFrequencyBand_instance21"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_5 :=  
    for $node_Polygon_0 in $doc_ObjectDescription
    let $value_Polygon_0 := fn:data($node_Polygon_0/@rdf:about)
    for $node_Entity_0 in $node_Polygon_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_Polygon_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    for $node_Polygon_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#Polygon"]
    let $value_Polygon_0 := fn:data($node_Polygon_0/@rdf:about)
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("Polygon_0"))
