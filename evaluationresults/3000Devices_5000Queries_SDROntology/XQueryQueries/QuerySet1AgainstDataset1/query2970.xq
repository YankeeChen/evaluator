...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CommunicationsSystem_0 in $node_Attribute_0/RFDevice:hasChargingTime
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_CommunicationsSystem_0)
    return
      <xqllib:result>
