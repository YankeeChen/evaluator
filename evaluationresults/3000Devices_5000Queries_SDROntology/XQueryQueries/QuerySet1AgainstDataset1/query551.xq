...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_CoverPulsesJamming_0) and fn:exists($node_Var0) and $node_CoverPulsesJamming_0/Nuvio:subprocessOf/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_CoverPulsesJamming_0 in $doc_ObjectDescription
    let $value_CoverPulsesJamming_0 := fn:data($node_CoverPulsesJamming_0/@rdf:about)
    for $node_Quantity_0 in $node_CoverPulsesJamming_0/Nuvio:hasProcessQuantity
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:resource)
    where fn:exists($node_CoverPulsesJamming_0) and fn:exists($value_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="CoverPulsesJamming_0">{$value_CoverPulsesJamming_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("CoverPulsesJamming_0"))
