...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_Voltage_0) and $node_Voltage_0/RFDevice:hasChargingTime/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Voltage_0 in $node_Var0/Nuvio:hasValue
    let $value_Voltage_0 := fn:data($node_Voltage_0/@rdf:resource)
    where fn:exists($node_Var0) and fn:exists($value_Voltage_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Value_0) and fn:exists($node_Var0) and $node_Value_0/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) = -706177494)]
  ...
  let $GRAPH_12 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Var1 in $node_Object_0/Nuvio:partOf
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_11,$GRAPH_12,("Var1"))
  ...
  let $GRAPH_19 := xqllib:and($GRAPH_2,$GRAPH_18,("Object_0","Voltage_0"))
  let $GRAPH_20 := xqllib:and($GRAPH_1,$GRAPH_19,("Object_0"))
  let $GRAPH_21 := xqllib:and($GRAPH_0,$GRAPH_20,("USRPB200_210_0"))
  return
    $GRAPH_21
return
