...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_PulseRadar_0 := fn:data($node_PulseRadar_0/@rdf:resource)
    where fn:exists($node_MobilePlatform_0) and fn:exists($value_PulseRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_MobilePlatform_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Moving_instance47"]/RFDevice:supportsTuning
