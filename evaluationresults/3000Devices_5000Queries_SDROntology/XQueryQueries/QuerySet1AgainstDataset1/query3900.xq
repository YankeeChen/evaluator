...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:about)
    for $node_Jamming_0 in $node_ElectronicAttackSystem_0/RFDevice:supportsTransmitting
    let $value_Jamming_0 := fn:data($node_Jamming_0/@rdf:resource)
    where fn:exists($node_ElectronicAttackSystem_0) and fn:exists($value_Jamming_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicAttackSystem_0">{$value_ElectronicAttackSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("Jamming_0"))
