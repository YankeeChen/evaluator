...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SDR_0 in $node_Receiver_0/Nuvio:compositeOf
    let $value_SDR_0 := fn:data($node_SDR_0/@rdf:resource)
    where fn:exists($node_Receiver_0) and fn:exists($value_SDR_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_RadioComponent_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#RFFrontEnd_instance92"]/Nuvio:compositeOf
    let $value_RadioComponent_0 := fn:data($node_RadioComponent_0/@rdf:resource)
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("RadioComponent_0"))
