...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicSupportSystem_0 := fn:data($node_ElectronicSupportSystem_0/@rdf:about)
    for $node_Component_0 in $node_ElectronicSupportSystem_0/Nuvio:aggregateOf
    let $value_Component_0 := fn:data($node_Component_0/@rdf:resource)
    where fn:exists($value_Component_0) and fn:exists($node_ElectronicSupportSystem_0)
    return
      <xqllib:result>
