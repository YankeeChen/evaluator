...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SearchRadar_0 := fn:data($node_SearchRadar_0/@rdf:about)
    for $node_ContinuousWaveRadarModulation_0 in $node_SearchRadar_0/RFDevice:hasCapability
    let $value_ContinuousWaveRadarModulation_0 := fn:data($node_ContinuousWaveRadarModulation_0/@rdf:resource)
    where fn:exists($value_ContinuousWaveRadarModulation_0) and fn:exists($node_SearchRadar_0)
    return
      <xqllib:result>
