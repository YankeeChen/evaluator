...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN200_0 := fn:data($node_USRPN200_0/@rdf:about)
    for $node_Var0 in $node_USRPN200_0/Nuvio:partOf
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Quantity_0) and fn:exists($node_Var0) and $node_Quantity_0/RFDevice:hasMaxPowerConsumption/@rdf:resource = $value_Var0
    return
      <xqllib:result>
