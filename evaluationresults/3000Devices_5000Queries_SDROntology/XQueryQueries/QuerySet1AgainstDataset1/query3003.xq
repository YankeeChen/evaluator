...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_i_0 in $node_FPGA_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_FPGA_0 in $node_XC6SLX75FPGA_0/Nuvio:aggregateOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0) and fn:exists($node_XC6SLX75FPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="FPGA_0">{$value_FPGA_0}</xqllib:var>
