...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:about)
    for $node_Var0 in $node_USRPB200mini_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Var0 in $node_Var1/RFDevice:hasPulseRepetitionFrequency
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_4 :=  
    ...
    for $node_DataValue0 in $node_Var2/RFDevice:hasDoubleValue
  ...
  let $GRAPH_6 :=  
    for $node_Var1 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Quantity"]
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Var1"))
