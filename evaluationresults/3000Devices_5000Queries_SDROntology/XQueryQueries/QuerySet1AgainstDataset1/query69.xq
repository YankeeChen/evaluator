...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    for $node_RadarModulation_0 in $node_PulseDopplerRadar_0/RFDevice:supportsSpectrumSensing
    let $value_RadarModulation_0 := fn:data($node_RadarModulation_0/@rdf:resource)
    where fn:exists($node_PulseDopplerRadar_0) and fn:exists($value_RadarModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseDopplerRadar_0">{$value_PulseDopplerRadar_0}</xqllib:var>
