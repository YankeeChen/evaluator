...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    for $node_Process_0 in $node_PulseDopplerRadar_0/RFDevice:supportsJamming
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Process_0 in $node_Weight_0/RFDevice:hasMaxSensedPower
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($value_Process_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
