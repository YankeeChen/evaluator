...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:about)
    for $node_Submarine_0 in $node_ElectronicAttackSystem_0/RFDevice:isDeployedOn
    let $value_Submarine_0 := fn:data($node_Submarine_0/@rdf:resource)
    where fn:exists($node_ElectronicAttackSystem_0) and fn:exists($value_Submarine_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicAttackSystem_0">{$value_ElectronicAttackSystem_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_UnitOfMeasure_0/Nuvio:hasUnitOfMeasure
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) > -52610179)]
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("Submarine_0"))
