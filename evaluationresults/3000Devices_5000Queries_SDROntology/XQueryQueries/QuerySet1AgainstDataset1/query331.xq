...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Half_DuplexCommunicationsSystem_0 in $node_Entity_0/RFDevice:supportsJamming
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) < 1973505204)]
  ...
  let $GRAPH_12 := $GRAPH_11[(data(xqllib:var[@name="DataValue1"]) >= -1694964994)]
  let $GRAPH_13 := xqllib:and($GRAPH_10,$GRAPH_12,("Var0"))
  let $GRAPH_14 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_DataValue2 in $node_Var0/RFDevice:hasDoubleValue
  ...
  let $GRAPH_17 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_AnalogModulation_0) and fn:exists($node_Entity_0) and $node_AnalogModulation_0/Nuvio:hasSubprocess/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_19 := xqllib:optional($GRAPH_17,$GRAPH_18,("AnalogModulation_0"))
  let $GRAPH_20 := xqllib:and($GRAPH_16,$GRAPH_19,("Entity_0"))
  let $GRAPH_21 := xqllib:and($GRAPH_7,$GRAPH_20,("Entity_0"))
  let $GRAPH_22 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_InformationContent_0 in $node_Entity_0/Nuvio:represents
  ...
  let $GRAPH_23 := xqllib:and($GRAPH_21,$GRAPH_22,("Entity_0"))
  let $GRAPH_24 := xqllib:and($GRAPH_2,$GRAPH_23,("Entity_0","InformationContent_0"))
