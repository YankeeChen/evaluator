...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/SDR.owl#Freq_100MHz"]/RFDevice:hasGain
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Var0 in $node_Value_0/RFDevice:supportsTuning
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) <= -2060170412)]
  let $GRAPH_9 := xqllib:and($GRAPH_6,$GRAPH_8,("Value_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_5,$GRAPH_9,("Value_0"))
  let $GRAPH_11 := xqllib:optional($GRAPH_4,$GRAPH_10,("Var0"))
