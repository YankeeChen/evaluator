...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ModelX_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_ModelX_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_IEDTrigger_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#RadarFrequencyBand_instance14"]/RFDevice:hasFrequencyRange
