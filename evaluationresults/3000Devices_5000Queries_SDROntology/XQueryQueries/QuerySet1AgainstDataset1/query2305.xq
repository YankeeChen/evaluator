...
let $result :=  
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "FrequencyUnit_instance11"]/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_9 := $GRAPH_8[(data(xqllib:var[@name="DataValue0"]) < 565523848)]
  ...
  let $GRAPH_15 := xqllib:and($GRAPH_0,$GRAPH_14,("USRPB2xx_0"))
  return
    $GRAPH_15
return
