...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarWarningReceiver_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:resource)
    where fn:exists($node_MobilePlatform_0) and fn:exists($value_RadarWarningReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_MobilePlatform_0 in $doc_ObjectDescription[RFDevice:supportsSpectrumSensing/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Moving_instance47"]
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:about)
    where fn:exists($node_MobilePlatform_0)
    return
      <xqllib:result>
