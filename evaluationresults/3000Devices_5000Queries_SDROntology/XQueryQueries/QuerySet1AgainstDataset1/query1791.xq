...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:about)
    for $node_FPGA_0 in $node_USRPB200mini_i_0/Nuvio:aggregateOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($value_FPGA_0) and fn:exists($node_USRPB200mini_i_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_FPGA_0 in $node_DirectConversionReceiver_0/Nuvio:aggregateOf
    let $value_FPGA_0 := fn:data($node_FPGA_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_FPGA_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("FPGA_0"))
