...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_RadarJammer_0/Nuvio:aggregateOf
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:resource)
    where fn:exists($node_RadarJammer_0) and fn:exists($value_USRPB2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_RadarJammer_0 in $node_RFFrontEnd_0/Nuvio:partOf
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_RFFrontEnd_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_RFFrontEnd_0 in $node_Object_0/Nuvio:partOf
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_NoiseFigure_0) and fn:exists($node_Object_0) and $node_NoiseFigure_0/RFDevice:hasMaxPowerConsumption/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("NoiseFigure_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("Object_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_3,$GRAPH_10,("RFFrontEnd_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_2,$GRAPH_11,("RFFrontEnd_0"))
