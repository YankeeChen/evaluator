...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN200_0 in $node_Var0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_SpotJamming_0) and $node_SpotJamming_0/Nuvio:hasParticipant/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_SpotJamming_0 in $doc_ObjectDescription
    let $value_SpotJamming_0 := fn:data($node_SpotJamming_0/@rdf:about)
    for $node_Power_0 in $node_SpotJamming_0/RFDevice:hasPulseRepetitionInterval
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("SpotJamming_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Object_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("Var0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("USRPN200_0"))
  return
    $GRAPH_8
return
