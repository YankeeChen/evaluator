...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:about)
    for $node_Receiver_0 in $node_USRPN210_0/Nuvio:partOf
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:resource)
    where fn:exists($value_Receiver_0) and fn:exists($node_USRPN210_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Receiver_0 in $doc_ObjectDescription[RFDevice:hasProducer/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Producer_instance385"]
