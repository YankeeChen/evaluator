...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Spartan6FPGA_0/Nuvio:partOf
    let $value_USRPB200mini_0 := fn:data($node_USRPB200mini_0/@rdf:resource)
    where fn:exists($node_Spartan6FPGA_0) and fn:exists($value_USRPB200mini_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Spartan6FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Producer_instance231"]/RFDevice:hasProducer
