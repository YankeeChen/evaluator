...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ContinuousWaveRadar_0 in $node_ContinuousWaveRadarModulation_0/RFDevice:supportsSpectrumSensing
    let $value_ContinuousWaveRadar_0 := fn:data($node_ContinuousWaveRadar_0/@rdf:resource)
    where fn:exists($node_ContinuousWaveRadarModulation_0) and fn:exists($value_ContinuousWaveRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 := xqllib:and($GRAPH_1,$GRAPH_2,("ContinuousWaveRadarModulation_0"))
