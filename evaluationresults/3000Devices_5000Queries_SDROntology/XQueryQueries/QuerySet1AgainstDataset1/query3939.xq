...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN3xx_0 in $node_InstantaneousFrequencyMeasurementReceiver_0/Nuvio:aggregateOf
    let $value_USRPN3xx_0 := fn:data($node_USRPN3xx_0/@rdf:resource)
    where fn:exists($node_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($value_USRPN3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_Transceiving_0/RFDevice:hasCapability
    ...
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_Transceiving_0)
    return
      <xqllib:result>
        <xqllib:var name="InstantaneousFrequencyMeasurementReceiver_0">{$value_InstantaneousFrequencyMeasurementReceiver_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_Transceiving_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#FormationJamming"]
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:about)
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Transceiving_0 in $node_Entity_0/RFDevice:hasPulseRepetitionInterval
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_Transceiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#LinearRing"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_4,$GRAPH_9,("Transceiving_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_3,$GRAPH_10,("Transceiving_0"))
  let $GRAPH_12 := xqllib:and($GRAPH_2,$GRAPH_11,("InstantaneousFrequencyMeasurementReceiver_0"))
  let $GRAPH_13 := xqllib:and($GRAPH_1,$GRAPH_12,("InstantaneousFrequencyMeasurementReceiver_0"))
  let $GRAPH_14 := xqllib:and($GRAPH_0,$GRAPH_13,("USRPN3xx_0"))
  return
    $GRAPH_14
return
