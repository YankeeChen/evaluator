...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CommunicationsSystem_0 in $node_Transceiving_0/RFDevice:supportsTransmitting
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:resource)
    where fn:exists($value_CommunicationsSystem_0) and fn:exists($node_Transceiving_0)
    return
      <xqllib:result>
        <xqllib:var name="CommunicationsSystem_0">{$value_CommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Transceiving_0 in $node_AntennaPolarization_0/Nuvio:hasProcessQuality
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:resource)
    where fn:exists($node_AntennaPolarization_0) and fn:exists($value_Transceiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("AntennaPolarization_0"))
  let $GRAPH_6 := xqllib:optional($GRAPH_2,$GRAPH_5,("Transceiving_0"))
