...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CR_0 in $node_Jammer_0/Nuvio:aggregateOf
    let $value_CR_0 := fn:data($node_CR_0/@rdf:resource)
    where fn:exists($value_CR_0) and fn:exists($node_Jammer_0)
    return
      <xqllib:result>
        <xqllib:var name="CR_0">{$value_CR_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Jammer_0 in $node_CommunicationsJamming_0/RFDevice:supportsTuning
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($node_CommunicationsJamming_0) and fn:exists($value_Jammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_CommunicationsJamming_0 in $node_Attribute_0/RFDevice:hasMaxSensedPower
    let $value_CommunicationsJamming_0 := fn:data($node_CommunicationsJamming_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_CommunicationsJamming_0)
    return
      <xqllib:result>
