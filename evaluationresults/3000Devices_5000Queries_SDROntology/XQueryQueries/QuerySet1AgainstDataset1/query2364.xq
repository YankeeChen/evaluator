...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_Transmitter_0/Nuvio:compositeOf
    let $value_USRPN2xx_0 := fn:data($node_USRPN2xx_0/@rdf:resource)
    where fn:exists($node_Transmitter_0) and fn:exists($value_USRPN2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Transmitter_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Transceiving_instance116"]/RFDevice:supportsJamming
