...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:about)
    for $node_CoverJamming_0 in $node_ElectronicAttackSystem_0/RFDevice:supportsTransmitting
    let $value_CoverJamming_0 := fn:data($node_CoverJamming_0/@rdf:resource)
    where fn:exists($value_CoverJamming_0) and fn:exists($node_ElectronicAttackSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_CoverJamming_0 in $node_Quantity_0/RFDevice:hasPulseWidth
    let $value_CoverJamming_0 := fn:data($node_CoverJamming_0/@rdf:resource)
