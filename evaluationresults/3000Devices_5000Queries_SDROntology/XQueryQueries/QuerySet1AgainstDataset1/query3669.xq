...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN2xx_0 in $node_Weight_0/RFDevice:hasWeight
    let $value_USRPN2xx_0 := fn:data($node_USRPN2xx_0/@rdf:resource)
    where fn:exists($value_USRPN2xx_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPN2xx_0">{$value_USRPN2xx_0}</xqllib:var>
  ...
  let $GRAPH_7 :=  
    ...
    for $node_Entity_0 in $node_Role_0/Nuvio:hasParticipant
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_Role_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_6,$GRAPH_11,("Entity_0","InformationRepresentation_0"))
