...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_PulseDopplerRadar_0 in $doc_ObjectDescription[RFDevice:isDeployedOn/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Aircraft_instance31"]
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    where fn:exists($node_PulseDopplerRadar_0)
    return
      <xqllib:result>
