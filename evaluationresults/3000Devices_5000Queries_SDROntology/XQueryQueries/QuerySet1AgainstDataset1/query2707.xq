...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ContinuousWaveRadar_0 in $node_Entity_0/RFDevice:hasCapability
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_SweepJamming_0) and $node_SweepJamming_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#USRPX300_instance51"]/Nuvio:relevantIndividual
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Entity_0","SweepJamming_0"))
  let $GRAPH_8 :=  
