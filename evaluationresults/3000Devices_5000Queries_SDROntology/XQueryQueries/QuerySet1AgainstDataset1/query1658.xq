...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_InboundRangeGatePullOffJamming_0 in $node_Object_0/RFDevice:hasCapability
    let $value_InboundRangeGatePullOffJamming_0 := fn:data($node_InboundRangeGatePullOffJamming_0/@rdf:resource)
    where fn:exists($value_InboundRangeGatePullOffJamming_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
