...
let $result :=  
  ...
  let $GRAPH_5 := $GRAPH_4[not((data(xqllib:var[@name="DataValue0"]) > xs:decimal("10")))]
  let $GRAPH_6 :=  
  ...
  let $GRAPH_7 :=  
    for $node_Var2 in $doc_ObjectDescription
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
    for $node_DataValue1 in $node_Var2/RFDevice:hasDecimalValue
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue1"]) < -96.25)]
  let $GRAPH_9 := xqllib:and($GRAPH_6,$GRAPH_8,("Var2"))
  let $GRAPH_10 := xqllib:and($GRAPH_5,$GRAPH_9,("Var2"))
  let $GRAPH_11 := xqllib:and($GRAPH_3,$GRAPH_10,("Var2"))
  let $GRAPH_12 :=  
  ...
  let $GRAPH_16 := xqllib:optional($GRAPH_0,$GRAPH_15,("USRPB2xxmini_0"))
  return
    $GRAPH_16
return
