...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Receiving_0 in $doc_ObjectDescription
    let $value_Receiving_0 := fn:data($node_Receiving_0/@rdf:about)
    for $node_RadarFrequencyBand_0 in $node_Receiving_0/RFDevice:hasFrequencyAccuracy
    let $value_RadarFrequencyBand_0 := fn:data($node_RadarFrequencyBand_0/@rdf:resource)
    where fn:exists($value_RadarFrequencyBand_0) and fn:exists($node_Receiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Attribute_0 in $node_RadarFrequencyBand_0/RFDevice:hasMax
