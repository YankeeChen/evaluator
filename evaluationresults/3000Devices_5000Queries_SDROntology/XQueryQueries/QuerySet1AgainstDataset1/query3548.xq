...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN3xx_0 in $node_XilinxZynq7100SoC_0/Nuvio:aggregateOf
    let $value_USRPN3xx_0 := fn:data($node_USRPN3xx_0/@rdf:resource)
    where fn:exists($value_USRPN3xx_0) and fn:exists($node_XilinxZynq7100SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPN3xx_0">{$value_USRPN3xx_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_XilinxZynq7100SoC_0 in $node_DirectConversionReceiver_0/Nuvio:compositeOf
    let $value_XilinxZynq7100SoC_0 := fn:data($node_XilinxZynq7100SoC_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_XilinxZynq7100SoC_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:about)
    for $node_Entity_0 in $node_DirectConversionReceiver_0/RFDevice:hasNoiseFigure
  ...
  let $GRAPH_11 := xqllib:and($GRAPH_9,$GRAPH_10,("Attribute_0"))
  ...
  let $GRAPH_18 := xqllib:optional($GRAPH_4,$GRAPH_17,("DirectConversionReceiver_0"))
  let $GRAPH_19 := xqllib:and($GRAPH_3,$GRAPH_18,("DirectConversionReceiver_0"))
  let $GRAPH_20 := xqllib:optional($GRAPH_2,$GRAPH_19,("XilinxZynq7100SoC_0"))
  let $GRAPH_21 := xqllib:and($GRAPH_1,$GRAPH_20,("XilinxZynq7100SoC_0"))
