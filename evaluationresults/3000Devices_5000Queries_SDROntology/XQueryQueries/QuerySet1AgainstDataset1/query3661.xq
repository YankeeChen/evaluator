...
let $result :=  
  ...
  let $GRAPH_4 := $GRAPH_3[(data(xqllib:var[@name="DataValue0"]) > xs:decimal("0"))]
  let $GRAPH_5 :=  
    for $node_Var1 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/RFDevice.owl#Meter"]/Nuvio:hasUnitOfMeasure
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_2,$GRAPH_6,("Var1"))
  let $GRAPH_8 := xqllib:and($GRAPH_1,$GRAPH_7,("Var0"))
  let $GRAPH_9 := xqllib:and($GRAPH_0,$GRAPH_8,("USRPB2xx_0"))
  return
    $GRAPH_9
return
