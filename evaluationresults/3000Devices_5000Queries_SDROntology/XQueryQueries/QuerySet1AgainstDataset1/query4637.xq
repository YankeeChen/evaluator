...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_CR_0 in $node_DirectConversionReceiver_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:about)
    for $node_Receiver_0 in $node_DirectConversionReceiver_0/Nuvio:aggregateOf
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_Receiver_0)
    return
      <xqllib:result>
        <xqllib:var name="DirectConversionReceiver_0">{$value_DirectConversionReceiver_0}</xqllib:var>
