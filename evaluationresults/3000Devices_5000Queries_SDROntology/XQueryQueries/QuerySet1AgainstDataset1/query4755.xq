...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RFSystem_0 in $node_Jammer_0/Nuvio:aggregateOf
    let $value_RFSystem_0 := fn:data($node_RFSystem_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_RFSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Jammer_0 in $doc_ObjectDescription
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:about)
    for $node_Receiver_0 in $node_Jammer_0/Nuvio:partOf
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_Receiver_0)
    return
      <xqllib:result>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:about)
    for $node_Quantity_0 in $node_Receiver_0/RFDevice:hasNoiseFigure
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:resource)
    where fn:exists($value_Quantity_0) and fn:exists($node_Receiver_0)
    return
      <xqllib:result>
