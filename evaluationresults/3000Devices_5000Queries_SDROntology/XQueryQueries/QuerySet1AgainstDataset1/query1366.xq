...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Object_0 in $node_Process_0/RFDevice:supportsJamming
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_Voltage_0 in $node_Process_0/RFDevice:hasTransmitPowerRange
    let $value_Voltage_0 := fn:data($node_Voltage_0/@rdf:resource)
    where fn:exists($node_Process_0) and fn:exists($value_Voltage_0)
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
