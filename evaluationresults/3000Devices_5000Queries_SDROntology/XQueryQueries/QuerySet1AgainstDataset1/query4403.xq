...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseDopplerRadar_0 in $node_ContinuousWaveRadarModulation_0/RFDevice:supportsJamming
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:resource)
    where fn:exists($node_ContinuousWaveRadarModulation_0) and fn:exists($value_PulseDopplerRadar_0)
    return
      <xqllib:result>
