...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    for $node_Object_0 in $node_PulseDopplerRadar_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Attribute_0) and fn:exists($node_Object_0) and $node_Attribute_0/Nuvio:hasParticipant/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
