...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:about)
    for $node_Entity_0 in $node_RadarSystem_0/RFDevice:hasProducer
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Jamming_0 in $node_Entity_0/RFDevice:supportsJamming
    let $value_Jamming_0 := fn:data($node_Jamming_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_Jamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
