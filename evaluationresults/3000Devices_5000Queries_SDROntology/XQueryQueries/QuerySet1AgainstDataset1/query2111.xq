...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Jammer_0 in $node_Process_0/RFDevice:hasCapability
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    for $node_Value_0 in $node_Process_0/Nuvio:hasSubprocess
    let $value_Value_0 := fn:data($node_Value_0/@rdf:resource)
    where fn:exists($node_Process_0) and fn:exists($value_Value_0)
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_VoltageUnit_0 in $node_Value_0/Nuvio:hasUnitOfMeasure
    let $value_VoltageUnit_0 := fn:data($node_VoltageUnit_0/@rdf:resource)
    where fn:exists($node_Value_0) and fn:exists($value_VoltageUnit_0)
    return
      <xqllib:result>
        <xqllib:var name="Value_0">{$value_Value_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("VoltageUnit_0"))
