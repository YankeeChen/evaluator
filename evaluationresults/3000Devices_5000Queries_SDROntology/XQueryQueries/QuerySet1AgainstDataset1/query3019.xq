...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Half_DuplexCommunicationsSystem_0 in $node_Process_0/RFDevice:supportsJamming
  ...
  let $GRAPH_2 :=  
    for $node_AbstractCurveSegment_0 in $doc_ObjectDescription
    let $value_AbstractCurveSegment_0 := fn:data($node_AbstractCurveSegment_0/@rdf:about)
    for $node_Process_0 in $node_AbstractCurveSegment_0/RFDevice:hasTarget
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($node_AbstractCurveSegment_0) and fn:exists($value_Process_0)
    return
      <xqllib:result>
        <xqllib:var name="AbstractCurveSegment_0">{$value_AbstractCurveSegment_0}</xqllib:var>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_3 :=  
    for $node_AbstractCurveSegment_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#AbstractCurveSegment"]
    let $value_AbstractCurveSegment_0 := fn:data($node_AbstractCurveSegment_0/@rdf:about)
    where fn:exists($node_AbstractCurveSegment_0)
    return
      <xqllib:result>
        <xqllib:var name="AbstractCurveSegment_0">{$value_AbstractCurveSegment_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("AbstractCurveSegment_0"))
