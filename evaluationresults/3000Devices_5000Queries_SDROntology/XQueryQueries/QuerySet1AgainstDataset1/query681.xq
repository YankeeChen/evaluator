...
let $result :=  
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_FrequencyShiftKeyingModulation_0 in $node_Object_0/RFDevice:supportsSpectrumSensing
    let $value_FrequencyShiftKeyingModulation_0 := fn:data($node_FrequencyShiftKeyingModulation_0/@rdf:resource)
    where fn:exists($value_FrequencyShiftKeyingModulation_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
