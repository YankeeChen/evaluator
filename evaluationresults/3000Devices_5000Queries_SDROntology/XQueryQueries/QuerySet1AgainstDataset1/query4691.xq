...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_Process_0/RFDevice:supportsTuning
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:resource)
    where fn:exists($node_Process_0) and fn:exists($value_SurveillanceRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_Length_0) and fn:exists($node_Process_0) and $node_Length_0/RFDevice:hasMaxSensedPower/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("Value_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_5,$GRAPH_8,("Var0"))
  let $GRAPH_10 := ($GRAPH_4,$GRAPH_9)
  let $GRAPH_11 := xqllib:and($GRAPH_2,$GRAPH_10,("Length_0"))
