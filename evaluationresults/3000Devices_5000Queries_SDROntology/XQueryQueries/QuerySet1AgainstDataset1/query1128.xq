...
let $result :=  
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription[RFDevice:hasMaxOutputPower/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#Power_45W"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) = -452364511)]
  let $GRAPH_9 :=  
