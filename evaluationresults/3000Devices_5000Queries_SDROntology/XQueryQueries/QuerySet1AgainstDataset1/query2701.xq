...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_CrossEyeJamming_0/RFDevice:supportsJamming
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:resource)
    where fn:exists($node_CrossEyeJamming_0) and fn:exists($value_JammingSystem_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_CrossEyeJamming_0 in $node_Quantity_0/RFDevice:hasPulseRepetitionInterval
    let $value_CrossEyeJamming_0 := fn:data($node_CrossEyeJamming_0/@rdf:resource)
    where fn:exists($value_CrossEyeJamming_0) and fn:exists($node_Quantity_0)
    return
      <xqllib:result>
        <xqllib:var name="CrossEyeJamming_0">{$value_CrossEyeJamming_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("CrossEyeJamming_0"))
