...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE313_0 in $node_XilinxZynq7020SoC_0/Nuvio:compositeOf
    let $value_USRPE313_0 := fn:data($node_USRPE313_0/@rdf:resource)
    where fn:exists($value_USRPE313_0) and fn:exists($node_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPE313_0">{$value_USRPE313_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_XilinxZynq7020SoC_0 in $node_Series7FPGA_0/Nuvio:compositeOf
    let $value_XilinxZynq7020SoC_0 := fn:data($node_XilinxZynq7020SoC_0/@rdf:resource)
    where fn:exists($node_Series7FPGA_0) and fn:exists($value_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_Series7FPGA_0 in $doc_ObjectDescription[Nuvio:aggregateOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#XC7K410TFPGA_instance16"]
    let $value_Series7FPGA_0 := fn:data($node_Series7FPGA_0/@rdf:about)
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("Series7FPGA_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("XilinxZynq7020SoC_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_1,$GRAPH_6,("XilinxZynq7020SoC_0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("USRPE313_0"))
  return
    $GRAPH_8
return
