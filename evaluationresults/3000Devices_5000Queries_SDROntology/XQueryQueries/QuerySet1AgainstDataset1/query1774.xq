...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_Object_0/RFDevice:isProducerOf
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($value_ElectronicAttackSystem_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicAttackSystem_0">{$value_ElectronicAttackSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_CommunicationsJamming_0) and fn:exists($node_Object_0) and $node_CommunicationsJamming_0/RFDevice:supportsTuning/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_CommunicationsJamming_0 in $node_Entity_0/Nuvio:hasProcessQuantity
  ...
  let $GRAPH_7 :=  
    ...
    for $node_Entity_0 in $node_Ratio_0/RFDevice:hasMaxOutputPower
  ...
  let $GRAPH_9 :=  
    ...
    for $node_Value_0 in $node_FrequencyUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Value_0 := fn:data($node_Value_0/@rdf:resource)
    where fn:exists($node_FrequencyUnit_0) and fn:exists($value_Value_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_11 := $GRAPH_10[(data(xqllib:var[@name="DataValue0"]) >= -1907021689)]
  let $GRAPH_12 := xqllib:and($GRAPH_9,$GRAPH_11,("Value_0"))
  let $GRAPH_13 :=  
  ...
  let $GRAPH_14 := xqllib:and($GRAPH_12,$GRAPH_13,("FrequencyUnit_0"))
  let $GRAPH_15 := ($GRAPH_8,$GRAPH_14)
  let $GRAPH_16 := xqllib:and($GRAPH_4,$GRAPH_15,("Entity_0","Value_0"))
  let $GRAPH_17 := xqllib:and($GRAPH_3,$GRAPH_16,("Entity_0"))
  let $GRAPH_18 := xqllib:and($GRAPH_2,$GRAPH_17,("CommunicationsJamming_0"))
