...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_Attribute_0 in $node_SurveillanceRadar_0/RFDevice:hasMaxPowerConsumption
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:resource)
    where fn:exists($value_Attribute_0) and fn:exists($node_SurveillanceRadar_0)
    return
      <xqllib:result>
