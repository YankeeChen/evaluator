...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:about)
    for $node_Object_0 in $node_Half_DuplexCommunicationsSystem_0/RFDevice:isProducerOf
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($node_Half_DuplexCommunicationsSystem_0) and fn:exists($value_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="Half_DuplexCommunicationsSystem_0">{$value_Half_DuplexCommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_Value_0) and $node_Value_0/Nuvio:participatesIn/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    for $node_InformationUnit_0 in $node_Value_0/Nuvio:hasUnitOfMeasure
    let $value_InformationUnit_0 := fn:data($node_InformationUnit_0/@rdf:resource)
    where fn:exists($value_InformationUnit_0) and fn:exists($node_Value_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) < 189492389)]
  let $GRAPH_6 :=  
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_5,$GRAPH_6,())
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,("InformationUnit_0","Value_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("Value_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("Object_0"))
  let $GRAPH_11 := xqllib:and($GRAPH_0,$GRAPH_10,("Half_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_11
return
