...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_RadarJammer_0 in $doc_ObjectDescription[RFDevice:hasFrequencyRange/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#RadarFrequencyBand_instance112"]
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:about)
    where fn:exists($node_RadarJammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("RadarJammer_0"))
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("RadarJammer_0"))
  let $GRAPH_6 := xqllib:optional($GRAPH_0,$GRAPH_5,("BistaticRadar_0"))
  return
    $GRAPH_6
return
