...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_XC3SD1800AFPGA_0 in $node_Entity_0/Nuvio:compositeOf
  ...
  let $GRAPH_14 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_CubicSpline_0) and fn:exists($node_Situation_0) and $node_CubicSpline_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
  ...
  let $GRAPH_16 := xqllib:and($GRAPH_14,$GRAPH_15,("CubicSpline_0"))
  let $GRAPH_17 := xqllib:and($GRAPH_13,$GRAPH_16,("Situation_0"))
