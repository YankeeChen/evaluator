...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarSystem_0 := fn:data($node_RadarSystem_0/@rdf:about)
    for $node_SuperheterodyneReceiver_0 in $node_RadarSystem_0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_SuperheterodyneReceiver_0 in $node_PhasedArrayAntenna_0/Nuvio:partOf
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($node_PhasedArrayAntenna_0) and fn:exists($value_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
