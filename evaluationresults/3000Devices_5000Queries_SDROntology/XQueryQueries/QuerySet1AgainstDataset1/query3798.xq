...
let $result :=  
  ...
  let $GRAPH_5 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_LineString_0) and fn:exists($node_Var0) and $node_LineString_0/Nuvio:relevantIndividual/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    for $node_LineString_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#LineString"]
    let $value_LineString_0 := fn:data($node_LineString_0/@rdf:about)
