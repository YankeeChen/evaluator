...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_Object_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_AutomaticGainControl_0) and fn:exists($node_Object_0) and $node_AutomaticGainControl_0/Nuvio:hasParticipant/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
