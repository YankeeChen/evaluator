...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicSupportSystem_0 := fn:data($node_ElectronicSupportSystem_0/@rdf:about)
    for $node_Platform_0 in $node_ElectronicSupportSystem_0/RFDevice:isDeployedOn
    let $value_Platform_0 := fn:data($node_Platform_0/@rdf:resource)
    where fn:exists($node_ElectronicSupportSystem_0) and fn:exists($value_Platform_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicSupportSystem_0">{$value_ElectronicSupportSystem_0}</xqllib:var>
