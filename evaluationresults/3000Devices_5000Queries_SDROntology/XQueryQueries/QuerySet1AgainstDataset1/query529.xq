...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_MannedPlatform_0 in $doc_ObjectDescription
    let $value_MannedPlatform_0 := fn:data($node_MannedPlatform_0/@rdf:about)
    for $node_Half_DuplexCommunicationsSystem_0 in $node_MannedPlatform_0/RFDevice:isDeployedOn
    let $value_Half_DuplexCommunicationsSystem_0 := fn:data($node_Half_DuplexCommunicationsSystem_0/@rdf:resource)
    where fn:exists($value_Half_DuplexCommunicationsSystem_0) and fn:exists($node_MannedPlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="Half_DuplexCommunicationsSystem_0">{$value_Half_DuplexCommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("Half_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_4
return
