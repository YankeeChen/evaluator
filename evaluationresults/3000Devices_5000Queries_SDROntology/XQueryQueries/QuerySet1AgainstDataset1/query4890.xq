...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_Object_0/Nuvio:aggregateOf
  ...
  let $GRAPH_3 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_RadarJamming_0 in $node_Object_0/RFDevice:supportsTuning
  ...
  let $GRAPH_4 :=  
    for $node_Object_0 in $doc_ObjectDescription
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_RadarJamming_0 in $node_Object_0/RFDevice:hasTarget
    let $value_RadarJamming_0 := fn:data($node_RadarJamming_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_RadarJamming_0)
    return
      <xqllib:result>
