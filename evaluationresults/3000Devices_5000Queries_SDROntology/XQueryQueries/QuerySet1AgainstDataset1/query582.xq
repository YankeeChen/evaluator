...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_Var0/RFDevice:supportsTuning
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Entity_0 in $node_XilinxZynq7020SoC_0/Nuvio:relevantIndividual
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_XilinxZynq7020SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_8,$GRAPH_9,("Process_0"))
