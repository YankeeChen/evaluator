...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[RFDevice:hasPulseRepetitionInterval/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Instance2076"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:optional($GRAPH_0,$GRAPH_5,("CR_0"))
  return
    $GRAPH_6
return
