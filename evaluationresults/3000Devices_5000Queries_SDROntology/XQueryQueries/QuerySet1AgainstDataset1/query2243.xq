...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB210_0 := fn:data($node_USRPB210_0/@rdf:about)
    for $node_XilinxFPGA_0 in $node_USRPB210_0/Nuvio:partOf
    let $value_XilinxFPGA_0 := fn:data($node_XilinxFPGA_0/@rdf:resource)
    where fn:exists($node_USRPB210_0) and fn:exists($value_XilinxFPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB210_0">{$value_USRPB210_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Entity_0 in $node_XilinxFPGA_0/Nuvio:aggregateOf
