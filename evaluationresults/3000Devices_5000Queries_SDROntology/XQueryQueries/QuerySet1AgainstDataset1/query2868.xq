...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE312_0 in $node_Receiver_0/Nuvio:aggregateOf
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:resource)
    where fn:exists($node_Receiver_0) and fn:exists($value_USRPE312_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_Receiver_0 in $doc_ObjectDescription
    let $value_Receiver_0 := fn:data($node_Receiver_0/@rdf:about)
    for $node_QuantityRange_0 in $node_Receiver_0/RFDevice:hasFrequencyRange
    let $value_QuantityRange_0 := fn:data($node_QuantityRange_0/@rdf:resource)
    where fn:exists($value_QuantityRange_0) and fn:exists($node_Receiver_0)
    return
      <xqllib:result>
