...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xx_0 in $node_Jammer_0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    for $node_Jammer_0 in $doc_ObjectDescription
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:about)
    for $node_RadioFrequencyBand_0 in $node_Jammer_0/RFDevice:hasFrequencyRange
    let $value_RadioFrequencyBand_0 := fn:data($node_RadioFrequencyBand_0/@rdf:resource)
    where fn:exists($node_Jammer_0) and fn:exists($value_RadioFrequencyBand_0)
    return
      <xqllib:result>
        <xqllib:var name="Jammer_0">{$value_Jammer_0}</xqllib:var>
