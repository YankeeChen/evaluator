...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE3xx_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_USRPE3xx_0 := fn:data($node_USRPE3xx_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_USRPE3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_IEDTrigger_0 in $node_ADC_0/Nuvio:aggregateOf
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:resource)
    where fn:exists($node_ADC_0) and fn:exists($value_IEDTrigger_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_ADC_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Producer_instance495"]/RFDevice:hasProducer
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("ADC_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("ADC_0"))
