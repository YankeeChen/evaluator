...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN210_0 in $node_Weight_0/RFDevice:hasHeight
    let $value_USRPN210_0 := fn:data($node_USRPN210_0/@rdf:resource)
    where fn:exists($value_USRPN210_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPN210_0">{$value_USRPN210_0}</xqllib:var>
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) != xs:double("-2.760994"))]
  ...
  let $GRAPH_14 := xqllib:and($GRAPH_8,$GRAPH_13,("Entity_0","UnitOfMeasure_0"))
  ...
  let $GRAPH_20 := xqllib:and($GRAPH_1,$GRAPH_19,("Weight_0"))
