...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_IEDJammer_0 in $node_Transceiver_0/Nuvio:compositeOf
    let $value_IEDJammer_0 := fn:data($node_IEDJammer_0/@rdf:resource)
    where fn:exists($value_IEDJammer_0) and fn:exists($node_Transceiver_0)
    return
      <xqllib:result>
        <xqllib:var name="IEDJammer_0">{$value_IEDJammer_0}</xqllib:var>
  ...
  let $GRAPH_7 :=  
    ...
    for $node_Transceiver_0 in $node_FPGA_0/Nuvio:partOf
    let $value_Transceiver_0 := fn:data($node_Transceiver_0/@rdf:resource)
    where fn:exists($node_FPGA_0) and fn:exists($value_Transceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("FPGA_0"))
  let $GRAPH_10 := ($GRAPH_6,$GRAPH_9)
  let $GRAPH_11 := xqllib:and($GRAPH_4,$GRAPH_10,("Transceiver_0"))
  let $GRAPH_12 := ($GRAPH_3,$GRAPH_11)
  let $GRAPH_13 := xqllib:and($GRAPH_1,$GRAPH_12,("IEDJammer_0"))
  let $GRAPH_14 := xqllib:optional($GRAPH_0,$GRAPH_13,("CR_0"))
  return
    $GRAPH_14
return
