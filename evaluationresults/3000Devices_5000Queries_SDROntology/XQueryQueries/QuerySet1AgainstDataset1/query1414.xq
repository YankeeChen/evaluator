...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE312_0 in $node_RadioComponent_0/Nuvio:partOf
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:resource)
    where fn:exists($node_RadioComponent_0) and fn:exists($value_USRPE312_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadioComponent_0 in $node_Producer_0/RFDevice:hasProducer
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("RadioComponent_0"))
