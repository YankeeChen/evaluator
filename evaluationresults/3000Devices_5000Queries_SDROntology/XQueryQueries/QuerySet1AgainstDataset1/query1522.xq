...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_SelfProtectionJamming_0/RFDevice:supportsTransmitting
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($value_ElectronicAttackSystem_0) and fn:exists($node_SelfProtectionJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicAttackSystem_0">{$value_ElectronicAttackSystem_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_SelfProtectionJamming_0 in $node_Entity_0/Nuvio:hasProcessQuantity
  ...
  let $GRAPH_10 := $GRAPH_9[(data(xqllib:var[@name="DataValue0"]) < 1293342964)]
  ...
  let $GRAPH_14 := xqllib:and($GRAPH_1,$GRAPH_13,("SelfProtectionJamming_0"))
