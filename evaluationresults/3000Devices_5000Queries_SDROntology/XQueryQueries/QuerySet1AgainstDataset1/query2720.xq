...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_Quantity_0 in $node_SurveillanceRadar_0/RFDevice:hasMaxPowerConsumption
    let $value_Quantity_0 := fn:data($node_Quantity_0/@rdf:resource)
    where fn:exists($value_Quantity_0) and fn:exists($node_SurveillanceRadar_0)
    return
      <xqllib:result>
