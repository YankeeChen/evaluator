...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX3xx_0 in $node_RadarJammer_0/Nuvio:partOf
    let $value_USRPX3xx_0 := fn:data($node_USRPX3xx_0/@rdf:resource)
    where fn:exists($node_RadarJammer_0) and fn:exists($value_USRPX3xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_RadarJammer_0 in $node_Filter_0/Nuvio:compositeOf
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_Filter_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
