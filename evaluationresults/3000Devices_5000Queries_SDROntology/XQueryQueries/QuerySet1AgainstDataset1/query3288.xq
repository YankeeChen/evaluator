...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicAttackSystem_0 in $node_MobilePlatform_0/RFDevice:isDeployedOn
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:resource)
    where fn:exists($value_ElectronicAttackSystem_0) and fn:exists($node_MobilePlatform_0)
    return
      <xqllib:result>
        <xqllib:var name="ElectronicAttackSystem_0">{$value_ElectronicAttackSystem_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_InformationContent_0 in $node_Entity_1/Nuvio:represents
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("MobilePlatform_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_1,$GRAPH_9,("MobilePlatform_0"))
