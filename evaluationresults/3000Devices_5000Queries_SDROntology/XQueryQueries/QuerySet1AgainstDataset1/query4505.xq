...
let $result :=  
  ...
  let $GRAPH_10 :=  
    for $node_RadarWarningReceiver_1 in $doc_ObjectDescription
    let $value_RadarWarningReceiver_1 := fn:data($node_RadarWarningReceiver_1/@rdf:about)
    for $node_Process_0 in $node_RadarWarningReceiver_1/Nuvio:hasParticipant
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($value_Process_0) and fn:exists($node_RadarWarningReceiver_1)
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
        <xqllib:var name="RadarWarningReceiver_1">{$value_RadarWarningReceiver_1}</xqllib:var>
      </xqllib:result>
  let $GRAPH_11 := xqllib:and($GRAPH_9,$GRAPH_10,("Process_0"))
  let $GRAPH_12 := xqllib:optional($GRAPH_8,$GRAPH_11,("Var0"))
