...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Value_0 in $node_ElectricChargeUnit_0/Nuvio:hasUnitOfMeasure
    let $value_Value_0 := fn:data($node_Value_0/@rdf:resource)
    where fn:exists($node_ElectricChargeUnit_0) and fn:exists($value_Value_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := $GRAPH_5[(data(xqllib:var[@name="DataValue0"]) != 1222361429)]
  let $GRAPH_7 :=  
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_2,$GRAPH_9,("Value_0"))
