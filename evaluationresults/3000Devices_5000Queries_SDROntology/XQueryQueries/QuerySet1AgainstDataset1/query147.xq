...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB2xxmini_0 in $node_Entity_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_4 := $GRAPH_3[(not((data(xqllib:var[@name="DataValue0"]) >= xs:decimal("-180"))) and (data(xqllib:var[@name="DataValue0"]) <= xs:decimal("180")))]
