...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RFSystem_0 in $node_Weight_0/RFDevice:hasChargingTime
    let $value_RFSystem_0 := fn:data($node_RFSystem_0/@rdf:resource)
    where fn:exists($value_RFSystem_0) and fn:exists($node_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="RFSystem_0">{$value_RFSystem_0}</xqllib:var>
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) <= xs:double("72.6685"))]
  ...
  let $GRAPH_9 := xqllib:optional($GRAPH_0,$GRAPH_8,("RFSystem_0"))
  return
    $GRAPH_9
return
