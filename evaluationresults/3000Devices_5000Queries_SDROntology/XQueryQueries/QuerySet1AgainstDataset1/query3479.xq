...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_JammingSystem_0 in $node_Jammer_0/Nuvio:compositeOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Jammer_0 in $node_BlinkingJamming_0/RFDevice:supportsTuning
    let $value_Jammer_0 := fn:data($node_Jammer_0/@rdf:resource)
    where fn:exists($node_BlinkingJamming_0) and fn:exists($value_Jammer_0)
    return
      <xqllib:result>
