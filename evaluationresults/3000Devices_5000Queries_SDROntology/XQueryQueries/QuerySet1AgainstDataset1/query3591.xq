...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ModelX_0 in $node_SuperheterodyneReceiver_0/Nuvio:compositeOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_SuperheterodyneReceiver_0 in $node_DipoleAntenna_0/Nuvio:aggregateOf
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($node_DipoleAntenna_0) and fn:exists($value_SuperheterodyneReceiver_0)
    return
      <xqllib:result>
