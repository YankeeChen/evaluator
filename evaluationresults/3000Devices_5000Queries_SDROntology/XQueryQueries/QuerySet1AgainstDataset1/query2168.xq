...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200_210_0 := fn:data($node_USRPB200_210_0/@rdf:about)
    for $node_Var0 in $node_USRPB200_210_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "Power_instance49"]/RFDevice:hasLevel/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
