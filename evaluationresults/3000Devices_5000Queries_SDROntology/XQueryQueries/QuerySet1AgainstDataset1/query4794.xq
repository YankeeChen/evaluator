...
let $result :=  
  ...
  let $GRAPH_10 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0) and $doc_ObjectDescription[@rdf:ID = "Instance3322"]/RFDevice:supportsSpectrumSensing/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
  ...
  let $GRAPH_21 := xqllib:and($GRAPH_0,$GRAPH_20,("PulseRadar_0"))
  return
    $GRAPH_21
return
