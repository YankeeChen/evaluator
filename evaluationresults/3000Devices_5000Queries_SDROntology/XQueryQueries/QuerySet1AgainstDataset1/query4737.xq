...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Entity_0 in $node_FormationJamming_0/Nuvio:subprocessOf
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($value_Entity_0) and fn:exists($node_FormationJamming_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_5 :=  
    for $node_FormationJamming_0 in $doc_ObjectDescription
    let $value_FormationJamming_0 := fn:data($node_FormationJamming_0/@rdf:about)
    for $node_Power_0 in $node_FormationJamming_0/RFDevice:hasTransmitPowerRange
    let $value_Power_0 := fn:data($node_Power_0/@rdf:resource)
    where fn:exists($node_FormationJamming_0) and fn:exists($value_Power_0)
    return
      <xqllib:result>
        <xqllib:var name="FormationJamming_0">{$value_FormationJamming_0}</xqllib:var>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_3,$GRAPH_8,("FormationJamming_0"))
