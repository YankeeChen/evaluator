...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_Process_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Process_0 := fn:data($node_Process_0/@rdf:about)
    where fn:exists($node_MultiPoint_0) and fn:exists($node_Process_0) and $node_MultiPoint_0/Nuvio:participatesIn/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_MultiPoint_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#MultiPoint"]
    let $value_MultiPoint_0 := fn:data($node_MultiPoint_0/@rdf:about)
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("MultiPoint_0"))
