...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_PulseDopplerRadar_0/Nuvio:compositeOf
    ...
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_PulseDopplerRadar_0)
    return
      <xqllib:result>
