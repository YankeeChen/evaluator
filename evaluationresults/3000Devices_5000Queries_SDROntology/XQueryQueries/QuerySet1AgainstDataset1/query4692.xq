...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ContinuousWaveRadar_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_ContinuousWaveRadar_0 := fn:data($node_ContinuousWaveRadar_0/@rdf:resource)
    where fn:exists($value_ContinuousWaveRadar_0) and fn:exists($node_IEDTrigger_0)
    return
      <xqllib:result>
        <xqllib:var name="ContinuousWaveRadar_0">{$value_ContinuousWaveRadar_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_IEDTrigger_0 in $node_Transceiving_0/RFDevice:supportsTuning
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:resource)
    where fn:exists($value_IEDTrigger_0) and fn:exists($node_Transceiving_0)
    return
      <xqllib:result>
        <xqllib:var name="IEDTrigger_0">{$value_IEDTrigger_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_Transceiving_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#StandoffJamming"]
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:about)
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:about)
    where fn:exists($node_Power_0) and fn:exists($node_Transceiving_0) and $node_Power_0/RFDevice:hasPulseRepetitionInterval/@rdf:resource = $value_Transceiving_0
    return
      <xqllib:result>
  ...
  let $GRAPH_9 := xqllib:and($GRAPH_7,$GRAPH_8,("PowerUnit_0"))
  let $GRAPH_11 :=  
  ...
  let $GRAPH_13 := $GRAPH_12[(data(xqllib:var[@name="DataValue0"]) <= -1639907898)]
  let $GRAPH_14 := ($GRAPH_11,$GRAPH_13)
  let $GRAPH_15 := xqllib:and($GRAPH_9,$GRAPH_14,("Var0"))
  let $GRAPH_16 := xqllib:and($GRAPH_6,$GRAPH_15,("Var0"))
  let $GRAPH_17 := xqllib:optional($GRAPH_5,$GRAPH_16,("Power_0"))
  let $GRAPH_18 := xqllib:and($GRAPH_4,$GRAPH_17,("Power_0"))
  let $GRAPH_19 := xqllib:and($GRAPH_3,$GRAPH_18,("Transceiving_0"))
  let $GRAPH_20 := xqllib:and($GRAPH_2,$GRAPH_19,("Transceiving_0"))
  let $GRAPH_21 := xqllib:and($GRAPH_1,$GRAPH_20,("IEDTrigger_0"))
  let $GRAPH_22 := xqllib:and($GRAPH_0,$GRAPH_21,("ContinuousWaveRadar_0"))
  return
    $GRAPH_22
return
