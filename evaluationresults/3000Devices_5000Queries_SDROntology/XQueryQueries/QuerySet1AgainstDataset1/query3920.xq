...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_DirectConversionReceiver_0/Nuvio:compositeOf
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_SurveillanceRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:about)
    for $node_ADC_0 in $node_DirectConversionReceiver_0/Nuvio:compositeOf
    let $value_ADC_0 := fn:data($node_ADC_0/@rdf:resource)
    where fn:exists($value_ADC_0) and fn:exists($node_DirectConversionReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_0,$GRAPH_6,("SurveillanceRadar_0"))
  return
    $GRAPH_7
return
