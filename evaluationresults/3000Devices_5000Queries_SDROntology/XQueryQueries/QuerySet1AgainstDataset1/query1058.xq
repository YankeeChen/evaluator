...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE312_0 := fn:data($node_USRPE312_0/@rdf:about)
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_USRPE312_0/Nuvio:aggregateOf
    ...
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_USRPE312_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_InstantaneousFrequencyMeasurementReceiver_0 in $node_XilinxZynq7100SoC_0/Nuvio:compositeOf
    let $value_InstantaneousFrequencyMeasurementReceiver_0 := fn:data($node_InstantaneousFrequencyMeasurementReceiver_0/@rdf:resource)
    where fn:exists($value_InstantaneousFrequencyMeasurementReceiver_0) and fn:exists($node_XilinxZynq7100SoC_0)
    return
      <xqllib:result>
        <xqllib:var name="InstantaneousFrequencyMeasurementReceiver_0">{$value_InstantaneousFrequencyMeasurementReceiver_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_XilinxZynq7100SoC_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#ADC_instance132"]/Nuvio:partOf
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_3,$GRAPH_4,("XilinxZynq7100SoC_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("XilinxZynq7100SoC_0"))
