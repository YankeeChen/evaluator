...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_Transmitter_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Transmitter_0 in $node_SpotJamming_0/RFDevice:hasCapability
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:resource)
    where fn:exists($node_SpotJamming_0) and fn:exists($value_Transmitter_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("SpotJamming_0"))
