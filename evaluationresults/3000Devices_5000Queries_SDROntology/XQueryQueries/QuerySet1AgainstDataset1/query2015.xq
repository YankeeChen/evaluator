...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:about)
    for $node_Var0 in $node_CommunicationsSystem_0/RFDevice:supportsJamming
  ...
  let $GRAPH_3 :=  
    for $node_Var0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://www.opengis.net/ont/gml#ArcByBulge"]
  ...
  let $GRAPH_6 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_FormationJamming_0) and fn:exists($node_Var0) and $node_FormationJamming_0/RFDevice:supportsTransmitting/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("FormationJamming_0"))
  ...
  let $GRAPH_13 := xqllib:and($GRAPH_0,$GRAPH_12,("CommunicationsSystem_0"))
  return
    $GRAPH_13
return
