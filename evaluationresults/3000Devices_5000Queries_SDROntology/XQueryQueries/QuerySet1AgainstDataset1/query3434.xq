...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_0 in $node_Transmitter_0/Nuvio:aggregateOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Transmitter_0 in $node_CrossEyeJamming_0/RFDevice:supportsTuning
    let $value_Transmitter_0 := fn:data($node_Transmitter_0/@rdf:resource)
    where fn:exists($node_CrossEyeJamming_0) and fn:exists($value_Transmitter_0)
    return
      <xqllib:result>
