...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:about)
    for $node_Object_0 in $node_USRPE310_0/RFDevice:hasProducer
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Object_0 in $node_AmplitudeModulation_0/RFDevice:supportsTuning
    let $value_Object_0 := fn:data($node_Object_0/@rdf:resource)
    where fn:exists($node_AmplitudeModulation_0) and fn:exists($value_Object_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("AmplitudeModulation_0"))
