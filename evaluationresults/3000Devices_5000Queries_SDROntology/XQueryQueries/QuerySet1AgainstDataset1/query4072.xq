...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB2xxmini_0 := fn:data($node_USRPB2xxmini_0/@rdf:about)
    for $node_Object_0 in $node_USRPB2xxmini_0/Nuvio:partOf
  ...
  let $GRAPH_2 :=  
    ...
    for $node_PhaseModulation_0 in $node_Object_0/Nuvio:participatesIn
    let $value_PhaseModulation_0 := fn:data($node_PhaseModulation_0/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_PhaseModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
