...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Full_DuplexCommunicationsSystem_0 in $node_Transceiving_0/RFDevice:hasCapability
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Transceiving_0 in $node_RadarFrequencyBand_0/RFDevice:hasFrequencyRange
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:resource)
    where fn:exists($node_RadarFrequencyBand_0) and fn:exists($value_Transceiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_3,$GRAPH_6,("Transceiving_0"))
  let $GRAPH_8 := xqllib:optional($GRAPH_2,$GRAPH_7,("Transceiving_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("Transceiving_0"))
