...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Var1 in $node_Attribute_0/Nuvio:expresses
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_9 :=  
    for $node_Attribute_0 in $doc_ObjectDescription
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:about)
    for $node_Var0 in $node_Attribute_0/RFDevice:hasMaxSensedPower
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_8,$GRAPH_9,("Attribute_0","Var0"))
  let $GRAPH_11 := xqllib:and($GRAPH_1,$GRAPH_10,("Var0"))
  let $GRAPH_12 := xqllib:and($GRAPH_0,$GRAPH_11,("USRPE312_0"))
  return
    $GRAPH_12
return
