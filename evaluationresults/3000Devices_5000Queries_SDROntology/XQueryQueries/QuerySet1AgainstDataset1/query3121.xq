...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_MonostaticRadar_0 in $node_DirectConversionReceiver_0/Nuvio:compositeOf
    let $value_MonostaticRadar_0 := fn:data($node_MonostaticRadar_0/@rdf:resource)
    where fn:exists($node_DirectConversionReceiver_0) and fn:exists($value_MonostaticRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    for $node_DirectConversionReceiver_0 in $doc_ObjectDescription
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:about)
    for $node_Entity_0 in $node_DirectConversionReceiver_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_Entity_0) and $doc_ObjectDescription[@rdf:ID = "SDR_instance50"]/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
      </xqllib:result>
