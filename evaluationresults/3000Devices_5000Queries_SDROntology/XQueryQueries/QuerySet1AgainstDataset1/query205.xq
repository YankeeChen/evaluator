...
let $result :=  
  ...
  let $GRAPH_10 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_EmitterLocationDetection_0) and fn:exists($node_Var0) and $node_EmitterLocationDetection_0/Nuvio:subprocessOf/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_13 :=  
    ...
    for $node_EmitterLocationDetection_0 in $node_Var2/Nuvio:hasParticipant
    let $value_EmitterLocationDetection_0 := fn:data($node_EmitterLocationDetection_0/@rdf:resource)
    where fn:exists($value_EmitterLocationDetection_0) and fn:exists($node_Var2)
    return
      <xqllib:result>
        <xqllib:var name="EmitterLocationDetection_0">{$value_EmitterLocationDetection_0}</xqllib:var>
  ...
  let $GRAPH_14 :=  
    ...
    for $node_Var2 in $node_RangeGatePullOffJamming_0/RFDevice:supportsSpectrumSensing
    let $value_Var2 := fn:data($node_Var2/@rdf:resource)
    where fn:exists($node_RangeGatePullOffJamming_0) and fn:exists($value_Var2)
    return
      <xqllib:result>
  ...
  let $GRAPH_15 :=  
    for $node_Var2 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#BistaticRadar"]
    let $value_Var2 := fn:data($node_Var2/@rdf:about)
  ...
  let $GRAPH_16 :=  
    ...
    for $node_Var2 in $node_Object_0/RFDevice:isProducerOf
    let $value_Var2 := fn:data($node_Var2/@rdf:resource)
    where fn:exists($node_Object_0) and fn:exists($value_Var2)
    return
      <xqllib:result>
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_16,$GRAPH_17,("Object_0"))
  let $GRAPH_19 := xqllib:and($GRAPH_15,$GRAPH_18,("Var2"))
  let $GRAPH_20 := xqllib:and($GRAPH_14,$GRAPH_19,("Var2"))
  let $GRAPH_21 := xqllib:and($GRAPH_13,$GRAPH_20,("Var2"))
  let $GRAPH_22 := ($GRAPH_12,$GRAPH_21)
  let $GRAPH_23 := xqllib:and($GRAPH_10,$GRAPH_22,("EmitterLocationDetection_0"))
  let $GRAPH_24 := xqllib:and($GRAPH_9,$GRAPH_23,("Var0"))
