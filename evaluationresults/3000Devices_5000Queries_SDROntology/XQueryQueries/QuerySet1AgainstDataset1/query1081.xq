...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_RadarWarningReceiver_0 in $node_Entity_0/RFDevice:hasHeight
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#DataTransmissionMode"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
  ...
  let $GRAPH_5 :=  
    ...
    where fn:exists($node_Object_0) and fn:exists($node_Quantity_0) and $node_Quantity_0/Nuvio:hasObjectQuality/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
        <xqllib:var name="Object_0">{$value_Object_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_3,$GRAPH_7,())
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("Entity_0","Object_0"))
