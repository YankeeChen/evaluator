...
let $result :=  
  ...
  let $GRAPH_3 :=  
    for $node_Entity_0 in $doc_ObjectDescription[Nuvio:partOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#RadarJammer_instance151"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) >= -1031507054)]
  ...
  let $GRAPH_19 := xqllib:and($GRAPH_17,$GRAPH_18,("Quality_0"))
