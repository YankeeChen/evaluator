...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_BistaticRadar_0 in $node_Entity_0/RFDevice:supportsTuning
  ...
  let $GRAPH_3 :=  
    for $node_Value_0 in $doc_ObjectDescription
    let $value_Value_0 := fn:data($node_Value_0/@rdf:about)
    for $node_TimeUnit_0 in $node_Value_0/Nuvio:hasUnitOfMeasure
    let $value_TimeUnit_0 := fn:data($node_TimeUnit_0/@rdf:resource)
    where fn:exists($value_TimeUnit_0) and fn:exists($node_Value_0)
    return
      <xqllib:result>
