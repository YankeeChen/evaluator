...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200mini_i_0 in $node_Spartan6FPGA_0/Nuvio:partOf
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:resource)
    where fn:exists($node_Spartan6FPGA_0) and fn:exists($value_USRPB200mini_i_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_Spartan6FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Producer_instance143"]/RFDevice:isProducerOf
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("Spartan6FPGA_0"))
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("Spartan6FPGA_0"))
