...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_CommunicationsSystem_0 := fn:data($node_CommunicationsSystem_0/@rdf:about)
    for $node_Entity_0 in $node_CommunicationsSystem_0/RFDevice:hasProducer
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
    where fn:exists($node_CommunicationsSystem_0) and fn:exists($value_Entity_0)
    return
      <xqllib:result>
        <xqllib:var name="CommunicationsSystem_0">{$value_CommunicationsSystem_0}</xqllib:var>
  ...
  let $GRAPH_6 :=  
    ...
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_AbstractCurveSegment_0) and fn:exists($node_Entity_0) and $node_AbstractCurveSegment_0/Nuvio:relevantIndividual/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
  ...
  let $GRAPH_11 := xqllib:optional($GRAPH_0,$GRAPH_10,("CommunicationsSystem_0"))
  return
    $GRAPH_11
return
