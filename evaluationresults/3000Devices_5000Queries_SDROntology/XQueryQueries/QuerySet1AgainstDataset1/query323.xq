...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE310_0 in $node_Var0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_TerrainBounceJamming_0) and fn:exists($node_Var0) and $node_TerrainBounceJamming_0/Nuvio:hasSubprocess/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_6 :=  
    ...
    for $node_TerrainBounceJamming_0 in $node_Quantity_0/Nuvio:hasProcessQuantity
