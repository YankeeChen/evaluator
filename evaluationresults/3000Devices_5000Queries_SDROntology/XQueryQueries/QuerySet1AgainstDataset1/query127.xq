...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE313_0 := fn:data($node_USRPE313_0/@rdf:about)
    for $node_RadarJammer_0 in $node_USRPE313_0/Nuvio:aggregateOf
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($value_RadarJammer_0) and fn:exists($node_USRPE313_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_RadarJammer_0 in $node_BlinkingJamming_0/RFDevice:supportsTuning
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_BlinkingJamming_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
