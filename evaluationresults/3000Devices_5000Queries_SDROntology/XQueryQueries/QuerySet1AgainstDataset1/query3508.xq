...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB2xx_0 := fn:data($node_USRPB2xx_0/@rdf:about)
    for $node_IEDTrigger_0 in $node_USRPB2xx_0/Nuvio:aggregateOf
    let $value_IEDTrigger_0 := fn:data($node_IEDTrigger_0/@rdf:resource)
    where fn:exists($value_IEDTrigger_0) and fn:exists($node_USRPB2xx_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_NoiseFigure_0 in $node_IEDTrigger_0/RFDevice:hasNoiseFigure
    let $value_NoiseFigure_0 := fn:data($node_NoiseFigure_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_NoiseFigure_0)
    return
      <xqllib:result>
        <xqllib:var name="IEDTrigger_0">{$value_IEDTrigger_0}</xqllib:var>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_0,$GRAPH_3,("USRPB2xx_0"))
  return
    $GRAPH_4
return
