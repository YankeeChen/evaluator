...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_RadarWarningReceiver_0 := fn:data($node_RadarWarningReceiver_0/@rdf:about)
    for $node_Attribute_0 in $node_RadarWarningReceiver_0/RFDevice:hasNoiseFigure
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:resource)
    where fn:exists($value_Attribute_0) and fn:exists($node_RadarWarningReceiver_0)
    return
      <xqllib:result>
