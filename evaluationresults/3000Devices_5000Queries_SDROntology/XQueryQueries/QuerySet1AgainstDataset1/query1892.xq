...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Var0 in $node_Quantity_0/RFDevice:hasTransmitPowerRange
  ...
  let $GRAPH_7 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0) and $doc_ObjectDescription[@rdf:ID = "Moving_instance8"]/RFDevice:supportsSpectrumSensing/@rdf:resource = $value_Var0
    return
      <xqllib:result>
        <xqllib:var name="Var0">{$value_Var0}</xqllib:var>
      </xqllib:result>
