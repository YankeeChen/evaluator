...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN300_0 in $node_SuperheterodyneReceiver_0/Nuvio:partOf
    let $value_USRPN300_0 := fn:data($node_USRPN300_0/@rdf:resource)
    where fn:exists($node_SuperheterodyneReceiver_0) and fn:exists($value_USRPN300_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_SuperheterodyneReceiver_0 in $node_Object_0/RFDevice:hasFrequencyRange
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    for $node_Length_0 in $node_Object_0/RFDevice:hasWidth
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Length_0"))
