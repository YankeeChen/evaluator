...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Entity_0 in $node_Var0/RFDevice:hasBeamwidth
  ...
  let $GRAPH_5 :=  
    for $node_Entity_0 in $doc_ObjectDescription
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    for $node_GainRange_0 in $node_Entity_0/Nuvio:expresses
    let $value_GainRange_0 := fn:data($node_GainRange_0/@rdf:resource)
    where fn:exists($node_Entity_0) and fn:exists($value_GainRange_0)
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
