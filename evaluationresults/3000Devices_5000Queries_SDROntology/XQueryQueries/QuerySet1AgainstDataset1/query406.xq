...
let $result :=  
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_ElectronicAttack_0 in $node_Var0/Nuvio:relevantIndividual
    let $value_ElectronicAttack_0 := fn:data($node_ElectronicAttack_0/@rdf:resource)
    where fn:exists($value_ElectronicAttack_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:optional($GRAPH_0,$GRAPH_9,("USRPB205mini_i_0"))
  return
    $GRAPH_10
return
