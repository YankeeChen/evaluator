...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Full_DuplexCommunicationsSystem_0 in $node_Entity_0/RFDevice:hasCapacity
  ...
  let $GRAPH_6 :=  
    ...
    for $node_Process_0 in $node_Role_0/RFDevice:hasTarget
    let $value_Process_0 := fn:data($node_Process_0/@rdf:resource)
    where fn:exists($value_Process_0) and fn:exists($node_Role_0)
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
  ...
  let $GRAPH_12 :=  
    for $node_Entity_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Frequency_instance212"]/RFDevice:hasBandwidth
  ...
  let $GRAPH_13 :=  
    ...
    for $node_Entity_0 in $node_Entity_1/RFDevice:hasBandwidthRange
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:resource)
