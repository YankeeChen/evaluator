...
let $result :=  
  ...
  let $GRAPH_5 := $GRAPH_4[(data(xqllib:var[@name="DataValue0"]) <= 1645587679)]
  let $GRAPH_6 :=  
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue1"]) >= 57.21)]
  ...
  let $GRAPH_13 :=  
    for $node_Entity_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://cogradio.org/ont/Nuvio.owl#Process" and Nuvio:hasSubprocess/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Instance3695"]
    let $value_Entity_0 := fn:data($node_Entity_0/@rdf:about)
    where fn:exists($node_Entity_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_14 := xqllib:optional($GRAPH_12,$GRAPH_13,("Entity_0"))
  let $GRAPH_15 := xqllib:and($GRAPH_1,$GRAPH_14,("Entity_0"))
  let $GRAPH_16 := xqllib:optional($GRAPH_0,$GRAPH_15,("RadarWarningReceiver_0"))
  return
    $GRAPH_16
return
