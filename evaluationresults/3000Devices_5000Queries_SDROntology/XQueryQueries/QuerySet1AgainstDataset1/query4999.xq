...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_PulseDopplerRadar_0 := fn:data($node_PulseDopplerRadar_0/@rdf:about)
    for $node_PulseRadarModulation_0 in $node_PulseDopplerRadar_0/RFDevice:supportsTransmitting
    let $value_PulseRadarModulation_0 := fn:data($node_PulseRadarModulation_0/@rdf:resource)
    where fn:exists($node_PulseDopplerRadar_0) and fn:exists($value_PulseRadarModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="PulseDopplerRadar_0">{$value_PulseDopplerRadar_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    for $node_PulseRadarModulation_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#TimeInterval_instance20"]/RFDevice:hasTransmitPowerRange
    let $value_PulseRadarModulation_0 := fn:data($node_PulseRadarModulation_0/@rdf:resource)
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("PulseRadarModulation_0"))
