...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN3xx_0 in $node_XilinxZynq7000Family_0/Nuvio:compositeOf
    let $value_USRPN3xx_0 := fn:data($node_USRPN3xx_0/@rdf:resource)
    where fn:exists($value_USRPN3xx_0) and fn:exists($node_XilinxZynq7000Family_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPN3xx_0">{$value_USRPN3xx_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_XilinxZynq7000Family_0 in $node_Kintex7FPGA_0/Nuvio:compositeOf
    let $value_XilinxZynq7000Family_0 := fn:data($node_XilinxZynq7000Family_0/@rdf:resource)
    where fn:exists($node_Kintex7FPGA_0) and fn:exists($value_XilinxZynq7000Family_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_Kintex7FPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Producer_instance234"]/RFDevice:isProducerOf
    let $value_Kintex7FPGA_0 := fn:data($node_Kintex7FPGA_0/@rdf:resource)
    where fn:exists($value_Kintex7FPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="Kintex7FPGA_0">{$value_Kintex7FPGA_0}</xqllib:var>
      </xqllib:result>
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Kintex7FPGA_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Kintex7FPGA_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("XilinxZynq7000Family_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("XilinxZynq7000Family_0"))
  let $GRAPH_10 := xqllib:and($GRAPH_0,$GRAPH_9,("USRPN3xx_0"))
  return
    $GRAPH_10
return
