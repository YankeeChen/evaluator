...
let $result :=  
  ...
  let $GRAPH_2 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Kintex7FPGA_0 in $node_Var0/Nuvio:hasParticipant
    let $value_Kintex7FPGA_0 := fn:data($node_Kintex7FPGA_0/@rdf:resource)
    where fn:exists($value_Kintex7FPGA_0) and fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_Kintex7FPGA_0 in $node_Object_0/RFDevice:hasProducer
    let $value_Kintex7FPGA_0 := fn:data($node_Kintex7FPGA_0/@rdf:resource)
    where fn:exists($value_Kintex7FPGA_0) and fn:exists($node_Object_0)
    return
      <xqllib:result>
        <xqllib:var name="Kintex7FPGA_0">{$value_Kintex7FPGA_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:optional($GRAPH_3,$GRAPH_6,("Kintex7FPGA_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Kintex7FPGA_0"))
