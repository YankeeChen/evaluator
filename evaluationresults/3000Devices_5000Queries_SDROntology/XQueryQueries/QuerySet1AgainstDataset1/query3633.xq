...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_JammingSystem_0 := fn:data($node_JammingSystem_0/@rdf:about)
    for $node_Weight_0 in $node_JammingSystem_0/RFDevice:hasGain
    let $value_Weight_0 := fn:data($node_Weight_0/@rdf:resource)
    where fn:exists($node_JammingSystem_0) and fn:exists($value_Weight_0)
    return
      <xqllib:result>
        <xqllib:var name="JammingSystem_0">{$value_JammingSystem_0}</xqllib:var>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("WeightUnit_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("Var0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("Weight_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("Weight_0"))
