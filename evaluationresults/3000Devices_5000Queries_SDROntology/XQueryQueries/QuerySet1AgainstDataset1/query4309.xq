...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ElectronicAttackSystem_0 := fn:data($node_ElectronicAttackSystem_0/@rdf:about)
    for $node_Attribute_0 in $node_ElectronicAttackSystem_0/RFDevice:hasMaxPowerConsumption
    let $value_Attribute_0 := fn:data($node_Attribute_0/@rdf:resource)
    where fn:exists($value_Attribute_0) and fn:exists($node_ElectronicAttackSystem_0)
    return
      <xqllib:result>
