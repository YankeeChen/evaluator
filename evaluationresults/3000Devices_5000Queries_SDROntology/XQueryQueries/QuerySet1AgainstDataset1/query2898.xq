...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_ElectronicSupportSystem_0 in $node_Component_0/Nuvio:compositeOf
    let $value_ElectronicSupportSystem_0 := fn:data($node_ElectronicSupportSystem_0/@rdf:resource)
    where fn:exists($node_Component_0) and fn:exists($value_ElectronicSupportSystem_0)
    return
      <xqllib:result>
