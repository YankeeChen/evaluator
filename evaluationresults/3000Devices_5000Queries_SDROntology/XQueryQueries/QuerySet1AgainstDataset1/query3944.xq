...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_DirectConversionReceiver_0/Nuvio:partOf
  ...
  let $GRAPH_3 :=  
    ...
    for $node_DirectConversionReceiver_0 in $node_Attribute_0/RFDevice:hasNoiseFigure
    let $value_DirectConversionReceiver_0 := fn:data($node_DirectConversionReceiver_0/@rdf:resource)
    where fn:exists($node_Attribute_0) and fn:exists($value_DirectConversionReceiver_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_0,$GRAPH_7,("PulseRadar_0"))
  return
    $GRAPH_8
return
