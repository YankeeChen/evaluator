...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_Var0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[Nuvio:subprocessOf/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Instance446"]
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Var0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 :=  
    ...
    for $node_Var1 in $node_SuperheterodyneReceiver_0/Nuvio:compositeOf
    let $value_Var1 := fn:data($node_Var1/@rdf:resource)
    where fn:exists($node_SuperheterodyneReceiver_0) and fn:exists($value_Var1)
    return
      <xqllib:result>
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_6,$GRAPH_9,())
  let $GRAPH_11 := xqllib:and($GRAPH_3,$GRAPH_10,("InformationEntity_0","Var0"))
