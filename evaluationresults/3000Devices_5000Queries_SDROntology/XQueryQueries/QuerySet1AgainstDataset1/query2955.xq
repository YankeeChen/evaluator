...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPN310_0 in $node_RadarJammer_0/Nuvio:partOf
    let $value_USRPN310_0 := fn:data($node_USRPN310_0/@rdf:resource)
    where fn:exists($node_RadarJammer_0) and fn:exists($value_USRPN310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_RadarJammer_0 in $node_NoiseJamming_0/RFDevice:supportsTransmitting
  ...
  let $GRAPH_6 :=  
    ...
    for $node_NoiseJamming_0 in $node_Quantity_0/RFDevice:hasEffectiveRadiatedPower
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_3,$GRAPH_9,("NoiseJamming_0"))
