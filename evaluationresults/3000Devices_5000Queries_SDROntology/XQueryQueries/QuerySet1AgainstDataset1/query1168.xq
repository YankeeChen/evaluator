...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:about)
    for $node_Var0 in $node_USRPB200_0/Nuvio:aggregateOf
  ...
  let $GRAPH_4 :=  
    for $node_Var0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#CommunicationsSystem_instance39"]/Nuvio:isExpressedBy
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_8 := $GRAPH_7[(data(xqllib:var[@name="DataValue0"]) = xs:double("18.454933"))]
  let $GRAPH_9 :=  
    for $node_Var1 in $doc_ObjectDescription
    let $value_Var1 := fn:data($node_Var1/@rdf:about)
  ...
  let $GRAPH_15 :=  
    ...
    for $node_Var0 in $node_Var2/RFDevice:hasLevel
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
  ...
  let $GRAPH_18 := xqllib:and($GRAPH_0,$GRAPH_17,("USRPB200_0"))
  return
    $GRAPH_18
return
