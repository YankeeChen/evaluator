...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ContinuousWaveRadar_0 := fn:data($node_ContinuousWaveRadar_0/@rdf:about)
    for $node_RadarModulation_0 in $node_ContinuousWaveRadar_0/RFDevice:supportsTransmitting
    let $value_RadarModulation_0 := fn:data($node_RadarModulation_0/@rdf:resource)
    where fn:exists($node_ContinuousWaveRadar_0) and fn:exists($value_RadarModulation_0)
    return
      <xqllib:result>
        <xqllib:var name="ContinuousWaveRadar_0">{$value_ContinuousWaveRadar_0}</xqllib:var>
