...
let $result :=  
  ...
  let $GRAPH_3 :=  
    ...
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    where fn:exists($node_Power_0) and fn:exists($node_Var0) and $node_Power_0/RFDevice:hasTransmitPowerRange/@rdf:resource = $value_Var0
    return
      <xqllib:result>
  ...
  let $GRAPH_7 :=  
    for $node_Var0 in $doc_ObjectDescription
    let $value_Var0 := fn:data($node_Var0/@rdf:about)
    for $node_Value_0 in $node_Var0/Nuvio:hasUnitOfMeasure
