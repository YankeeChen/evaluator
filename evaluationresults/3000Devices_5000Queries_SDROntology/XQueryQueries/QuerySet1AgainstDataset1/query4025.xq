...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SearchRadar_0 in $node_Entity_0/RFDevice:isDeployedOn
  ...
  let $GRAPH_4 :=  
    ...
    where fn:exists($node_Entity_0) and fn:exists($node_InformationRepresentation_0) and $node_InformationRepresentation_0/Nuvio:representedBy/@rdf:resource = $value_Entity_0
    return
      <xqllib:result>
        <xqllib:var name="Entity_0">{$value_Entity_0}</xqllib:var>
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_4,$GRAPH_7,("InformationRepresentation_0"))
  ...
  let $GRAPH_12 := xqllib:and($GRAPH_0,$GRAPH_11,("SearchRadar_0"))
  return
    $GRAPH_12
return
