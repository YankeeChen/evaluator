...
let $result :=  
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Situation_0 := fn:data($node_Situation_0/@rdf:about)
    where fn:exists($node_GPS_0) and fn:exists($node_Situation_0) and $node_GPS_0/Nuvio:relevantIndividual/@rdf:resource = $value_Situation_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_Situation_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Producer_instance949"]/Nuvio:relevantIndividual
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_6,$GRAPH_7,("GPS_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("Situation_0"))
