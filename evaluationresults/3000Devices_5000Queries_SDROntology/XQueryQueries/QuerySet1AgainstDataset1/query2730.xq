...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:about)
    for $node_MobilePlatform_0 in $node_USRPB200mini_i_0/RFDevice:isDeployedOn
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:resource)
    where fn:exists($value_MobilePlatform_0) and fn:exists($node_USRPB200mini_i_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    for $node_MobilePlatform_0 in $doc_ObjectDescription[RFDevice:supportsTransmitting/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Moving_instance58"]
    let $value_MobilePlatform_0 := fn:data($node_MobilePlatform_0/@rdf:about)
    where fn:exists($node_MobilePlatform_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:optional($GRAPH_2,$GRAPH_3,("MobilePlatform_0"))
