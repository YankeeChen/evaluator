...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseRadar_0 in $node_PulseDopplerRadarModulation_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_3 :=  
    for $node_PulseDopplerRadarModulation_0 in $doc_ObjectDescription[RFDevice:hasPulseWidth/@rdf:resource = "http://ece.neu.edu/ontologies/ObjectDescription3000.owl#TimeInterval_instance8"]
    let $value_PulseDopplerRadarModulation_0 := fn:data($node_PulseDopplerRadarModulation_0/@rdf:about)
    where fn:exists($node_PulseDopplerRadarModulation_0)
    return
      <xqllib:result>
