...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:about)
    for $node_SuperheterodyneReceiver_0 in $node_USRPE310_0/Nuvio:partOf
    let $value_SuperheterodyneReceiver_0 := fn:data($node_SuperheterodyneReceiver_0/@rdf:resource)
    where fn:exists($value_SuperheterodyneReceiver_0) and fn:exists($node_USRPE310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_RadioFrequencyBand_0 in $node_Distance_0/RFDevice:hasMin
    let $value_RadioFrequencyBand_0 := fn:data($node_RadioFrequencyBand_0/@rdf:resource)
    where fn:exists($node_Distance_0) and fn:exists($value_RadioFrequencyBand_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("Distance_0"))
  let $GRAPH_7 := xqllib:and($GRAPH_3,$GRAPH_6,("RadioFrequencyBand_0"))
  let $GRAPH_8 := xqllib:and($GRAPH_2,$GRAPH_7,("SuperheterodyneReceiver_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_1,$GRAPH_8,("SuperheterodyneReceiver_0"))
  let $GRAPH_10 := xqllib:optional($GRAPH_0,$GRAPH_9,("USRPE310_0"))
  return
    $GRAPH_10
return
