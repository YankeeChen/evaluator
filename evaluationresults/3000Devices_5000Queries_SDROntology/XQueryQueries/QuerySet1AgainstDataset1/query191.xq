...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_ModelX_0 := fn:data($node_ModelX_0/@rdf:about)
    for $node_Entity_0 in $node_ModelX_0/RFDevice:isProducerOf
  ...
  let $GRAPH_3 :=  
    ...
    where fn:exists($node_SpatialVolumeUnit_0) and fn:exists($node_Value_0) and $node_SpatialVolumeUnit_0/Nuvio:hasUnitOfMeasure/@rdf:resource = $value_Value_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 := xqllib:optional($GRAPH_3,$GRAPH_4,("SpatialVolumeUnit_0"))
  let $GRAPH_6 := xqllib:and($GRAPH_2,$GRAPH_5,("Value_0"))
