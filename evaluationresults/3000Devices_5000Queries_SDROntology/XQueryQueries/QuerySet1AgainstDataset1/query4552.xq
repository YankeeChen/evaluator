...
let $result :=  
  ...
  let $GRAPH_1 :=  
    for $node_USRPN3xx_0 in $doc_ObjectDescription[RFDevice:supportsSpectrumSensing/@rdf:resource = "http://ece.neu.edu/ontologies/SDR.owl#USRP_SpectrumSensing_Instance4"]
    let $value_USRPN3xx_0 := fn:data($node_USRPN3xx_0/@rdf:about)
    where fn:exists($node_USRPN3xx_0)
    return
      <xqllib:result>
