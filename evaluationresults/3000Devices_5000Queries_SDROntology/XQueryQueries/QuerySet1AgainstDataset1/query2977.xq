...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_USRPB200mini_i_0 := fn:data($node_USRPB200mini_i_0/@rdf:about)
    for $node_Var0 in $node_USRPB200mini_i_0/Nuvio:partOf
    let $value_Var0 := fn:data($node_Var0/@rdf:resource)
    where fn:exists($node_USRPB200mini_i_0) and fn:exists($value_Var0)
    return
      <xqllib:result>
        <xqllib:var name="USRPB200mini_i_0">{$value_USRPB200mini_i_0}</xqllib:var>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_FrequencyAccuracy_0 in $node_Var0/RFDevice:hasMaxPowerConsumption
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("FrequencyAccuracy_0"))
