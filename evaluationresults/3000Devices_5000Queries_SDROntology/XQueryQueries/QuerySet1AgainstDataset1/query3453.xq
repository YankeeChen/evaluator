...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_BistaticRadar_0 in $node_Entity_0/RFDevice:supportsSpectrumSensing
  ...
  let $GRAPH_5 :=  
    ...
    where fn:exists($node_InformationEntity_0) and fn:exists($node_LinearRing_0) and $node_LinearRing_0/Nuvio:isExpressedBy/@rdf:resource = $value_InformationEntity_0
    return
      <xqllib:result>
        <xqllib:var name="InformationEntity_0">{$value_InformationEntity_0}</xqllib:var>
  ...
  let $GRAPH_7 := xqllib:and($GRAPH_5,$GRAPH_6,("LinearRing_0"))
  let $GRAPH_8 := ($GRAPH_4,$GRAPH_7)
  let $GRAPH_9 := xqllib:and($GRAPH_2,$GRAPH_8,("InformationRepresentation_0"))
