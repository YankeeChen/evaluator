...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_Full_DuplexCommunicationsSystem_0 in $node_SuperheterodyneReceiver_0/Nuvio:compositeOf
  ...
  let $GRAPH_4 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_Entity_0) and fn:exists($node_Object_0) and $node_Entity_0/Nuvio:participatesIn/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    ...
    for $node_Entity_0 in $node_Object_1/Nuvio:expresses
  ...
  let $GRAPH_8 := xqllib:and($GRAPH_1,$GRAPH_7,("SuperheterodyneReceiver_0"))
  let $GRAPH_9 := xqllib:and($GRAPH_0,$GRAPH_8,("Full_DuplexCommunicationsSystem_0"))
  return
    $GRAPH_9
return
