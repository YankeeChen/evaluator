...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_SurveillanceRadar_0 in $node_IEDTrigger_0/Nuvio:compositeOf
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:resource)
    where fn:exists($node_IEDTrigger_0) and fn:exists($value_SurveillanceRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_IEDTrigger_0 in $node_Transceiving_0/RFDevice:supportsJamming
  ...
  let $GRAPH_5 :=  
    for $node_Transceiving_0 in $doc_ObjectDescription[rdf:type/@rdf:resource = "http://ece.neu.edu/ontologies/RFDevice.owl#CrossPolarizationJamming"]
    let $value_Transceiving_0 := fn:data($node_Transceiving_0/@rdf:about)
    where fn:exists($node_Transceiving_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_8 := xqllib:optional($GRAPH_2,$GRAPH_7,("IEDTrigger_0"))
