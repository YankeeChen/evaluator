...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    let $value_SurveillanceRadar_0 := fn:data($node_SurveillanceRadar_0/@rdf:about)
    for $node_RadarJammer_0 in $node_SurveillanceRadar_0/Nuvio:partOf
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($value_RadarJammer_0) and fn:exists($node_SurveillanceRadar_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_4 :=  
    ...
    for $node_RadarJammer_0 in $node_NoiseJamming_0/RFDevice:hasCapability
    let $value_RadarJammer_0 := fn:data($node_RadarJammer_0/@rdf:resource)
    where fn:exists($node_NoiseJamming_0) and fn:exists($value_RadarJammer_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_5 :=  
    for $node_NoiseJamming_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#Power_instance63"]/RFDevice:hasPulseRepetitionInterval
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_4,$GRAPH_5,("NoiseJamming_0"))
