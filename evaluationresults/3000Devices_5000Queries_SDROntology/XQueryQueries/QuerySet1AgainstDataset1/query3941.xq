...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPX300_0 in $node_XilinxFPGA_0/Nuvio:aggregateOf
    let $value_USRPX300_0 := fn:data($node_USRPX300_0/@rdf:resource)
    where fn:exists($value_USRPX300_0) and fn:exists($node_XilinxFPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="USRPX300_0">{$value_USRPX300_0}</xqllib:var>
  ...
  let $GRAPH_4 :=  
    for $node_XilinxFPGA_0 in $doc_ObjectDescription[@rdf:about="http://ece.neu.edu/ontologies/ObjectDescription3000.owl#OmnidirectionalAntenna_instance3"]/Nuvio:compositeOf
  ...
  let $GRAPH_6 := xqllib:and($GRAPH_1,$GRAPH_5,("XilinxFPGA_0"))
