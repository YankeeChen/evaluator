...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB200_0 in $node_Aircraft_0/RFDevice:isDeployedOn
    let $value_USRPB200_0 := fn:data($node_USRPB200_0/@rdf:resource)
    where fn:exists($node_Aircraft_0) and fn:exists($value_USRPB200_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_7 := $GRAPH_6[(data(xqllib:var[@name="DataValue0"]) != -68985885)]
  let $GRAPH_8 :=  
  ...
  let $GRAPH_10 := xqllib:and($GRAPH_5,$GRAPH_9,("Entity_0","PowerUnit_0"))
  let $GRAPH_11 :=  
