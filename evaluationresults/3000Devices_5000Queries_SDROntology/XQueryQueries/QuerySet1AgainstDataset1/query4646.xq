...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPE310_0 in $node_Processor_0/Nuvio:partOf
    let $value_USRPE310_0 := fn:data($node_USRPE310_0/@rdf:resource)
    where fn:exists($node_Processor_0) and fn:exists($value_USRPE310_0)
    return
      <xqllib:result>
  ...
  let $GRAPH_2 :=  
    ...
    for $node_Processor_0 in $node_Spartan3ADSPFPGA_0/Nuvio:aggregateOf
    let $value_Processor_0 := fn:data($node_Processor_0/@rdf:resource)
    where fn:exists($value_Processor_0) and fn:exists($node_Spartan3ADSPFPGA_0)
    return
      <xqllib:result>
        <xqllib:var name="Processor_0">{$value_Processor_0}</xqllib:var>
  ...
  let $GRAPH_3 :=  
    ...
    for $node_Spartan3ADSPFPGA_0 in $node_Var0/RFDevice:hasProducer
  ...
  let $GRAPH_5 := xqllib:and($GRAPH_1,$GRAPH_4,("Processor_0"))
