...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_PulseDopplerRadar_0 in $node_Process_0/RFDevice:supportsTuning
  ...
  let $GRAPH_2 :=  
    ...
    where fn:exists($node_Process_0) and fn:exists($node_Ratio_0) and $node_Ratio_0/RFDevice:hasMaxSensedPower/@rdf:resource = $value_Process_0
    return
      <xqllib:result>
        <xqllib:var name="Process_0">{$value_Process_0}</xqllib:var>
