...
let $result :=  
  ...
  let $GRAPH_1 :=  
    ...
    for $node_USRPB210_0 in $node_Object_0/RFDevice:isProducerOf
  ...
  let $GRAPH_2 :=  
    ...
    let $value_Object_0 := fn:data($node_Object_0/@rdf:about)
    where fn:exists($node_FrequencyScanRate_0) and fn:exists($node_Object_0) and $node_FrequencyScanRate_0/RFDevice:hasLevel/@rdf:resource = $value_Object_0
    return
      <xqllib:result>
  ...
  let $GRAPH_4 := xqllib:and($GRAPH_2,$GRAPH_3,("FrequencyScanRate_0"))
